...
class KLOGIC:
    ...
    def get_user_information(self, tables):
        ...
        # print(user_td)
        self.user_info[self.word_mapping["USER"][self.language]] = re.search(r"(?<!\d)\d{13}(?!\d)", user_td[0].text).group(0)
        self.user_info[self.word_mapping["TIME"][self.language]] = user_td[1].text.replace("\xa0", "")
        self.user_info[self.word_mapping["CURRENTSEMESTER"][self.language]] = re.search(r"(?<!\d)\d{1,2}/\d{4}(?!\d)", user_td[2].text).group(0)
        self.user_info[self.word_mapping["CURRENTSEMESTERWORK"][self.language]] = re.search(r"(?<!\d)\d{1,2}/\d{4}(?!\d)", user_td[3].text).group(0)
        # print(user_info)
        ...
        # print(user_td2)
        self.user_info[self.word_mapping["STUDENT_ID"][self.language]] = re.search(r"(?<!\d)\d{13}(?!\d)", user_td2[0].text).group(0)
        self.user_info[self.word_mapping["STUDENT_NAME"][self.language]] = user_td2[1].text.replace("\xa0", "").replace("ชื่อ", "").replace("\r\n", "")

        user_td3 = tables[1].find_all("td")
        # print(user_td3)

        self.user_info[self.word_mapping["STUDENT_MAJOR"][self.language]] = user_td3[0].text.replace("สาขา", "").replace("\xa0", "")
        self.user_info[self.word_mapping["STUDENT_DEPARTMENT"][self.language]] = user_td3[1].text.replace("ภาควิชา", "").replace("\xa0", "")
        self.user_info[self.word_mapping["STUDENT_FACULTY"][self.language]] = user_td3[2].text.replace("คณะ", "").replace("\xa0", "")
        ...
        for tb in tables:
            ...
            for tr in ft_tr[2:]:
                try:
                    ...
                    course_info[self.word_mapping["COURSE_NAME"][self.language]] = ft_tr2_td[0].text.replace(course_id, "").replace("\xa0", "").strip()
                    ...
                    term_info[ft][self.word_mapping["SEMESTER"][self.language]] = {self.word_mapping["AVERAGE_SCORE"][self.language]: self.remove_xa(result_first_row[0].text),
                    ...
                    self.term_info[ft][self.word_mapping["SEMESTER"][self.language]] = {"คะแนนฉลี่ย": self.remove_xa(result_first_row[0].text),
                                                 self.word_mapping["REGISTERED_CREDIT"][self.language]: self.remove_xa(result_first_row[1].text),
                                                 self.word_mapping["ACHIEVE_CREDIT"][self.language]: self.remove_xa(result_first_row[2].text),
                                                 self.word_mapping["TOTAL_SCORE"][self.language]: self.remove_xa(result_first_row[3].text)}
                    result_first_row = result_first_row[5:]
                    # print(result_first_row)
                    term_info[ft][self.word_mapping["CUMULATIVE"][self.language]] = {self.word_mapping["AVERAGE_SCORE"][self.language]: self.remove_xa(result_first_row[0].text),
                                             self.word_mapping["REGISTERED_CREDIT"][self.language]: self.remove_xa(result_first_row[1].text),
                                             self.word_mapping["ACHIEVE_CREDIT"][self.language]: self.remove_xa(result_first_row[2].text),
                                             self.word_mapping["TOTAL_SCORE"][self.language]: self.remove_xa(result_first_row[3].text)}
                    self.term_info[ft][self.word_mapping["CUMULATIVE"][self.language]] = {self.word_mapping["AVERAGE_SCORE"][self.language]: self.remove_xa(result_first_row[0].text),
                                             self.word_mapping["REGISTERED_CREDIT"][self.language]: self.remove_xa(result_first_row[1].text),
                                             self.word_mapping["ACHIEVE_CREDIT"][self.language]: self.remove_xa(result_first_row[2].text),
                                             self.word_mapping["TOTAL_SCORE"][self.language]: self.remove_xa(result_first_row[3].text)}
    ...
    def get_information(self):
        ...
        # print(tables[::2])
        self.result_all[self.word_mapping["USER_INFOMATION"][self.language]] = self.user_info
        ...
        if self.term_info:
            ...
            for term in self.term_info:
                ...
                for row in self.term_info[term][self.word_mapping["COURSES_LIST"][self.language]]:
                    db = db.append({'Course Id': row[self.word_mapping["COURSE_ID"][self.language]], 'Course Name': row[self.word_mapping["COURSE_NAME"][self.language]], 'Year': year,
                               'Semester': semester, 'Credit': row[self.word_mapping["COURSE_CREDIT"][self.language]], 'Section': row[self.word_mapping["COURSE_SECTION"][self.language]],
                               'Grade': row[self.word_mapping["COURSE_GRADE"][self.language]], 'Grade(Score)': grade_table[row[self.word_mapping["COURSE_GRADE"][self.language]]]}, ignore_index=True)
