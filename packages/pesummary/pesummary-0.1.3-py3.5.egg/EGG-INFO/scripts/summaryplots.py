#!/home/c1737564/virtualenvs/python3.5/bin/python
...
class PlotGeneration(PostProcessing):
    ...
    def generate_plots(self):
        ...
        if self.add_to_existing:
            ...
            """
            approximants = list(existing_file.keys())
            parameters = [[i.decode("utf-8") for i in \
                existing_file["%s/parameter_names" %(j)]] for j in approximants]
            samples = [[i for i in existing_file["%s/samples" %(j)]] \
                for j in approximants]
            existing_file.close()
            for num, i in enumerate(approximants):
                if i not in self.approximant:
                    self.approximant.append(i)
                    self.result_files.append(self.existing+"/samples/posterior_samples.h5")
                    self.samples.append(samples[num])
                    self.parameters.append(parameters[num])
                else:
                    logger.warning("Data for the approximant %s already exists. "
                                   "This approximant is being ignored" %(i))
            """
