...
def reproducible(namespace=None):
    """
        ...
        a string returned by call result of reproduction_string function or its __repr__ method.
        I.e.:
    ...
    def decorator(decorated_class):
        ...
        decorated_class.__repr__ = reproduction_string
...
def reproduction_string(object_):
    return "".join(_repr_dispatcher(object_))


def _repr_dispatcher(object_):
...
def _is_instance_of_namedtuple(object_):
    t = type(object_)
    b = t.__bases__
    ...
    if not isinstance(f, tuple):
        return False
    return all(type(n) in six.string_types for n in f)
