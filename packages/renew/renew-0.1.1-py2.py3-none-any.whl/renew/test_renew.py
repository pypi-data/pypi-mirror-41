...
def test_repr_plain_types(expression):
    assert renew.reproduction_string(eval(expression)) == expression
...
def test_raises_unsupported_signature():
    ...
    with pytest.raises(AttributeError, match=msg):
        renew.reproduction_string(Kws(kak=1))
...
def test_repr_namedtuple():
    ...
    assert renew.reproduction_string(a) == expected_str
    assert eval(renew.reproduction_string(a)) == a
