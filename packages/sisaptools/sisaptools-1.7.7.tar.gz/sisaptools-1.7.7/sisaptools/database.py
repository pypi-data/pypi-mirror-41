...
class Database(object):
    ...
    def get_table_count(self, table, dblink=None):
    ...
    def get_table_partitions(self, table, dblink=None):
        """
        ...
        if self.engine == 'my':
            ...
            if is_merge:
                ...
                tables = [table.replace('`', '') for table in raw_tables]
                sql = "select table_name, table_rows \
                       from information_schema.tables \
    ...
    def get_table_columns(self, table, dblink=None):
        ...
        if self.engine == 'my':
            ...
            owner, table_real = self.get_table_owner(table, dblink=dblink)
            dblink_txt = "@{}".format(dblink.upper()) if dblink else ""
    ...
    def get_column_information(self, column, table, desti='my', dblink=None):
        """
        ...
        conv = {('my', 'date', 'my', 'create'): '{column} {done}',
                ...
                ('ora', 'NUMBER', 'my', 'create'):
                    '{column} int' if not scale else '{column} double',
