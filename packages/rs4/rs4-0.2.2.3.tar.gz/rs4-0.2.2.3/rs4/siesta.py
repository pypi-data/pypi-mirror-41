...
class Resource(object):
	def __init__(self, uri, api, logger, reraise_http_error = True):
		self._api = api
		self._uri = uri
		self._logger = logger
	...
	def __getattr__(self, name):	
		key = self._uri + '/' + name
		return Resource(uri=key, api=self._api, logger = self._logger, reraise_http_error = self._reraise_http_error)		
	...
	def _getresponse(self, resp):
		if resp.status_code == 202:
			...
			resource = Resource(uri=status_uri, api=self._api, logger = self._logger, reraise_http_error = self._reraise_http_error).get()
			retries = 0
			MAX_RETRIES = 3
			...
			return Resource(uri=urlparse(location).path, api=self._api, logger = self._logger, reraise_http_error = self._reraise_http_error).get()			
...
class API(object):
	REQ_TIMEOUT = 60
	def __init__(self, base_url, auth=None, logger = None, reraise_http_error = True):
	...
	def __getattr__(self, name):
		if name in ('get', 'post', 'put', 'patch', 'delete', 'options'):
			r = Resource(uri='', api=self, logger = self.logger, reraise_http_error = self.reraise_http_error)
			return getattr (r, name)
		return Resource(uri=name, api=self, logger = self.logger, reraise_http_error = self.reraise_http_error)
