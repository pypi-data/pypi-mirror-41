...
class DataGraph(KadmosGraph):
    ...
    F_SAMPLES_B2k = 20  # LHS sample factor for BLISS-2000 (N_SAMPLES = F_SAMPLES*number_of_variables)
    ...
    def create_mathematical_problem(self, n_disciplines, coupling_density=None, n_clusters=1, cluster_strength=1,
        ...
        for global_var in range(n_global_var):  # Global design variables
            self.add_node('/data_schema/global_design_variables/x0{0}'.format(global_var + 1),
                          category='variable', label='x0{0}'.format(global_var + 1), instance=1)
        for constraint in range(n_global_constraints):  # Global constraints
            self.add_node('/data_schema/global_constraints/g0{0}'.format(constraint + 1),
                          category='variable', label='g0{0}'.format(constraint + 1), instance=1)
        self.add_node('/data_schema/objective/f',  # Objective
                      category='variable', label='f', instance=1)
        for discipline in range(n_disciplines):
            for local_var in range(n_local_var[discipline]):  # Local design variables
                self.add_node('/data_schema/local_design_variables/x{0}_{1}'.format(discipline + 1, local_var + 1),
                              category='variable', label='x{0}_{1}'.format(discipline + 1, local_var + 1), instance=1)
            for coupling_var in range(n_coupling_var[discipline]):  # Coupling variables
                self.add_node('/data_schema/coupling_variables/y{0}_{1}'.format(discipline + 1, coupling_var + 1),
                              category='variable', label='y{0}_{1}'.format(discipline + 1, coupling_var + 1),
                              instance=1)
            for local_constraint in range(n_local_constraints[discipline]):  # Local constraints
                self.add_node('/data_schema/local_constraints/g{0}_{1}'.format(discipline+1, local_constraint+1),
                              category='variable', label='g{0}_{1}'.format(discipline+1, local_constraint+1),
                              instance=1)
...
class FundamentalProblemGraph(DataGraph):
    ...
    def add_function_problem_roles(self):
        ...
        # Input assertions
        if self.find_all_nodes(subcategory='all problematic variables'):
            raise AssertionError('Problem roles could not be determined. Graph still has problematic variables.')
        ...
        coupled = set()
        post_coupled = set()
        _coupled_node = '__coupled__'
        _des_vars_node = '__des_vars__'

        # First construct the function graph
        fgraph = self.get_fdg()  # fdg: function dependency graph

        # Get the coupled functions
        cycles = nx.simple_cycles(fgraph)
        for cycle in cycles:
            coupled.update(cycle)

        # Merge the coupled functions into one node
        if coupled:
        ...
        g_prob_form['function_ordering'] = {f_roles[0]: [f for f in f_order if f in uncoupled_dvi | uncoupled_dvd],
    ...
    def get_mdg(self, name='MDG'):
        ...
        if mdao_arch == graph.OPTIONS_ARCHITECTURES[0]:  # unconverged-MDA
            ...
            for idx, subsys_functions_dict in enumerate(sa['functions_dicts'][1]):
                ...
                for node in subsys_functions:
                    if mdg.nodes[node]['problem_role'] == self.FUNCTION_ROLES[1]:  # coupled
                        ...
                        mg_function_ordering['coupled'].remove(node)
                        mg_function_ordering['post-desvars'].append(node)
                        mdg.nodes[node]['problem_role'] = self.FUNCTION_ROLES[4]
                    elif mdg.nodes[node]['problem_role'] == self.FUNCTION_ROLES[2]:  # post-coupling
                        mdg.nodes[node]['architecture_role'] = self.ARCHITECTURE_ROLES_FUNS[6]
                        mg_function_ordering['post-coupling'].remove(node)
                        mg_function_ordering['post-desvars'].append(node)
                        mdg.nodes[node]['problem_role'] = self.FUNCTION_ROLES[4]
            ...
            for bnode, targets in balance_nodes.items():
                node_matches = [x for x in ccf_mapping.values() if
...
class MdaoDataGraph(DataGraph, MdaoMixin):
    ...
    def connect_weighted_couplings_objective_function(self, group_idx, couplings):
        ...
        for idx, var1 in enumerate(couplings):
            ...
            self.add_edge(var2, new_function_node, equation_label=eq_lab2)
            self.add_edge(new_scaler_node, new_function_node, equation_label=eq_lab3)
            # TODO: remove later, this is a workaround for testing of SSBJ
            low_bound = -2.
            upp_bound = 2.
            nom_value = 1.
            # TODO: remove later, this is a workaround for testing of SSBJ
            #ref_idx = ['D', 'WE', 'WT', 'Theta', 'ESF', 'L'].index(var_ref_label)
            #low_bound = [-0.73071874,  2.7587791,   2.99509154, -3.42137717, -1.23671802,  4.45046035][ref_idx]
            #nom_value = [0.02605991,  2.82133722,  3.05824303, -3.24180687, -1.1322568,   4.99826503][ref_idx]
            #upp_bound = [0.93145296,  2.88375257,  3.31556474, -2.86580697, -1.02751769,  4.99826503][ref_idx]

            self.mark_as_design_variable(var2, lower_bound=low_bound, upper_bound=upp_bound, nominal_value=nom_value,
                                         global_lb=float('-inf'), global_ub=float('inf'))
            if idx == 0:
        ...
        if converger is None or self.nodes[converger]['architecture_role'] == self.ARCHITECTURE_ROLES_FUNS[1] or \
            ...
            if 'distr_function_ordering' in self.graph:
                ...
                for fun in coupled_functions+post_functions:
                    self.nodes[fun]['architecture_role'] = self.ARCHITECTURE_ROLES_FUNS[6]
                mg_fun_ord[self.FUNCTION_ROLES[1]] = []
                mg_fun_ord[self.FUNCTION_ROLES[2]] = []
                mg_fun_ord[self.FUNCTION_ROLES[4]] += coupled_functions + post_functions
