...
class XDSM(object):
    ...
    def addDep(self, out, inp, style, string, stack=False):
        ...
        if inp[0:5] != 'EMPTY':
            if python_version == 2:
                ...
                inp = unicodedata.normalize('NFKD', inp)
        string = format_string_for_latex(string)
        ...
        for chain_elem in chain_list:
            if chain_elem[0:5] != 'EMPTY':
                if python_version == 2:
                    ...
                    chain_list_new.append(unicodedata.normalize('NFKD', chain_elem))
            else:
    ...
    # noinspection PyMethodMayBeStatic
    def _compilepdf(self, filename, timeout, pdflatex_path=None):
        """Use pdflatex to compile the tex file"""
        def target():
            subprocess.check_output(pdflatex_path + filename + '.tex', shell=True,
                                    stderr=subprocess.STDOUT, cwd=os.path.dirname(filename))
        ...
        if pdflatex_path is None:
            ...
            pdflatex_path = pdflatex_path.strip() + ' '
    ...
    def create(self, file_path, keep_tex_file, compile_pdf, pdflatex_path):
        ...
        if find_executable('latex') is not None:
            ...
            if compile_pdf:
                self._compilepdf(file_name, timeout=10, pdflatex_path=pdflatex_path)
