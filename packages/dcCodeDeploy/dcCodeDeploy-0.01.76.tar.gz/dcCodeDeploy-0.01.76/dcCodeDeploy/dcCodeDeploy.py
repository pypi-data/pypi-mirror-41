...
#        sudo('unlink /data/deploy/pending')
    ...
    with settings(shell_escape=False, warn_only=True, pty=False):
        t=run("ls -t|grep 20|head -2|ls|grep 20|sort -r|head -2|tail -1|xargs -I q sudo ln -s /data/deploy/q /data/deploy/current")
