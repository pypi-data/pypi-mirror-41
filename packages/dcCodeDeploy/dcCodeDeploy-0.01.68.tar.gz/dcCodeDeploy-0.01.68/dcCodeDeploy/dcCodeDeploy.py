...
def rollback_code():
    ...
    with settings(shell_escape=False):
        t=sudo("ls -t|grep 20|head -2|ls|grep 20|sort -r|head -2|tail -1|xargs -I \\% ln -s /data/deploy/\\% /data/deploy/current",shell_escape=False) 
        logger.info("real: %s",t.real_command)
        logger.info("cmd:  %s",t.command)  

        sudo("ls -t|grep 20|head -2|ls|grep 20|sort -r|head -2|tail -1|xargs -I \\% ln -s /data/deploy/\\% /data/deploy/pending",shell_escape=False)

        t=sudo("ls -t|grep 20|head -2|ls|grep 20|sort -r|head -1 |xargs -I \\% rm -rf \\%",shell_escape=False)  
        logger.info("real: %s",t.real_command)
        logger.info("cmd:  %s",t.command)  
