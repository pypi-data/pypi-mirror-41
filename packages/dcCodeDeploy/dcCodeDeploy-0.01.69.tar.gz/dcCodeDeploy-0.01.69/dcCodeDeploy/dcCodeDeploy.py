...
def rollback_code():
    ...
    with settings(shell_escape=False, warn_only=True):
        t=sudo("ls -t|grep 20|head -2|ls|grep 20|sort -r|head -2|tail -1|xargs -I \\% ln -s /data/deploy/\\% /data/deploy/current") 
        logger.info("real: %s",t.real_command)
        logger.info("cmd:  %s",t.command)  

        sudo("ls -t|grep 20|head -2|ls|grep 20|sort -r|head -2|tail -1|xargs -I \\% ln -s /data/deploy/\\% /data/deploy/pending")
