...
from ...nntoolbox.utils import DEMO_DEBUG_MODEL, DEMO_DEBUG_PARALLEL  #, DEMO_DEBUG_ANALYSIS,
...
#     container.markdate = datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S")
    ...
    if not (savingformat in [C.SF_CPYTHON, C.SF_CCODEMULTI]):
        ...
        if not (C.USE_PARALLEL and USE_PARALLEL_MODEL and (max_workers > 0) and not (debug & DEMO_DEBUG_PARALLEL)):
            # cas serie
