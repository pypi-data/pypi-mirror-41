...
class PFSA_CATEGORICAL_FORCASTER(SupervisedLearnerPrimitiveBase[Inputs, Outputs, Params, Hyperparams]):
    """
    ...
    def set_training_data(self, *, inputs: Inputs, outputs: Outputs) -> None:  # type: ignore
        ...
        self._RUNLEN = (train_vs_test['d3mIndex'].loc['TRAIN'])-1
        self._FITLEN = (train_vs_test['d3mIndex'].loc['TEST']) + self._RUNLEN
        print(self._RUNLEN)
        print(self._FITLEN)
    ...
    def fit(self, *, timeout: float = None, iterations: int = None, outputs: Outputs) -> CallResult[None]:
        ...
        xgstr=self._XgenESeSS+' -f '+self._DATAFILE+ ' -k "' + KEY+ '"  -B 0  -E 30 -n 2 -N '+self._NAME_PATH + ' -T symbolic -m -G 10000 -v 0 -A .5 -q -w '+self._MODEL_PATH+run_name +' -l ' + log_File   
