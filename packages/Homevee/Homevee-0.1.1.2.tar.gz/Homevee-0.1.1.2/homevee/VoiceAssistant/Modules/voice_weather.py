...
from homevee.VoiceAssistant.voice_patterns import PATTERN_WEATHER
from homevee.db_utils import get_server_data

def get_pattern(db):
    return PATTERN_WEATHER

def get_label():
    return "weather"

def run_command(username, text, context, db):
    return get_weather(username, text, context, db)

def get_weather(username, text, context, db):
    ...
    search_weekday_word, search_weekday = get_weekday(text, cur_weekday)
    ...
    search_daytime = get_day_time(text)
    ...
    for day in weather_list:
        ...
        if (cur_date.date() == search_date.date()):
            ...
            weather_descs = get_weather_description(weather_ids)
            ...
            print "set context"
            helper.set_context(username, context_keys.GET_WEATHER, {}, db)

    return {'msg_speech': output, 'msg_text': output}

def get_weekday(text, cur_weekday):
    words = text.split(' ')
...
def get_weather_description(codes):
    description_map = {
        #Gewitter
