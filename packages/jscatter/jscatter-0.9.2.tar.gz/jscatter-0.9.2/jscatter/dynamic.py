...
def finiteZimm(t, q, NN=None, pmax=None, ll=None, Dcm=None, Dcmfkt=lambda q: 1., tintern=0., mu=0.5, viscosity=1.,
               Temp=293):
    """
    ...
    Bmn = lambda t, NN, l, mu: np.array(
        [[np.array([abs(n - m) ** (2 * mu) * l ** 2] * len(t))] + spmax(t, NN, n, m, mu, modeamplist, ps)
...
def finiteRouse(t, q, NN=None, pmax=None, ll=None, frict=None, Dcm=None, Wl4=None, Dcmfkt=lambda q: 1., tintern=0.,
                Temp=293):
    """
    ...
    Additional Attributes
     ...
     - tr1 is rotational correlation time or rouse time
...
@formel.memoize()
def _msd_trap(t, Dg, u, rt, gamma=1):
...
def diffusionPeriodicPotential(t, q, u, rt, Dg, gamma=1):
    ...
    sqttrap = lambda q, t, Dg, u, rt, gamma=1: np.exp(-q ** 2 / 6 * (_msd_trap(t, Dg, u, rt, gamma)))
...
def rotDiffusion(t, q, cloud, Dr, lmax='auto'):
    ...
    if isinstance(cloud, (float, int)):
        ...
        NN = 10
        grid = np.mgrid[-R:R:1j * NN, -R:R:1j * NN, -R:R:1j * NN].reshape(3, -1).T
        inside = lambda xyz, R: (np.abs(xyz[:, 0]) / R) ** 2 + (np.abs(xyz[:, 1]) / R) ** 2 + (
                    np.abs(xyz[:, 2]) / R) ** 2 <= 1
    ...
    if blcoh is not None:
        ...
        bjlylmcoh = [np.sum((blcoh * spjn(l, qr) * Ylm(np.r_[-l:l + 1], l, pp, tt).T).sum(axis=0) ** 2) for l in
                     np.r_[:lmax + 1]]
...
# noinspection PyUnusedLocal,PyUnusedLocal,PyUnusedLocal,PyIncorrectDocstring
def resolution(t, s0=1, m0=0, s1=None, m1=None, s2=None, m2=None, s3=None, m3=None, s4=None, m4=None, s5=None, m5=None,
    ...
    w = np.atleast_1d(w)
    Y = np.r_[[a * _gauss(t, m, s) for s, m, a in zip(sigmas, means, amps) if (s is not None) & (m is not None)]].sum(
        axis=0)
...
def diffusionHarmonicPotential_w(w, q, tau, rmsd, ndim=3, nmax='auto'):
    ...
    ::
     ...
     iqt1=js.dL([js.dynamic.time2frequencyFF(js.dynamic.diffusionHarmonicPotential,
                                     'elastic',w=np.r_[-100:100:0.01],dw=0,q=q, rmsd=u0, tau=t0 ,ndim=1) for q in ql])
     p.plot(iqt2)
     p.plot(iqt3)
    ...
    if ndim == 3:
        ...
        def An(q, n):
            ...
            An = np.r_[
                (q * q * u2) ** n[s] / factorial(n[s]), (q * q * u2 / n[~s] * np.e) ** n[~s] / (2 * pi * n[~s]) ** 0.5]
            An *= np.exp(-q * q * u2)
            return An
...
def rotDiffusion_w(w, q, cloud, Dr, lmax='auto'):
    ...
    if not isinstance(lmax, int):
        ...
        lmax = min(max(int(pi * qr.max() / 6. * 2), 7), 100)
