...
def memoize(**memkwargs):
    ...
    During fits it may be faster to calc result for a list of and take from cache.
    ...
    the function for a list of q , put it to cache and take in the fit from there. B is only calculated once.
...
def lognorm(x, mean=1, sigma=1):
    ...
    Returns
    -------
        lognorm distribution : dataArray
...
def voigt(x, center=0, fwhm=1, lg=1, asym=0, amplitude=1):
    ...
    val = amplitude / (afwhm(FWHM, asym) / (2 * np.sqrt(2 * ln2))) / math.sqrt(2 * np.pi) * special.wofz(
        z(afwhm(FWHM, asym))).real
...
def scatteringLengthDensityCalc(composition, density=None, T=293, units='mol', mode='all'):
    ...
    Returns
    -------
        sld : float or list of float corresponding to mode
...
def cstar(Rg, Mw):
    ...
    .. math:: c^* = \frac{ 3M_w }{ 4N_A \pi R_g^3 }
    ...
    """
    cstar = 3. * Mw / (constants.Avogadro * 4 * np.pi * (Rg * 1E-9) ** 3) / 1000  # in g/l
    return cstar
...
def molarity(objekt, c, total=None):
    ...
    Returns
    -------
    molarity : float in mol/liter (= mol/1000cm^3)
...
def rphitheta2xyz(RPT, transpose=False):
    ...
    Returns
    -------
        xyz : array Nx3 with [x,y,z] coordinates
...
def DrotfromT12(t12=None, Drot=None, F0=20e6, Tm=None, Ts=None, T1=None, T2=None):
    ...
    Returns
    -------
        Correlation time or Drot : float
...
def sedimentationProfileFaxen(t=1e3, rm=48, rb=85, number=100, rlist=None, c0=0.01, s=77, Dt=1.99e-11, w=246,
                              Rh=None, visc='h2o', temp=293, densitydif=None):
    """
...
def sedimentationProfile(t=1e3, rm=48, rb=85, number=100, rlist=None, c0=0.01, S=None, Dt=None, omega=246,
                         Rh=10, temp=293, densitydif=0.41, visc='h2o'):
    """
    Approximate solution to the Lamm equation of sedimenting particles
    in centrifuge including bottom equilibrium distribution.
    ...
    t : float or list
        ...
        If list, results at these times is given as dataList.
    rm : float
        Axial position of meniscus in mm.
    rb : float
        Axial position of bottom in mm.
    ...
    Returns
    -------
        concentration profile : dataArray, dataList
            Columns  [position in [mm]; ; conc_meniscus_part; conc_bottom_part]
    ...
    Lipid volume fraction is 33% for 50nm radius (10% for 200 nm) for a bilayer thickness of 6.5 nm. ::
     ...
     omega=g*246/21000
     D100nm=js.formel.sedimentationProfile(t=t1,Rh=50,c0=0.05,omega=omega,rm=58,rb=85,densitydif=0.0033)
     D400nm=js.formel.sedimentationProfile(t=t1,Rh=200,c0=0.05,omega=omega,rm=58,rb=85,densitydif=0.001)
     # plot it
     p[0].plot(D100nm,li=[1,2,-1],sy=0,legend='%s nm-> t=$time s' )
     p[1].plot(D400nm,li=[1,2,-1],sy=0,legend='%s nm-> t=$time s' )
     # pretty up
     ...
     p[0].legend(x=50,y=0.2,charsize=0.5)
     p[1].legend(x=50,y=0.2,charsize=0.5)
     p[0].title('Concentration profile in first 6000s')
    ...
    c2 = lambda tau: -(exp(b * w / eps) / (1 - b)) * erfc(
        (w + 2 * b * (exp(tau / 2.) - 1)) / (2 * sqrt(2 * eps * (exp(tau / 2.) - 1))))
    ...
    # same here
    # c3=lambda tau:(2-b)/(1-b)*exp( (w+2*(exp(tau/2.)-1)*(1-b))/eps)*
    #             erfc( (w+2*(exp(tau/2.)-1)*(2-b)) / (2*sqrt(2*eps*(exp(tau/2.)-1)))  )
    # c3=lambda tau:(2-b)/(1-b)*exp(((w+2*(exp(tau/2.)-1)*(1-b))/eps)+
    #                          np.log(erfc( (w+2*(exp(tau/2.)-1)*(2-b)) / (2*sqrt(2*eps*(exp(tau/2.)-1))))))
    def c3(tau):
        ...
        res = (2 - b) / (1 - b) * exp(
            ((w + 2 * (exp(tau / 2.) - 1) * (1 - b)) / eps) + -xxerfc ** 2 + np.log(erfcx(xxerfc)))
        return res
