...
The scattering intensity I(q) of a single particle with real scattering length densities is calculated.
If the scattering length density is not defined as
e.g. for beaucage model the normalized particle form factor F(q) is calculated.
...
The forward scattering per particle is :math:`I_0=V_p^2(\rho-\rho_{solvent})^2` with particle
volume :math:`V_p` and scattering length density :math:`\rho`.
...
def sphereGaussianCorona(q, R, Rg, Ncoil, coilequR, coilSLD=0.64e-4, sphereSLD=4.186e-4, solventSLD=6.335e-4, d=1):
    ...
    res += 2 * Ncoil * fa_sphere * fa_coil * np.sin(Q * (R + d * Rg)) / (
            Q * (R + d * Rg))  # N times interference between one coil and one sphere
    res += Ncoil * (Ncoil - 1) * (fa_coil * np.sin(Q * (R + d * Rg)) / (
            Q * (R + d * Rg))) ** 2  # interference between one coils with distance R+d*Rg
...
def sphereCoreShellGaussianCorona(q, Rc, Rs, Rg, Ncoil, thicknessCoils, coilSLD=0.64e-4, coreSLD=4.186e-4,
    ...
    res = fa_coreshell.Y ** 2  # core shell scattering
    res += Ncoil * fa_coil ** 2  # N * coil scattering
    res += 2 * Ncoil * fa_coreshell.Y * fa_coil * np.sin(Q * (Rs + d * Rg)) / (
            Q * (Rs + d * Rg))  # N times interference between one coil and one sphere
    res += Ncoil * (Ncoil - 1) * (fa_coil * np.sin(Q * (Rs + d * Rg)) / (
            Q * (Rs + d * Rg))) ** 2  # interference between one coils with distance R+d*Rg
...
def multiShellSphere(q, shellthickness, shellSLD, solventSLD=0):
    ...
    #  Volume  *  formfactor
    fflin = lambda qr, r: 4 / 3. * np.pi * r * r * r * 3. * r * (
            2. * qr * np.sin(qr) - (qr * qr - 2.) * np.cos(qr)) / qr / qr / qr / qr  # lin increasing part
    ffsph = lambda qr, r: 4 / 3. * np.pi * r * r * r * 3. * (
            np.sin(qr) - qr * np.cos(qr)) / qr / qr / qr  # constant part
...
# noinspection PyUnusedLocal
def _scattering_Debye(q, r, blength, formfactor):
    """
...
def cloudScattering(q, cloud, relError=50, V=0, formfactor=None, rms=0, ffpolydispersity=0, ncpu=0):
    ...
    - By default explicit spherical is done. If rms and polydispersity are not needed the Debye-function
      can be used (for particle numbers<500 faster).
    ...
    relError : float
        Determines calculation method.
         ...
         - relError=0   The Debye equation is used (no asymmetry factor beta, no rms, no ffpolydispersity).
                        Computation of Order :math:`N^2` opposite to above which is order :math:`N`.
    ...
    if relError == 0:
        if useFortran:
            ...
            result = dA(np.c_[q, [_scattering_Debye(qx, cloud, blength, formfactor) / sumblength ** 2 for qx in q]].T)
...
# noinspection PyUnusedLocal,PyUnusedLocal
def ellipsoid(q, Ra, Rb, SLD=1, solventSLD=0, alpha=None, tol=1e-6, beta=False):
    r"""
...
def multilayer(q, layerd, SLD, gaussw=None, solventSLD=0):
    ...
    **How to use in a fit model**.
    ...
    contrastprofile = np.zeros_like(x)
