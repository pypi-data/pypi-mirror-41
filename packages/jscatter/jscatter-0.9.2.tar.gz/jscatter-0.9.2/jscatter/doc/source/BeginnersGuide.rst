...
For convenience we use Ipython (colors highlighting, history, ...), start it typing "ipython" in your shell.

Optionally Jscatter can also be run in an environment like Jupyter.

In a shell two simple things help the user.
...
- js.showDoc() opens the local documentation in a web browser.
...
::
 ...
 # define model, q will get a list of values (numpy array)
...
- Use  a model from the libraries and change it to your needs.
...
with a polydispersity in radius ::
 ...
 def psphere(q,R,A,bgr,dR):
    ...
    result=A*js.formel.pDA(js.ff.sphere,dR,'radius',q=q,radius=R).Y+bgr
    return result
