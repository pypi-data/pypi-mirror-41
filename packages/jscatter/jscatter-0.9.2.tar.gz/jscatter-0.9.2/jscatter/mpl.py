...
def contourImage(x, y=None, z=None, levels=8, fontsize=10, colorMap='jet', scale='norm', lineMap=None, title=None,
    ...
    if np.ndim(x) < 2 or hasattr(x, '_isdataArray'):
        if hasattr(x, '_isdataArray'):
            x, y, z = x.X, x.Z, x.Y
        # regrid tries to make regular grid (matrixlike data in x,y,z coordinates without missing data)
        # returns None if this fails
        if np.unique(x).shape[0] * np.unique(y)[0] == np.shape(z)[0]:
            ...
            norm.autoscale(z)
            im = ax.tricontour(x, y, z, levels=levels, linewidths=1, cmap=lmap, extent=extend, origin=origin, norm=norm)
            im2 = ax.tricontourf(x, y, z, levels=levels, cmap=cmap, extent=extend, origin=origin, norm=norm)
            im3 = ax.plot(x, y, 'ko', ms=1)
            fig.colorbar(im2)  # note that colorbar is a method of the figure, not the axes
