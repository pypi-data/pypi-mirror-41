...
def _readfile(xfile, encoding=None):
    ...
    """
    if isinstance(xfile, list) and all([isinstance(zz, basestring) for zz in xfile]):
        # a list of strings
        return xfile
    try:
...
def _read(xfile, block=None,
    ...
    elif isinstance(takeline, basestring):
        # single word
...
def _maketxt(dataa, name=None, fmt='%.5e'):
    ...
    tail = []
    partxt = []
    comment = [dataa.comment] if isinstance(dataa.comment, basestring) else dataa.comment
    ...
    for parameter in dataa.attr:
        ...
        if isinstance(dataapar, dataArray):
            ...
            continue
        if isinstance(dataapar, basestring):
...
def inheritDocstringFrom(cls):
    ...
    def docstringInheritDecorator(fn):
        if isinstance(fn.__doc__, basestring):
            ...
            for ff in filelist:
                ...
                if inputz[0]['para']['@name'] == 'header_of_common_parameters':
                    for k, v in inputz[0]['para'].items():
                        setattr(self, k, v)
                    inputz = inputz[1:]
                if isinstance(inputz, basestring):
                    print(inputz)
                else:
    ...
    def fit(self, model, freepar={}, fixpar={}, mapNames={}, method='leastsq', xslice=slice(None), condition=None,
        ...
        **Fit result attributes** ::
        ...
        if ier not in [True, 1, 2, 3, 4]:  # NOT successful fit
            ...
            if output: print(CSIr + 'last result : ' + CSIe)
            i = 0
...
class dataArrayBase(np.ndarray):
    ...
    def __new__(cls, input=None,
        ...
        **Rules for reading of ASCII files**
        ...
        """
        if isinstance(input, basestring):  # if a filename is given
    ...
    def __array_wrap__(self, out_arr, context=None):
        ...
        # some ufuncs return context as 3-element tuple: (name, argument, domain) of the ufunc.
        #
    ...
    def regrid(self, xgrid=None, zgrid=None, wgrid=None, method='nearest', fill_value=0):
        ...
        Add missing points in a regular grid (e.g. an image)
        or interpolate on a regular grid from irregular arranged data points.
        For 1D data use *.interpolate*
        ...
        if xgrid is None:
            try:
                ...
                raise Exception('No X column defined. ')
        if zgrid is None:
            try:
                zgrid = np.unique(self.Z)
            except AttributeError:
                raise Exception('No second dimension (Z column) defined. ')
        if wgrid is None:
            try:
        ...
        if wgrid is None:
            ...
            zz = zz.flatten()
            # ww = None
            f = griddata(points=self[[self._ix, self._iz]].T, values=self[self._iy],
                         xi=(xx, zz), method=method, fill_value=fill_value)
            if hasattr(self, '_iey'):
                # linear interpolate the errors if present
                fe = griddata(points=self[[self._ix, self._iz]].T, values=self[self._iey],
            ...
            zz = zz.flatten()
            ww = ww.flatten()
            f = griddata(points=self[[self._ix, self._iz, self._iw]].T, values=self[self._iy],
    ...
    def fit(self, model, freepar={}, fixpar={}, mapNames={}, xslice=slice(None), condition=None, output=True, **kw):
        ...
        for key, value in freepar.items():
            free[key] = (value if isinstance(value, (int, float, np.integer, np.float)) else value[0])
        for key, value in fixpar.items():
            fix[key] = (value if isinstance(value, (int, float, np.integer, np.float)) else value[0])
        if 'debug' in kw:
