...
class DomainAncillaries(with_metaclass(abc.ABCMeta, Container)):
...
.. versionadded:: 1.7.0
    ...
    def __init__(self, domain_ancillaries=None, source=None,
                 copy=True):
        '''**Initialization**
...
:Parameters:
    ...
    domain_ancillary: `str`
        TODO The name of the domain ancillary to be deleted.

        *Parameter example:*
           ``domain_ancillary='domainacnillary2'``

    default: optional
        TODO Return *default* if the domain ancillary has not been set.

:Returns:

        TODO The removed domain. If the property has not been then the
