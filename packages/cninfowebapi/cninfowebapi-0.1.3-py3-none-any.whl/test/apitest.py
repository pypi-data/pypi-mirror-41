...
if __name__ == "__main__":
    # 设置保存文件的类型 SaveType.JSON SaveType.CSV SaveType.EXCEL 文件默认保存到cninfowebapi/data/目录
    datamart.setSaveType(SaveType.NONE)
    # 根据关键词在API描述或返回字段名、字段描述查找API
    #datamart.apisearch("资产负债表")
    # API帮助：查看API详细元数据信息
    datamart.apihelp('p_stock2301')
    # API调用方式一
    dfprofit = getprofit(scode='000001', rdate='20180630')
    # API调用方式二
