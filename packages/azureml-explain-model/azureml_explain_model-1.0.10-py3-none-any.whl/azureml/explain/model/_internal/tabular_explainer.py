...
from .common import module_logger, _sort_features, _sort_feature_list_single, _sort_feature_list_multiclass
from .base_explainer import BaseExplainer
from .model_summary import ModelSummary
from .constants import Attributes, Defaults, ExplainType, History, SKLearn
...
class TabularExplainer(BaseExplainer):
    ...
    def __init__(self, workspace=None, experiment_name=None, run_id=None):
        ...
        if workspace is not None:
            self.assets_client = AssetsClient.create(workspace)
    ...
    def explain_model(self, model, initialization_examples, evaluation_examples=None,
        ...
        if self.run is not None:
            ...
            self._upload_artifact(upload_dir, History.SHAP_VALUES, shap_values)
            if expected_values is not None:
            ...
            if classification:
                ...
                # TODO update these for new asset schema structure??
    ...
    def _compute_summary(self, shap_values, classification, features, top_k):
        ...
        if classification:
            ...
            global_importance_values = global_importance_values[global_importance_rank]
        if features is not None:
            global_importance_names = _sort_features(features, global_importance_rank)
            if classification:
