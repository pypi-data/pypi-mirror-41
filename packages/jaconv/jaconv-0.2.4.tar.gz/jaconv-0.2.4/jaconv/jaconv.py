...
def kana2alphabet(text):
    ...
    text = text.replace('てぃ', 'thi').replace('とぅ', 'twu').replace('とぉ', 'two')
...
def alphabet2kana(text):
    ...
    text = text.replace('thi', 'てぃ').replace('twu', 'とぅ')
...
def extract_vowel(text):
    alphabets = kana2alphabet(text)
    alphabets = re_vowels.sub('', alphabets)
