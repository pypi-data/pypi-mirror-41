...
class Storage(BaseStorage):
    ...
    def put(self, bytes, callback=None):
        ...
        content_type = self.get_content_type(buff)
    ...
    def get(self, callback):
        ...
        try:
            ...
            result.buffer = buff
            result.metadata = {
                'Content-Type': objkey.get_metadata("Content-Type"),
                'Content-Disposition': objkey.get_metadata("Content-Disposition"),
                'Cache-Control': objkey.get_metadata("Cache-Control")
            }
    ...
    def get_content_type(self, buff, name):
        try:
            ...
            mimetypes.init()
            return mimetypes.types_map['.'.join(name_segments.split('.')[1:])]