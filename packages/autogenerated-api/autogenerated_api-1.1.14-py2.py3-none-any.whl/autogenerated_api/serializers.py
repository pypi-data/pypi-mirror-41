...
def DeadlySerializerFactory(model, fields=None, exclude=None, nest=[], nested_fields={}, force_nesting=False, exclude_nest=[]):
    ...
    class ModelDeadlySerializer(object):
        ...
        def postprocess_values_list(self, dict_list, fields, related_model):
            ...
            for f in fields:
                field_ref = related_model._meta.get_field_by_name(f)[0]
        ...
        def load_fields(self):
            ...
            if ModelDeadlySerializer.model_related_foreignkeys is None:
                ModelDeadlySerializer.model_related_foreignkeys = set([field.name for field in model._meta.get_all_related_objects() if field.one_to_many])
            ...
            if ModelDeadlySerializer.model_reverse_o2o_fields is None:
                ModelDeadlySerializer.model_reverse_o2o_fields = set([field.name for field in model._meta.get_all_related_objects() if field.one_to_one])
            ...
            if ModelDeadlySerializer.model_reverse_m2m_fields is None:
                ModelDeadlySerializer.model_reverse_m2m_fields = set([field.name for field in model._meta.get_all_related_many_to_many_objects()])
            ...
            if type(queryset) is list:
                for item in queryset:
                    ...
                    for f in self.local_concrete_fields:
                        if model._meta.get_field_by_name(f)[0].is_relation:
            ...
            if self.force_nesting:
                for x in self.fields:
                    ...
                    if x in ModelDeadlySerializer.model_relational_fields:
                        fk_field = model._meta.get_field_by_name(x)[0]
