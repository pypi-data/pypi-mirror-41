...
class LbCIService(BaseService):
    ...
    def getEnvKitUrl(flavour, platform, version, url=None):
        if url:
            return url
        return "http://lhcb-rpm.web.cern.ch/lhcb-rpm/lbenv-kits/" \
               "lbenv-kit.%s.%s.%s.tar.bz2" % (flavour, platform, version)

    @staticmethod
    def getEnvKitPrefix(flavour, platform):
        env_kit_top_dir = '/cvmfs/lhcbdev.cern.ch/new-env'
