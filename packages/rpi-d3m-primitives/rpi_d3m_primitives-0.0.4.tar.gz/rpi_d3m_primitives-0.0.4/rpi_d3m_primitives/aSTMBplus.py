...
class Hyperparams(hyperparams.Hyperparams):
    threshold = hyperparams.Hyperparameter[Union[int, float]](
        default=0.02,
        description="threshold for independence test",
        semantic_types=['https://metadata.datadrivendiscovery.org/types/TuningParameter']
    )


class aSTMBplus(SupervisedLearnerPrimitiveBase[Inputs, Outputs, Params, Hyperparams]):
    """
    A primitive which select a set of features condition on which the target is independent of other features
    """

    metadata = metadata_base.PrimitiveMetadata({
        'id': '6961e9ad-0272-4ecc-82c4-c28c8345a1fb',
        'version': '2.1.5',
        'name': 'aSTMBplus feature selector',
        'keywords': ['Markov Blanket Discovery','Feature Selection'],
        'description':'This algorithm will learn a Markov Blanket of the target node with adaptive mutual information estimation. Nodes in the learned Markov Blanket will be the selected features',
        'source': {
        ...
        'python_path': 'd3m.primitives.feature_selection.adaptive_simultaneous_markov_blanket.Manual_RPI',
        'algorithm_types': [
    ...
    def fit(self, *, timeout: float = None, iterations: int = None) -> None:
        ...
        model = ASTMB(Trainset, discTrainset,self._problem_type)
