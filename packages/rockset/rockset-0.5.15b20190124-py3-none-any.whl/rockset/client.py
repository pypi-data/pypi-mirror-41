...
class Client(object):
    ...
    def __init__(self, api_key=None, api_server=None,
                 profile=None, driver=None, **kwargs):
    ...
    def call_api(self, resource, method, *args, **kwargs):
        try:
            ...
            if e.status == 401:
                raise AuthError(response=e.body) from None
            elif e.status == 403 or e.status == 413 or e.status == 429:
                raise LimitReached(response=e.body) from None
            elif e.status == 400:
                body = json.loads(e.body)
                raise InputError(
                    code='400', message=body['message'], type=body['type']
                ) from None
            elif e.status == 501:
                raise NotYetImplemented(response=e.body) from None
            elif e.status == 503 or e.status == 504:
                raise TransientServerError(response=e.body) from None
            elif e.status == 502:
                ...
                raise InputError(response=e.body) from None
            elif e.status >= 500 and e.status < 530:
                raise ServerError(response=e.body) from None
            elif e.status == 530:
    ...
    def retrieve(self, name):
        ...
        """
        return Collection.retrieve(name=name, client=self)
...
class ClientCollection(object):
    ...
    def list(self):
        return Collection.list(client=self.client)

    def retrieve(self, name):
        return Collection.retrieve(name=name, client=self.client)
