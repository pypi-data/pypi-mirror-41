...
def _response_errmsg(response, default='unexpected error occurred'):
    m = None
    try:
        if type(response) is str:
            # response from apiserver is string json format
            response = json.loads(response)
            m = response.get('message', None)
        else:
            # requests.Response object
            m = response.json().get('message', None)
    except:
        pass
    if m is None:
        # Http Error like 502 are not json
        if response.status_code is not None:
            m = '{} {}'.format(response.status_code, response.reason)
    if m is None:
        m = default
    return m

def _response_errtype(response, default='Error'):
    t = None
    try:
        if type(response) is str:
            response = json.loads(response)
            t = response.get('type', None)
        else:
            t = response.json().get('type', None)
    except:
        pass
    if t is None:
        t = default
...
def _response_errcode(response, default='?'):
    c = None
    try:
        if type(response) is str:
            response = json.loads(response)
            c = response.get('code', None)
        else:
            c = response.json().get('code', None)
    except:
        pass
    if c is None:
...
class HTTPError(Error):
    ...
    def __init__(self, **kwargs):
        kwargs['response'] = kwargs.get('response', None)
        if kwargs['response'] is not None:
            r = kwargs['response']
            kwargs['code'] = _response_errcode(r)
            kwargs['message'] = _response_errmsg(r)
            kwargs['type'] = _response_errtype(r)
        kwargs['message'] = kwargs.get('message', 'Unexpected error from server')
