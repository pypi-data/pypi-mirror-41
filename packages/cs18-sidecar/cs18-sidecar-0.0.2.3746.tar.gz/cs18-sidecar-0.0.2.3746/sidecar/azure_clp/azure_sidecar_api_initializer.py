...
class AzureSidecarApiInitializer:
    ...
    def initialize(self):
        ...
        self._logger.error(f"appdata : {azure_credentials_provider.management_resource_group } "
                           f" APPKEY: {azure_credentials_provider.application_key} "
                           f"subscription_id: {azure_credentials_provider.subscription_id} "
                           f"appid: {azure_credentials_provider.application_id} "
                           f"tenant: {azure_credentials_provider.tenant}")

        azure_clients_manager = AzureClientsManager(credentials_provider=azure_credentials_provider)
        # todo gil remove cosmos
        # data_store_service = AzureCosmosService(clients_manager=azure_clients_manager)
