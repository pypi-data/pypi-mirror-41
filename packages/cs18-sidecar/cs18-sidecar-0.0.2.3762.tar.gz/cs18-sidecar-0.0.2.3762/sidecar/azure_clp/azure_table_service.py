...
class AzureTableService(DataStoreService):
    ...
    def create_table(self, table_name: str = None):
        ...
        # if not self.table_service.exists(table_name=table_name):
        self.table_service.create_table(table_name=table_name if table_name is not None else self.table_name,
                                        fail_on_exist=False)

    def insert_data(self, data: dict, data_id: str = None):
        entity = self._to_entity(data=data, data_id=data_id)
        self.table_service.insert_entity(table_name=self.table_name,
                                         entity=entity)
    ...
    def find_data_by_ids(self, id_column_name: str, ids: List[str] = None) -> List[dict]:
        ...
        query_entities_gen = self.table_service.query_entities(table_name=self.table_name,
                                                               filter=ids_filter)
    ...
    def delete_data_by_id(self, data_id: str) -> bool:
        entity = self.table_service.delete_entity(table_name=self.table_name,
                                                  partition_key=self.default_partition_key,
                                                  row_key=data_id)
