...
class AzureCosmosService:
    ...
    def find_document_by_id(self, id: str, db: str = default_db_name,
                            collection: str = default_sandboxes_collection_name) -> Dict:
        return self._get_collection(collection, db).find_one({'_id': id})

    def update_document(self, id: str,
                        update_param: dict,
                        db: str = default_db_name,
                        collection: str = default_sandboxes_collection_name):
        self._get_collection(collection, db).update_one(filter={'_id': id}, update=update_param)