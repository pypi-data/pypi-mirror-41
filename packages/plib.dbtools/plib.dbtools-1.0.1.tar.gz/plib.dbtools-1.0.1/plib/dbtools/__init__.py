...
def maybe(t, null_values=None, null_fields=None,
          null_default_values=(None,), null_default_fields={bool: False, int: 0, unicode: u"", str: "", float: 0.0}):
...
class DBInterface(object):
    ...
    def get_tablespecs(cls, tablename):
        ...
        return (
            tables[tablename] if isinstance(tablename, basestring) else
            list(OrderedDict(
                (fieldspec, None)
                for table in tablename
    ...
    def create_tables(self, clear=False, commit=True, verbose=True):
        ...
        for tablename, fields in tables.iteritems():
    ...
    def _index_sql(self, tablename, columns, primary=False):
        return "ALTER TABLE {0} ADD {1} ({2});".format(
            tablename,
            "PRIMARY KEY" if primary else "INDEX",
            columns if isinstance(columns, basestring) else ", ".join(columns)
        )
    
    @cursor_method
    def create_index(self, cursor, tablename, columns, primary=False, commit=True, verbose=True):
        sql = self._index_sql(tablename, columns, primary)
        if verbose:
            print sql
        result = cursor.execute(sql)
        if commit:
    ...
    def _create_indexes(self, index_map, primary, commit=True, verbose=True):
        results = []
        for tablename, indexspec in index_map.iteritems():
            indexes = (indexspec,) if isinstance(indexspec, basestring) else indexspec
