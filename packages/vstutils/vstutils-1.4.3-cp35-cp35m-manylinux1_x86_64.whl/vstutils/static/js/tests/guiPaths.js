...
guiTests.createUser=function(env,pk_obj){let path='/user/new/';let password=rundomString(6);let fieldsData={username:{value:rundomString(6)},email:{value:rundomString(6)+"@gmail.com"},password:{value:password,do_not_compare:true},password2:{value:password,do_not_compare:true},};guiTests.openPage(path);guiTests.setValuesAndCreate(path,fieldsData,(data)=>{env.user_id=data.id;env.user_name=data.username;pk_obj.api_user_id=data.id;},true);}
