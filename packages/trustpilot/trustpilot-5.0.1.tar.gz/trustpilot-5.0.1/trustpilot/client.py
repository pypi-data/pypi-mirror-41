...
def disable_ssl_warnings():
    try:
        ...
        logging.error("Error importing urllib3 when disabling its logging")
...
class TrustpilotSession(requests.Session):
    ...
    def setup(self, api_host=None, api_key=None, api_secret=None,
        ...
        try:
            ...
            self.username=username or environ['TRUSTPILOT_USERNAME']
            self.password=password or environ['TRUSTPILOT_PASSWORD']
            self.access_token=access_token
    ...
    def _request_auth_headers(self):
        self.access_token = auth.request_new_access_token(self)
    ...
    def _post_request_callback(self, response, *args, **kwargs):
        ...
        if retry and response.status_code == requests.codes.unauthorized:
            ...
            req.headers.update(self._request_auth_headers())
