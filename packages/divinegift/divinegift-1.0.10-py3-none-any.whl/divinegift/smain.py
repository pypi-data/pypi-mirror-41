...
class Settings:
    ...
    def initialize_cipher(self):
        ...
        write_key('ck', self.cipher_key)

    def encrypt_password(self, conn_name):
        if not self.cipher_key:
            self.cipher_key = read_key('ck')
            if not self.cipher_key:
                self.initialize_cipher()
        self.settings[conn_name]['db_pass'] = encrypt_str(self.settings[conn_name]['db_pass'], self.cipher_key)

    def decrypt_password(self, conn_name):
        if not self.cipher_key:
            self.cipher_key = read_key('ck')
        self.settings[conn_name]['db_pass'] = decrypt_str(self.settings[conn_name]['db_pass'], self.cipher_key)
