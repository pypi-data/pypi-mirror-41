...
from orchestrate.core.docker.service import DockerLoginCredentials
...
class AWSCluster(Cluster):
  ...
  def get_registry_login_credentials(self, repository):
    registry_id = self.services.ecr_service.ensure_repositories([repository])['repositories'][0]['registryId']
  ...
  def generate_image_tag(self, repository):
    if self.registry is not None:
      return safe_format('{}/{}', self.registry, repository)

    descriptions = self.services.ecr_service.describe_repositories([repository])
