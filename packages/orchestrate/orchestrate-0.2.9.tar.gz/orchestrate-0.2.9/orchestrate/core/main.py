...
class OrchestrateController(ArgparseController):
  ...
  def runner(self, local=False, quiet=False):
    ...
    repository_name = options.get('repository')
    aws_ecr_repository_name = options.get('aws', {}).get('ecr', {}).get('repository')
    if repository_name and aws_ecr_repository_name:
      raise OrchestrateException('aws.ecr.repository is deprecated, please use repository')

    repository_name = repository_name or aws_ecr_repository_name
    ...
    if local:
      ...
      target = cluster.generate_image_tag(repository=repository_name)
      ...
      self.app.services.docker_service.push(target, quiet=quiet)
...
class OrchestrateClusterController(ArgparseController):
  ...
  def create(self):
    ...
    print(self.app.services.aws_service.node_access_instructions(cluster_name))

  @expose(
    help='destroy a cluster',
    arguments=[
      (['--cluster-name', '-n'], dict(help='cluster name', required=True)),
      # TODO: make required in 0.3.0
      (['--provider'], dict(help='The provider used to create this cluster')),
      (['--cloud-provider'], dict(help='The cloud provider used to create this cluster')),
    ],
  )
  def destroy(self):
    cluster_name = self.app.pargs.cluster_name
    cloud_provider_string = self.app.pargs.cloud_provider
    provider_string = self.app.pargs.provider

    if cloud_provider_string is not None and provider_string is not None:
      raise OrchestrateException('Cannot give both --cloud-provider and --provider arguement')

    # TODO(alexandra): when provider is required remove aws default
    provider_string = provider_string or cloud_provider_string or 'aws'
