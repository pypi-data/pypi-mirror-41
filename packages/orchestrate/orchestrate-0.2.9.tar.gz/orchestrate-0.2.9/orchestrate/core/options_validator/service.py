...
class OptionsValidatorService(Service):
  def validate_orchestrate_options(
    ...
    if run:
      assert is_string(run) or (is_string_sequence(run) and any(run)), safe_format(
        'run is not a string or a list of strings: {}',
        run
      )
    else:
    ...
    # TODO(alexandra): make repository a required field in 0.3.0
    assert repository is None or is_string(repository) and len(repository) > 0, safe_format(
  ...
  def validate_aws_for_orchestrate(
    ...
    if ecr:
      assert is_mapping(ecr), safe_format('aws.ecr is not a mapping: {}.', ecr)
