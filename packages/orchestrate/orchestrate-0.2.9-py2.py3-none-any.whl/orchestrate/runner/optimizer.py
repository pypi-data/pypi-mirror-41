...
class SigOptOptimizer(BaseOptimizer):
  ...
  def is_experiment_running(self):
    ...
    observation_count = experiment.progress.observation_count
    open_suggestion_count = self.conn.experiments(self.experiment_id).suggestions().fetch(state="open", limit=0).count
    return observation_count + open_suggestion_count < experiment.observation_budget
