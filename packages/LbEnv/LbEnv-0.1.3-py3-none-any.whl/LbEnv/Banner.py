...
def generate(width=80, env=None):
    ...
    if 'CMTCONFIG' in env:
        lines.append('*%s*' % _center(
            'Using system configuration %s' % env['CMTCONFIG'], width - 2))
    lines.append('*' * width)
    if 'User_release_area' in env:
        lines.append(' --- User_release_area is set to %s' %
                     env['User_release_area'])
    if env.get('LHCBPROJECTPATH'):
        lines.append(' --- LHCBPROJECTPATH is set to:')
        lines.extend('    %s' % item
                     for item in env['LHCBPROJECTPATH'].split(pathsep))
    else:
        lines.append(' --- LHCBPROJECTPATH is NOT set!')
    lines.append('-' * width)
    return '\n'.join(lines)
