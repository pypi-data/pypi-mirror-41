...
class SetupProject(object):
    ...
    def _prepare_parser(self):
        ...
        def nightly_option(_option, opt_str, _value, _parser):
            ...
            if os.path.exists(
                ...
                extraCMTPROJECTPATH = filter(str,
    ...
    def _prepare_tmp_local_project(self):
        ...
        for e in exts + self.args + self.ext_versions.keys(
        ) + self.site_externals:
    ...
    def _gen_setup(self, root_dir):
        ...
        for k in [
                k for k in new_env.keys()[:]  # I need a copy of the keys
...
def expandAllVars(env, iterations=10):
    ...
    new_env = dict(
        (key, expandVars(value, env)) for key, value in env.iteritems())
