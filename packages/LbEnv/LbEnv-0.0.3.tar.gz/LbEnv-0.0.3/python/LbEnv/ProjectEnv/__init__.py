...
class SearchPath(object):
    ...
    def __iter__(self):
        ...
        return ifilter(None, chain.from_iterable(
            str(entry).split(os.pathsep) for entry in self.entries
        ))
    ...
    def toCMake(self):
        ...
        return '\n'.join(
            _toEntry(entry).toCMake() for entry in self.entries[::-1])

    def toCMT(self, shell='sh'):
        # we need to add them in reverse order because the 'prepend'
        return '\n'.join(
            _toEntry(entry).toCMT(shell) for entry in self.entries[::-1])
