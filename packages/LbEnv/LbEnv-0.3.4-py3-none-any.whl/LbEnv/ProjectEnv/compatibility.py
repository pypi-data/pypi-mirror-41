...
class TemporaryEnvironment:
    ...
    def restore(self):
        ...
        """
        for key, value in list(self.old_values.items()):
            if value is None:
                del self.env[key]
            else:
                self.env[key] = value
...
def isProject(path, ignore_not_ready=False, cvmfs=True):
    ...
    if cvmfs:
        ...
        try:
            ...
            if v.startswith(p + "_"):
                v = v[len(p) + 1:]
                return lhcb_style_version.match(v) or lcg_style_version.match(
                    v)
            else:
...
def _extract_version(project, version):
    ...
    if version.startswith(project):
        if len(version) > len(
                project):  # the version directory could be == project
...
def FindProjectVersions(project,
                        search_path,
                        user_area=None,
    ...
    if user_area and os.path.isdir(user_area):
        user_projects = [
            d for d in os.listdir(user_area)
    ...
    for d in search_path:
        ...
        if os.path.isdir(p):
            if isProject(p, ignore_not_ready, cvmfs=True):
                ...
                versions += [(project, _extract_version(
                    PROJECT, v), os.path.join(
                        PROJECT, v), d) for v in os.listdir(p) if isProject(
                            os.path.join(p, v), ignore_not_ready, cvmfs=True)]
    return versions
...
def FindLCGForROOT(version, versions):
    ...
    for req, vers in [(join(base, name, 'LCG_Configuration', 'cmt',
                            'requirements'), vers)
...
class ProjectInfo:
    ...
    no_platform_projects = [
        'Compat', 'LbScripts', 'LCGCMT', 'ExtraPackages', 'LHCbDirac',
        'LHCbGrid'
    ...
    def __init__(self, project, version, realName, path):
        ...
        if self.version and self.sys:
            self.syscmtfullpath = os.path.join(self.project_dir, self.sys,
                                               self.version, 'cmt')
        else:
    ...
    def _container(self):
        ...
        """
        container = None
        project_cmt = os.path.join(self.path, self.realName, "cmt",
                                   "project.cmt")
    ...
    def __str__(self):
        if self.version:
            return "%s %s from %s" % (self.name, self.version,
                                      self.project_dir)
        else:
    ...
    def supportsPlatform(self, platform, user_area=None):
        ...
        return (self.path == user_area
                or self.name in self.no_platform_projects or os.path.isdir(
                    os.path.join(self.project_dir, 'InstallArea', platform)))
...
def makeProjectInfo(project=None,
                    version=None,
                    versions=None,
                    search_path=None,
                    user_area=None,
    ...
    if versions is None:
        if not project:
            raise TypeError(
                "makeProjectInfo() requires either 'project' or 'versions'")
...
if sys.platform.startswith("win"):
    ...
    _prepare_cmt_cmd = lambda cmd, args: \
                        " ".join(["cmt", cmd] + ['"%s"' % s for s in args])
...
def cmt(command, args=[], environment=None, cwd=None):
    ...
    return Popen(
        cmd, shell=True, env=environment, cwd=cwd, stdout=PIPE,
...
def _get_cmt_user_context(env=None):
    ...
    if env['LHCb_release_area']:
        ...
        if os.access(context_file, os.F_OK | os.R_OK):
            ...
            for l in open(context_file):
                m = re.match(
                    "^#.*created.*from.*LCGCMT_(([0-9]+)([a-z]?)|HEAD)", l)
                if m:
    ...
    if not current_lcgcmt:
        ...
        log.debug(
            "----- get_cmt_user_context(): LCGCMT not found, use %s -----",
            context_path)
        return context_path
    ...
    if int(context_validity[1]) > int(current_lcgcmt[1]):
        ...
        log.debug("----- get_cmt_user_context(): use %s -----", context_path)
        return context_path
    elif (context_validity[1] == current_lcgcmt[1]) and (context_validity[2] >
                                                         current_lcgcmt[2]):
...
class SetupProject(object):
    ...
    def set_build_env(self):
        ...
        if not which('cmt'):
            self._debug('"cmt" not in the default environemnt, '
                        'I look for it...')
            ...
            cmt_cmd = which(
                ...
                path=os.pathsep.join(
                    ...
                    bin_path([os.path.dirname(d)
                              for d in self.search_path if d.endswith('lhcb')],
        ...
        if self.project_info.policy == 'old':  # old style project
            ...
            ShellParser[self.shell](
                self.cmt("setup", "-" + self.shell,
                         self.project_info._projectenv_cmt_dir), localEnv)
            ...
            self._debug("----- CMTPATH set to '%s' -----",
                        self.environment['CMTPATH'])
        else:
            self.environment['CMTPROJECTPATH'] = os.pathsep.join(
                self.search_path)
            self._debug("----- CMTPROJECTPATH set to '%s' -----",
    ...
    def _prepare_parser(self):
        ...
        class MyHelpFormatter(BasicHelpFormatter):
            ...
            def format_option_strings(self, option):
                ...
                if option.takes_value():
                    ...
                    short_opts = [
                        sopt + " " + metavar for sopt in option._short_opts
                    ]
                    long_opts = [
                        lopt + " " + metavar for lopt in option._long_opts
                    ]
                else:
        ...
        parser = MyOptionParser(
            usage=
            "%prog [options] <project_name> [version|--ask] [options] [externals]",
            version=__version__,
        ...
        parser.add_option(
            ...
            action="store_true",
            help=
            "ask for the version of the project to use (overrides the version specified)"
        )
        parser.add_option(
            "--disable-CASTOR",
            action="store_true",
        ...
        parser.add_option(
            "--tag_add", action="append", help="specify extra CMT tags")
        parser.add_option(
            "--use", action="append", help="add a CMT use statement")
        parser.add_option(
            "--verbose",
            action="store_const",
            dest="loglevel",
        ...
        parser.add_option(
            ...
            action="store_true",
            help=
            "print available versions of the specified project and exit (all other options are ignored)"
        )
        parser.add_option(
            "--external-only",
            action="store_true",
            help=
            "sets only the environment for the externals (the project is used only to select the version of LCG)"
        )
        ...
        parser.add_option(
            ...
            help='path to the installation root, used to add default search '
            'path')

        def external_version_option(_option, opt_str, value, parser):
            if len(parser.largs) < 2:
                raise OptionValueError(
                    "%s must be preceded by the name of the external" %
                    opt_str)
        ...
        parser.add_option(
            ...
            help=
            "must be used after the name of an external to specify a non default version for it"
        )
        ...
        parser.add_option(
            ...
            action="store_true",
            help=
            "Do not override the value of the environment variable CMTPROJECTPATH"
        )
        ...
        def nightly_option(_option, opt_str, _value, _parser):
            ...
            if len(parser.rargs) < 1:
                raise OptionValueError(
                    "%s must be followed by the slot of the nightlies and optionally by the day"
                    % opt_str)
            ...
            global nightly_bases
            nightly_bases += [
                self.environment.get("LHCBNIGHTLIES",
            ...
            if os.path.exists(
                ...
                extraCMTPROJECTPATH = list(filter(str,
                                             data.get("cmtProjectPathList",
                                                      [])))
            elif os.path.exists(config_file):  # Try with the XML configuration
                extraCMTPROJECTPATH = getNightlyCMTPROJECTPATH(
                    config_file, slot, day)
        ...
        if 'CMTSITE' in self.environment and \
            ...
            parser.set_defaults(site_externals=self.default_externals[
                self.environment["CMTSITE"]])
        else:
                ...
                if os.path.isdir(dirname):
                    try:
                        ...
                        touchline = 'touch %s/%s/%s_%s_%s >& /dev/null\n' % (
                            dirname, self.project_info.name.upper(),
                            self.project_info.name.upper(),
    ...
    def _prepare_tmp_local_project(self):
        ...
        self.tmp_root = TempDir(
            prefix="SetupProject", keep_var="SPKEEPTEMPDIR")

        tmp_dir = os.path.join(
            str(self.tmp_root),
            "v23")  # add a fake version directory to please CMT
        os.mkdir(tmp_dir)
                ...
                if hat is None:
                    ...
                    for p in getPackageNames():
                        if p.endswith(suffix):
                            ...
                            break
                    if hat:
                        self._warning('wrong data package name used (%s), the'
                                      ' correct name is %s/%s',
                                      pack, hat, pack)
                req.write("use %s %s %s\n" % (pack, ver, hat))
        ...
        for e in exts + self.args + list(self.ext_versions.keys(
        )) + self.site_externals:
        ...
        for p in self.overriding_projects + [self.project_info
        ...
        if self._logger.level <= 2:
            ...
            self._verbose(
                open(os.path.join(tmp_dir, "cmt", "project.cmt")).read())
    ...
    def _gen_setup(self, root_dir):
        ...
        if 'CMTPATH' not in self.environment \
            ...
            raise SetupProjectError(
                'neither CMTPATH nor CMTPROJECTPATH are set')
        ...
        for v in ("PATH", "PYTHONPATH", "LD_LIBRARY_PATH", "ROOT_INCLUDE_PATH",
            ...
            if v in self.environment:
                self.environment[v] = os.pathsep.join(
                    x for x in self.environment[v].split(os.pathsep)
                    if '/gcc/' not in x)
        ...
        for v in ["PATH", "PYTHONPATH", "LD_LIBRARY_PATH", "HPATH", "MANPATH"]:
            if v in new_env:
                ...
                new_env[v] = os.pathsep.join([
                    d for d in new_env[v].split(os.pathsep)
        ...
        for k in [
                k for k in list(new_env.keys())[:]  # I need a copy of the keys
        ...
        if self.context_path:
            if not self.opts.quiet:
                lines.append(
                    "echo Using CMTUSERCONTEXT = '%s'" % self.context_path)
    ...
    def prepare(self, args=None):
        ...
        self.search_path = list(filter(os.path.exists, self.search_path))

        #------------- discover all project versions
        # debug printout: print project, search path and, optionally, user area
        self._debug("Look for all versions of '%s' in %s" %
                    (self.project_name, self.search_path) + (
                        (self.user_area and
                         (" with user area in '%s'" % self.user_area)) or ""))
        ...
        for p in self.overriding_projects + [self.project_info
            ...
            if not p.supportsPlatform(platform, self.user_area):
                self._error("Project %s %s is not available for platform %s" %
                            (p.name, p.version, platform))
                return 1
            self._verbose("Project %s %s uses %s policy" % (p.name, p.version,
                                                            p.policy))
    ...
    def run(self, args=None):
        ...
        try:
            ...
            if err and self.force:
                self._always(
                    "WARNING - ignoring warnings and errors from CMT:")
                self._always("\n".join(err))
        ...
        # Feedback
        if 'CMTPROJECTPATH' in self.environment:
            messages.append("Using CMTPROJECTPATH = '%(CMTPROJECTPATH)s'" %
                            self.environment)
            if self.keep_CMTPROJECTPATH:
        ...
        # add decorators
        if len(lines) > 1:
            lines = ['(%s,' % lines[0]] + [' %s,' % l for l in lines[1:-1]
                                           ] + [' %s)' % lines[-1]]
        else:
            lines[0] = '(%s)' % lines[0]
        messages += lines
        ...
        for p in path2normalize:
            if p in list(self.environment.keys()):
...
def expandAllVars(env, iterations=10):
    ...
    new_env = dict(
        (key, expandVars(value, env)) for key, value in env.items())
...
def getOldEnvironment(args):
    ...
    if rc:
        sys.exit(rc)
    return expandAllVars(dict(list(comp.environment.items())))
