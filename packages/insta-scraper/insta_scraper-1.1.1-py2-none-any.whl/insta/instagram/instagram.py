...
def get_query_hash(target_user, location):
    ...
    sidecar_query_hash = re.search(r'var E="[\w\d]+', sidecar_query_hash_response).group().split('"')[-1]
    ...
    story_downloader_query_hash_page = insta_sess.get(url + re.search(r'/static/bundles/metro/ConsumerCommons.js/[\w\d]+.js', profile_url_content, re.IGNORECASE).group()).content
    story_downloader_query_hash = re.search(r'var E=50\WR="[\w\d]+', story_downloader_query_hash_page).group().split('"')[-1]
