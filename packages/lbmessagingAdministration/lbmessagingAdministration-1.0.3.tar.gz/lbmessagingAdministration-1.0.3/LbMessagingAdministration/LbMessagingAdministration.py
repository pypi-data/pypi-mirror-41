...
class LbMaNightliesClient(object):
    ...
    def __init__(self, configType, arguments=None, prog="LbMaNightlies"):
        ...
        self._add_slot_info_parsing(parser)
        parser.add_argument('source_exchange',
                            help="Specify original exchange for message")
        ...
        if args.get('source_exchange', None):
            self.source_exchange = args['source_exchange']
    ...
    def move(self):
        if not self.source_exchange:
            raise Exception("The origin exchange used"
        ...
        tools = AdminHelper(self.source_exchange,
                            channel=self.channel)
