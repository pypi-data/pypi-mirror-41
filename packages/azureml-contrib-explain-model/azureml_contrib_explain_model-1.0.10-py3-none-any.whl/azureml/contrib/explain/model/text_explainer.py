...
from azureml.explain.model._internal.constants import Attributes


class TextExplainer(PureStructuredModelExplainer):
    ...
    def __init__(self, model, **kwargs):
        ...
        :type model: object
        """
        super(TextExplainer, self).__init__(model, **kwargs)
        self._logger.debug('Initializing TextExplainer')

    def _explain_instance(self, row, model):
        ...
        def fromNumericPredict(data):
            ...
            prediction = model.predict_proba(filteredData)
            return prediction

        exp = shap.KernelExplainer(fromNumericPredict, zeros)
    ...
    def explain_local(self, evaluation_examples, **kwargs):
        ...
        shap_values, explainer, features = self._explain_instance(golden_doc,
                                                                  self.model)
