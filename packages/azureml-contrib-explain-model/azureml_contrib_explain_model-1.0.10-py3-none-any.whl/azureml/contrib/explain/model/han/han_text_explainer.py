...
from azureml.explain.model._internal.common import _sort_features, module_logger
from azureml.explain.model._internal.constants import Tensorflow
...
from ..common.structured_model_explainer import TextStructuredInitModelExplainer
from ..explanation.explanation import create_local_explanation


class HANTextExplainer(TextStructuredInitModelExplainer):
    ...
    def __init__(self, model, initialization_examples, **kwargs):
        ...
        :type model: object
        :param initialization_examples: A matrix of feature vector examples (# examples x # features) on which
    ...
    def explain_local(self, evaluation_examples, classes=None, tune_epochs=5, **kwargs):
        ...
        golden_doc = _find_golden_doc(self.model, evaluation_examples)
    ...
    def _sort_shap_values_and_features(self, shap_values, features):
        ...
        shap_imp = _sort_features(features[0].flatten(), shap_order)
