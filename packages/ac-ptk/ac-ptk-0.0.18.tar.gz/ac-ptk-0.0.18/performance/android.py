...
from performance.excel import Excel
...
class AndroidApp:
    ...
    def dump_memory(self):
        ...
        try:
            ...
            while self.run:
                ...
                logd('(%s) add data: %s' % (datas.values().__str__(), self.series))
                sleep(DUMP_INTERVAL)
        except Exception as e:
            self.success = False
            raise e
    ...
    def record(self, plt_simple):
        ...
        matlab_plt.plt_export_history(histories, history_temp)
    ...
    def get_anr_files(self):
        ...
        if not os.path.exists(self.ptk_anr_dir):
            return []
        return os.listdir(self.ptk_anr_dir)
    ...
    def get_crash_files(self):
        ...
        if not os.path.exists(self.ptk_crash_dir):
            return []
        return os.listdir(self.ptk_crash_dir)
