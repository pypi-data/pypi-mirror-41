...
def smeftpar(scale, C, basis):
    ...
    # epsP_ub = 0
    # r_Bu = 0
    # R_us_ud = p['Vus'] / abs(1 + epsA_us - r_pi * epsP_us)
    # Vus = sqrt(R_us_ud) / sqrt(1 + R_us_ud)
    # Vub = p['Vub'] / abs(1 + epsA_ub - r_Bu * epsP_ub)
    # Vub = p['Vcb'] / abs(1 + epsA_ub - r_Bu * epsP_ub)
    # K = ckmutil.ckm.ckm_tree(Vus, Vub, Vcb, p['gamma'])
