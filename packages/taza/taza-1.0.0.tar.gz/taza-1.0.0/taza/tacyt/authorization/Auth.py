...
logger = logging.getLogger('tacyt.auth')
...
class Auth(object):
    ...
    def http_get_proxy(self, url, query_params=None):
        try:
            ...
            logger.exception("Failed to make get request")
            return None
    ...
    def http_delete_proxy(self, url):
        try:
            return self.http_delete(self.get_api_host() + url,
                                    self.authentication_headers(self.HTTP_METHOD_DELETE, url, None), None)

        except:
            logger.exception("Failed to make delete request")
            return None

    def http_post_proxy(self, url, data=None, body=None):
        try:
            if data is not None:
                return self.http_post(self.get_api_host() + url,
                                      self.authentication_headers(self.HTTP_METHOD_POST, url, None, data), data)

            elif body is not None:
                return self.http_post(self.get_api_host() + url,
                                      self.authentication_headers_with_body(self.HTTP_METHOD_POST, url, None, body),
                                      body)

        except:
            logger.exception("Failed to make post request")
            return None

    def http_put_proxy(self, url, data=None, body=None):
        try:
            if data is not None:
                return self.http_put(self.get_api_host() + url,
                                     self.authentication_headers(self.HTTP_METHOD_PUT, url, None, data), data)

            elif body is not None:
                return self.http_put(self.get_api_host() + url,
                                     self.authentication_headers_with_body(self.HTTP_METHOD_PUT, url, None, body), body)
        except:
            logger.exception("Failed to make put request")
            return None
    ...
    def _http(self, method, url, x_headers=None, body=None, file=None, content_type=None):
        ...
        try:
            ...
            logger.exception("Failed to make http request with body {}".format(body))
            ret = None
        ...
        if x_headers:
            ...
            for key, value in sorted(headers.items()):
                if not key.startswith(Auth.X_11PATHS_HEADER_PREFIX.lower()):
                    logging.error(
                        "Error serializing headers. Only specific " + Auth.X_11PATHS_HEADER_PREFIX + " headers need to be signed")
                    return None
    ...
    def get_serialized_params(self, params):
        ...
        if params:
            ...
            for key in sorted(params):
                serialized_params += key + Auth.PARAM_VALUE_SEPARATOR + urllib.quote_plus(
                    params[key]) + Auth.PARAM_SEPARATOR
    ...
    def http_post_file(self, url, headers, file_stream, file_name, tag_name):
        try:
            ...
            return Response(error=Error.Error({"code": "-1",
                                               "message": "The Python \"requests\" library was not found. Please, install it before call the upload method."}))
