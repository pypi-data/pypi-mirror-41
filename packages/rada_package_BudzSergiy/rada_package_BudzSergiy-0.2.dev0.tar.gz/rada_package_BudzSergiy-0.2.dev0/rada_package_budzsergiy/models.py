...
from rada_package_budzsergiy.utils import Sorting
from rada_package_budzsergiy.descriptors import IntDescriptor, StrDescriptor, DateDescriptor
...
class Spiker(Human):
    ...
    int_types = ('age', 'weight', 'height')

    def __init__(self, weight, height, last_name, first_name, age):
        super(Spiker, self).__init__(weight, height)
        self.last_name = last_name
        self.first_name = first_name
        self.age = age

    def notify_all_habarnikiv(self, verkhovna_rada):
        for fraction in verkhovna_rada.fractions:
            for deputat in fraction.deputies:
                if deputat.bribe_taker:
                    deputat.update()
...
class UkraineDeputat(Deputat):
    ...
    def give_tribute(self):
        if not self.bribe_taker:
            print('{} doesnt take bribes'.format(self))
        else:
            bribe_amount = int(input("Enter bribe amount\n"))
            if bribe_amount > 10000:
                print("The police put the deputy")
            else:
...
class Fraction(ABC):
    ...
    def __str__(self):
        return 'Name: {}. Deputies: {}'.format(self.name, len(self.deputats))
    ...
    def add_deputat(self, deputat):
        if deputat in self.deputats:
            print('{} already in fraction'.format(deputat))
        else:
    ...
    def remove_deputat(self, deputat):
        if deputat in self.deputats:
            ...
            print('{} was removed'.format(deputat))
        else:
            print('{} is not in {}'.format(deputat, self))
...
class UkraineFraction(Fraction):
    ...
    def get_deputat(cls):
        input_data = input(
            'Enter surname name data_f_birth weight height bribe_taker\n'
...
class UkraineRada(VerhovnaRada):
    ...
    def add_spiker(self, spiker):
        if not getattr(self, "spiker", None):
            self.spiker = spiker
            print("spiker was added")
