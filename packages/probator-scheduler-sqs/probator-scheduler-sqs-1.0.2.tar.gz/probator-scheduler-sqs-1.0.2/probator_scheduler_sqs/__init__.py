...
from probator import app_config, get_local_aws_session, AWS_REGIONS
from probator.config import dbconfig
from probator.constants import SchedulerStatus, ConfigOption
from probator.database import db
from probator.exceptions import ProbatorError, SchedulerError, ProbatorFatalError
...
from probator.schema.base import SchedulerBatch, SchedulerJob
from probator.utils import get_hash
...
class SQSScheduler(BaseScheduler):
    ...
    options = (
        ...
        ConfigOption(
            ...
            2,
            'float',
            'Time between each scheduled job, in seconds. Can be used to avoid spiky load during execution of tasks'
        ),
    )
    ...
    def send_worker_queue_message(self, *, batch_id, job_name, entry_point, worker_args, retry_count=0):
        ...
        try:
            ...
            self.log.exception(f'Error when processing worker task: {ex}')
    ...
    def execute_worker(self):
        ...
        try:
            ...
            for message in messages:
                ...
                except ProbatorError as ex:
                    if isinstance(ex, ProbatorFatalError):
                        # Fail immediately if its a Fatal error
