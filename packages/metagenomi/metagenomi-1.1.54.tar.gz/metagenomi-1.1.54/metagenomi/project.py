...
class MgProject:
    ...
    def __init__(self, mgproject, type=None, db=dbconn, check=True):
        ...
        self.assemblies = [Assembly(db=db, check=check, **i) for i in self.items
                           if i['mgtype'] == 'assembly']
        self.sequencings = [Sequencing(db=db, check=check, **i) for i in self.items
                            if i['mgtype'] == 'sequencing']
        self.samples = [Sample(db=db, check=check, **i) for i in self.items
                        if i['mgtype'] == 'sample']
    ...
    #     return [Assembly(db=db, check=check, **i) for i in items
    #                       if i['mgtype'] == 'assembly']
    #
    # def load_sequencings(self):
    #     return [Sequencing(db=db, check=check, **i) for i in self.items
    #                         if i['mgtype'] == 'sequencing']
    #
    # def load_samples(self):
    #     return [Sample(db=db, check=check, **i) for i in self.items
    #                     if i['mgtype'] == 'sample']

    def test_query(self, value, index='mgproject-mgtype-index', key='mgproject',
        ...
        if filtervalue is not None:
            filtering_exp = Key(filterkey).eq(filtervalue)
