...
class MgObj:
    ...
    def __init__(self, mgid, db=dbconn, key=None, **data):
        self.db = db
        ...
        for k, v in vars(self).items():
            if k not in self.not_required and v is not None:
                if isinstance(v, list):
                    ...
                    newdict[k] = []
                    for i in v:
                        newdict[k] = newdict[k] + [i.to_dict(validate=validate, clean=clean)]
                elif isinstance(v, pd.DataFrame):
                    newdict[k] = v.to_dict(orient='list')
                else:
                    try:
                        newdict[k.capitalize()] = v.to_dict(validate=validate, clean=clean)
