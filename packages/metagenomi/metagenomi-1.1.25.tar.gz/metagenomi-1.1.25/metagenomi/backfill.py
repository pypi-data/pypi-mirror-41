...
def fix_prey_sequencing(s, db=testdb):
    newd = {}
    newd['mgid'] = s['mg-identifier']
    newd['associated'] = {'sample': s['associated']['sample'], 'assembly': s['associated']['assembly']}
    ...
    if 'metadata' in s:
        ...
        newtask['cmd_run'] = 'None'
        newtask['jobid'] = 'None'
        newtask['avg_length'] = to_decimal(i['avgLength'])
        newtask['bases'] = to_int(i['bases'])
        newtask['insert_size'] = to_int(i['InsertSize'])
        newtask['library_layout'] = i['LibraryLayout']
        newtask['library_selection'] = i['LibrarySelection']
        newtask['library_source'] = i['LibrarySource']
        newtask['library_strategy'] = i['LibraryStrategy']
        newtask['model'] = i['Model']
        newtask['platform'] = i['Platform']

        newtask['raw_read_paths'] = {'fwd': i['raw_reads'][0], 'rev': i['raw_reads'][1]}
        newtask['sample_name'] = i['SampleName']
        newtask['size_mb'] = {'fwd': list(i['size_MB'].keys())[0], 'rev': list(i['size_MB'].keys())[1]}
        newtask['spots'] = i['spots']
        newtask['spots_with_mates'] = i['spots_with_mates']
...
def main(project='PREY', db=testdb):
    ...
    for i in preyitems:
        ...
        if 'read' in i['mg-identifier']:
            print(i['mg-identifier'])
            s = fix_prey_sequencing(i, db=db)
            print(s.to_dict(validate=False, clean=True))
            s.write(force=False)
