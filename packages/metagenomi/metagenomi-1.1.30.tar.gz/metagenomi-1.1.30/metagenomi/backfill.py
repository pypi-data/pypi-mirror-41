...
def fix_prey_sequencing(s, db=dbconn):
    ...
    if 'metadata' in s:
        ...
        newtask['raw_read_paths'] = {'fwd': i.pop('raw_reads')[0],
                                     'rev': i.pop('raw_reads')[1]}
        newtask['sample_name'] = i.pop('SampleName')
        newtask['size_mb'] = {'fwd': list(i.pop('size_MB').keys())[0],
                              'rev': list(i.pop('size_MB').keys())[1]}
...
def fix_htsp_sample(s, db=testdb):
    newd = {}
    newd['mgid'] = s['mg-identifier']
    newd['associated'] = {'sequencing': s['associated']['read']}
    # s.pop('associated')
...
def main(project='PREY', db=testdb):
    ...
    for i in htspitems:
        ...
        #     print(i['mg-identifier'])
        #     s = fix_htsp_sequencing(i, db=db)
        #     # s.write(force=True)
        #     print(s.created)
        #     # print(s.to_dict(validate=False, clean=True))
        #     s.write(force=False)

        if 'HTSP_0037_USA_SRA-samp' in i['mg-identifier']:
            print(i['mg-identifier'])
