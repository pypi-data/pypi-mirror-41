...
class Exam:
    ...
    def get_exam_id(self, exam_name: str = None) -> str:
        ...
        :return:
        """
        if exam_name and "-" in exam_name: return exam_name
        exam_names = self.get_exam_names()
        if exam_name is None:
            return list(exam_names.values())[0]
        elif exam_name in exam_names:
            return exam_names[exam_name]
        else:
            return ""

    def get_exam_names(self) -> dict:
        """
        获取所有考试信息
        :return:
        """
        ...
        json_data = loads(re.findall(r"examMap = ({.+?}})", r.text.strip())[0])
        for one in json_data["examList"]:
            exam_id = one["examId"]
            exam_name = one["examName"]
            exam_names[exam_name] = exam_id
        return exam_names
