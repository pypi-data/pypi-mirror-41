...
class HP_CMTools(ECMSPluginBase):
    ...
    def create_folder(self,folder_rep):
        ...
        if reccontainer != None:
            payload["RecordContainer"] = (None, reccontainer)
        if folder_rep.ecmsclassification != None:
        ...
        if response.status_code != requests.codes.ok:
            try:
                ...
                self.logger.error('Response failed with error: {0}'.format(
                str(e)))
            return folder_rep
    ...
    def create_document(self,doc_rep):
        ...
        try:    
            ...
            self.logger.error("{0} failed with error {1}").format(str(ex))
            doc_rep.ecmsid = None
            return doc_rep
        ...
        if len(responsedata["Results"]) > 0:
            ...
            if uri == 0:
                errmsg = responsedata["ResponseStatus"]["Message"]
                self.logger.error("Error creating document: {0}".format(errmsg))
            else:
                doc_rep.ecmsid = uri
        return doc_rep
