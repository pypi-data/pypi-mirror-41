...
def manualTimeStampEntry(frame, dialog, flashFrames=[]):
    ...
    if result == QDialog.Accepted:
        ...
        if frameNum2text != '':
            ...
            return 'Both frame entries must be supplied', \
                   time, dataEntered, nf, ef
        ...
        if dialog.radioButtonNTSC.isChecked():
            ...
            try:
                expectedFrameDeltaTime = float(dialog.frameDeltaTime.text())
            except:
                return '"' + dialog.frameDeltaTime.text() + '" is invalid as frameDeltaTime', \
                       time, dataEntered, nf, ef
