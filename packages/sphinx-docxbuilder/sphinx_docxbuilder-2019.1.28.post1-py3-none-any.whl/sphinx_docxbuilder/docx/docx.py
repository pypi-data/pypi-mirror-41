...
nsprefixes = {
    ...
    # Package Relationships (we're just making up our own namespaces here to
    # save time)
...
class DocxDocument:
    ...
    def get_coverpage(self):
        coverInfo = get_attribute(
            self.docbody,
            'w:sdt/w:sdtPr/w:docPartObj/w:docPartGallery',
            'w:val')
...
class DocxComposer:
    ...
    def set_props(self, title, subject, creator, company='',
                  category='', descriptions='', keywords=[]):
        '''
    ...
    def save(self, docxfilename):
        ...
        docxfile = self.styleDocx.restruct_docx(
            self.template_dir, docxfilename, list(treesandfiles.values()))
...
#################
# Output Paragraph
    def paragraph(self, paratext=None, style='BodyText',
                  block_level=0, create_only=False):
        '''
    ...
    def set_indent(self, paragraph, lskip):
        ...
        '''
        ind = set_attributes(paragraph, 'w:pPr/w:ind',
                             {'w:leftChars': '0', 'w:left': str(lskip)})
...
# Output Headinng
    def heading(self, headingtext, headinglevel):
        ...
        self.insert_paragraph_property(
            paragraph, 'Heading' + str(headinglevel))
...
########
# Output ListItem
    def list_item(self, itemtext, style='ListBullet', lvl=1, nid=0,
                  enum_prefix=None, enum_prefix_type=None, start=1):
        '''
        ...
        self.insert_numbering_property(
            paragraph,
            lvl - 1,
            nid,
            start,
...
# Numbering Style
    ...
    def insert_numbering_property(
            self, paragraph, lvl=0, nId=0, start=1, enum_prefix=None, enum_type=None):
        '''
           Insert paragraph property element with style.
        '''
        ...
        if nId <= 0:
            ...
            if num_id not in self.get_numbering_ids():
                ...
                if enum_prefix:
                    lvl_text = enum_prefix
                newid = self.get_max_numbering_id() + 1
                if newid < nId:
                    newid = nId
                num_id = str(
                    self.new_ListNumber_style(
                        newid, start, lvl_text, enum_type))

        numPr_tree = [['w:numPr'], [['w:ilvl', {'w:val': str(ilvl)}]], [
    ...
    def new_ListNumber_style_org(
            self, nId, start_val=1, lvl_txt='%1.', typ=None):
        '''
...
# Table
    ...
    def create_table_row(self, n_cells, cellsize=None,
                         contents=None, nline=0, firstCol=0):
        '''
          Create table row
        '''
        if nline < 0:
        ...
        for i in range(n_cells):
            ...
            if cellsize > 0:
                cellwidth = make_element_tree(
                    [['w:tcW', {'w:w': str(cellsize[i]), 'w:type':'dxa'}]])
    ...
    def create_table(self, colsize, tstyle='NormalTable'):
        ...
        '''
        table_tree = [['w:tbl'],
                      [['w:tblPr'], [['w:tblStyle', {'w:val': tstyle}]], [
        ...
        for csize in colsize:
            tablegrid.append(make_element_tree(
                [['w:gridCol', {'w:w': str(csize)}]]))
...
# for reStructuredText (FieldList and Admonitions)
    ...
    def insert_option_list_item(self, table, contents, nrow=0):
        ...
        '''
        row = self.create_table_row(
            1, [self.max_table_width - 500], nline=nrow)
        table.append(row)
    ...
    def insert_option_list_table(self):
        ...
        table = self.create_table(
            [self.max_table_width - 500], tstyle='OptionList')
        self.append(table)
        return table

    def insert_admonition_table(
            self, contents, title='Note: ', tstyle='NoteAdmonition'):
        '''
        ...
        self.append_paragrap_to_table_cell(
            table, self.paragraph(
                title, create_only=True), [
                0, 0])
