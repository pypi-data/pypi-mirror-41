...
class JSQLExecutor:
    ...
    def executeSelect(self, request):
        ...
        try:
            ...
            if not finalQuery.strip().lower().startswith("select"):
    ...
    def executeUpdateAndDelete(self, request):
        ...
        try:
            finalQuery = self.substituteParams(query["data"], params)
            if not finalQuery.strip().lower().startswith("update") and not finalQuery.strip().lower().startswith("delete"):
    ...
    def executeInsert(self, request):
        ...
        result = ""
        finalQuery = self.substituteParams(query["data"], params)
        if not finalQuery.strip().lower().startswith("insert"):
