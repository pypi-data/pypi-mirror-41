...
def relax_dgo(tmodel, reactions_to_ignore=(), solver=None):
    ...
    my_dgo = slack_model.get_variables_of_type(DeltaGstd)
    ...
    slack_model.logger.info('Extracting relaxation')
    ...
    epsilon = slack_model.solver.configuration.tolerances.feasibility

    # Apply reaction delta G standard bound change
    for this_reaction in slack_model.reactions:
        ...
        if dgo_delta_lb > 0 or dgo_delta_ub > 0:
            ...
            # Apply change
            the_dgo.variable.lb -= (dgo_delta_lb + 1*epsilon)
            the_dgo.variable.ub += (dgo_delta_ub + 1*epsilon)
    ...
    relaxed_model.objective = relaxed_model.growth_reaction
