...
class AbstractProtocol:
    ...
    def __init__(self, address: IPv4Address, server: 'AbstractFactory'):
        ...
        self.console = logging.getLogger(repr((address.host, address.port)))  # TODO: fix
        self.server = server
...
class AbstractFactory:
    ...
    def prepare(self, port, *args, **kwargs):
        self.port = reactor.listenTCP(port, self)
        log.info('Listen %s server at %s' % (self.protocol, port))

    def run(self, port=8956, *args, **kwargs):
        self.prepare(port, *args, **kwargs)
        reactor.run()
...
class WebSocketFactory(_WebSocketServerFactory, AbstractFactory):
    ...
    def prepare(self, port, *args, **kwargs):
        if not self.secure_context:
            return super().prepare(port, *args, **kwargs)
        self.reactor.listenSSL(port, self, self.secure_context)
        log.info('Listen %s wss server at %s' % (self.protocol, port))
