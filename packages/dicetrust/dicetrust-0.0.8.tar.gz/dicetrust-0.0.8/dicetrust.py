...
class Client:
    ...
    def participate(self, session_id: str) -> Iterator[Tuple[int, int]]:
...
def _cmd_create_session(client: Client, args) -> str:
    return client.create_session(args.mod)['id']
...
def _cmd_participate(client: Client, args) -> str:
    for number, mod in client.participate(args.session_id):
        print('%s %% %s' % (number, mod))
