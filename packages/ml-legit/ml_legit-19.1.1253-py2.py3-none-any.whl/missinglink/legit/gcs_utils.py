...
def _wrap_azure_call(callback, bucket_name, key, *args, **kwargs):
    ...
    try:
        ...
        six.raise_from(NotFound('container %s not found' % bucket_name), None)
...
def __retry_if_retry_possible_error(exception):
    ...
    return not isinstance(exception, (ImportError, AttributeError, TypeError, AssertionError, ValueError, NonRetryException))
...
def _s3_download(bucket_name, object_name):
    if object_name.startswith(s3_moniker):
        ...
        object_bucket_name, object_name = object_name.split('/', 1)
        return S3DownloadDirectDownload().download(object_bucket_name, object_name)
