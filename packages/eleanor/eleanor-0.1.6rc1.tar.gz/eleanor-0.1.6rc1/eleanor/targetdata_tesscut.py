...
#                self.post_obj = Postcard(source.postcard, source.ELEANORURL)
    ...
    def open_tesscut(self, url, cache=True):
        fn = download_file(url, cache=cache)
        with ZipFile(fn, "r") as f:
            with fits.open(BytesIO(f.read(f.namelist()[0]))) as hdu:
                flux = hdu[1].data
                hdr = hdu[1].header
        return flux, hdr
    ...
    def get_tpf_from_postcard(self, pos, postcard, height, width, bkg_size, save_postcard):
        ...
        post_url = "https://mast.stsci.edu/tesscut/api/v0.1/astrocut?ra={0[ra]}&dec={0[dec]}&y={1}&x={2}&units=px&sector=All".format(self.source_info.tic,
                                                                                                                                     105, 148)
        print(post_url)
        post_flux, post_hdr = self.open_tesscut(post_url, save_postcard)

        xy = WCS(post_hdr).all_world2pix(pos[0], pos[1], 1)
        ...
        url = "https://mast.stsci.edu/tesscut/api/v0.1/astrocut?ra={0[ra]}&dec={0[dec]}&y={1}&x={2}&units=px&sector=All".format(self.source_info.tic,
                                                                                                                                height,
                                                                                                                                width)
        self.tpf, self.header = self.open_tesscut(url)
...
#        self.dimensions = np.shape(self.tpf)
        ...
        # Determines if a target is in a crowded field
