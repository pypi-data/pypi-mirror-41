...
# There are ohter compatibility fixes throughout the code.
...
class Database(object):
	...
	def import_tabular_file(self, schema_name, table_name, csv_file_obj, skipheader):
		...
		csv_cols = next(f)
		csv_cols = [c.strip() for c in csv_cols]
		f.close()
...
class PostgresDatabase(Database):
	...
	def import_tabular_file(self, schema_name, table_name, csv_file_obj, skipheader):
		...
		csv_file_cols = [ch.lower().strip() for ch in csv_file_obj.column_headers()]
		if conf.import_common_cols_only:
...
class IfLevels(object):
	...
	def nest(self, tf_value):
		self.if_levels.append(tf_value)
	def unnest(self):
	...
	def invert(self):
		if len(self.if_levels) == 0:
			...
			self.if_levels[-1] = not self.if_levels[-1]
	def replace(self, tf_value):
		if len(self.if_levels) == 0:
			raise ErrInfo(type="error", other_msg="Can't change the IF state; no IF block is active.")
		else:
			self.if_levels[-1] = tf_value
	def current(self):
		if len(self.if_levels) == 0:
			raise ErrInfo(type="error", other_msg="No IF block is active.")
		else:
			return self.if_levels[-1]
	def all_true(self):
		return all(self.if_levels)
	def only_current_false(self):
		...
		if len(self.if_levels) == 0:
			return False
		return not self.if_levels[-1] and all(self.if_levels[:-1])
...
class CommandList(object):
	...
	def run_next(self):
		...
		if self.cmdptr > len(self.cmdlist) - 1:
			if len(if_stack.if_levels) != self.init_if_level:
				write_warning("IF level mismatch at beginning and end of script.")
			raise StopIteration
	...
	if row1:
		...
		if not isinstance(dataval, stringtypes):
			if sys.version_info < (3,):
				...
				dataval = str(dataval)
		subvarset.add_substitution(subvar, dataval)
	return None
...
def x_sub_encrypt(**kwargs):
	subvarset = subvars if varname[0] != '~' else commandliststack[-1].localvars
	subvarset.add_substitution(kwargs['subvar'], Encrypt().encrypt(kwargs['plaintext']))
	return None
...
def x_sub_decrypt(**kwargs):
	subvarset = subvars if varname[0] != '~' else commandliststack[-1].localvars
	subvarset.add_substitution(kwargs['subvar'], Encrypt().decrypt(kwargs['crypttext']))
	return None
...
def x_executescript(**kwargs):
	...
	if script_id not in savedscripts.keys():
		raise ErrInfo("cmd", other_msg="There is no SCRIPT named %s." % script_id)
	...
	cl = copy.deepcopy(savedscripts[script_id])
	arg_exp = kwargs["argexp"]
	args_rx_str =r'(?P<param>#?\w+)\s*=\s*(?P<arg>(?:(?:[^"\'\[][^,\)]*)|(?:"[^"]*")|(?:\'[^\']*\')|(?:\[[^\]]*\])))'
	args_rx = re.compile(args_rx_str, re.I)
	#If any argument expressions were found, parse the arguments
	if arg_exp:
		#Clean arg_exp
		all_args = re.findall(args_rx, arg_exp)
...
def write_warning(warning_msg):
	...
	warn = "%s (line %s of %s)" % (warning_msg, line_no, script_name)
	exec_log.log_status_warning(warn)
	if conf.write_warnings:
		output.write_err(u"**** Warning %s" % warn)
