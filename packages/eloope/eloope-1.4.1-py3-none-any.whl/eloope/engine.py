...
class Engine:
    def __init__(self, name=None, pool_size: int = 50):
    ...
    def run(self):
        ...
        for task in self._tasks:
            task[0].__globals__.update({'__engine__': self})
    ...
    def add_task(self, exe_func, *params):
        if self._is_run:
            exe_func.__globals__.update({'__engine__': self})
            self._task_pool.spawn(exe_func, *params)
        else:
            self._tasks.append((exe_func, *params))
