from eloope.task_queue import TaskQueue
...
class Manager:
    ...
    def start(self):
        ...
        for engine in self.processes:
            self._pipeline.hsetnx(f'manager-{self.name}', engine.name, 'Ready')
        if 0 in self._pipeline.execute():
            raise Exception('Duplicate engine name.')
...
def _forever():
    pipe = __engine__.redis.pipeline()
    while True:
        # 获取其他进程发送过来的任务
        pipe.spop(__engine__.name, 1000)
        pipe.scard('_')

        # 向其他进程发送任务
        for engine_name, task_queue in __engine__.task_set.items():
        ...
        if tasks:
            for task in tasks:
                __engine__.add_task(*loads(task))

        # 从公共任务库中获取任务
        if public_task_count and __engine__.task_count < __engine__.pool_size:
            tasks = __engine__.redis.spop('_', __engine__.pool_size)
            if tasks:
                for task in tasks:
                    __engine__.add_task(*loads(task))

        # 无任务时结束进程
        if __engine__.task_count == 1:
            pipe.hset(f'manager-{__engine__.manager_name}', __engine__.name, 'Done')
