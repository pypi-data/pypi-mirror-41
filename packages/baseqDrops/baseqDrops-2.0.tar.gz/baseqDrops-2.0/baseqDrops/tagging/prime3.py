...
def check_reference_files(genome):
    ...
    for refpath in [gtf_path, chrom_name_path, chrom_start_path, tx_path, ex_path]:
        if not os.path.exists(refpath):
            sys.exit("[ERROR] Annotation File Not Exist: {} !".format(refpath))
        else:
            print("[INFO] Annotation file exists: {}".format(refpath))

def tagging_reads(genome, bam, outpath):
    """
    **tagging_reads** will read the bam file which sorted by header or barcode, report the UMIs and reads counts for
    each expressed genes of a barcode.
    
    Args:
        genome: The name of genome (hg38);
        bam: The bam file (should sorted by header);
        outpath: The path of genes expression files;
        
    Returns:
        path: A file for tagged reads;
        ::
            BARCODE#{}
    """
    ...
    if os.path.exists(outpath):
        print("[INFO] The Tagging File Exists. It will be deleted...")
        os.remove(outpath)
    ...
    #Build the correspondance of transc name and gene name 
    tx2gene = {}

    print("[INFO] Building Mappings of Transcript and Gene Name...")
    with open(gtf_path, "r") as infile:
        for line in infile:
            ...
            if cells[2] != "transcript":
                continue
            transc_name = re.split('\"',cells[13])[1]
            gene_name = re.split('\"',cells[17])[1]
            tx2gene[transc_name] = gene_name
    ...
    print("[INFO] Tagging Bam File To Get Barcode Level Gene Expression: {}".format(outpath))
