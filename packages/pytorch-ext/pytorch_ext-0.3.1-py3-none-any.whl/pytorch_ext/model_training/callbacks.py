...
class RNNCurriculumLearningTrainer(BatchTrainingCallback):
    ...
    class BoundedMovingAverage:
        ...
        def __init__(self, max_len: int, threshold: float):
            ...
            self.past_elements = deque(maxlen=max_len)

        def append(self, elem: Union[int, float]) -> None:
            self.past_elements.append(elem)

        def is_bounded(self) -> bool:
            min_val = min(self.past_elements)
            max_val = max(self.past_elements)
            return max_val - min_val < self.threshold
    ...
    def __init__(self, threshold: float, max_predicted_frames: int,
        ...
        self.moving_average = RNNCurriculumLearningTrainer.BoundedMovingAverage(self.max_under_threshold_batches, self.threshold)
    ...
    def _raise_difficulty(self, loss: float) -> None:
        if self.moving_average.is_full() and self.moving_average.is_bounded():
