...
from stu.sensor import SensorManager
...
def __send(data1, data2):
    ...
    # USB
        try:
            ...
            print('Not connected to the Studuino board')
            # raise
        except:
...
def _analogout(pin, value):
    ...
    """
    id = pin - 2   # PinID to Dx ID (Dxx begins with 2.)
    data1 = 0xc0 + ((id << 1) & 0x0e) + (value >> 7 & 0x01)
    data2 = value & 0x7f 
    #print('Analog output', hex(data1), hex(data2))
    __send(data1, data2)
...
def _initSensor(pin, snstype):
    ...
    data2 = snstype << 4 | pin
    try:
        sensor.registerSensorType(pin, snstype)
    except AttributeError:
        print('Not connected to the Studuino board')
        # raise
    __send(data1, data2)
