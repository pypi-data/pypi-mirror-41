...
class DigitalIn(DigitalSensor):
	...
	def _canAttach(self, connector):
		...
		:return: 戻り値の説明
		"""
		return (isinstance(connector, ConnectorSensor), connector.id <= 5)


class AnalogIn(Sensor):
	"""
	Analog Input Class
	"""
	@property
	...
	def attach(self, connector):
		Part.attach(self, connector)
		command._initSensor(self.connector.id, 6)


class DigitalOut(Part):
	"""
...
class AnalogOut(Part):
	...
	def setValue(self, value):
		...
		command._analogout(self.connector._getGlobalId(), value)
