...
class Selenium_yes24(object):
	...
	def __init__(self, idperf, x=100, y=100, w=512, h=350):
		self.idperf = idperf
	...
	def handleCaptcha(self):
		...
		## 1) pop captchaKey
		URL = "https://wangticket.com/captcha2/pop_captcha.php"
		r = requests.get(URL)
		print("[pop captcha]", r.status_code, r.reason)
		if r.text != '':
			print(r.text)
			data = r.json()
		...
		insertURL = "https://wangticket.com/captcha/new_captcha.php?captchaKey={}&requester={}".format(captchaKey, self.userid)
		r = requests.get(insertURL)
		print("[new captcha]", r.status_code, r.reason)
		print(r.text)
		...
		while True:
			time.sleep(5)
			getURL = "https://wangticket.com/captcha/get_captcha.php?captchaId={}".format(captchaId)
			r = requests.get(getURL)
			#print("[get captcha]", r.status_code, r.reason)
			print(r.text)
			data = r.json()
			captchaText = data['captchaText']
	...
	def handleBookSuccess(self):
		...
		if len(content)>0 and content[0].is_displayed():
			...
			# register bookingno
			URL = "https://wangticket.com/depute/new_booking.php?yes24id={}&bookingno={}".format(self.userid, bookno)
			r = requests.get(URL)
			print("[new bookingno]", r.status_code, r.reason)
			print(r.text)
