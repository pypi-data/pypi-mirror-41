...
class CheckPayloadFile(Check):
    ...
    def check(self, build):
        ...
        if self.fullPath.encode('utf-8') not in rpmHdr[rpm.RPMTAG_FILENAMES]:
...
class CheckSourceFile(Check):
    ...
    def check(self, build):
        ...
        if self.name.encode('utf-8') not in srpmHdr[rpm.RPMTAG_FILENAMES]:
            raise FailedCheck(self)
...
class CheckTrigger(Check):
    ...
    def check(self, build):
        ...
        for t in rpmHdr[rpm.RPMTAG_TRIGGERTYPE]:
            ...
            if t==self.trigger.event.encode('utf-8') and rpmHdr[rpm.RPMTAG_TRIGGERCONDS][index]==self.trigger.triggerConds.encode('utf-8'):
                if rpmHdr[rpm.RPMTAG_TRIGGERSCRIPTS][index]!=self.trigger.script.encode('utf-8'):
                ...
                if rpmHdr[rpm.RPMTAG_TRIGGERSCRIPTPROG][index]!=expectedProgram.encode('utf-8'):
...
class TestSimpleRpmBuild(unittest.TestCase):
    def assert_header_has_item(self, rpmFilename, tagId, item, msg=None):
        ...
        self.assertIn(item.encode('utf-8'), h[tagId], msg)
