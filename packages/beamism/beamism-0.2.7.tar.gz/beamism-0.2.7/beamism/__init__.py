...
BASE_OUTPUT_SCHEMA = {
    ...
    "properties": {
        ...
        "results": { "type": "array", "items": { "type": ["object", "null"] } }
    }
}
...
class Lambda(beam.DoFn):
    ...
    def process(self, msg_text, save_mode=True):
        ...
        # メインの処理
        output_of_main_func = self.func(indata)
        if validate_json_based_on_schema(output_of_main_func, BASE_OUTPUT_SCHEMA):
            results = [{}]
            is_func_success = False
            func_msg = 'Output of main_func should follow the BASE_OUTPUT_SCHEMA. The output is follows: ' + str(output_of_main_func)
        else:
            results = output_of_main_func['results']
            is_func_success = output_of_main_func['is_func_success']
            func_msg = output_of_main_func['func_msg']
