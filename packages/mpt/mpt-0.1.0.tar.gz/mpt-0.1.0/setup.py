...
with open('mpt/__version__.py') as f:
    version_file_contents = "\n".join(f.readlines())
    ver_dic = {}
    exec(compile(version_file_contents, "mpt/__version__.py", 'exec'), ver_dic)
...
def load_entry_points():
    ...
    for file in glob.glob('mpt/cli_scripts/*.py'):
        ...
        if command_class_name is not None:
            script = '{command_name} = mpt.cli_scripts.{module_name}:{class_name}.console_script'.format(
