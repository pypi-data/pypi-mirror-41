...
def map_contenturl2casecorrect(v):
    low_keys = [x.lower() for x in v.keys()]
    
    if 'contenturl' in low_keys and 'contentUrl' not in v.keys():
        
        key_map = [[x.lower(), x] for x in v.keys()]
        
        for k in key_map:
            if k[0] == 'contenturl':
                v['contentUrl'] = v.pop(k[1])
                break
