...
function postAndRead(commandPath) {
    if (REQUEST_RUNNING) {
        return false;
    }
    ...
    try {
        ...
        let runner = new ExecuteAndProcessOutput(input_form, commandPath);
        runner.run();
    } catch (e) {
        console.error(e);
...
class ExecuteAndProcessOutput {
    ...
    run() {
        let submit_btn = document.getElementById("submit_btn");
        const command_url = window.location.href + this.commandPath;
        this.post(command_url)
            .then(response => {
