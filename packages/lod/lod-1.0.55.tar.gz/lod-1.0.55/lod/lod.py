...
def tag(symbol, comment=None, weight=None, arguments=None, also_create_signal=False):
    ...
    if arguments is None:
        arguments = []
    tag = basics.Tag(basics.create_preliminary_identifier('tag'), arguments, symbol_name=symbol, comment=comment, weight=weight)
    ...
    if also_create_signal:
        signal_weight = 1.0 if tag.weight is None else tag.weight
        tag('!set_signal_weight', comment=tag.comment, weight=signal_weight, arguments=[tag])
    return tag
