...
def calculateLeave(enter, lunchbreak):
    leave = (datetime.datetime.combine(today, enter) + datetime.timedelta(hours=7, minutes=48)).time()
    normal = datetime.datetime.combine(today, leave)
    exit = normal + datetime.timedelta(minutes=lunchbreak)
    return exit
...
def main():
    ...
    if args.lunch == None:
        ...
        lunch = args.lunch
        print("Specified --lunch break of {} minutes".format(lunch))
    ...
    print("You are allowed to leave at " +
          leave.strftime("%H:%M" + " my fren."))
