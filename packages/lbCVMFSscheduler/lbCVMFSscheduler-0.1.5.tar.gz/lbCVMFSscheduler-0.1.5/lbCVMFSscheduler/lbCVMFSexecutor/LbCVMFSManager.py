...
class LockManager(object):
    ...
    def __enter__(self):
        ...
        try:
            ...
            self.logCreator.write('...'.join(messages), 0,
...
class LbCVMFSManager(object):
    ...
    def __init__(self, vhost='/lhcb', topic='topic.cvmfs',
        ...
        self.server = RPCServer(self.vhost, self.topic, self.request)
    ...
    def request(self, body):
        ...
        if self.past_jobs.get(command, None):
            now = time.time()
            if now < self.past_jobs[command] + delay:
                return
        ...
        try:
            ...
            self.past_jobs[command] = time.time()
            self.logCreator.write(
