...
import urllib
import datetime
from lbCVMFSscheduler.lbcommons.MonitoringGW import MonitoringGW


class CVMFSStats(object):

    volpath = "/var/spool/cvmfs/lhcbdev.cern.ch"

    def __init__(self, logger=None, log_filename=None):
        self.logger = logger
    ...
    def _getRootCatalogURL(self):
        h = self._getRootHash()
        prefix = "http://localhost/cvmfs/lhcbdev.cern.ch/data"
        topdir = h[:2]
        fname = h[2:] + "C"
