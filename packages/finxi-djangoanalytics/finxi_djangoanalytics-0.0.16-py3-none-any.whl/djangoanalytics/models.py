...
class UserManager(models.Manager):
    def auth_user(self, hit):
        try:
            cid = hit['cid']
            user = self.get(cid=cid)
        except self.model.DoesNotExist:
            user = self.new_user(hit)
        user.update_user_id(hit) 
        return user

    def new_user(self, hit):
        user = self.model(cid=hit['cid'],
                          first_session_source=hit.get('cs', ''),
                          first_session_medium=hit.get('cm', ''),
                          first_campaign_name=hit.get('cn', ''),
                          first_campaign_keyword=hit.get('ck', ''))
        user.save()
        return user
...
class AnalitcsUser(TimeStampedModel):
    ...
    def update_last_hit(self, hit):
        self.last_hit = hit.id
        self.save()
        return self

    def update_user_id(self, hit):
        uid = hit.get('uid', None)
        if uid is not None and self.user_id == '':
            self.user_id = uid
            self.attribution_path = RawHit.objects.attribution_path(self)
            self.save()
        return self
...
class HitManager(models.Manager):
    ...
    def last_hit(self, user):
        try:
            return self.model.get(id=user.last_hit)
        except Exception:
            return None

    def get_source_and_medium(self, hit, user, expired):
        last_hit = self.last_hit(user)
        if last_hit is None:
        ...
        if expired:
            ...
            return last_hit.session_source, \
                   last_hit.session_medium
    ...
    def get_session_vars(self, hit, user, expired):
        ...
        old_metrics = last_hit.session_custom_metrics
        old_dimensions = last_hit.session_custom_dimensions
    ...
    def create(self, hit, user):
        last_hit = self.last_hit(user)
        expired = last_hit.session_expired(hit) \
