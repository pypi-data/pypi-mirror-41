...
class SegmentationAnalyzer(object):
    ...
    def update(self):
        ...
        np.testing.assert_array_almost_equal(self._scalar_im.affine, self._segmentation.affine)
        np.testing.assert_array_almost_equal(self._scalar_im.shape, self._segmentation.shape)

        self._one_voxel_volume = np.round(np.abs(np.prod(np.diag(self._segmentation.affine))), decimals=6)
