...
error_data = [
  ...
  "", "", "", "", "", "", "9999-12-28", "False", email.utils.make_msgid(uuid.uuid4().hex),
...
def get_datestart_filename(data, is_draft):
  ...
  ext = DRAFT_EXT if data[SENT_L]=="True" else MAIL_EXT
  return s + ext

# return full path to mailfile
def get_filename(data, is_draft):
  s = get_datestart_filename(data, is_draft)
  d = config.drafts_dir if is_draft else config.email_dir
  return os.path.join(d, s)

# where its attachments are stored
def get_attach_dir(data, is_draft):
  s = get_datestart_filename(data, is_draft)
  return os.path.join(config.attach_dir, s)

def get_trash_attach_dir(data, is_draft):
  s = get_datestart_filename(data, is_draft)
  return os.path.join(config.trash_dir, "attach", s)

# return full file path
def get_trashfilename(data, is_draft):
  s = get_datestart_filename(data, is_draft)
  if is_draft:
    return os.path.join(config.trash_dir, "draft", s)
  return os.path.join(config.trash_dir, s)

# return fullfile path
def get_rawfilename(data):
  s = get_datestart_file_part(data)
  return os.path.join(config.old_rawmail_dir, s + MIME_EXT)
