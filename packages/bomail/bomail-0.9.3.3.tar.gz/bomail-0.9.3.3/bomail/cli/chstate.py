...
def remove_from_lists(flist):
  if os.path.exists(config.openlist_file):
    remove_lines.do_sorted(config.openlist_file, sorted(flist))
  if os.path.exists(config.scheduledlist_file):
    remove_lines.do_sched(config.scheduledlist_file, set(flist))
...
def make_open(flist, mgr, remove_old=True):
  ...
  merge_lines.do(config.openlist_file, sorted(flist))
  for f in flist:
...
def rewrite_open_list(flist):
  ...
  with open(config.openlist_file, "w") as f:
    f.write("\n".join(flist))
...
def rewrite_scheduled_list(prlist):
  ...
  with open(config.scheduledlist_file, "w") as f:
    f.write("\n".join(slist))
