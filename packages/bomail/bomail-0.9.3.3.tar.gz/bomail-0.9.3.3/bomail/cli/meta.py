...
def migrate(newdir):
  ...
  try:
    ...
    print("Please make change in " + config.config_file)
...
def main_cli():
  ...
  if "-l" in sys.argv:
    ...
    if os.path.exists(config.acts_log_file):
      os.remove(config.acts_log_file)
