...
# Don't write config to disk, it's too confusing
from demyst.common.config import load_volatile_config
...
def watch(env, region):
    ...
    c = load_volatile_config(env=env, region=region)
...
def channel(config_file, channel_id, inputs_file, sample_mode, pipes_only, env, region):
    ...
    spinner.start()
    c = load_volatile_config(config_file=config_file, env=env, region=region)
...
def tile_json(ctx, config_file, spinner=None):
    ...
    import function
    result_dict = function.tile_data()
...
def create(name, path, version, config_file, env, region, key, username, password):
    c = load_volatile_config(config_file=config_file, env=env, region=region, key=key, username=username, password=password)
    ...
    response = c.auth_post(register_url, data={'name': name})
