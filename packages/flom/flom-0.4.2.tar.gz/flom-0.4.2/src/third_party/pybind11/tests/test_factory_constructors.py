...
def test_init_factory_basic():
    ...
    cstats = [ConstructorStats.get(
        c) for c in [m.TestFactory1, m.TestFactory2, m.TestFactory3]]
...
def test_init_factory_casting():
    ...
    cstats = [ConstructorStats.get(
        c) for c in [m.TestFactory3, m.TestFactory4, m.TestFactory5]]
    cstats[0].alive()  # force gc
    n_inst = ConstructorStats.detail_reg_inst()
    ...
    class BrokenTF6(m.TestFactory6):
        def __init__(self, bad):
            if bad == 1:
                ...
                m.TestFactory6.__init__(NotPybindDerived.__new__(
                    NotPybindDerived), tag.base, 1)
            elif bad == 4:
                m.TestFactory6.__init__(NotPybindDerived.__new__(
                    NotPybindDerived), tag.alias, 1)
