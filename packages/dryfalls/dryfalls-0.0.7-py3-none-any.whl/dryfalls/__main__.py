...
def creategitconfig(gituser, gitmail, reponame):
    template = read_string_from_file("dryfalls/configtemplate", "")
...
def createreadme(title, description):
    template = read_string_from_file("dryfalls/README.md.template", "")
    template = template.replace("${title}", title)
    template = template.replace("${description}", description)
    return template

def creategitignore(reponame):
    template = read_string_from_file("dryfalls/gitignore.template", "")
    template = template.replace("${reponame}", reponame)    
    return template

def createmetayaml(gituser, reponame):
    template = read_string_from_file("dryfalls/meta.yaml.template", "")
    template = template.replace("${gituser}", gituser)
    template = template.replace("${reponame}", reponame)
    return template

def createsetuppy(gituser, gitmail, reponame, projectShortDescription, projectDescription):
    template = read_string_from_file("dryfalls/setup.py.template", "")
    template = template.replace("${gituser}", gituser)
    template = template.replace("${gitmail}", gitmail)
    template = template.replace("${reponame}", reponame)
...
def createtravistest(reponame):
    template = read_string_from_file("dryfalls/travistest.template", "")    
    template = template.replace("${reponame}", reponame)
    return template
...
if args.create:    
    ...
    if args.force:
        rmtree(repopath())
    create_dir(repopath())
    write_string_to_file(repoconfigpath(), read_string_from_file("dryfalls/repotemplate.json", "{}"), force = args.force)    
...
if args.populate:
    ...
    write_string_to_file(repofilepath("LICENSE"), read_string_from_file("dryfalls/LICENSE.template", ""), force = args.force)    
    print("written LICENSE")
    write_string_to_file(repofilepath("bld.bat"), read_string_from_file("dryfalls/bld.bat.template", ""), force = args.force)    
    write_string_to_file(repofilepath("build.sh"), read_string_from_file("dryfalls/build.sh.template", ""), force = args.force)    
    print("written PyPi build files")
    create_dir(repofilepath(reponame))
    write_string_to_file(repofilepath(reponame + "/__init__.py"), read_string_from_file("dryfalls/initpy.template", ""))
    print("created package dir")
    write_string_to_file(repofilepath("Pipfile"), read_string_from_file("dryfalls/Pipfile.template", ""), force = args.force)    
    write_string_to_file(repofilepath("Pipfile.lock"), read_string_from_file("dryfalls/Pipfile.lock.template", ""), force = args.force)    
