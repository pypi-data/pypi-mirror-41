...
from anahita.text.normalize_company_name import normalize_company_name
...
class Bing(BasicAPI):
	...
	def search_name(self, name, convert_response=True, echo=0):
		echo = max(0, echo)
		...
		if convert_response:
			return self.convert_name_response_to_dicts(response=response)
		else:
			return response

	def search(self, name=None, ticker=None, country_code=None, exchange_code=None, echo=0):
		echo = max(0, echo)
		...
		name = name or ticker
		entities = self.search_name(name=name, convert_response=True, echo=echo)
		...
		entities.sort(key=lambda x: x['result_distance'])
		return entities
