...
if [ -z "${GPG_SECRET_KEY}" ]; then
  ...
  if [ -e ${GPG_SECRET_KEY} -a -f ${GPG_SECRET_KEY} ]; then
    ...
    GPG_COMMAND="--allow-secret-key-import -a --import $GPG_SECRET_KEY"
  else
	echo "using gpg secret key string: [hidden]"
	USE_SECRET_KEY=$(echo "${GPG_SECRET_KEY}" | sed 's|\\n|\n|g')
	GPG_COMMAND="--allow-secret-key-import -a --import <(echo "${USE_SECRET_KEY}")"
  fi

  if [ -z "${GPG_SECRET_KEY_PASSPHRASE}" ]; then
    echo "No secret key passphrase given."
    GPG_COMMAND="$GPG ${GPG_COMMAND}"
  else
    ...
    GPG_COMMAND="/tmp/signproxy ${GPG_COMMAND}"
  fi
  eval "${GPG_COMMAND}"
  CAN_SIGN="true"
fi
...
PKG_COMMAND="dpkg-buildpackage -rfakeroot -b ${DPKG_SIGN_ARG} ${SIGN_KEY}"
if [ -z "${GPG_SECRET_KEY_PASSPHRASE}" ]; then
  echo "No secret key passphrase given."
else
  PKG_COMMAND="$PKG_COMMAND --sign-command=/tmp/signproxy"
fi
eval "${PKG_COMMAND}"
