...
class BaseFeaturizer:
	...
	def featurize(self, data, ):
		...
		for index, key in enumerate(self._config.feature_confs):
			for execs in self._config.feature_confs[index].executions:
				featurized_list.append(self._apply_featurization(key=key.name, exec=execs))
	...
	def _apply_featurization(self, key, exec):
		...
		:param execution: Execution data, with args, kwargs and other configuration settings.
		...
		if exec.model:
			func_model = get_attr_from_module(exec.model['preprocessor'])
			model = func_model(self._transformed_data, **exec.model['kwargs'])
