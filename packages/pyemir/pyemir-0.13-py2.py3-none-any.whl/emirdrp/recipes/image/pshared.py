...
from emirdrp.recipes.aiv.flows import init_filters_bdf
from emirdrp.recipes.aiv.flows import basic_processing_with_update
...
def truncated(arr, frac=0.1):
    "Simple truncated mean"""

    # FIXME: see also stats.trim_mean and stats.trimboth
    nf = int(arr.size * frac)
    arr.sort()
    new = arr[nf:-nf]
    lnew = len(new)
    if lnew == 0:
...
def extract_metadata(frame, master_bpm, keywords, scalewindow, target_is_sky):
    """Extract required metadata from frame header."""
    ...
    # FIXME: this is a disk path reference, dangerous in GTC
    ...
    if frame.itype == 'TARGET':
        ...
        raise ValueError('itype must be TARGET or SKY, not %s' % (frame.itype,))
...
class DirectImageCommon(EmirRecipe):
    ...
    def _basicComputation(self, rinput):
        _logger.info('Basic processing')

        flow = init_filters_bdf(rinput)

        basic_processing_with_update(rinput, flow=flow)
