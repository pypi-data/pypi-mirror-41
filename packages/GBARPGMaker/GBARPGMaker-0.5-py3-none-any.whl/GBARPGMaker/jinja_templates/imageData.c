...
const unsigned short {{ sprite_image_name }}RealSize[2] = { {{sprite_image.size[0]}}, {{ sprite_image.size[1] }} };

const unsigned short {{ sprite_image_name }}Palette[{{ sprite_image.palette|length }}] __attribute__((aligned(4))) __attribute__((visibility("hidden")))=
{
    {% for o in range(sprite_image.palette|length) -%}
    {{ "{0:#06x}".format(sprite_image.palette[o]) }},
    {%- if loop.index is divisibleby 12 and not loop.last %}
    {% endif -%}
    {%- endfor %}
};

const unsigned int {{ sprite_image_name }}Tileset[{{ sprite_image.tileset|length }}] __attribute__((aligned(4))) __attribute__((visibility("hidden")))=
{
    {% for o in range(sprite_image.tileset|length) -%}
    {{ "{0:#010x}".format(sprite_image.tileset[o]) }},
    {%- if loop.index is divisibleby 8 and not loop.last %}
    {% endif -%}
    {%- endfor %}
};
...
const unsigned int BorderTextTileset[24] __attribute__((aligned(4))) __attribute__((visibility("hidden")))=
{
    0xeeeeee00,0xffffffe0,0xeeeeeefe,0xcccceefe,0xdddccefe,0xddddcefe,0xddddcefe,0xddddcefe,
    0xeeeeeeee,0xffffffff,0xeeeeeeee,0xcccccccc,0xdddddddd,0xdddddddd,0xdddddddd,0xdddddddd,
    0xddddcefe,0xddddcefe,0xddddcefe,0xddddcefe,0xddddcefe,0xddddcefe,0xddddcefe,0xddddcefe,
};

const unsigned short BorderTextPal[4] __attribute__((aligned(4))) __attribute__((visibility("hidden")))=
{
	0x14A5,0x4580,0x4E73,0x7FFF,
};
