...
from altuscli.paramfile import ParamFileVisitor
...
class TestHTTPBasedResourceLoading(unittest.TestCase):
    def setUp(self):
        self.requests_patch = mock.patch('altuscli.paramfile.requests')
...
class TestParamFileVisitor(unittest.TestCase):
    def setUp(self):
        self.model = yaml.load(open(os.path.join(MODEL_DIR, 'service.yaml')))
