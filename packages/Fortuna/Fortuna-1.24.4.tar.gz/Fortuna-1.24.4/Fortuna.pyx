...
class TruffleShuffle:
    ...
    def beta_rotate(self):
        if self.max_id < 3:
            ...
            result = self.data.pop(1 + half_len + _max_extreme(half_len - 1))
            self.data.insert(_zero_extreme(half_len - 1), result)
        return result
...
def distribution_timer(func: staticmethod, *args, call_sig=None, num_cycles=10000, dist=True, **kwargs):
    ...
    print(f"{call_sig} x {num_cycles}: Average time: {average_time} ns")
    if dist:
