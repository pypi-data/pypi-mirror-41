...
long long ability_dice(long long num) {
    const long long n = Fortuna::min_max(num, 3, 9);
    if (n == 3) return Fortuna::dice(3, 6);
    std::vector<long long> theRolls(n);
    std::generate(begin(theRolls), end(theRolls), []() { return Fortuna::d(6); });
    std::sort(begin(theRolls), end(theRolls), std::greater<long long>());
