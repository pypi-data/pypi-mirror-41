...
int half_the_zeros(const std::function<int(int)> & func, int num) {
    if (percent_true(50)) return func(num);
    else {
        int t = 0;
...
int plus_or_minus_linear_down(int num) {
    if (num == 0) return 0;
    const int n = std::abs(num);
    if (percent_true(50)) return half_the_zeros(max_cool, n);
    else return half_the_zeros(max_cool, -n);
}

int plus_or_minus_curve_down(int num) {
    if (num == 0) return 0;
    const int n = std::abs(num);
    if (percent_true(50)) return half_the_zeros(max_extreme, n);
    else return half_the_zeros(max_extreme, -n);
}

int mostly_not_middle(int num) {
    if (num > 0) {
        const int mid_point = num / 2;
        if (num % 2 == 0) return plus_or_minus_linear_down(mid_point) + mid_point;
        else if (percent_true(50)) return zero_cool(mid_point);
        else return 1 + max_cool(mid_point) + mid_point;
    } else return analytic_continuation(mostly_not_middle, num);
}

int mostly_not_center(int num) {
    if (num > 0) {
        const int mid_point = num / 2;
        if (num % 2 == 0) return plus_or_minus_curve_down(mid_point) + mid_point;
        else if (percent_true(50)) return zero_extreme(mid_point);
        else return 1 + max_extreme(mid_point) + mid_point;
    } else return analytic_continuation(mostly_center, num);
}
