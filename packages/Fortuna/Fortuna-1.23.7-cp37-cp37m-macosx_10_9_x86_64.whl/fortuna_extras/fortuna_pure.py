...
class TruffleShuffle:
    ...
    def beta_rotate(self):
        if self.max_id < 3:
            ...
            result = self.data.pop(max_extreme(self.max_id))
            self.data.insert(zero_extreme(self.max_id), result)
        else:
            half_len = self.max_id // 2
            result = self.data.pop(1 + half_len + max_extreme(self.max_id - half_len - 1))
            self.data.insert(zero_extreme(self.max_id - half_len - 1), result)
        return result
