...
cdef extern from "Fortuna.hpp":
    ...
    int _mostly_not_middle "mostly_not_middle"(int)
    int _mostly_not_center "mostly_not_center"(int)
    double _random_float "random_float"()
    int _plus_or_minus_linear_down "plus_or_minus_linear_down"(int)
    int _plus_or_minus_curve_down "plus_or_minus_curve_down"(int)
...
def percent_true_float(num: float) -> bool:
    return _random_below(100) + _random_float() <= num
...
class TruffleShuffle:
    ...
    def delta_swap(arr: list):
        max_idx = len(arr) - 1
        result, target = _mostly_middle(max_idx), _mostly_not_middle(max_idx)
        arr[result], arr[target] = arr[target], arr[result]
        return arr[target]
    ...
    def gamma_swap(arr: list):
        max_idx = len(arr) - 1
        result, target = _mostly_center(max_idx), _mostly_not_center(max_idx)
        arr[result], arr[target] = arr[target], arr[result]
        return arr[target]
