...
def percent_true_float(num: float) -> bool:
...
class TruffleShuffle:
    ...
    def __call__(self):
        if self.first_block:
            ...
            return flatten(self.beta_rotate(self.data), self.flat)

    @staticmethod
    def beta_rotate(arr: list):
        arr_len = len(arr)
        max_idx = arr_len - 1
        if arr_len < 3:
            return random_value(arr)
        elif arr_len < 6:
            result = arr.pop(max_extreme(max_idx))
            arr.insert(zero_extreme(max_idx), result)
        else:
            half_len = max_idx // 2
            result = arr.pop(1 + half_len + max_extreme(max_idx - half_len - 1))
            arr.insert(zero_extreme(max_idx - half_len - 1), result)
        return result
    ...
    @staticmethod
    def delta_swap(arr: list):
        """ linear inside-out swap """
        max_idx = len(arr) - 1
        result, target = mostly_middle(max_idx), mostly_not_middle(max_idx)
        arr[result], arr[target] = arr[target], arr[result]
        return arr[target]
    ...
    def gamma_swap(arr: list):
        ...
        max_idx = len(arr) - 1
        result, target = mostly_center(max_idx), mostly_not_center(max_idx)
        arr[result], arr[target] = arr[target], arr[result]
        return arr[target]


class QuantumMonty:
    __slots__ = ("flat", "data", "max_id", "truffle_shuffle", "quantum_monty", "quantum_not_monty")
