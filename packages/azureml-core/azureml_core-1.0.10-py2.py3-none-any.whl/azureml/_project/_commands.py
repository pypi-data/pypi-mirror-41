...
from azureml._base_sdk_common.project_map import ProjectMap
...
def attach_project(workspace_object, cloud_project_name, project_path="."):
    ...
    experiment_client = ExperimentClient.create(workspace_object, experiment_name)
    experiment_client._prepare_experiment()

    scope = "{}/{}/{}".format(experiment_client.get_workspace_uri_path(), PROJECT_KEY, experiment_name)
    # End temporary fix

    # There is no duplicate save here, if entry already exists.
    ProjectMap().save_project(experiment_name, scope)
...
def create_workspace(auth, resource_group_name, workspace_name, subscription_id,
    ...
    try:
        ...
        if e.status_code == 404:
            ...
            from azureml._base_sdk_common.common import get_http_exception_response_string
            raise WorkspaceException(get_http_exception_response_string(e))
...
def share_workspace(auth, resource_group_name, workspace_name, subscription_id, user, role):
    ...
    create_role_assignment(auth, role, assignee=user, resource_group_name=None, scope=scope)
