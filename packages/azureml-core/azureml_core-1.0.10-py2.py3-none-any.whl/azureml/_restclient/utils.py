...
from .constants import (PAGE_SIZE_KEY, TOP_KEY, CALLER_KEY, CUSTOMER_HEADERS_KEY, CALL_FUNCTION)
...
def _generate_client_kwargs(**kwargs):
    if not kwargs:
        return None
    ...
    if kwargs.get(CALLER_KEY):
        call = {CALL_FUNCTION: kwargs.pop(CALLER_KEY)}
        if kwargs.get(CUSTOMER_HEADERS_KEY):
            kwargs[CUSTOMER_HEADERS_KEY].update(call)
        else:
            kwargs[CUSTOMER_HEADERS_KEY] = call
