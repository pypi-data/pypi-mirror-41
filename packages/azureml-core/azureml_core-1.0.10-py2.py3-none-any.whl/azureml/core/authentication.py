...
"""Manages Azure Machine Learning authentication."""
import datetime
import logging
import os
import pytz
...
class AbstractAuthentication(object):
    ...
    def _perform_subscription_check(self, subscription_id):
        """Check if the auth has access to the provided subscription id.
        
        If auth doesn't have access then azureml.exceptions.AuthenticationException is thrown.
        :param subscription_id:
        :type subscription_id: str
        :return: None if successful
        :rtype: None
        """
        all_subscription_list = self._get_all_subscription_ids()
        if not _check_if_subscription_exists(subscription_id, all_subscription_list):
            raise AuthenticationException("You don't have access to {} subscription. All the subscriptions "
                                          "that you have access to = {}. ".format(subscription_id,
    ...
    def _get_service_client(self, client_class, subscription_id, subscription_bound=True, base_url=None):
        ...
        if subscription_id:
            self._perform_subscription_check(subscription_id)
    ...
    def _get_adal_auth_object(self):
...
class InteractiveLoginAuthentication(AbstractAuthentication):
    ...
    def __init__(self, force=False, tenant_id=None):
        ...
        if "MSI_ENDPOINT" in os.environ:
            ...
            self._msi_auth = self._get_in_msi_scope()

        if self._msi_auth is None:
            if force:
    ...
    def _get_arm_token(self):
        ...
        """
        if self._msi_auth:
            return self._msi_auth._get_arm_token()
        else:
    ...
    def _get_all_subscription_ids_internal(self, arm_token):
        if self._msi_auth:
            return self._msi_auth._get_all_subscription_ids()
        else:
    ...
    def _get_arm_token_using_interactive_auth(self, force_reload=False):
        ...
        arm_token = _get_arm_token_with_refresh(profile_object, cloud_type, ACCOUNT, CONFIG, SESSION,
                                                get_config_dir(), force_reload=force_reload)
        # If a user has specified a tenant id then we need to check if this token is for that tenant.
        if self._tenant_id and fetch_tenantid_from_aad_token(arm_token) != self._tenant_id:
    ...
    def _fallback_to_azure_cli_credential(self):
        import logging
        logging.getLogger().warning("Falling back to use azure cli credentials. This fall back to use "
                                    "azure cli credentials will be removed in the next release. \n"
                                    "Make sure your code doesn't require 'az login' to have happened "
                                    "before using azureml-sdk, except the case when you are specifying "
                                    "AzureCliAuthentication in azureml-sdk.")
...
class AzureCliAuthentication(AbstractAuthentication):
    ...
    def _get_default_subscription_id(self):
        ...
        if not ACCOUNT.data:
            ...
            SESSION.load(os.path.join(get_config_dir(), 'az.sess'), max_age=AzureCliAuthentication._TOKEN_TIMEOUT_SECs)
...
class ArmTokenAuthentication(AbstractAuthentication):
    ...
    def _get_all_subscription_ids(self):
        ...
        """
        from azureml._vendor.azure_cli_core.adal_authentication import AdalAuthentication
        auth_object = AdalAuthentication(lambda: ("Bearer", self._get_arm_token(), None))
...
class ServicePrincipalAuthentication(AbstractAuthentication):
    ...
    def __init__(self, tenant_id, username, password):
        ...
        The class allows for authentication using a service principle instead of users own identity. The class is
        ideal for automation and CI/CD scenarios.

        :param tenant_id: The active directory tenant that the service identity belongs to.
        :type tenant_id: str
        :param username: The service principal id.
        :type username: str
        :param password: The service principal key.
        :type password: str
        """
        self._tenant_id = tenant_id
    ...
    def _get_sp_credential_object(self):
        ...
        sp_credentials = AdalAuthentication(
            ...
            resource, self._username, self._password)
    ...
    def _get_all_subscription_ids(self):
        ...
        :rtype: list
        """
        arm_auth = ArmTokenAuthentication(self._get_arm_token())
...
class AzureMLTokenAuthentication(AbstractAuthentication):
    ...
    def __init__(self, azureml_access_token, expiry_time=None, host=None, subscription_id=None,
        ...
        self._expiry_time = expiry_time if expiry_time is not None else AzureMLTokenAuthentication._parse_expiry_time_from_token(self._aml_access_token)
        self._host = host
    ...
    def _register_auth(self):
        ...
        if auth_key in AzureMLTokenAuthentication._registered_auth:
            ...
            if host_key not in AzureMLTokenAuthentication._host_clientbase:
                AzureMLTokenAuthentication._host_clientbase[host_key] = ClientBase(self._host, self)
...
class MsiAuthentication(AbstractAuthentication):
    ...
    def _get_all_subscription_ids(self):
        ...
        from msrestazure.azure_active_directory import MSIAuthentication
        msi_auth = MSIAuthentication()
        return _get_subscription_ids_via_client(msi_auth)
...
def _check_if_subscription_exists(subscription_id, subscription_id_list):
...
def _get_arm_token_with_refresh(profile_object, cloud_type, account_object, config_object, session_object,
                                config_directory, force_reload=False):
    ...
    try:
        auth, _, _ = profile_object.get_login_credentials()
    ...
    try:
        ...
        if user_type == "user":
            _, refresh_token, access_token, tenant = profile_object.get_refresh_token()
            if (_get_exp_time(access_token) - time.time()) < TOKEN_REFRESH_THRESHOLD_SEC:
                ...
                with NewLoggingLevel(ADAL_LOGGER_NAME, level=logging.WARNING):
                    adal_token = context.acquire_token_with_refresh_token(
                        refresh_token, AZ_CLI_AAP_ID, cloud_type.endpoints.management)
