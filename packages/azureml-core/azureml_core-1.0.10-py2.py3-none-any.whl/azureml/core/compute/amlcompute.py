...
class AmlCompute(ComputeTarget):
    ...
    def _initialize(self, workspace, obj_dict):
        ...
        subnet_id = obj_dict['properties']['properties']['subnet']['id'] \
            if obj_dict['properties']['properties'] and obj_dict['properties']['properties']['subnet'] else ""
        if subnet_id:
    ...
    def _build_create_payload(config, location, subscription_id):
        ...
        if not config.vm_size and not config.vm_priority and not config.vnet_resourcegroup_name and \
                not config.vnet_name and not config.subnet_name:
    ...
    def serialize(self):
        ...
        amlcompute_properties = {'vmSize': self.vm_size, 'vmPriority': self.vm_priority,
                                 'scaleSettings': scale_settings, 'subnet': {'id': subnet_id}}
...
class AmlComputeStatus(object):
    ...
    def serialize(self):
        ...
        return {'allocationState': self.allocation_state,
                ...
                'currentNodeCount': self.current_node_count, 'errors': self.errors, 'modifiedTime': modified_time,
                ...
                'scaleSettings': scale_settings, 'targetNodeCount': self.target_node_count,
...
class AmlComputeNodeStateCounts(object):
    ...
    def serialize(self):
        ...
        return {'idleNodeCount': self.idle_node_count, 'leavingNodeCount': self.leaving_node_count,
                'preemptedNodeCount': self.preempted_node_count, 'preparingNodeCount': self.preparing_node_count,
                'runningNodeCount': self.running_node_count, 'unusableNodeCount': self.unusable_node_count}
