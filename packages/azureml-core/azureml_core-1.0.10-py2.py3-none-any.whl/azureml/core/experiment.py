...
class Experiment(ChainedIdentity, HasExperimentPortal):
    ...
    def list(workspace):
        ...
        :rtype: :class:`list[azureml.core.Experiment]`
        """
        workspace_client = WorkspaceClient.create(workspace)
        experiments = workspace_client.list_experiments()
        return [Experiment(workspace, experiment.name) for experiment in experiments]
    ...
    def _prompt_message(message):
        """Ask for confirmation for a CLI action.

        :param message:
        :type message: string
        :return: Returns confirmation option
        :rtype: bool
        """
        ...
        if choice in no_set:
            return False
        elif choice in yes_set:
            return True
        else:
            sys.stdout.write("Error reading specified input. Choices are 'yes' or 'no'")
