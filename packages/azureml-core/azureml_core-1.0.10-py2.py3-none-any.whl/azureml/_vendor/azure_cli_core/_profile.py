...
from azureml._vendor.azure_cli_core._session import ACCOUNT
# from azure.cli.core.util import get_file_json, in_cloud_console, open_page_in_browser, can_launch_browser
from azureml._vendor.azure_cli_core.util import get_file_json, in_cloud_console, open_page_in_browser, can_launch_browser

# from azure.cli.core.cloud import get_active_cloud, set_cloud_subscription

# from azureml._vendor.azure_cli_core.cloud import get_active_cloud, set_cloud_subscription

from azureml._vendor.azure_cli_core.cloud import AZURE_PUBLIC_CLOUD

from azureml._vendor.azure_cli_core.profiles._shared import get_client_class
from azureml._vendor.azure_cli_core.profiles import ResourceType, get_api_version, get_sdk
from azureml._vendor.azure_cli_core._debug import change_ssl_cert_verification
from azureml._vendor.azure_cli_core._debug import allow_debug_adal_connection
...
def load_subscriptions(cloud_type, all_clouds=False, refresh=False):
    profile = Profile(cloud_type=cloud_type)
    if refresh:
...
def _get_authority_url(cloud_type, tenant):
    authority_url = cloud_type.endpoints.active_directory
...
def _authentication_context_factory(cloud_type, tenant, cache):
    authority_url, is_adfs = _get_authority_url(cloud_type, tenant)
...
def _load_tokens_from_file(file_path):
    if os.path.isfile(file_path):
        ...
        except (AuthenticationException, ValueError) as ex:
            raise AuthenticationException("Failed to load token files. If you have a repro, please log an issue at "
                                          "https://github.com/Azure/azure-cli/issues. At the same time, you can clean "
                                          "up by running 'az account clear' and then 'az login'. "
                                          "(Inner Error: {})".format(ex))
    return []
...
def get_credential_types(cloud_type):

    class CredentialType(Enum):  # pylint: disable=too-few-public-methods
        # cloud = get_active_cloud(cli_ctx)
        cloud = cloud_type
        management = cloud_type.endpoints.management
        rbac = cloud_type.endpoints.active_directory_graph_resource_id
...
class Profile(object):
    ...
    def __init__(self, storage=None, auth_ctx_factory=None, use_global_creds_cache=True,
                 async_persist=True, cloud_type=None):
        self.cloud_type = cloud_type or AZURE_PUBLIC_CLOUD
        ...
        if use_global_creds_cache:
            ...
            if not Profile._global_creds_cache:
                Profile._global_creds_cache = CredsCache(self.cloud_type, self.auth_ctx_factory,
                                                         async_persist=async_persist)
            self._creds_cache = Profile._global_creds_cache
        else:
            self._creds_cache = CredsCache(self.cloud_type, self.auth_ctx_factory, async_persist=async_persist)

        self._management_resource_uri = self.cloud_type.endpoints.management
        self._ad_resource_uri = self.cloud_type.endpoints.active_directory_resource_id
        self._ad = self.cloud_type.endpoints.active_directory
    ...
    def find_subscriptions_on_login(self,
        ...
        if not subscription_finder:
            subscription_finder = SubscriptionFinder(self.cloud_type,
        ...
        if interactive:
            ...
            if not use_device_code:
                try:
                    authority_url, _ = _get_authority_url(self.cloud_type, tenant)
            ...
            if is_service_principal:
                if not tenant:
                    raise AuthenticationException('Please supply tenant using "--tenant"')
        ...
        if not allow_no_subscriptions and not subscriptions:
            raise AuthenticationException("No subscriptions were found for '{}'. If this is expected, use "
                                          "'--allow-no-subscriptions' to have tenant level accesses".format(username))
        ...
        if allow_no_subscriptions:
            ...
            profile = Profile(cloud_type=self.cloud_type)
    ...
    def _normalize_properties(self, user, subscriptions, is_service_principal, cert_sn_issuer_auth=None,
        ...
        for s in subscriptions:
            consolidated.append({
                ...
                _ENVIRONMENT_NAME: self.cloud_type.name
            })
    ...
    def _new_account(self):
        SubscriptionType, StateType = get_sdk(self.cloud_type, ResourceType.MGMT_RESOURCE_SUBSCRIPTIONS,
    ...
    def find_subscriptions_in_vm_with_msi(self, identity_id=None):
        resource = self.cloud_type.endpoints.active_directory_resource_id
        if identity_id:
        ...
        subscription_finder = SubscriptionFinder(self.cloud_type, self.auth_ctx_factory, None)
        subscriptions = subscription_finder.find_from_raw_token(tenant, token)
        if not subscriptions:
            raise AuthenticationException('No access was configured for the VM, hence no subscriptions were found')
    ...
    def find_subscriptions_in_cloud_console(self):
        _, token, _ = self._get_token_from_cloud_shell(self.cloud_type.endpoints.active_directory_resource_id)
        logger.info('MSI: token was retrieved. Now trying to initialize local accounts...')
        decode = jwt.decode(token, verify=False, algorithms=['RS256'])
        tenant = decode['tid']
        ...
        subscriptions = subscription_finder.find_from_raw_token(tenant, token)
        if not subscriptions:
            raise AuthenticationException('No subscriptions were found in the cloud shell')
    ...
    def _set_subscriptions(self, new_subscriptions, merge=True, secondary_key_name=None):
        ...
        if subscriptions:
            ...
            # set_cloud_subscription(self.cloud_type, active_cloud.name, default_sub_id)
    ...
    def set_active_subscription(self, subscription):  # take id or name
        ...
        if len(result) != 1:
            raise AuthenticationException("The subscription of '{}' {} in cloud '{}'.".format(
        ...
        # set_cloud_subscription(self.cloud_type, active_cloud.name, result[0][_SUBSCRIPTION_ID])
        self._storage[_SUBSCRIPTIONS] = subscriptions
    ...
    def get_current_account_user(self):
        try:
            ...
            raise AuthenticationException('There are no active accounts.')
    ...
    def get_subscription(self, subscription=None):  # take id or name
        ...
        if not result and subscription:
            raise AuthenticationException("Subscription '{}' not found. "
                                          "Check the spelling and casing and try again.".format(subscription))
        elif not result and not subscription:
            raise AuthenticationException("No subscription found. Run 'az account set' to select a subscription.")
        elif len(result) > 1:
            raise AuthenticationException("Multiple subscriptions with the name '{}' found. "
    ...
    def get_login_credentials(self, resource=None, subscription_id=None, aux_subscriptions=None):
        ...
        resource = resource or self.cloud_type.endpoints.active_directory_resource_id
    ...
    def refresh_accounts(self, subscription_finder=None):
        ...
        allow_debug_adal_connection()
        subscription_finder = subscription_finder or SubscriptionFinder(self.cloud_type,
                                                                        self.auth_ctx_factory,
                                                                        self._creds_cache.adal_token_cache)
        refreshed_list = set()
        result = []
    ...
    def get_sp_auth_info(self, subscription_id=None, name=None, password=None, cert_file=None):
        ...
        if name and (password or cert_file):
            ...
            if user_type == _SERVICE_PRINCIPAL:
                ...
                raise AuthenticationException('SDK Auth file is only applicable on service principals')
        ...
        for e in endpoint_mappings:
            result[endpoint_mappings[e]] = getattr(self.cloud_type.endpoints, e)
        return result
...
class SubscriptionFinder(object):
    ...
    """
    def __init__(self, cloud_type, auth_context_factory, adal_token_cache, arm_client_factory=None):
        ...
        def create_arm_client_factory(credentials):
            ...
            api_version = get_api_version(self.cloud_type, ResourceType.MGMT_RESOURCE_SUBSCRIPTIONS)
            return change_ssl_cert_verification(client_type(credentials, api_version=api_version,
                                                            base_url=self.cloud_type.endpoints.resource_manager))
    ...
    def find_through_authorization_code_flow(self, tenant, resource, authority_url):
        ...
        if not results.get('code'):
            raise AuthenticationException('Login failed')  # error detail is already displayed through previous steps
    ...
    def _create_auth_context(self, tenant, use_token_cache=True):
        ...
        return self._auth_context_factory(self.cloud_type, tenant, token_cache)
...
class CredsCache(object):
    ...
    def __init__(self, cloud_type, auth_ctx_factory=None, async_persist=True):
    ...
    def retrieve_token_for_user(self, username, tenant, resource):
        context = self._auth_ctx_factory(self._cloud_type, tenant, cache=self.adal_token_cache)
        token_entry = context.acquire_token(resource, username, _CLIENT_ID)
        if not token_entry:
            raise AuthenticationException("Could not retrieve token from local cache.{}".format(
    ...
    def retrieve_token_for_service_principal(self, sp_id, resource, tenant, use_cert_sn_issuer=False):
        ...
        if not matched:
            raise AuthenticationException("Please run 'az account set' to select active account.")
        cred = matched[0]
        context = self._auth_ctx_factory(self._cloud_type, cred[_SERVICE_PRINCIPAL_TENANT], None)
    ...
    def retrieve_secret_of_service_principal(self, sp_id):
        ...
        if not matched:
            raise AuthenticationException("No matched service principal found")
        cred = matched[0]
...
class ServicePrincipalAuth(object):
    ...
    def __init__(self, password_arg_value, use_cert_sn_issuer=None):
        if not password_arg_value:
            raise AuthenticationException('missing secret or certificate in order to '
