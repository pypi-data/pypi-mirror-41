...
from azureml._vendor.azure_cli_core.util import get_config_parser
...
def _set_active_cloud_removed(cli_ctx, cloud_name):
    cli_ctx.config.set_value('cloud', 'name', cloud_name)


def get_active_cloud_name_removed(cli_ctx):
    try:
...
def get_clouds_removed(cli_ctx):
    ...
    for section in config.sections():
        ...
        if c.profile not in API_PROFILES:
            raise AuthenticationException('Profile {} does not exist or is not supported.'.format(c.profile))
        ...
        clouds.append(c)
    active_cloud_name = get_active_cloud_name_removed(cli_ctx)
    for c in clouds:
...
def get_cloud_removed(cli_ctx, cloud_name):
    cloud = _get_cloud_removed(cli_ctx, cloud_name)
    if not cloud:
        raise CloudNotRegisteredException(cloud_name)
    return cloud


def get_active_cloud_removed(cli_ctx=None):
    if not cli_ctx:
        from azure.cli.core import get_default_cli
        cli_ctx = get_default_cli()
    try:
        return get_cloud_removed(cli_ctx, get_active_cloud_name_removed(cli_ctx))
    except CloudNotRegisteredException as err:
        logger.warning(err)
        logger.warning("Resetting active cloud to'%s'.", AZURE_PUBLIC_CLOUD.name)
        _set_active_cloud_removed(cli_ctx, AZURE_PUBLIC_CLOUD.name)
        return get_cloud_removed(cli_ctx, AZURE_PUBLIC_CLOUD.name)
...
def set_cloud_subscription_removed(cli_ctx, cloud_name, subscription):
    if not _get_cloud_removed(cli_ctx, cloud_name):
        raise CloudNotRegisteredException(cloud_name)
    config = get_config_parser()
    config.read(CLOUD_CONFIG_FILE)
    if subscription:
...
def _set_active_subscription_removed(cli_ctx, cloud_name):
...
def switch_active_cloud_removed(cli_ctx, cloud_name):
    if cli_ctx.cloud.name == cloud_name:
        return
    if not _get_cloud_removed(cli_ctx, cloud_name):
        raise CloudNotRegisteredException(cloud_name)
    _set_active_cloud_removed(cli_ctx, cloud_name)
...
def add_cloud_removed(cli_ctx, cloud):
    if _get_cloud_removed(cli_ctx, cloud.name):
        raise CloudAlreadyRegisteredException(cloud.name)
    _save_cloud(cloud)


def update_cloud_removed(cli_ctx, cloud):
    if not _get_cloud_removed(cli_ctx, cloud.name):
        raise CloudNotRegisteredException(cloud.name)
    _save_cloud(cloud, overwrite=True)


def remove_cloud_removed(cli_ctx, cloud_name):
    if not _get_cloud_removed(cli_ctx, cloud_name):
        raise CloudNotRegisteredException(cloud_name)
