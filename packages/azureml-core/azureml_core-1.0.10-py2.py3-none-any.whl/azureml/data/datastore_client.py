...
class _DatastoreClient:
    ...
    def register_azure_blob_container(workspace, datastore_name, container_name, account_name, sas_token=None,
        ...
        return _DatastoreClient._register_azure_storage(
            ...
            sas_token or account_key, protocol, endpoint, overwrite, create_if_not_exists)
    ...
    def _register_azure_storage(ws, datastore_name, storage_type, container_name, account_name,
                                ...
                                create_if_not_exists, auth=None, host=None):
        ...
        datastore = DataStoreDto(datastore_name, storage_type, storage_dto)
        module_logger.debug("Converted data into DTO")
        return _DatastoreClient._register(ws, datastore, create_if_not_exists, overwrite, auth, host)
    ...
    def _register_azure_data_lake(ws, datastore_name, store_name, tenant_id, client_id, client_secret,
        ...
        datastore = DataStoreDto(datastore_name, constants.AZURE_DATA_LAKE, azure_data_lake_section=storage_dto)
        module_logger.debug("Converted data into DTO")
        return _DatastoreClient._register(ws, datastore, False, overwrite, auth, host)
    ...
    def _register_azure_sql_database(ws, datastore_name, server_name, database_name, tenant_id, client_id,
        ...
        datastore = DataStoreDto(datastore_name, constants.AZURE_SQL_DATABASE, azure_sql_database_section=storage_dto)
        module_logger.debug("Converted data into DTO")
        return _DatastoreClient._register(ws, datastore, False, overwrite, auth, host)
    ...
    def _register_azure_postgre_sql(ws, datastore_name, server_name, database_name, user_id, user_password,
        ...
        datastore = DataStoreDto(datastore_name, constants.AZURE_POSTGRESQL, azure_postgre_sql_section=storage_dto)
        module_logger.debug("Converted data into DTO")
        return _DatastoreClient._register(ws, datastore, False, overwrite, auth, host)

    @staticmethod
    def _register(ws, dto, create_if_not_exists, overwrite, auth, host):
        try:
            client = _DatastoreClient._get_client(ws, auth, host)
            ...
            if e.response.status_code == 400 and overwrite:
                ...
                client.data_store.update(dto.name, ws._subscription_id, ws._resource_group,
                                         ws._workspace_name, dto, create_if_not_exists)
            else:
