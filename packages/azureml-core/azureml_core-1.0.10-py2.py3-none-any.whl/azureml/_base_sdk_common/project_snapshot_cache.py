...
class ContentSnapshotCache(object):
    ...
    def get_workspace_hash(self):
        h = hashlib.sha256()
        h.update(self.project_context.get_workspace_id().encode('utf-8'))
        return h.hexdigest()
    ...
    def get_latest_snapshot(self):
        ...
        if not os.path.isfile(os.path.abspath(latest_snapshot_cache_file)):
            ...
            url = self.project_context.get_history_service_uri() + \
                "/content/v1.0" + self.project_context.get_workspace_uri_path() + "/snapshots/latest/metadata"

            headers = self.project_context.get_auth().get_authentication_header()
