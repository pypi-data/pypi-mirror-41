...
class Tagger(BaseEstimator):
    ...
    def fit(self,
        ...
        for epoch in range(epochs):
            ...
            flow = self.batch_flow(X, y, batch_size)
            losses = []
            if verbose > 0:
            ...
            #     X, y, batch_size=predict_batch_size, verbose=1)
            # dev_score = None
            # if X_dev is None:
            #     if verbose > 0:
            ...
            if X_dev is not None:
                ...
                with torch.no_grad():
                    ...
                    flow = self.batch_flow(X_dev, y_dev, predict_batch_size)
                    dev_losses = []
    ...
    def batch_flow(self,
        ...
        if sample_shuffle:
            inds = shuffle(inds)
        while True:
            for i in range(math.ceil(len(inds) / batch_size)):
                x_batch = x_data[i * batch_size:(i + 1) * batch_size]
                y_batch = y_data[i * batch_size:(i + 1) * batch_size]
                diff = batch_size - len(x_batch)
                if diff:
                    x_batch += x_data[:diff]
                    y_batch += y_data[:diff]
                ...
                if self._sentence_lm.use_char:
                    ...
                    tcx, tcy, tcl = (torch.from_numpy(np.asarray(x_batch)),
                                     torch.from_numpy(np.asarray(y_batch)),
                                     torch.from_numpy(np.asarray(len_batch)))
                    yield tcx, tcy, tcl, None, None
