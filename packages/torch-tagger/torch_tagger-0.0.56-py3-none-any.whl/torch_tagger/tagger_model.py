...
class TaggerModel(nn.Module):
    ...
    def __init__(self, vocab_size, tag_to_ix, embedding_dim, hidden_dim,
                 ...
                 rnn_type, device, embedding_trainable, use_crf, use_char,
        ...
        self._encoder = EncoderRNN(
            ...
            device, embedding_trainable, use_char, char_hidden_dim)

        if use_crf:
            self._decoder = DecoderCRF(tag_to_ix, device)
        else:
        ...
        if isinstance(use_char, str):
            if use_char.lower() == 'rnn':
                self._char_features = CharRNN(
                    char_vocab_size, char_max_len, char_embedding_dim,
                    char_hidden_dim, char_dropout_p, char_bidirectional,
                    rnn_type, device, embedding_trainable)
            elif use_char.lower() == 'cnn':
                self._char_features = CharCNN(char_vocab_size, char_max_len,
                                              char_embedding_dim,
                                              char_hidden_dim, char_dropout_p,
                                              device, embedding_trainable)
