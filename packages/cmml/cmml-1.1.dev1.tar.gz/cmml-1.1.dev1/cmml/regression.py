...
def standRegressionpredict(xArr, yArr,xtest):
    w = standRegression(trainX,trainY)
    #print (wMat)
    y=xtest*w
    return y
...
def regularize(xMat):  # 按列进行规范化
    #浅拷贝的时候，修改原来的对象，深拷贝的对象不会发生改变,copy是深拷贝
...
def crossValidation(xArr,yArr,numVal=10):
    ...
    for i in range(numVal):
        ...
        for k in range(30):
            ...
            matTestX = (matTestX-meanTrain)/varTrain
            # 测试回归效果并存储
            yEst = matTestX * mat(wMat[k,:]).T + mean(trainY)c
            # 计算误差
...
def abaloneTest(abX, abY):
    ...
    # 使用不同的核进行预测
    oldyHat002 = lwlrTest(abX[0:99], abX[0:99], abY[0:99], 0.02)
...
def ridgepredict(trainX,trainY,TestX):
    wMat = ridgeTest(trainX,trainY)
    #print (wMat)
    matTestX=TestX
    for k in range(30):
...
if __name__ == '__main__':
    #trainX,trainY = loadDataSet("C:\\zf.txt")
    #trainX,trainY = loadDataSet("zf.txt")s
    #trainX,trainY = loadDataSet("zf1.txt")
    trainX,trainY = loadDataSet("zf2.txt")
    
    crossValidation(trainX,trainY)
    ...
    print (y)
    '''
    #ridgeTest(trainX,trainY)
    #wMat=ridgeTest(trainX,trainY)
    #y=stageWisepredict(trainX,trainY,[1,1,2,5])
    y=ridgepredict(trainX,trainY,[1,1,2,5])
    print (y)
    #y=predict([1,1])
    #print (y)
    # regression2()
    ...
    # regression4()
    #regression5()
    ridgeWeights = ridgeTest(trainX,trainY)
    fig = plt.figure()
    ax = fig.add_subplot(111)
    ax.plot(ridgeWeights)
    ...
    y_test=ridgepredict(trainX,trainY,X_test)
    error=sqrt(rssError(y_test, Y_test))
    print (error)
    pass