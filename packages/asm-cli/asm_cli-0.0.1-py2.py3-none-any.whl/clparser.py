...
class CreateParser:
    def __init__(self):
        self.version = '2.8.5'
        self.script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'settings.ini')

        cf = ConfigParser()
        for filename in os.listdir('..'):
            if re.match("asm_api*", filename):
                settings_path = filename
        cf.read(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "{}/settingsz.ini".format(settings_path)))
    ...
    def create_parser(self):
        ...
        parser = argparse.ArgumentParser(
            ...
            epilog='(c) 2019 Alphaopen LLC | www.alphaopen.com',
            add_help=False
        )
        ...
        cert_group.add_argument('-i', '--info', action='store_true',
                    help='View Trusted Certificates')
