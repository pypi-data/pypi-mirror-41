...
class TestJonesMatrix(object):
    ...
    def test_from_matrix(self):
        ...
        proposal = M1.get()
        assert np.linalg.norm(proposal - solution) < eps, sys._getframe(
        ).f_code.co_name + ".py --> example: (1,2,3,4j)"
    ...
    def test_diattenuator_linear(self):
        ...
        M1 = Jones_matrix('M_linear')
        M1.diattenuator_lineal(p1=.75, p2=0.25, angle=0 * degrees)
        proposal = M1.get()
        assert np.linalg.norm(proposal - solution) < eps, sys._getframe(
        ...
        M1 = Jones_matrix('M_linear')
        M1.diattenuator_lineal(p1=.75, p2=0.25, angle=90 * degrees)
        proposal = M1.get()
        assert np.linalg.norm(proposal - solution) < eps, sys._getframe(
        ...
        M1 = Jones_matrix('M_linear')
        M1.diattenuator_lineal(p1=.75, p2=0.25, angle=45 * degrees)
        proposal = M1.get()
        assert np.linalg.norm(proposal - solution) < eps, sys._getframe(
    ...
    def test_retarder_linear(self):
        ...
        proposal = M1.get()
        assert np.linalg.norm(proposal - solution) < eps, sys._getframe(
        ).f_code.co_name + ".py --> example: no retarder"
        ...
        proposal = M1.get()
        assert np.linalg.norm(proposal - solution) < eps, sys._getframe(
        ).f_code.co_name + ".py --> example: delay = 90*degrees, angle=45"
    ...
    def test_retarder_material(self):
        ...
        proposal = M1.get()
        assert np.linalg.norm(proposal - solution) < eps, sys._getframe(
        ).f_code.co_name + ".py --> example: lambda/4"
    ...
    def test_diattenuator_azimuth_ellipticity(self):
        ...
        M1 = Jones_matrix('diat_az_el')
        M1.diattenuator_azimuth_ellipticity(
            p1=0.75, p2=0.25, az=0 * degrees, el=90 * degrees, angle=0)
        proposal = M1.get()
        assert np.linalg.norm(proposal - solution) < eps, sys._getframe(
        ).f_code.co_name + ".py --> 2"
        ...
        M1 = Jones_matrix('diat_az_el')
        M1.diattenuator_azimuth_ellipticity(
            p1=1, p2=0, az=0 * degrees, el=45 * degrees, angle=0)
        proposal = M1.get()
        assert np.linalg.norm(proposal - solution) < eps, sys._getframe(
        ).f_code.co_name + ".py --> 3"
        ...
        M1 = Jones_matrix('diat_az_el')
        M1.diattenuator_azimuth_ellipticity(
            p1=0.75, p2=0.25, az=45 * degrees, el=0 * degrees, angle=0)
        proposal = M1.get()
        assert np.linalg.norm(proposal - solution) < eps, sys._getframe(
        ).f_code.co_name + ".py --> 4"
        ...
        M1 = Jones_matrix('diat_az_el')
        M1.diattenuator_azimuth_ellipticity(
            p1=1, p2=0, az=45 * degrees, el=45 * degrees, angle=0)
        proposal = M1.get()
        assert np.linalg.norm(proposal - solution) < eps, sys._getframe(
        ).f_code.co_name + ".py --> 5"  # TODO: Faltan parameters
