...
class TestStokes(object):
    ...
    def test_sum(self):
        ...
        proposal = M3.get()
        assert np.linalg.norm(proposal - solution) < eps, sys._getframe(
        ).f_code.co_name + ".py --> example: 0-0*degrees"
        ...
        proposal = M3.get()
        assert np.linalg.norm(proposal - solution) < eps, sys._getframe(
        ).f_code.co_name + ".py --> example: 0-45*degrees"
    ...
    def test_general_carac_angles(self):
        ...
        M1.general_carac_angles(
            alpha=0 * degrees, delta=0 * degrees, intensity=1, pol_degree=0)
        proposal = M1.get()
        assert np.linalg.norm(proposal - solution) < eps, sys._getframe(
        ).f_code.co_name + ".py --> example: (0,0,1,0)"

        solution = matrix([[1], [1], [0], [0]])
        M1 = Stokes('l_gen')
        M1.general_carac_angles(
            alpha=0 * degrees, delta=0 * degrees, intensity=1, pol_degree=1)
        proposal = M1.get()
        assert np.linalg.norm(proposal - solution) < eps, sys._getframe(
        ).f_code.co_name + ".py --> example: (0,0,1,1)"

        solution = matrix([[1], [0], [0], [0.5]])
        M1 = Stokes('l_gen')
        M1.general_carac_angles(
            alpha=45 * degrees, delta=90 * degrees, intensity=1, pol_degree=.5)
        proposal = M1.get()
        assert np.linalg.norm(proposal - solution) < eps, sys._getframe(
        ).f_code.co_name + ".py --> example: (45,90,1,0.5)"

        solution = matrix([[1], [-0.5], [0], [0]])
        M1 = Stokes('l_gen')
        M1.general_carac_angles(
            alpha=90 * degrees, delta=45 * degrees, intensity=1, pol_degree=.5)
        proposal = M1.get()
        assert np.linalg.norm(proposal - solution) < eps, sys._getframe(
        ).f_code.co_name + ".py --> example: (90,45,1,0.5)"
    ...
    def test_circular_light(self):
        ...
        proposal = M1.get()
        assert np.linalg.norm(proposal - solution) < eps, sys._getframe(
        ).f_code.co_name + ".py --> example: kind='r'"
        ...
        proposal = M1.get()
        assert np.linalg.norm(proposal - solution) < eps, sys._getframe(
        ).f_code.co_name + ".py --> example: kind='l'"
