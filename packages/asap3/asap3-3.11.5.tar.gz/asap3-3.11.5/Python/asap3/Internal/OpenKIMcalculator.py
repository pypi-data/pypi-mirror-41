...
def _get_openkim_model_string(verbose):
    exename = "kim-api-v2-collections-info"
    args = ' models'
    sistername = 'kim-api-v2-collections-management'
    ...
    exepath = None
    try:
        prefix = subprocess.check_output("pkg-config libkim-api-v2 --variable=libexecdir", shell=True).decode('utf-8').strip()
    except subprocess.CalledProcessError:
        pass
    else:
        exepath = os.path.join(prefix, 'kim-api-v2', exename)
        if verbose:
            print("Found using pkg-config:", exepath)

    # Finally, try to look for where kim-api-v2-collections-management and then check ../libexec
    if not exepath and hasattr(shutil, 'which'):
        ...
        if management:
            ...
            exepath = os.path.join(prefix, 'libexec', 'kim-api-v2', exename)
            if verbose:
