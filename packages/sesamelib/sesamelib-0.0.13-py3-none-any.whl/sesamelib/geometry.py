...
def load_geometry(gml_path, only_exteriors=False, dTolerance=None):
    ...
    geometry["total_points"] = sum([sum([len(p) for p in ps]) for ps in geometry["points"]])
    geometry["total_polygons"] = sum([len(p) for p in geometry["points"]])
