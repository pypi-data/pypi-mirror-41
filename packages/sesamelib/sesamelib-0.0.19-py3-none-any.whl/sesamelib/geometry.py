...
class SesameGeometry:
    ...
    def from_gml_file(cls,
        ...
        if self.envelope in [ENVELOPE_MULTI, ENVELOPE_MULTI_UNION]:
        ...
        elif self.envelope in [ENVELOPE_MULTI, ENVELOPE_MULTI_UNION]:
            _geoms = []
...
class IndexedGeometry:
    ...
    def from_gml_file(cls,
        ...
        def _build_index(geometry, env, cell_size, max_distance):
            _index = dict()
            x1, x2, y1, y2 = env
            ...
            for x in X:
                for y in Y:
                    ...
                    within  = geometry.ogr_contains(poly)
                    intersect = geometry.ogr_intersects(poly)
                    _index[(x, y)] = {
                        "intersect": intersect,
                        "within": within,
                        "distance": d}
        ...
        self = cls()
        self.sesame_geometry = SesameGeometry.from_gml_file(gml_path, **kwargs)
        self.cell_size = cell_size
        self.max_distance = max_distance

        # build the index
        # Among all the possible cells to explore we limit to the envelops + max_distance
        envs = self.sesame_geometry.get_envelopes()
        logger.debug("envelope to explore %s" % envs)

        index = {}
        for env in envs:
            _index = _build_index(self.sesame_geometry,
                                  env,
                                  cell_size,
                                  max_distance)
            ...
            for k, v in _index.items():
                if k in index:
                    # merge
                    index[k]["intersect"] = index[k]["intersect"] or v["intersect"]
                    index[k]["within"] = index[k]["within"] or v["within"]
                    index[k]["distance"] = min(index[k]["distance"], v["distance"])
                else:
                    index[k] = v
    ...
    def distance(self, x, y, enlarge=0.0):
        ...
        if intersect:
            return self.sesame_geometry.distance(x, y, enlarge=enlarge)
