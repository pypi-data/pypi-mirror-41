...
layout(binding = 9) uniform sampler2D diffuseSampler;
layout(binding = 10) uniform sampler2D specSampler;
...
void main() {
	...
	for (int i = 0; i < lbo.numLights; ++i) {
		...
		diffuseColor += diffuseTexColor.rgb * ubo.kd.rgb * lbo.lights[i].diffuse.rgb * diffterm * dist * shadow;
		
		specularColor += specTexColor.rgb * ubo.ks.rgb * lbo.lights[i].specular.rgb * pow(specterm, 80.) * dist * shadow;
	}
