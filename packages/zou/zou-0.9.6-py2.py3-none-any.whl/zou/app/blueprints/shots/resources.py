...
class SequenceResource(Resource):
    ...
    def delete(self, sequence_id):
        ...
        """
        try:
            sequence = sequences_service.get_sequence(sequence_id)
            user_service.check_manager_project_access(sequence["project_id"])

            deleted_sequence = sequences_service.remove_sequence(
                sequence_id,
                force=force
            )
        except ShotNotFoundException:
            ...
            return {
                "error": True,
                "message": \
                  "There are shots related to this sequence, please remove them."
            }, 400

        return deleted_sequence, 204
