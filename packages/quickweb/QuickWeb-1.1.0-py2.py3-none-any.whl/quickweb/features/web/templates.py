...
class TemplateController(object):
    ...
    @controller.publish
    def index(self):
        return self._template_engine.render(self._path)
...
class QWTemplateLoader(BaseLoader):
    ...
    def get_source(self, environment, template):
        ...
        if template_ext == '.md':
            source = markdown(source, extensions=[
        ...
        if template_ext in ['.html', '.md'] and not re.match(r'^\s*{% extends "', source) \
            ...
            if template_ext == '.md':
                extend_file = '_base_md.html'
                extend_block = 'markdown'
            else:
                extend_file = '_base.html'
                extend_block = 'content'
            _base_filename = join(self.path, extend_file)
            if exists(_base_filename):
                extended_source = '{%% extends "%s" %%}\n' % extend_file
                extended_source += '{%% block %s %%}' % extend_block
