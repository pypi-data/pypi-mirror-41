...
class Convolutional:
    ...
    def forword(flow_data, layer, is_train):
        ...
        # 开始卷积
        for batch in range(output.shape[0]):  # 遍历输出的样本数
            for channel in range(output.shape[1]):  # 遍历输出的通道数，输出的通道数等于卷积核的个数
                for height in range(output.shape[2]):  # 遍历输出的高
                    for width in range(output.shape[3]):  # 遍历输出的宽
                        # 滑动窗口截取部分
                        sliding_window = flow_data[batch][:,
                                         height * layer['stride']:height * layer['stride'] + kernel_height,
                                         width * layer['stride']:width * layer['stride'] + kernel_width
                                         ]
                        output[batch][channel][height][width] = np.sum(
                            (sliding_window * layer["W"][channel]) + layer["b"][channel])
        return output, layer
    ...
    def backword(flow_data, layer, config):
        ...
        for batch in range(flow_data.shape[0]):  # 遍历输入梯度的样本数
            for channel in range(flow_data.shape[1]):  # 遍历输入梯度的通道数，输入梯度的通道数等于卷积核的个数
                for height in range(flow_data.shape[2]):  # 遍历输入梯度的高
                    for width in range(flow_data.shape[3]):  # 遍历输入梯度的宽
                        # 前向传播输入数据，滑动截取窗口
                        sliding_window = forword_input[batch][:,
                                         height * layer['stride']:height * layer['stride'] + kernel_height,
                                         width * layer['stride']:width * layer['stride'] + kernel_width
                                         ]
                        # dx
                        ...
                        height * layer['stride']:height * layer['stride'] + kernel_height,
                        width * layer['stride']:width * layer['stride'] + kernel_width
                        ] += flow_data[batch][channel][height][width] * layer['W'][channel]

                        # 单个卷积核梯度 = 前向输入数据的滑动窗口 * 梯度对应通道（卷积核），对应高宽
                        layer["dW"][channel] += (flow_data[batch][channel][height][width] * sliding_window) / batch_size

                        layer["db"][channel][0][0][0] += flow_data[batch][channel][height][width] / batch_size
        if layer['padding'] != 0:
