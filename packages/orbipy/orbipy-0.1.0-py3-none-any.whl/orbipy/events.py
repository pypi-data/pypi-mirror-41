...
Event defined as time when some function (called event-function) reaches
specific value. Event-function calculates in each step of integration
process.
...
from .solout import base_solout, default_solout
...
#            raise ValueError("Empty events list")
    ...
    def prop(self, s0, t0, t1, ret_df=True):
        ...
        df = self.model.prop(s0, t0, t1, ret_df)
        evdf = self.accurate(ret_df)
...
#                print('not acc shape:', len(sa))
            evout.append([e[0], e[1], *sa])
        
#        self.model.solout = self.solout
        
        if ret_df:
            df = pd.DataFrame(evout, columns=event_detector.columns+self.model.columns)
            df.set_index(event_detector.columns[0], inplace=True)
            return df
...
#    error_empty_event_list = 'Events list is empty'
    
    def __init__(self, events):
#        if not events:
#            raise ValueError(self.error_empty_event_list)
        self._events = events
        self.reset()
...
class event_chain(base_event):
    '''
    event_chain looks like last event in chain but works like sequence of 
    events: events should occur in specified order and only last event will
    behave like event
    '''
...
class center_event(base_event):
    ...
    '''
    def __init__(self, value, center, direction=0, terminal=True, accurate=True, count=-1):
...
class model_event(base_event):
    ...
    '''
    def __init__(self, value, model, direction=0, terminal=True, accurate=True, count=-1):
        super().__init__(value, direction, terminal, accurate, count)
        self.model = model
...
class eventInsidePathXY(center_event):
    def __init__(self, path, center, direction=0, terminal=True, accurate=True, count=-1):
        super().__init__(0, center, direction, terminal, accurate, count)
        if len(center) < 2:
    ...
    def setPath(self, path):
        ...
        r = r[order]
        self.rint = interp1d(theta, r, fill_value='extrapolate')
        self.theta = theta
        self.r = r
...
class eventFOV(center_event):
    def __init__(self, orbit, r, center, direction=0, terminal=True, accurate=True, count=-1):
        super().__init__(r, center, direction, terminal, accurate, count)
        self.setOrbit(orbit)
