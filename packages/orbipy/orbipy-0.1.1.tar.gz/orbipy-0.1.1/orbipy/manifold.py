...
#            print('Manifold calculation:')
    ...
    def calc_branch(self,
        ...
        for i, s in enumerate(arr):
            ...
            df, ev = detector.prop(s1, 0.0, maxt)

            df.reset_index(inplace=True)
            df.insert(0, 'tr', i)
            df.insert(0, 'br', branch_idx)
            df.set_index('br', inplace=True)

            ev.reset_index(inplace=True)
            ev.insert(0, 'tr', i)
            ev.insert(0, 'br', branch_idx)
            ev.set_index('br', inplace=True)
...
#        print('Trajectories calculated:', len(tr_lst))
        ...
        if self.trajs is None:
            self.trajs = pd.concat(tr_lst)
        else:
            self.trajs = self.trajs.append(tr_lst)
        
        if self.evout is None:
            self.evout = pd.concat(ev_lst)
        else:
            self.evout = self.evout.append(ev_lst)
