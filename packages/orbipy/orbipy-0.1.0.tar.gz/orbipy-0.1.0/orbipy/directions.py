...
class unstable_direction(base_direction):
    ...
    '''
    def __init__(self, model, event=np.pi, maxt=10*np.pi):
    ...
    def __call__(self, t, s):
        ...
        s0[:s.shape[0]] = s
        if self.is_event:
            _, ev = event_detector(self.model, self.event).prop(s0, t, t+self.maxt)
            # i(0),cnt(1),t(2),x(3),y(4),z(5),vx(6),vy(7),vz(8),stm00(9), ...
            stm = ev[9:].reshape(6,6)
        else:
        ...
        # first two eigenvalues should be real
        # choose positive
        idx = 0 if eigvals[0] > 0 else 1
        d = eigvecs[idx,3:6].real
