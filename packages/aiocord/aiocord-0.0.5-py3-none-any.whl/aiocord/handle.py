...
class Handle:
    ...
    def ready(self, data):
        ...
        guilds = self._cache.ready(user_data, channels_data, guilds_data)

        return version, guilds, session_id
    ...
    def presence_update(self, data):
        ...
        values = self._cache.presence_update(guild_id, user_id, presence_data)

        guild, member, old_member, presence, old_presence = values

        return guild, member, old_member, presence, old_presence
    ...
    def voice_state_update(self, data):
        ...
        guild_id, channel_id, user_id, voice_state_data = parts

        parts = (guild_id, channel_id, user_id, voice_state_data)

        values = self._cache.voice_state_update(*parts)

        guild, channel, user, voice_state = values

        return guild, channel, user, voice_state

    def voice_server_update(self, data):

        guild_id, endpoint, token = breaks.voice_state_update(data)

        guild = self._cache.voice_state_update(guild_id)
