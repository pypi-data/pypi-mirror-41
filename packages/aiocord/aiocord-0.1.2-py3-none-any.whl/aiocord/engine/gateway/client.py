...
class Client:
    ...
    async def ready(self, shard, data):
        ...
        version, guilds, session_id = self._handle.ready(data)

        guild_ids = tuple(guild.id for guild in guilds)
    ...
    async def presence_update(self, shard, data):
        ...
        parts = self._handle.presence_update(data)
    ...
    async def voice_state_update(self, shard, data):
        ...
        guild, channel, user, voice_state = parts

        name = 'voice state update'

        values = (shard, guild, channel, user, voice_state)

        await self._dispatch(name, values)

    async def voice_server_update(self, shard, data):

        guild_id, endpoint, token = self._handle.voice_server_update(data)
