...
class TestInitiators(TestCase):
    ...
    def test_folder(self):
        lst = Query.folder(path('test_fluq_extensions', 'folder')).select(lambda z: z.name).order_by(lambda z: z).to_list()
        self.assertListEqual(['file1', 'file2'], lst)

    def test_folder_as_path(self):
        lst = Query.folder(Path(path('test_fluq_extensions', 'folder'))).select(lambda z: z.name).order_by(
            lambda z: z).to_list()
        self.assertListEqual(['file1', 'file2'], lst)
