...
class TestPlots(TestCase):
    ...
    def test_series_hist(self):
        ...
        (Query
         .en([
            ('no_trimmer',trimmer()),
            ('unite',trimmer(unite_before=-1,unite_after=1)),
            ('unite_perc',trimmer(unite_before_percentile=10,unite_after_percentile=90)),
            ('drop',trimmer(drop_before=-1,drop_after=1)),
            ('drop_perc',trimmer(drop_before_percentile=10,drop_after_percentile=90))
            ])
    ...
    @unittest.skip("Something whong with hist2d, will seriously refactor it soon anyway")
