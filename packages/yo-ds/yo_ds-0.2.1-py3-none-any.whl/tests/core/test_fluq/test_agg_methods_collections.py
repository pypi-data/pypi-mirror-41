...
class ToCollectionMethodsTests(LinqTestBase):
    ...
    def test_to_dictionary_special_vase(self):
        result = Query.args(1,2,3,4).group_by(lambda z: z%2).to_dictionary(lambda z: z.key, lambda z: len(z.value))
        self.assertDictEqual({0:2,1:2},result)
