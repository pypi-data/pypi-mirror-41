...
class with_plots(Callable[[Queryable[T]],Queryable[ItemWithPlot[T]]]):
    ...
    def __call__(self, q: Queryable[T]) -> Queryable[ItemWithPlot[T]]:
        return Queryable(self._make(q),q.length)


class with_progress_bar(Callable[[Queryable[T]],Queryable[T]]):
    def __init__(self, console=False, hide=False, **kwargs):
        self.console = console
        self.hide = hide
        self.kwargs = kwargs
    def __call__(self, q: Queryable[T])->Queryable[T]:
        if self.hide:
            return q
        try:
            length = q.length
