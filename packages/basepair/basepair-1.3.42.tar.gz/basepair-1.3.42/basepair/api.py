...
class BpApi(object):
    ...
    def __init__(self, conf=None, scratch=None, use_cache=False,
        ...
        self.DATATYPES = ['dna-seq', 'chip-seq', 'rna-seq', 'atac-seq', 'other']
        self.LIST_TYPES = ['samples','analyses','genomes','workflows','analysis']
    ...
    def _check_genome(self,genome):
        ...
        if not any([genome in i['name'] for i in self.genomes]):
            print(('The provided genome, {genome}, does not exist in Basepair!' +
                ' Proceeding anyway...').format(genome=genome), file=sys.stderr)
    ...
    def create_sample(self, data, source='api', upload=True):
        ...
        if 'id' not in data:
            ...
            if res.status_code == 201:
                ...
                if self.verbose:
                    print('created: sample with id', sample_id, file=sys.stderr)
    ...
    def get_analysis_files(self, analysis, tags=None, kind='exact',
        ...
        if tags is not None:
            if type(tags) != list:
                print('Invald tags argument. Provide a list of list of tags.',file=sys.stderr)
                return
            else:
    ...
    def get_file_by_tags(self, sample, tags=None,
                         kind=None, exclude=None, analysis_tags=None,
    ...
    def download_analysis(self,uid,tags=None,tagkind=None,outdir='.'):
        ...
        if tagkind not in ['exact','diff','subset']:
            print('Invalid tagkind, choose one of: exact, diff, subset',file=sys.stderr)
            return
    ...
    def print_data(self,data_type='',uid=None,json=False):
        ...
        if data_type not in self.LIST_TYPES:
            print(
                '--list must be one of the following: {}'.format(','.join(self.LIST_TYPES)))
            return
