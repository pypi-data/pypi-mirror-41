...
# Y变量统一默认为"y"，设置为其他会报错
class logistic_score_card:

    def __init__(self,
                 max_deeps=2,
                 min_info_gain_epos=0.01,
                 multiprocessing=0,
                 base_score=600,
                 increase_score = 50,
                 min_iv = 0.1,
        ...
        self.max_corr = max_corr #选取相关性的标准（选取相关性小于0.6的值）
        ...
        self.cut_pionts_list = []  # 连续变量的切分点，按小于等于，大于切分。（如有空值返回为'null'）
        self.cut_pionts_result = []  # 连续变量的切分点，按小于等于，大于切分。（如有空值返回为'null'）例如:[col,['null',3,7]]
    ...
    def get_cut_all_not_null(self, data_not_null, col, deeps=2):
        if deeps > 0:
            ...
            if cut_point != '无最优切分点':
                self.cut_pionts_list.append([col, cut_point])
    ...
    # 排序切分点以及展示样式
    def transform_cut_pionts_list(self):
        # 排序切分点
        self.cut_pionts_list = [[i, j] for i, j in
                                pd.DataFrame(self.cut_pionts_list, columns=['col', 'col_cut']).sort_values(
                                    by=['col', 'col_cut']).values]
        for i in pd.DataFrame(self.cut_pionts_list, columns=['col', 'col_cut'])['col'].unique():
            col_cuts = []
            for col, col_cut in self.cut_pionts_list:
                if col == i:
                    col_cuts.append(col_cut)
            self.cut_pionts_result.append([i, col_cuts])
        ...
        for col, col_type in tqdm(self.col_type):
            if col_type == 1:
                ...
                if len(data_not_null) != len(data_col):
                    self.cut_pionts_list.append([col, 'null'])
                    self.get_cut_all_not_null(data_not_null, col,self.max_deeps)
                else:
                    self.get_cut_all_not_null(data_not_null, col,self.max_deeps)
        self.transform_cut_pionts_list()
    ...
    def get_cut_result(self, data,cut_pionts_result):
        logging.info('根据cut离散化连续变量进行中。。。')
        data_cut_result = data.copy()
        for col, cut_points in tqdm(cut_pionts_result):
            data_col=data[[col]]
            cluster = 1
            data_cut_result.loc[data_col[col].notnull(),col] = cluster
            for cut_point in cut_points:
                if cut_point != 'null':
                    cluster += 1
                    data_cut_result.loc[data_col[col] > cut_point, col] = cluster
    ...
    # 通过IV值和相关性选择变量（默认参数提出IV值<0.1，相关系数>0.6的变量）
    def get_iv_corr_col(self, data_woe, min_iv=0.1, max_corr=0.6):
    ...
    def get_iv_corr_logistic_l1_col(self, data_woe, min_iv=0.1, max_corr=0.6,C=0.01,penalty='l1'):
    ...
    def get_logistic_l1_col(self,data):

        lr = linear_model.LogisticRegression(penalty='l1',n_jobs=-1)
        lr.fit(data[[col for col in data.columns if col!='y']], data['y'])

    # -------------------------------------------logistic模型训练-----------------------------------------------------------------

    def plot_roc(self, test_x, test_y):
        predictions = self.logistic_reuslt.predict_proba(test_x)  # 每一类的概率
    ...
    def plot_ks(self, test_x, test_y):
        predictions = self.logistic_reuslt.predict_proba(test_x)  # 每一类的概率
        false_positive_rate, recall, thresholds = roc_curve(test_y, predictions[:, 1], drop_intermediate=False)
        pre = sorted(predictions[:, 1], reverse=True)
        num = []
        for i in range(10):
    ...
    def lr_auc_ks(self, train_x, test_x, train_y, test_y):
        start = time.time()
        self.logistic_reuslt = linear_model.LogisticRegression()
        self.logistic_reuslt.fit(train_x, train_y)
        print('跑完逻辑回归花费时间：%s' % (time.time() - start))
        plt.figure(figsize=(15, 15))
        plt.subplot(221)
        self.plot_roc(train_x, train_y)
        plt.subplot(222)
        self.plot_roc(test_x, test_y)
        plt.subplot(223)
        self.plot_ks(train_x, train_y)
        plt.subplot(224)
        self.plot_ks(test_x, test_y)
        # plt.savefig(filename_auc_ks)
        plt.show()
    ...
    def get_cut_str(self,col_cuts_test):
        ...
        for col, cut in col_cuts_test:
            ...
            col_result.append([col, num, '(-,' + str(cut[0]) + ']'])
            for i in range(len(cut) - 1):
                num += 1
                if cut[i] != 'null':
                    cut_detail = '(' + str(cut[i]) + ',' + str(cut[i + 1]) + ']'
                    col_result.append([col, num, cut_detail])
            col_result.append([col, len(cut) + 1, '(' + str(cut[-1]) + ',+)'])
        result = pd.DataFrame(col_result)
        result.columns = ['col', 'cut_point', 'cut_str']
        return result

    def get_logistic_socre_card(self,data):
        logging.info('制作评分卡进行中。。。')

        b = -self.increase_score / np.log(2)
        a = self.base_score - self.logistic_reuslt.intercept_[0] * b

        self.score_card_parameter['a'] = a
        self.score_card_parameter['b'] = b

        data_result = data[self.col_result].copy()
        data_result['y'] = data['y']
        get_cut_result = self.get_cut_result(data_result, [i for i in self.cut_pionts_result if i[0] in self.col_result])
        ...
        for cut_point, woe, col in result.values:
            ...
            woe_y_rate = len(data_woe[(data_woe[col] == woe) & (data_woe['y'] == 1)]) / len(data_woe)
        ...
        score_card_result = pd.DataFrame(score_card,
            ...
            by=['col', 'cut_point'])
        # score_card_result.sort_values(by=['col','cut_point'])
        col_coef = pd.DataFrame(self.col_result, self.logistic_reuslt.coef_[0]).reset_index()
        ...
        col_cuts_test = [i for i in self.cut_pionts_result if i[0] in self.col_result].copy()

        col_cuts_str = self.get_cut_str(col_cuts_test)
        ...
        self.score_card = score_card_result_coef_cut_iv[['col','col_type','iv','cut_point','cut_str','woe','col_coef','score','woe_num_rate','woe_y_rate']]
        # self.score_card = score_card_result_coef_cut_iv[['变量名','变量类型','iv值','切分点','切分区间','woe值','logistic系数','得分','用户占比','逾期率']]
    ...
    def fit(self, data):
        ...
        get_cut_result = self.get_cut_result(data,self.cut_pionts_result) #按切分点划分数据，得到全部的离散数据

        self.get_col_type_cut_iv(get_cut_result)  # 按切分点切分数据，求所有变量IV值
        data_woe = self.get_transform_woe(get_cut_result)  # 根据切分点讲数据woe化
        # self.col_result = self.get_iv_corr_col(data_woe, min_iv=self.min_iv, max_corr=self.max_corr)
        ...
        self.get_logistic_socre_card(data)
        logging.info('任务结束')


    def predict_score_proba(self, data):
        logging.info('预测用户分数中。。。')
        col_result_cut_pionts=[i for i in self.cut_pionts_result if i[0] in self.col_result]
        data_cut_points = self.get_cut_result(data[self.col_result], col_result_cut_pionts).fillna('null')
        ...
        for col in data_cut_points.columns:
            ...
            for cut_point in data_cut_points_col[col].unique():
                cut_point_score=list(self.score_card['score'][(self.score_card['col'] == col) & (self.score_card['cut_point'] == cut_point)])
        ...
        data_score_proba['proba']=1-1/(1+np.e**((data_score_proba['total_score']-self.score_card_parameter['a'])/self.score_card_parameter['b']))
        logging.info('预测用户分数完成')
        return data_score_proba

    def score(self,data):
        data_score_proba=self.predict_score_proba(data)
