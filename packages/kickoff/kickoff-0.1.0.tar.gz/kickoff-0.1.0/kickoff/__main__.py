...
from .exceptions import internal_exception_guard
from .logger import log

def main():
    """An entry point of `kickoff` command"""

    with internal_exception_guard():
        ...
        groups_mgr = CmdGroupsManager(galaxy, config, app_doc)

        for name, cmd in commands.items():
            groups_mgr.add_command(name, cmd)

        groups_mgr.run_top_group(app_args, app_name)
