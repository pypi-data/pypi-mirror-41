...
class MultiStart(FittingMethod):
    ...
    def __init__(self, obj_fun, ls_method="Powell",
    ...
    def fit(self, model, folds):
        ...
        def measurement_wrapper(mod_params):
            ...
            dist_b = \
                np.clip(mod_params - np.array(upper_b), a_min=0.0, a_max=None) \
                + \
                np.clip(mod_params - np.array(lower_b), a_min=None, a_max=0.0)
        ...
        assert log['best']['params'], "No params were found"
