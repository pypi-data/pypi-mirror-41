...
class CovarianceFunction(WithParameters):
    ...
    def is_psd(cov, dims=1, items=100, n_data_sets=10):
        """
    ...
    def check_psd_matrix(cov_matrix):
        ...
        if np.min(np.diagonal(cov_matrix)) < 0.0:
            return False
        # Covariance matrix is not symmetric
        if np.max(np.abs(cov_matrix - cov_matrix.T)) > 1e-4:
            return False
        ...
        if (np.max(diagless_matrix) - np.min(diagless_matrix)) < 1e-4:
            return False
        ...
        # L*L' != M
        if np.max(np.abs(cov_matrix - np.dot(l_mat, l_mat.T))) > 1e-4:
            return False
