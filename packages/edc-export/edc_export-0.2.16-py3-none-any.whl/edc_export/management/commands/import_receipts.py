...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        print('reading file...')
        error_filepath = os.path.join(os.path.expanduser(
            self.export_plan.target_path) or '', self.error_filename)
        with open(self.ack_filename, 'r') as f, (
            ...
            for row in rows:
                ...
                try:
                    ...
                    print('Failed to process file {0}'.format(
                        self.ack_filename))
                    raise ValueError(e)
                if ExportTransaction.objects.filter(export_uuid=export_uuid):
                    for export_transaction in (
                            ExportTransaction.objects.filter(export_uuid=export_uuid)):
                        self.update_or_create_export_transaction(
                            export_transaction)
    ...
    def clean_up(self, has_rejects):
        if has_rejects:
            target_path = os.path.join(
                os.path.expanduser(self.export_plan.target_path))
