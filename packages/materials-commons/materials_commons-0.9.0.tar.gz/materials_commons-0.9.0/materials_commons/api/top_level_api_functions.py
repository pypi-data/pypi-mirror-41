...
def get_project_by_id(project_id, apikey=None, remote=None):
    ...
    return Project(data=results, apikey=apikey, remote=remote)
...
def get_all_projects(apikey=None, remote=None):
    ...
    projects = []
    for r in results:
        projects.append(Project(data=r, apikey=apikey, remote=remote))
    return projects
...
def get_apikey(email, password, remote=None):
    """
    Fetch the apikey for the given user here
