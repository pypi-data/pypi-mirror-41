...
class GitManager:
    ...
    def _commitFile(self, filename, comment, branch):
        ...
        try:
            self.cloned_repo.git.commit(m=comment)
            # Push
    ...
    def UpdateListOfFilesToInstall(self, prefixlist, exclusionlist,
                                   lbinstallManager):
        ...
        if (len(prefixlist) > 0):
            ...
            self._commitFile(self.list_path, comment, self.GITLAB_BRANCH)
