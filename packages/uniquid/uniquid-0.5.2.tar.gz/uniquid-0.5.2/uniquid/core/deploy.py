...
# ssh command line with options
_SSH = 'ssh -o \"IdentitiesOnly=yes\" -i '
# scp command line with options
_SCP = 'scp -o \"IdentitiesOnly=yes\" -i '
...
_DEV_CFG_FILE = 'aws_device_cfg.json'
...
class Deploy:
    ...
    def _aws_deploy(self):
        ...
        if not self._yes_mode:
            if self._console.confirm("Do you want to proceed in automatic mode (answer yes to all dialogs)?",
                                     in_default=False):
                self._yes_mode = True
        self._console.ok(_TAB + 'The following tools must be installed and configured: awscli, Node.js (>= v8.0), npm, ssh, scp.')
    ...
    def _aws_deploy_secgroup(self, in_step):
        ...
        if (self._yes_mode or
            ...
            # tag Name of the new security group
    ...
    def _aws_deploy_instance(self, in_step):
        ...
        linux_cmd = (_SSH + self._keypair_path +
                     ' -o \"StrictHostKeyChecking=accept-new\" ubuntu@' +
                     self._ec2_public_ip + ' \"uname -a && exit 0\"')
        result = self._run_command(linux_cmd, _NUM_RETRIES, _RETRY_DELAY)
        self._console.ok(_TAB + cons.TXT_PASS)
        # install java in ec2 instance
        step = self._print_step(step, 'Provision the AWS EC2 instance.')
        linux_cmd = (_SSH + self._keypair_path + ' ubuntu@' + self._ec2_public_ip +
    ...
    def _aws_deploy_agent(self, in_step):
        ...
        self._aws_orch_props = self._get_orch_properties()
        if self._aws_orch_props is None:
        ...
        with open(config_path, 'r') as cf:
            appconfig = cf.read()
        appconfig = appconfig.replace('NETWORK_PARAMS', self._aws_orch_props.get('network', '[Error]'))
        appconfig = appconfig.replace('MQTT_BROKER', self._aws_orch_props.get('mqttBroker', '[Error]'))
        ...
        upload_cmd = _SCP + self._keypair_path + ' ' + config_path + ' ubuntu@' + self._ec2_public_ip + ':appconfig.properties'
        result = self._run_command(upload_cmd, _NUM_RETRIES, _RETRY_DELAY)
        # find untarred jar file
        for f in glob.glob(_OUTPUT_DIR + '/**/aws*.jar', recursive=True):
            jar_path = f
        if jar_path is None:
            self._console.error(ERR_AWS_CANNOT_FIND_JAR_FILE)
        upload_cmd = _SCP + self._keypair_path + ' ' + jar_path + ' ubuntu@' + self._ec2_public_ip + ':awsagent.jar'
        result = self._run_command(upload_cmd, _NUM_RETRIES, _RETRY_DELAY)
        upload_cmd = (_SSH + self._keypair_path + ' ' +
                      ' ubuntu@' + self._ec2_public_ip +
    ...
    def _aws_deploy_mqtt_proxy(self, in_step):
        ...
        with urllib.request.urlopen(package_url) as response, \
            ...
            shutil.copyfileobj(response, outfile)
        upload_cmd = _SCP + self._keypair_path + ' ' + _AWS_PROXY_FILE + ' ubuntu@' + self._ec2_public_ip + ':' + _AWS_PROXY_FILE
        result = self._run_command(upload_cmd, _NUM_RETRIES, _RETRY_DELAY)
        untar_cmd = (_SSH + self._keypair_path + ' ' +
                      ' ubuntu@' + self._ec2_public_ip +
                      ' \"tar xvz -f  ' + _AWS_PROXY_FILE + '\"')
        result = self._run_command(untar_cmd)
        self._console.ok(_TAB + cons.TXT_PASS)
        ...
        install_cmd = (_SSH + self._keypair_path + ' ' +
                      ' ubuntu@' + self._ec2_public_ip +
                      ' \"' + apt_cmd + '\"')
        result = self._run_command(install_cmd)
        make_cmd = (_SSH + self._keypair_path + ' ' +
                      ' ubuntu@' + self._ec2_public_ip +
                      ' \"cd ' + awscons.AWS_PROXY_REPO + ' && make' + '\"')
        result = self._run_command(make_cmd)
        self._console.ok(_TAB + cons.TXT_PASS)
        ...
        upload_cmd = _SCP + self._keypair_path + ' ' + proxy_cert_file + ' ubuntu@' + self._ec2_public_ip + ':' + upload_path
        result = self._run_command(upload_cmd, _NUM_RETRIES, _RETRY_DELAY)
        upload_path = os.path.join(awscons.AWS_PROXY_REPO, 'mqtt-proxy.key')
        upload_cmd = _SCP + self._keypair_path + ' ' + proxy_key_file + ' ubuntu@' + self._ec2_public_ip + ':' + upload_path
        result = self._run_command(upload_cmd, _NUM_RETRIES, _RETRY_DELAY)
        upload_path = os.path.join(awscons.AWS_PROXY_REPO, _AWS_VERISIGN_FILE)
        upload_cmd = _SCP + self._keypair_path + ' ' + _AWS_VERISIGN_FILE + ' ubuntu@' + self._ec2_public_ip + ':' + upload_path
        result = self._run_command(upload_cmd, _NUM_RETRIES, _RETRY_DELAY)
        self._console.ok(_TAB + cons.TXT_PASS)
        step = self._print_step(step, 'Upload the device configuration file.')
        upload_path = os.path.join(awscons.AWS_PROXY_REPO, _DEV_CFG_FILE)
        upload_cmd = _SCP + self._keypair_path + ' ' + _DEV_CFG_FILE + ' ubuntu@' + self._ec2_public_ip + ':' + upload_path
        result = self._run_command(upload_cmd, _NUM_RETRIES, _RETRY_DELAY)
        self._console.ok(_TAB + cons.TXT_PASS)
        step = self._print_step(step, 'Start the MQTT Proxy process.')
        start_cmd = (_SSH + self._keypair_path + ' ' +
                      ' ubuntu@' + self._ec2_public_ip +
                      ' \"cd ' + os.path.join('.', awscons.AWS_PROXY_REPO) +
                      '; nohup ' + os.path.join('.', 'mqtt-proxy') +
                      ' < /dev/null > /dev/null 2>&1 &\"')
        result = self._run_command(start_cmd, 1, _RETRY_DELAY)
        self._console.ok(_TAB + cons.TXT_PASS)
        return step
    ...
    def _aws_deploy_lambda(self, in_step):
        ...
        cmd = 'aws lambda create-function --function-name ' + self._lambda_name + ' --runtime nodejs8.10 --role ' + aws_role_arn + ' --handler UniquidLambda.handler --zip-file fileb://' + node_dir + '/UniquidLambda.zip --region ' + self._aws_region
        result = self._run_command(cmd)
        result_string = result.stdout.decode('utf-8').strip()
        result_json = json.loads(result_string)
    ...
    def _aws_deploy_dev_cfg(self, in_step):
        ...
        _aws_device_config['orgId'] = self._login_mgr.get_org_id()
        _aws_device_config['mqttUrl'] = self._aws_orch_props.get('mqttBroker', '[Error]')
        mqtt_url = self._aws_orch_props.get('mqttBroker', '[Error]')
        _aws_device_config['mqttsUrl'] = mqtt_url.replace('1883','8883') # FIXME work-around for lack of field in reply from orchestrator (DEV-75)
        _aws_device_config['mqttTopic'] = self._login_mgr.get_org_id() + '/announce' # FIXME work-around # self._aws_orch_props.get('mqttTopic', '[Error]')
        _aws_device_config['proxyUrl'] = 'tcp://' + self._ec2_public_ip + ':8883' # FIXME improve creation of URL
        _aws_device_config['registryUrl'] = 'http://' + self._login_mgr.get_api_ip() + ':8060' # FIXME work-around # self._aws_orch_props.get('registryUrl', '[Error]')
        ...
        network_type = self._aws_orch_props.get('network', '').casefold()
        if _aws_network_map.get(_LTC_REGTEST,'[Error]') in network_type:
            _aws_device_config['network'] = _LTC_REGTEST
        elif _aws_network_map.get(_LTC_TESTNET,'[Error]') in network_type:
            _aws_device_config['network'] = _LTC_TESTNET
        else:
            _aws_device_config['network'] = _LTC_MAINNET
        # save device configuration to file
        with open(_DEV_CFG_FILE, 'w') as f:
            f.write(json.dumps(_aws_device_config, indent=2))
            self._console.ok(_TAB + 'Configuration file written: '+ _DEV_CFG_FILE)
        self._console.ok(_TAB + cons.TXT_PASS)
        return step
    ...
    def _aws_print_summary(self, in_step):
        ...
        self._console.ok(_TAB + 'Configuration for devices: ' + str(_DEV_CFG_FILE))
        ...
        for timestamp in in_timestamps:
                ...
                for inst in obj.get('Instances',[]):
                    for tag in inst.get('Tags',[]):
                        if (_AWS_EC2_NAME_PREFIX in tag.get('Value', '') and
                            ...
                            delete_ec2_name = tag.get('Value', None)
                            delete_ec2_id = inst.get('InstanceId', None)
            if delete_ec2_id:
            ...
            for obj in json_obj.get('SecurityGroups',[]):
                if (_SEC_GROUP_NAME in obj.get('GroupName', '') and
                    ...
                    delete_sg_name = obj.get('GroupName', None)
                    delete_sg_id = obj.get('GroupId', None)
            if delete_sg_id:
            ...
            for obj in json_obj.get('KeyPairs',[]):
                if (_KEYPAIR_NAME in obj.get('KeyName','') and
                    ...
                    delete_kp_name = obj.get('KeyName', None)
            if delete_kp_name:
            ...
            for obj in json_obj.get('Functions',[]):
                if (_AWS_LAMBDA_FUNC_PREFIX in obj.get('FunctionName','') and
                    ...
                    delete_lambda_name = obj.get('FunctionName', None)
                    delete_lambda_arn = obj.get('FunctionArn', None)
            ...
            for obj in json_obj.get('authorizers',[]):
                if (_AWS_AUTH_PREFIX in obj.get('authorizerName','') and
                    ...
                    delete_authorizer_name = obj.get('authorizerName', None)
            ...
            for obj in json_obj.get('Roles',[]):
                if (_AWS_IAM_ROLE_PREFIX in obj.get('RoleName','') and
                    ...
                    delete_role_name = obj.get('RoleName', None)
            if delete_role_name:
