...
class EmailNotification(Notifier):
    def __init__(
        ...
        self.default_template_filepath = "html_template.txt"
    ...
    def set_template_prop(self, key, value):
        ...
        """
        self.template_obj.update({key: value})
        print(self.template_obj)
        ...
        if self.message_type == 'html':
            if self.template_filepath is None:
                ...
                if self.message_subject == "" and subject == "":
                    self.message_subject = self.template_obj["app_name"] + " Notification"

                try:
                    msg = self.tempalte_message.substitute(self.template_obj)
                except Exception as err:
