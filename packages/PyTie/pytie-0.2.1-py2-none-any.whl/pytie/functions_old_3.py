...
def int_data(las_files, N_files, DT_id, RHOB_id, Depth_id):
 ...
 # Fills the gaps at the beginning and at the end of the logs
        ...
        if RHOB['Depth'][0] > DT['Depth'][0]:
            Depth = np.array([np.arange(DT['Depth'][0], RHOB['Depth'][0]-STEP, STEP)]).T
        ...
        if RHOB['Depth'][0] < DT['Depth'][0]:
            Depth = np.array([np.arange(RHOB['Depth'][0], DT['Depth'][0]-STEP, STEP)]).T
            nan = np.array([np.full(Depth.size, np.nan)]).T
        ...
        if RHOB['Depth'].iloc[-1] > DT['Depth'].iloc[-1]:
            Depth = np.array([np.arange(DT['Depth'].iloc[-1]+STEP, RHOB['Depth'].iloc[-1], STEP)]).T
            nan = np.array([np.full(Depth.size, np.nan)]).T
            data = pd.DataFrame(np.concatenate((Depth,nan), axis=1), columns= ('Depth', 'DT'))
...
# Checks if there are gaps in the depth parameter:
        ...
        for i in range(Dept.size - 1):
            if np.round((Dept[i+1] - Dept[i]), decimals=4) != STEP:
                ...
                DT = DT.drop_duplicates(subset='Depth', keep='last', inplace=False).reset_index(drop=True)
        ...
        for i in range(Dept2.size - 1):
            if np.round((Dept2[i+1] - Dept2[i]), decimals=4) != STEP:
                ...
                Depth = np.array([np.arange(start+STEP, end-STEP, STEP)]).T
                rhob = np.array([np.full(Depth.size, np.nan)]).T
                ...
                RHOB = RHOB.drop_duplicates(subset='Depth', keep='last', inplace=False).reset_index(drop=True)




        #result = pd.merge(DT, RHOB, on='Depth')

        DEPTH = DT['Depth'].values
        DT = DT['DT'].values
