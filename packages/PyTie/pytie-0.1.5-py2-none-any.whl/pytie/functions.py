...
def int_data(las_files, N_files, DT_id, RHOB_id, Depth_id):
    ...
    if N_files == 1:
        ...
        for i in range(DEPT.size - 1):          # Checks if there are gaps between the samples
            if np.round((DEPT[i+1] - DEPT[i]), decimals=4) != STEP:
                start = DEPT[i]
                end = DEPT[i+1]
        
                Depth = np.array([np.arange(start+STEP, end, STEP)]).T
                dt = np.array([np.full(Depth.size, np.nan)]).T
                rhob = np.array([np.full(Depth.size, np.nan)]).T
        
                data = pd.DataFrame(np.concatenate((Depth,dt, rhob), axis=1), columns= ('Depth', 'DT', 'RHOB'))
        
                DEPT = np.array([DEPT]).T
                DT = np.array([DT]).T
                RHOB = np.array([RHOB]).T
        
                data2 = pd.DataFrame(np.concatenate([DEPT,DT, RHOB], axis=1), columns= ('Depth', 'DT', 'RHOB'))
        
                result = pd.concat([data,data2]).sort_values(by=['Depth']).reset_index(drop=True)
            
                DEPT = result['Depth'].values
                DT = result['DT'].values
                RHOB = result['RHOB'].values
        ...
        for i in range(np.asarray(count_d_i).size):
            ...
            if count_d[count_d_i[0][i]] == 1:                          # verifies if the same log is divided in different files
                ...
                DT = DT.drop_duplicates(subset='Depth', keep='first', inplace=False)
            
            
                DT = DT.sort_values(by='Depth', axis=0, ascending=True, inplace=False, kind='quicksort', na_position='last').reset_index(drop=True)
            ...
            if count_d[count_d_i[0][i]] > 1:   # verifies if the same log is divided in different columns
                ...
                globals()['%s%s%s' % (x,y,z)] = globals()['log_S_%s' % i]['DT']  # reading the first column
            
                DT_idx = np.append(DT_idx, globals()['%s%s%s' % (x,y,z)])
                ...
                DT = DT.dropna(axis=0, how='any', thresh=None, subset=None, inplace=False)
        ...
        for i in range(np.asarray(count_r_i).size):
            ...
            RHOB = RHOB.dropna(axis=0, how='any', thresh=None, subset=None, inplace=False)
            
            RHOB = RHOB.sort_values(by='Depth', axis=0, ascending=True, inplace=False, kind='quicksort', na_position='last').reset_index(drop=True)
            
            
            if count_r[count_r_i[0][i]] > 1:   # verifies if the same log is divided in different columns
        
                globals()['log_B_%s' % i] = lasio.read(las_files[count_r_i[0][i]])  # saves the logs which has RHOB log
                ...
                globals()['%s%s%s' % (x,y,z)] = globals()['log_B_%s' % i]['RHOB']  # reading the first column
            
                RHOB_idx = np.append(RHOB_idx, globals()['%s%s%s' % (x,y,z)])
        ...
        if RHOB['Depth'][0] > DT['Depth'][0]:
            Depth = np.array([np.arange(DT['Depth'][0], RHOB['Depth'][0], STEP)]).T
            nan = np.array([np.full(Depth.size, np.nan)]).T
            data = pd.DataFrame(np.concatenate((Depth,nan), axis=1), columns= ('Depth', 'RHOB'))
            RHOB = RHOB.append(data).sort_values(by=['Depth']).reset_index(drop=True)
    
        if RHOB['Depth'][0] < DT['Depth'][0]:
            Depth = np.array([np.arange(RHOB['Depth'][0], DT['Depth'][0], STEP)]).T
            nan = np.array([np.full(Depth.size, np.nan)]).T
            data = pd.DataFrame(np.concatenate((Depth,nan), axis=1), columns= ('Depth', 'DT'))
            DT = DT.append(data).sort_values(by=['Depth']).reset_index(drop=True)
        
        if RHOB['Depth'].iloc[-1] > DT['Depth'].iloc[-1]:
            Depth = np.array([np.arange(DT['Depth'].iloc[-1], RHOB['Depth'].iloc[-1], STEP)]).T
            nan = np.array([np.full(Depth.size, np.nan)]).T
            data = pd.DataFrame(np.concatenate((Depth,nan), axis=1), columns= ('Depth', 'DT'))
            DT = DT.append(data).reset_index(drop=True)
    
        if RHOB['Depth'].iloc[-1] < DT['Depth'].iloc[-1]:
            Depth = np.array([np.arange(RHOB['Depth'].iloc[-1], DT['Depth'].iloc[-1], STEP)]).T
            nan = np.array([np.full(Depth.size, np.nan)]).T
            data = pd.DataFrame(np.concatenate((Depth,nan), axis=1), columns= ('Depth', 'RHOB'))
            RHOB = RHOB.append(data).reset_index(drop=True)
        ...
        for i in range(Dept.size - 1):
            if np.round((Dept[i+1] - Dept[i]), decimals=4) != STEP:
                ...
                data = pd.DataFrame(np.concatenate((Depth,dt), axis=1), columns= ('Depth', 'DT'))
        
                DT = pd.concat([data,DT]).sort_values(by=['Depth']).reset_index(drop=True)
        ...
        for i in range(Dept2.size - 1):
            if np.round((Dept2[i+1] - Dept2[i]), decimals=4) != STEP:
                ...
                data = pd.DataFrame(np.concatenate((Depth,rhob), axis=1), columns= ('Depth', 'RHOB'))
        
                RHOB = pd.concat([data,RHOB]).sort_values(by=['Depth']).reset_index(drop=True)
...
def ext_line(d_sediment, depth_source, ground_level, DEPTH, DT, RHOB, STEP): 
    ...
    if ground_level < 0:
        ...
        V_sed = (378*((d_sediment)**4))*0.3048    # in m/s
        
        DT_sed = 1e6/(V_sed/0.3048)   # converting m/s to Us/foot
        
        X2 = np.arange(SB, DEPT[0],STEP)
        ...
        DEPT = np.insert(DEPT, 0, X2)
        DEPT = np.insert(DEPT, 0, X)
    ...
    if ground_level >= 0:
        ...
        V_sed = (357*((d_sediment)**4))*0.3048    # in m/s
        
        DT_sed = 1e6/(V_sed/0.3048)   # converting m/s to Us/foot
        
        X = np.arange(depth_source, DEPT[0],STEP)
...
def time_depth(DT, DEPT, depth_source):
