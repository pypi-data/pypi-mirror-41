...
from .feature_toggles import GIT_PATCH, GPU_MONITOR
from .file_uploader import compress_git_patch, write_file_like_to_tmp_file
...
from .utils import is_list_like, local_timestamp, read_unix_packages
...
class BaseExperiment(object):
    ...
    def _report_summary(self):
        ...
        shown.
        """
        topics = [("Metrics", "metric"), ("Other", "other")]
    ...
    def log_epoch_end(self, epoch_cnt, step=None):
        ...
        if self.alive:
            self._summary["metric"]["epoch_end"] = epoch_cnt
            if step is not None:
                self._summary["metric"]["step"] = step
            message = self._create_message()
    ...
    def log_figure(self, figure_name=None, figure=None, overwrite=False):
        ...
        self._file_upload(filename, "visualization", url_params)

        return self._get_uploaded_figure_url(figure_id)

    def log_asset(
        self, file_path=None, file_like_object=None, file_name=None, overwrite=False
    ):
        """
        Logs the Asset located at given file_path

        Args:
            file_path: String - the file path of the file you wants to log
            file_like_object: File-like - a file-like asset
            file_name: String - Optional. A custom file name to be displayed
            If not provided the filename from the `file_path` argument will be used.
            overwrite: if True will overwrite all existing assets with the same name.
        """
        if self.alive is False:
            return

        if file_path is None and file_like_object is None:
            raise TypeError(
                "log_asset takes either file_path or file_like_object argument"
            )

        copy_to_tmp = True

        if file_like_object is not None:
            # Read the file_like_object and write it to a temporary file
            file_path = write_file_like_to_tmp_file(file_like_object)
            copy_to_tmp = False

        # Check the file size before reading it
        try:
            file_size = os.path.getsize(file_path)
            if file_size > self.asset_upload_limit:
                LOGGER.error(
                    UPLOAD_ASSET_TOO_BIG, file_path, file_size, self.asset_upload_limit
                )
                return

        except OSError:
            LOGGER.error(UPLOAD_FILE_OS_ERROR, file_path)
            return
        ...
        self._file_upload(file_path, "asset", url_params, copy_to_tmp=copy_to_tmp)

        return self._get_uploaded_asset_url(asset_id)

    def log_image(self, file_path, file_name=None, overwrite=False):
        """
        Logs the image located at given file_path. Images are displayed on the
        Graphics tab on Comet.ml

        Args:
            file_path: String - the file path of the image you wants to log
            file_name: String - Optional. A custom file name to be displayed
                on the dashboard. If not provided the filename from the `file_path`
        ...
        self._file_upload(file_path, "visualization", url_params, copy_to_tmp=True)
    ...
    def log_current_epoch(self, value):
        ...
        """
        if self.alive:
            self._summary["metric"]["curr_epoch"] = value
            message = self._create_message()
    ...
    def _upload_git_patch(self):
        ...
        if not self.feature_toggles[GIT_PATCH]:
            LOGGER.debug("Git patch uploading turned off by feature toggle")
            return
