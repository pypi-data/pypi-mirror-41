...
from .api import API
from .comet import Streamer, config, generate_guid, get_api_key, is_valid_guid
...
class Experiment(BaseExperiment):
    ...
    def display(self, clear=False, wait=True, new=0, autoraise=True):
        ...
        if self._in_jupyter_environment():
            ...
            display(
                IFrame(
                    src=config.experiment._get_experiment_url(),
                    width="100%",
                    height="800px",
                )
            )
            ...
            webbrowser.open(
                config.experiment._get_experiment_url(), new=new, autoraise=autoraise
            )
...
class ExistingExperiment(Experiment):
    ...
    and is not currently running. This is useful when your training and testing happen on different scripts.
    ...
    def __init__(self, api_key, previous_experiment, **kwargs):
        """
        Creates a new experiment on the Comet.ml frontend.
        Args:
            api_key: Your API key obtained from comet.ml
