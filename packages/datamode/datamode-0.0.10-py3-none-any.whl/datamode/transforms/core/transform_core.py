...
class Transform(abc.ABC):
  ...
  def drop_rows(self):
    ...
    # Category columns can have lingering categories if their rows are dropped.
    # If we have any, recategorize them.
    for colname in self.result.df.select_dtypes('category').columns:
      col = self.result.df[colname]

      # Convert to object and back to drop extraneous categories
      col = col.astype('object').astype('category')
      self.result.df[colname] = col
...
def expand_dtype_columns(df, cols):
  ...
  for col in cols:
    if col in ['@object', '@number', '@datetime', '@bool']:
      dtype = col[1:]  # Get the value without the @
      cols_by_dtype = df.select_dtypes(dtype).columns

      # Account for category-represented @object
      if dtype == 'object':
        cols_by_dtype += df.select_dtypes('category').columns
