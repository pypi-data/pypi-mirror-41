...
from .altaircharts.altair_build import build_altair_spec, build_altair_subset, build_altair_counts
...
class ChartBuilder():
  ...
  def build_chart(self, msg_data):
    ...
    comm_data['chartdata']['current_record_count'] = df.shape[0]
    buffers = None
    if chart_type == 'multi':
  ...
  def build_altair(self, comm_data, df, chartset, chart_type):
    ...
    if chart_type == 'multi':
      ...
      df, current_record_count = build_altair_counts(df, picks)
      if current_record_count != None:
        comm_data['chartdata']['current_record_count'] = current_record_count

    comm_data['chartdata']['altair_spec'] = build_altair_spec(df, chart_type, chartset)
  ...
  def build_wordcloud(self, comm_data, df, chartset):
    ...
    # Make sure we still have rows - if not, bail
    if col.shape[0] == 0:
      comm_data['chartdata']['wordcounts'] = []
      return

    # Convert to strings because some objects only have ints left
    col = col.astype(str)
    ...
    # The token pattern will trap words of one character (the default only uses 2)
