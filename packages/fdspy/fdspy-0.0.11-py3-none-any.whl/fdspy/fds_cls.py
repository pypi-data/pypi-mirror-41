...
class FdsJob(object):
    def __init__(
            self, uid, path_fds, num_mpi=1, num_omp=1, path_work=None, user='n/a', path_destination=None, status=0,
            **red_kwargs,
    ):
...
class ClientAgentBack(object):
    ...
    def start(self, time_wait=30.):
        ...
        while True:
            ...
            if count_cpu_available >= count_pt:
                ...
                self.update_queue_json(j.uid, status=1)
    ...
    def update_queue_progress(self):
        ...
        for i, FdsJob in enumerate(self._list_FdsJob):
            if not FdsJob.check_is_live():
                queue[FdsJob.uid]['status'] = 2
                if FdsJob.path_destination: FdsJob.copy_files()
                del self._list_FdsJob[i]
            queue[FdsJob.uid]['progress'] = FdsJob.get_current_progress()
    ...
    def update_queue_live(self, uid, status):
        for Job in self._list_FdsJob:
            if Job.uid == uid and int(status) == 4:
                Job.status = 4
                Job.sp.kill()
