...
def bginfo():
    ...
    while True:
        ...
        table_top_boarder = "{uid}{user}{datetime_start}{path_fds}{num_omp}{num_mpi}{progress}{status}\n".format(
            uid="="*7, user="="*10, datetime_start="="*16, path_fds="="*30, num_omp="="*7, num_mpi="="*7, progress="="*11, status="="*6
        )

        table_header_boarder = table_top_boarder.replace('=', '-')

        table_header = "{uid:7.7}{user:10.10}{datetime_start:16.16}{path_fds:30.30}{num_omp:7.7}{num_mpi:7.7}{progress:11.11}{status:6.6}\n".format(
            uid="UID",
            user="User",
        ...
        if len(dict_queue) > 0:
            for uid, v in dict_queue.items():
                ...
                # v["datetime_start"] = datetime.strptime(v["datetime_start"], "%Y-%m-%d %H:%M:%S")
