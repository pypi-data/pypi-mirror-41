...
class TorBuild:
    ...
    def __init__(self, tormax=1, db_type=None, database_config=None, db_con_type=None):
        ...
        self.tors = {}
        while True:
            tor = _database.select(_view_name, view=True)
            if len(tor) >= tormax:
                break
...
class TorScanner:
    ...
    def __init__(self, db_type, database_config, db_con_type, drives=None):
        ...
        self.torrc_scanner()
        if self.opened_tors:
            _database.merge(_table_name, self.opened_tors, filters={'dat_pro': None}, on=['ipv4', 'pid', 'control_port'])
        else:
            _database.callproc(['tor_list_updall'])
        del self.tor
...
class TorService:
    ...
    def main(self, id_change=30):
        ...
        tor_list = _database.select(_table_name)
