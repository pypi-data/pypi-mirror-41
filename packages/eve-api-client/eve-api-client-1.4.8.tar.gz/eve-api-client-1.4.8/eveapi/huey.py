...
def esi(method, url, headers, params, data, fetch_pages, auth_callback, cache, page=False, parent_expires=None):
    """
    ESI call method.
    ...
    if 200 <= r.status_code < 300:  # is 2xx
        ...
        if fetch_pages and total_pages > 1:
            ...
            pages = [
                esi_task(method, url, headers, params, data, False, auth_callback, cache, page=p + 2,
                         parent_expires=r.headers['Expires'])
                # +2 since 1-based indexing
        ...
        if int(r.headers.get('X-ESI-Error-Limit-Remain', '100')) == 0:
            ...
            return error_limit_retry(auth_callback, data, fetch_pages, headers, method, page, params, url)

        raise Exception('ESI error')
