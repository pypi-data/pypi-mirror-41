...
from dingtalk_simple_test.api.callback import Callback
from dingtalk_simple_test.api.department import Department
from dingtalk_simple_test.api.user import User
from dingtalk_simple_test.client.base import BaseClient
from dingtalk_simple_test.core.utils import random_string, DingTalkSigner
from dingtalk_simple_test.crypto import DingTalkCrypto
from dingtalk_simple_test.storage.base import DingTalkCache
...
class CorpClient(DingTalkClient):
    # callback 必须提供token, aes_key
    callback = Callback()

    def __init__(self, corp_id, app_key, app_secret, storage=None, token=None, aes_key=None, timeout=None,
                 auto_retry=True):
        prefix = 'secret:%s:%s' % (corp_id, app_key)
        super(CorpClient, self).__init__(corp_id, prefix, storage, timeout, auto_retry)
