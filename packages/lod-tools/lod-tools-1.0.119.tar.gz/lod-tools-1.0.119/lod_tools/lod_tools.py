...
api_version_number_of_this_library = 1
...
def test_program_offline(args):
    ...
    if args.preparation_folder is not None:
        ...
        dst = os.path.abspath(args.execution_folder)
        shutil.rmtree(dst)
    ...
    try:
        ...
        client.containers.run(image=tag, volumes=folder_mapping)
    except docker.errors.ContainerError as error:
        print("Encountered error while executing Docker Container:")
        error_message = error.stderr.decode("utf-8")
        print(error_message)
    # Print either an error message or the content of the log file
    if args.quiet:
        return
    print("Finished executing the Docker Container.")
    error_file_location = os.path.join(dst, "error.txt")
    log_file_location = os.path.join(dst, "log.txt")
    try:
