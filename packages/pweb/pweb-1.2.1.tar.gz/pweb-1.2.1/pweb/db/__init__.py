...
from pweb.utils import iters, iterbetter, safestr
from pweb.utils.datastructs import threadeddict, storage, sqlrow
...
class DB:
    ...
    def __init__(self, db_module, keywords):
        """Creates a database.
        """
        if hasattr(db_module, "__name__") and db_module.__name__ != 'pyodbc':
...
class MSSQLDB(DB):
    def __init__(self, **keywords):
        _odbc = "pyodbc"
        if '_odbc' in keywords:
            _odbc = keywords.pop('_odbc').strip().lower()

        if _odbc == 'pymssql':
            db, keywords = self._use_pymssql(**keywords)
        else:
            try:
                db, keywords = self._use_pyodbc(**keywords)
            except:
                db, keywords = self._use_pymssql(**keywords)
    ...
    def _use_pyodbc(self, **keywords):
        ...
        keywords['unicode_results'] = True
        keywords['port'] = 1433
        self.paramstyle = "qmark"
        return db, keywords

    def _use_pymssql(self, **keywords):
...
class BaseSQLClass:
    ...
    def init_db(self):
        self._db = storage()
        params = dict(dbn='ENGINE', pw='PASSWORD', user='USER', db='NAME', host='HOST', port='PORT', _odbc='ODBC',
                      printing='PRINTING')
        for key, cfg in self.configs.iteritems():
            ...
            _cfg['_odbc'] = cfg.get('_odbc') and cfg.get('_odbc') or 'pyodbc'

            self._db[key] = database(**_cfg)

    def has_db(self, dbn):
        if self._db is None: self.init_db()
        ...
        if c > 0:
            ...
            for arg in args:
                if isinstance(arg, basestring):
                    params['_pweb_qmark_%d' % (len(params) + 1)] = safeunicode(arg)
                else:
    ...
    def selectpage(self, table, fields, page, pagesize, where=None, args=(), **kwargs):
        ...
        clauses_n = [self.db.gen_clause(sql, val, vars) for sql, val in clause_n if
                     (val is not None and not (sql.lower().strip() == 'where' and val.strip() == ''))]
        ...
        clauses_m = [self.db.gen_clause(sql, val, vars) for sql, val in clause_m if
                     (val is not None and not (sql.lower().strip() == 'where' and val.strip() == ''))]
