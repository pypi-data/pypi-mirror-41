...
class Template:
    ...
    def __init__(self, text, filename='<template>', env=None, fvars=None):
        ...
        self.source_code = self._load_source(text, filename)

        _, ext = os.path.splitext(filename)
        self.content_type = self.CONTENT_TYPES.get(ext, settings.DEF_CONTENT_TYPES)

    def _load_source(self, text, filename):
        basefile, filename = os.path.split(filename)
        _name, ext = os.path.splitext(filename)
        source_file = os.path.join(basefile, "compile", "%s.py" % _name)
        if os.path.isfile(source_file) and (not settings.DEBUG):
            return self.__readfile(source_file)

        text = Template.normalize_text(text)
        source_code = self._compile_class(text)
        if not settings.DEBUG:
            source_path = os.path.join(basefile, "compile")
            if not os.path.exists(source_path):
                os.makedirs(source_path)
            self.__writefile(source_file, source_code)
        return source_code
    ...
    def __writefile(self, filepath, content):
        try:
            f = open(filepath, "wb")
            try:
                f.write(content)
