...
class GitRepository(object):
    ...
    def merge(self, commit):
        ...
        '''
        results = log_call(['git', 'merge', '--no-ff', commit], cwd=self.path)
        if results['retcode']:
...
class GitLabMergeRequestHandler(GitRepository):
    ...
    '''
    def __init__(self, iid, path=os.curdir):
        '''
        ...
        self.commit, self.commit_id = \
            self.resolve_commit('merge-requests/{0}'.format(iid))
...
def git(proj, url=None, commit=None, export=False, merges=None):
    ...
    @param merge: merge options as (<url>, <commit> [, <remote_name>]) or
                  as merge request id, or a list of them, or 'all' for all
    ...
    if not commit:
        if proj.version.lower() == 'head':
            ...
            if not merges:
                ...
                commit = getMRTargetBranch(gitlab_name, merges[0])
        else:
            commit = proj.version
    ...
    def merge_hdlr(merge):
        ...
        if isinstance(merge, int):
            h = GitLabMergeRequestHandler(merge, path=dest)
            return h
        ...
        if hasattr(proj, 'slot') and proj.slot and proj.slot.build_id:
            ...
            def tag_name(previous=False):
                ...
                if os.environ.get('flavour', 'nightly') != 'nightly':
                    fragments.append(os.environ['flavour'])
                fragments.append(proj.slot.name)
                fragments.append(str((proj.slot.build_id - 1)
