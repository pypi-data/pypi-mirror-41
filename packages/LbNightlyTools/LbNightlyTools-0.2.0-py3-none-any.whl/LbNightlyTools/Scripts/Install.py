...
def requiredPackages(files, slot=None, build_id=None, projects=None, platforms=None, skip=None,
                     metadataurl=None,
    ...
    for filename in files:
        ...
        slot_tmp, build_id_tmp, project, platform = tokens[2], tokens[3], tokens[0], tokens[-3]
        if len(tokens) > 6 and (slot_tmp != slot or build_id_tmp != build_id):
            continue
...
class Script(PlainScript):
    ...
    def main(self):
        ...
        try:
            urllist = listdir(url + '/packs')
            tarfiles = ['packs/' + f for f in urllist
            ...
            tarfiles = requiredPackages(tarfiles, slot, build_id,
            ...
            required_files.extend(other_files.intersection(listdir(url)) -
                                  already_installed)
            if required_files:
            ...
            if ('confSummary.py' in installed and
                ...
                data = {}
                exec(compile(open(os.path.join(dest, 'confSummary.py')).read(), os.path.join(dest, 'confSummary.py'), 'exec'), data)
