...
class Project(object, metaclass=_ProjectMeta):
    ...
    def _fixProjectConfigJSON(self, patchfile=None):
        '''
        ...
        if exists(configfile):
            ...
            # case insensitive list of projects
            projs = dict((p.name.lower(), p.version)
                         for p in self.slot.projects)
    ...
    def patch(self, patchfile=None):
        ...
        self._fixProjectConfigJSON(patchfile)
...
class Package(object):
    ...
    def baseDir(self):
        ...
        if self.container:
            return os.path.join(self.container.baseDir,
    ...
    def build(self, **kwargs):
        ...
        return {'retcode': 0, 'stdout': '%s does not require build' % self,
                'stderr': ''}
    ...
    def checkout(self, **kwargs):
        ...
        for package in self.packages:
            for link in package.getVersionLinks():
                dest = os.path.normpath(os.path.join(package.baseDir, os.pardir,
                                                     link))
...
class Slot(object, metaclass=_SlotMeta):
    ...
    def __init__(self, name, projects=None, **kwargs):
        ...
        while caller.f_code.co_name in ['fromDict', 'cloneSlot']:
    ...
    def fromDict(cls, data):
        ...
        slot = cls(name=data.get('slot', None), projects=list(containers.values()),
                   env=data.get('env', []),
                   desc=data.get('description'),
                   disabled=data.get('disabled', False),
                   deployment=data.get('deployment', []))
        slot.platforms = data.get('platforms',
                                  data.get('default_platforms', []))
        ...
        slot.projects.extend([Project.fromDict(prj)
    ...
    def __str__(self):
        ...
        return ('{0}.{1}'.format(self.name, self.build_id) if self.build_id
                else self.name)
...
def getSlot(name, configdir=os.curdir):
    ...
    '''
    slot = loadConfig(configdir).get(name)
    if slot:
    ...
    def decorate(method):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                _failures_count += 1
                __log__.warning(msg.format(*args, **kwargs) +
...
def loadConfig(configdir=os.curdir):
    '''
    Load all slots from a config directory.
    '''
    ...
    submodules = [f[0] for f in map(splitext, names)
                  if f[1] == ".py"]
...
def findSlot(name):
    ...
    if os.path.exists(name.split('#')[0]):
        ...
        return getSlot(name,
                       'configs' if os.path.exists('configs') else os.curdir)


def pushDataToFrontEnd(config_path):
    '''
    ...
    params = {
        ...
        'slots': ';'.join([name for name in sorted(loadConfig(config_path))])
    }
...
if __name__ == '__main__':
    ...
    slots = loadConfig(sys.argv[1])

    print(('{0} slots configured ({1} enabled)'
          .format(len(slots), len([s for s in list(slots.values()) if s.enabled]))))

    from tabulate import tabulate
    print((tabulate([[name, 'X' if slots[name].enabled else ' ',
                     ', '.join(slots[name].deployment), slots[name]._source]
