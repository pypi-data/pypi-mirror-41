...
import gitlab
# make sure we have the right gitlab implementation
if not hasattr(gitlab, '__title__') or gitlab.__title__ != 'python-gitlab':
    from LbNightlyTools import __contrib_dir__
    from imp import load_source
    gitlab = load_source('gitlab',
                         os.path.join(__contrib_dir__, 'gitlab',
                                      '__init__.py'))
    del load_source
...
TAR_WITH_HARD_DEREF = None
def _tar_has_hard_dereference():
    'helper functio nto check if tar supports thje option --hard-dereference'
    global TAR_WITH_HARD_DEREF
    if TAR_WITH_HARD_DEREF is None:
        _test_tar = subprocess.Popen(['tar', '--hard-dereference'],
                                     stdout=subprocess.PIPE,
                                     stderr=subprocess.PIPE)
        _test_tar.communicate()
        TAR_WITH_HARD_DEREF = _test_tar.returncode == 2
    return TAR_WITH_HARD_DEREF
...
class Dashboard(object):
    ...
    def update(self, key, changes):
        ...
        self._log.debug('updating %s/%s', self.db.resource.url, key)
...
def _packcmd(srcs, dest, cwd='.', dereference=True, exclude=None):
    ...
    if dereference:
        ...
        if _tar_has_hard_dereference():
            cmd.append('--hard-dereference')
    if exclude:
        cmd.extend(['--exclude=%s' % x for x in exclude])
    fd, fname = mkstemp()
    cmd.extend(['--bzip2', '--null', '--files-from=%s' % fname, '--file', dest])
    with os.fdopen(fd, 'w') as file_list:
        file_list.write('\0'.join(srcs))
        file_list.write('\0')
        file_list.flush()
        try:
...
def getAllMergeRequestIDs(name_or_id, state='opened', include_wip=False,
    ...
    @param include_wip: if False (default) Work In Progress merge requests are
                        ignored
    '''
    server = gitlab.Gitlab('https://gitlab.cern.ch/',
                           token or os.environ['GITLAB_TOKEN'])
    logging.debug('looking for %s merge requests in project %s%s',
                  state, name_or_id, ' including WIP' if include_wip else '')
    try:
        if filters:
        ...
        def accepted(mr):
            ...
            #  - non-WIP if include_wip is False else any
            return ((labels and labels.intersection(mr.labels)) or
                    (include_wip or not mr.work_in_progress))
