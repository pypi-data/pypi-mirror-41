...
def test_cmt_build():
    dummy_src = join(_testdata, 'artifacts', 'packs', 'TestProject.HEAD.testing-slot.src.tar.bz2')

    config = os.environ.get('CMTCONFIG') or os.environ.get('BINARY_TAG', '')
...
def test_cmake_build():
    ...
    with TemporaryDir(chdir=True):
        ...
        assert '=== cleaning ===' in res['dummy'].stdout
        assert exists(join('DUMMY', 'DUMMY_HEAD', 'all-installed'))
