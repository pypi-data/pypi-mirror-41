...
build_slot flavour slot slot_build_id platform
    ...
    if [ "$JENKINS_MOCK" != "true" ] ; then
        ...
        rsync_opt="--rsync-dest ${ARCHIVE_DIR}"
    fi
    ...
    if [ -e build/.ccache ] ; then
        ...
        time tar -c -j -f "${directory}/packs/ccache_dir.${slot}.${platform}.tar.bz2" -C build .ccache
    fi
    ...
    if [ "${flavour}" = "release" -o -n "${make_rpm}" ] ; then
        ...
        if [[ "$platform" != *-slc5-* ]] ; then
            ...
            ~/.local/bin/pip install --extra-index-url https://lhcb-pip.cern.ch/simple/ --trusted-host lhcb-pip.cern.ch --user lbinstall -U
            lbn-rpm-validator -d --build-folder=${directory}/rpms --repo-url=file:${directory}/rpms ${platform}
        fi
    fi

    if [ "$JENKINS_MOCK" != "true" ] ; then
        lbn-manage-remote --verbose "${directory}" "${ARCHIVE_DIR}"
    fi
    ...
    if [ "${flavour}" = "release" -o -n "${run_indexer}" ] ; then
        if which glimpseindex &> /dev/null ; then
            ...
            rm -rf build
            mkdir build
            for tarfile in "${directory}"/packs/*.tar.bz2 ; do
