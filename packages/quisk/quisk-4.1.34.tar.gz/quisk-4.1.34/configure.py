...
class Configuration:
  ...
  def UpdateConf(self):		# Called second to update the configuration for the selected radio
    ...
    for k, v in list(radio_dict.items()):	# radio_dict may change size during iteration
      if k == 'favorites_file_path':	# A null string is equivalent to "not entered"
        if not v.strip():
          continue
      if k == 'power_meter_local_calibrations':	# present in configuration data but not in the config file
        continue
      try:
  ...
  def OnPageChanging(self, event):
    event.Skip()
    if event.GetEventObject() != self.notebk:	# We get events from the sub-notebook too
      return
...
"This is the end of the band in megahertz.")
    ...
    try:
      ...
      for band in transverter_offset:
        if band not in band_list:
          band_list.append(band)
    try:
    ...
    try:
      ...
      for band in hermes_bus:
        if band not in band_list:
          band_list.append(band)
    band_list.sort(self.SortCmp)
    if 'Time' in band_list:
