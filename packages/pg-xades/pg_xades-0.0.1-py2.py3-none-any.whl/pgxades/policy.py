...
# Modificado por Pragmatic SAS para ajustar a los requerimientos de Colombia
...
from pgxades.constants import NS_MAP, MAP_HASHLIB
from cryptography.hazmat.backends import default_backend
from cryptography.x509 import load_der_x509_certificate
from pgxades.utils import rdns_to_map, dict_compare
from pgxmlsig.constants import TransformUsageDigestMethod
from pgxmlsig.ns import DSigNs
from pgxmlsig.utils import create_node, USING_PYTHON2, get_rdns_name
...
class Policy(object):
    ...
    def calculate_policy_node(self, node, sign=False):
        ...
        return node.find('xades:SignaturePolicyImplied', namespaces=NS_MAP)
    ...
    def validate_certificate(self, node, signature):
        certs = node.findall('xades:Cert', namespaces=NS_MAP)
        ...
        if x509_data is not None:
            ...
            assert str(parsed_x509.serial_number) == certificate.find(
                'xades:IssuerSerial/ds:X509SerialNumber', namespaces=NS_MAP
            ).text
            dict_compare(
            ...
            digest = certificate.find(
                'xades:CertDigest', namespaces=NS_MAP
            )
            assert b64encode(
...
class PolicyId(Policy):
    ...
    def calculate_policy_node(self, node, sign=False):
        if sign:
            ...
            remote = self.remote or self.id
        else:
            policy_id = node.find('xades:SignaturePolicyId', namespaces=NS_MAP)
            identifier = policy_id.find('xades:SigPolicyId',  namespaces=NS_MAP)
        ...
        if sign:
            ...
            hash_method = policy_id.find(
                'xades:SigPolicyHash/ds:DigestMethod', namespaces=NS_MAP
            ).get('Algorithm')
            digest_value  = policy_id.find(
                'xades:SigPolicyHash/ds:DigestValue', namespaces=NS_MAP
            )
