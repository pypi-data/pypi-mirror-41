...
class MyMQTTClient(object):
    ...
    def set_will(self, topic, will):
        ...
        if topic is None:
            self._logger.error("MyMQTTClient.set_will - topic ({}) must be not None.".format(topic))
            raise ValueError("MyMQTTClient.set_will - topic ({}) must be not None.".format(topic))
