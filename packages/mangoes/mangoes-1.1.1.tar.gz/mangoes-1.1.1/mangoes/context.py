...
class Window(Context):
    ...
    def __init__(self, vocabulary=None, size=1, dirty=False, dynamic=False, n_grams=1, distance=False,
        ...
        self._window = _create_window_method(vocabulary, self.size_before, self.size_after,
                                             dirty, dynamic, n_grams, distance)

    def __call__(self, sentence, mask=False):
        filtered_sentence = self.filter_sentence(sentence)
        return [self._window(position, filtered_sentence)
                if not mask or mask[position] else [] for position in range(len(filtered_sentence))]


def _create_window_method(vocabulary, size_before, size_after,
                          dirty, dynamic, n, distance):
    ...
    get_items_from = (lambda sentence: list(ngrams(sentence, n))) if n > 1 else (lambda sentence: sentence)

    # so, if n > 1, all tokens in n-grams have to be checked if in vocabulary
    check_in_vocabulary = (lambda ng: all((t in vocabulary for t in ng))) if n > 1 else (lambda t: t in vocabulary)

    if not dirty:
        def _window(position, sentence):
            before, after = get_size()

            if position - before < 0:
                start_index = 0
                start_distance = position - before + n - 1
            else:
                start_index = position - before
                start_distance = - before + n - 1

            # distance_start is the distance from the first token in before list to the target token (position)
            before_list = [format_output(w, d)
                           for d, w in enumerate(get_items_from(sentence[start_index:position]),
                                                 start=start_distance)
                           if not vocabulary or check_in_vocabulary(w)]

            end_index = min(position + after + 1, len(sentence))
            after_list = [format_output(w, d)
                          for d, w in enumerate(get_items_from(sentence[position + 1:end_index]),
                                                start=1)
                          if not vocabulary or check_in_vocabulary(w)]
        ...
        def _window(position, sentence):
            ...
            before_list = []
            d = -1
            while position + d >= 0 and len(before_list) < before:
                items = get_items_from(sentence[position + d + 1 - n:position + d + 1])
                before_list += [format_output(item, d) for item in items if check_in_vocabulary(item)]
                d -= 1
            before_list = list(reversed(before_list))

            after_list = []
            d = 1
            while position + d < len(sentence) and len(after_list) < after:
                items = get_items_from(sentence[position + d:position + d + n])
                after_list += [format_output(item, d) for item in items if check_in_vocabulary(item) ]
                d += 1
