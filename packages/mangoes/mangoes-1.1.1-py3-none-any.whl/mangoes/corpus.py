...
class Corpus:
    ...
    def _count_words(self, nb_workers=None):
        if self.annotated:
            counter = _count_words_annotated
        else:
            counter = _count_words_raw

        if not nb_workers:
            nb_workers = multiprocessing.cpu_count() - 1

        data_parallel = mangoes.utils.multiproc.DataParallel(counter,
                                                             _reduce_counter,
                                                             nb_workers=nb_workers)

        nb_sentences_per_worker = self._nb_sentences
        if self._nb_sentences and nb_workers > 1:
            nb_sentences_per_worker = self._nb_sentences // (nb_workers - 1)

        self._words_count, self._nb_sentences = data_parallel.run(self, nb_sentences_per_worker)

    @mangoes.utils.decorators.timer(display=_logger.info, label="counting occurrences of each bigram in corpus")
    def _count_bigrams(self, nb_workers=None, batch=1000000):
        if self.annotated:
            counter = _count_bigrams_annotated
        else:
            counter = _count_bigrams_raw
        ...
        self._bigrams_count, self._nb_sentences = data_parallel.run(self, nb_sentences_per_worker)
...
def _count_words_raw(sentences, nb_sentences=None):
    words_count = collections.Counter()
    real_nb_sentences = 0

    for sentence in ProgressBar(sentences, total=nb_sentences):
        words_count.update(sentence)
        real_nb_sentences += 1

    yield (words_count, real_nb_sentences)


def _count_bigrams_raw(sentences, nb_sentences=None, batch=1000000):
    bigrams_count = collections.Counter()
    real_nb_sentences = 0
        ...
        while True:
            try:
                for _ in range(batch):
                    ...
                    bigrams_count.update(nltk.bigrams(sentence))
                    real_nb_sentences += 1
                    progress_bar.update()

                yield (bigrams_count, real_nb_sentences)
                bigrams_count = collections.Counter()
                real_nb_sentences = 0
...
def _count_words_annotated(sentences, nb_sentences=None):
    words_count = collections.Counter()
    real_nb_sentences = 0

    tokens_filter = mangoes.vocabulary.create_tokens_filter(("form", "lemma", "POS"))

    for sentence in ProgressBar(sentences, total=nb_sentences):
        words_count.update([Token(*token) for token in tokens_filter(sentence)])
        real_nb_sentences += 1

    yield (words_count, real_nb_sentences)


def _count_bigrams_annotated(sentences, nb_sentences=None, batch=1000000):
    bigrams_count = collections.Counter()
    real_nb_sentences = 0
    ...
    with ProgressBar(total=nb_sentences) as progress_bar:
        while True:
            try:
                for _ in range(batch):
                    sentence = sentences.__next__()
                    sentence = [Token(*token) for token in tokens_filter(sentence)]

                    bigrams = nltk.bigrams(sentence)
                    bigrams_count.update(bigrams)
                    real_nb_sentences += 1
