...
class Mask_RCNN(Application):
    ...
    def predict(self, weights_path):
        """Predicts image data, then returns the loaded image data and predicted results dictionary.
        """
        ...
        dataset_val.load_class(self.data_path, "val")
        dataset_val.prepare()
        self.class_names = dataset_val.class_names
        inference_config.NUM_CLASSES = len(dataset_val.class_names) # Background + other classes
        ...
        image_data = modellib.load_image_gt(dataset_val, inference_config, image_id, use_mini_mask=False)
    ...
    def prepare_train_data(self, data_path = None, get_image_from = 'directory', data_array = None, target_array = None):
    ...
    def train(self, epochs = 3, weights_path = None):
        """Train the model."""
        # Training dataset
        dataset_train = ImageDataset()
        dataset_train.load_class(self.data_path, "train")
        dataset_train.prepare()
        print("Images: {}\nClasses: {}".format(len(dataset_train.image_ids), dataset_train.class_names))
        self.config.NUM_CLASSES = len(dataset_train.class_names) # Background + other classes
        ...
        self.config.NUM_CLASSES = len(dataset_val.class_names) # Background + other classes

        assert sorted(dataset_train.class_names) == sorted(dataset_val.class_names), "train classes have to be same with val classes"
    ...
    def visualize(self, image_data, r):
