...
class Style_Transfer(Application):
    ...
    def prepare_train_data(self, get_base_image_from = 'directory', get_reference_image_from = 'directory'):
        '''
        Keyword Arguments:
            get_base_image_from {str} -- "directory" or "argument" (default: {'directory'})
            get_reference_image_from {str} -- "directory" or "argument" (default: {'directory'})
            style_reference_image {str} -- Path to style reference image (default: {'origin.jpg'})

        Raises:
            AssertionError -- If get_base_image_from or get_reference_image_from is not 'directory' or 'argument'
        '''
        assert get_base_image_from in ["directory", "argument"], "get_base_image_from should be either 'directory' or 'argument'."
        assert get_reference_image_from in ["directory", "argument"], "get_reference_image_from should be either 'directory' or 'argument'."

        self.get_base_image_from = get_base_image_from
        self.get_reference_image_from = get_reference_image_from

    def generate(self, base_image_path = "dataset", output_type = "file", output_path = "result", style_reference_image = 'origin.jpg',
                iterations = 10, total_variation_weight = 1.0, style_weight = 1.0, content_weight = 0.025):
        '''
        Keyword Arguments:
            base_image_path {str or numpy.ndarray} -- Path to base image or numpy array of base image (default: {"dataset"})
            output_type {str} -- 'file' or 'array' (default: {"file"})
            output_path {str} -- Path to save output image if output_type is 'file' (default: {"result"})
            iterations {int} -- To specify the number of iterations the style transfer takes place (default: {10})
            total_variation_weight {float} -- The weight given to the total variation loss (default: {1.0})
            style_weight {float} -- The weight given to the style loss (default: {1.0})
            content_weight {float} -- The weight given to the content loss (default: {0.025})
        
        Raises:
            FileNotFoundError -- If self.get_base_image_from is 'directory' and base_image_path is not a directory
            AssertionError -- If output_type is not 'file' or 'array'
        
        Returns:
            numpy.ndarray -- If output_type is 'array', return numpy.ndarray of generated images(resized to (224,224,3)). If not, return nothing.
        '''
        self.total_variation_weight = total_variation_weight
        self.style_weight = style_weight
        self.content_weight = content_weight

        if self.get_reference_image_from == 'directory':
            try:
                load_img(style_reference_image)
        ...
        if self.get_base_image_from == "directory":
            ...
            if base_image_path.ndim == 3:
                base_image_path = np.array([base_image_path])
            elif base_image_path.ndim < 4:
                raise ValueError("base_image_path is not an image ndarray")
            parent = base_image_path
        ...
        for child in parent:
            ...
            if self.get_base_image_from == 'directory':
                ...
                width = base_image_path[1]
                height = base_image_path[2]
            img_nrows = 400
            ...
            style_reference_image_tensor = K.variable( self.preprocess_image(style_reference_image, img_nrows, img_ncols, model_type) )
            ...
            input_tensor = K.concatenate([base_image, style_reference_image_tensor, self.combination_image], axis=0)
