    ...
    for line in loglines:
        ...
        try:
            ...
            if len(list) is 1:
                ...
                #comment = str(arguments).replace(",","\n").replace("}","").replace("{","")
                #fioCallbacks["TEST_START"](myStream, starttimeStamp + (list[8] / 1000), comment)
                continue
            ...
            if (iterator == 0):
                ...
                #comment = str(arguments).replace(",","\n").replace("}","").replace("{","")
                #newTime = starttimeStamp + (float(list[8]) / 1000)
                #fioCallbacks["TEST_START"](myStream, newTime, comment)
                #fioCallbacks["TEST_START"](myStream, timeStamp, comment)
