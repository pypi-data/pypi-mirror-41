...
def external_request(remote_host, remote_fn_name, args=(), kwargs={},
    ...
    if secure is not None:
        if len(args) > 0 and not rpcable(args):
            args = ('pickled_data', Binary(pickle.dumps(args)))
        if len(kwargs) > 0 and not rpcable(kwargs):
    ...
    if secure is True and response is not None:
        ...
        print('calling public remote fn: {}'.format(remote_fn_name)) if verbosity > 1 else None
