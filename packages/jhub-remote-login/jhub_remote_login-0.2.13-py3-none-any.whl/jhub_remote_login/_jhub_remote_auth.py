    ...
    def prepare(self):
        ...
        if self.get_current_user() is not None:
            ...
            self.log.info(
                ...
                f"{self.authenticator.header_name}")
            # begin extract_headers
            user_data = {}
            for _, header in enumerate(self.authenticator.header_name):
                ...
                if value:
                    try:
                        user_data[header] = value
                        self.log.info(f"user_data[header] -> "
                                      f"{user_data[header]}")
                    except KeyError:
                        pass
            # end extract_headers
            ...
            for item in self.authenticator.header_name:
                if item not in user_data:
                    ...
                    argument = self.get_argument('user', None, True)
                    self.log.info(f"argument -> {argument}")
                    if argument is not None:
                        self.log.info(f"self.redirect(argument) -> "
                                      f"{self.redirect(argument)}")
                        self.redirect(argument)
                    else:
                        user = self.user_from_username(argument)
                        self.set_login_cookie(user)
                        self.log.info(f"surl_path_join(self.hub.server.base_url, 'home') -> "
