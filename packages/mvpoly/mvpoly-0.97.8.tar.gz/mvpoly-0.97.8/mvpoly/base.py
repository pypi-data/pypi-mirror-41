...
class MVPoly(object):
    ...
    def intd(self, *intervals):
        ...
        pn = len(pv)
        if len(intervals) != pn:
            msg = "{0:d} variables, {1:d} intervals".format(pn, len(intervals))
    ...
    def norm_L(self, p, n=None):
        ...
        if nvar == 0:
            ...
            if n == 1:
                Ip = scipy.integrate.quad(
                    lambda t : f([t], p, self),
                    *limits(1)
                )[0]
            elif n == 2:
                Ip = scipy.integrate.dblquad(
                    lambda *ts : f(ts, p, self),
                    *limits(2)
                )[0]
            elif n == 3:
                Ip = scipy.integrate.tplquad(
                    lambda *ts : f(ts, p, self),
                    *limits(3)
                )[0]
            elif n > 3:
                if not mvpoly.util.version.at_least('scipy', (0, 13, 0)):
                    raise NotImplementedError("need SciPy 0.13.0 or later")
                Ip = scipy.integrate.nquad(
                    lambda *ts : f(ts, p, self),
                    *limits(n + 1),
                    args = None,
                    opts = None
                ...
                raise ValueError("bad dimension {0:d}".format((n)))
    ...
    def maxmodnd(self, epsilon = 1e-12, i0 = None):
        ...
        *t* is an *n*-row matrix whose columns are the midpoints of
        ...
        if i0 is None:
            i0 = 3 * deg
        if i0 <= 2 * deg:
            msg = "i0 must be at least twice the polynomial degree"
            raise RuntimeError(msg)
    ...
    @classmethod
    def bell(cls, n, **kwargs):
        """
        Return the output of :meth:`mvpoly.dict.MVPolyDict.bell`
        converted to other polynomial subclasess.
        """
        from mvpoly.dict import MVPolyDict
        return cls(MVPolyDict.bell(n, **kwargs), **kwargs)
    ...
    @classmethod
    def chsym(cls, k, n, **kwargs):
        """
        Return the output of :meth:`mvpoly.dict.MVPolyDict.chsym`
        converted to other polynomial subclasess.
        """
        from mvpoly.dict import MVPolyDict
        return cls(MVPolyDict.chsym(k, n, **kwargs), **kwargs)
...
def add_scipy_special(cls, meth):
    ...
    special.__doc__ = "Wrapper for :meth:`scipi.special.{0!s}`".format((name))
