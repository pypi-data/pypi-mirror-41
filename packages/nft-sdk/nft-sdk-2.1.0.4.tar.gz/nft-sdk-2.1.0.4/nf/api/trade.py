...
from nf.api.basic import _error_call_back, _check_frequency, _reform_tick_data
...
def order_future(symbol, volume, side, order_type, leverage=0, price=0,
                 exec_inst='', stop_price=0, account=''):
    if not symbol or not volume or not side or not order_type or len(symbol) <= 0:
    ...
    return _place_order(symbol=symbol, value=-1, volume=volume, side=side, price=price,
                        ...
                        subject=SecurityType_Future, exec_inst=exec_inst, stop_price=stop_price, account_id=account)
...
def get_unfinished_orders(exchange=None, symbols=None):
    ...
    if symbols:
        ...
        for symbol in symbols:
            symbol = str(symbol).split(".")  # 只能要 'BINANCE.BTCUSDT' 中的 BTCUSDT
            if len(symbol) > 1:
                symbol = symbol[1]
