...
class LogParser(Constant):
    # datas = {}  # Cause shared self.datas between test functions!
    logger = logger

    def __init__(self, scrapyd_server, scrapyd_logs_dir, parse_round_interval, log_encoding,
                 log_extensions, log_head_lines, log_tail_lines, delete_existing_json_files_at_startup,
    ...
    def calc_elapsed(self, start_string, end_string):
        try:
    ...
    def find_text_to_ignore(self, text):
        ...
        if m:
            self.logger.warning("Found log ending:%s", self.format_log_block('log ending', m.group()))
            text_to_ignore = ''
        else:
    ...
    def get_default_stats(self):
        ...
        return dict(status='ok', datas={}, logparser_version=self.logparser_version,
    ...
    def handle_logfile(self, log_path):
        ...
        project, spider, job = self.parse_log_path(log_path)
        ...
        if log_path not in self.datas:
            ...
            if loaded_data.get('size', -1) == size:
                ...
                self.logger.warning("Skip parsing since data with same size found: %s (%s bytes)",
                                    log_path, loaded_data['size'])
                return
            else:
                self.logger.debug("Go on to parse the new logfile.")
        elif self.datas[log_path]['size'] == size:
            self.logger.debug("Old logfile with old size: %s (%s bytes)", log_path, size)
            return
        else:
            self.logger.warning("Old logfile with new size: %s (%s bytes)", log_path, size)
    ...
    def main(self):
        while True:
            ...
            try:
                self.run()
                end_time = time.time()
                self.logger.debug("Cost %.1f seconds this round", (end_time - start_time))
                ...
                if self.main_pid:
                    msg = "!!! LogParser subprocess (pid: %s) cancelled by KeyboardInterrupt" % self.logparser_pid
                else:
    ...
    def parse_appended_log(self, data, appended_log):
        ...
        if data.get('first_log_time'):
            ...
            data['elapsed'] = self.calc_elapsed(data['first_log_time'], data['latest_log_time'])

            data['datas'].extend(data_['datas'])
            data['pages'] = data_['pages'] or data['pages']
            data['items'] = data_['items'] or data['items']
        ...
        if self.count_actual_lines(data['tail']) < self.LOG_TAIL_LINES:
            if tail_last_time:
                if appended_log:
                    data['tail'] = '%s\n%s' % (tail_last_time, appended_log)
                else:
                    data['tail'] = tail_last_time
            else:
    ...
    def parse_log_path(log_path):
        ...
        job, __ = os.path.splitext(_job)  # ('job', '.log')
    ...
    def read_data(self, json_path):
        ...
        if not os.path.exists(json_path):
            self.logger.warning("NOT found: %s", json_path)
        else:
            try:
                with io.open(json_path, 'r', encoding='utf-8') as f:
                    data = json.loads(f.read())
            except Exception as err:
                self.logger.warning("Fail to load json file: %s", json_path)
                self.logger.error(err)
            else:
    ...
    def save_data(self, data):
        data['logparser_version'] = self.logparser_version
