...
class PropagateSeedsDialog(object):
    ...
    SMALLEST_XCORR_WINDOW_SIZE = 3
    MAX_XCORR_WINDOW_SIZE = 512
    ...
    def setupUi(self, Dialog, firstslice=None, lastslice=None, xcorrwinsz=None):
        ...
        if PropagateSeedsDialog.firstSlice < 0 and firstslice:
            PropagateSeedsDialog.firstSlice = firstslice
        elif PropagateSeedsDialog.firstSlice < 0:
            PropagateSeedsDialog.firstSlice = 0

        if PropagateSeedsDialog.lastSlice < 0 and lastslice:
            PropagateSeedsDialog.lastSlice = lastslice
        elif PropagateSeedsDialog.lastSlice < 0:
            PropagateSeedsDialog.lastSlice = 1

        if PropagateSeedsDialog.xcorrWindowSize < 0:
            PropagateSeedsDialog.xcorrWindowSize = rcallbacks.rcbsegment.RCBSegment.DEFAULT_FLOW_WINDOW_SZ
        else:
            PropagateSeedsDialog.xcorrWindowSize = xcorrwinsz
        ...
        self.sbXcorrWindowSz.setMinimum(PropagateSeedsDialog.SMALLEST_XCORR_WINDOW_SIZE)
        self.sbXcorrWindowSz.setMaximum(PropagateSeedsDialog.MAX_XCORR_WINDOW_SIZE)
        self.sbXcorrWindowSz.setValue(PropagateSeedsDialog.xcorrWindowSize)
    ...
    def parameters(self):
        ...
        PropagateSeedsDialog.xcorrWindowSize = self.sbXcorrWindowSz.value()

        theparameters = {'first': PropagateSeedsDialog.firstSlice, 'last': PropagateSeedsDialog.lastSlice, 'xcorr_win_sz': PropagateSeedsDialog.xcorrWindowSize}
