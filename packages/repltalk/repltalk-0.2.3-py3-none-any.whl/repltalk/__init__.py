...
class Comment():
	def __init__(
		...
		self.author = user # TODO: Turn this into a User object
...
class Post():
	def __init__(
		...
		self.url = url
		self.board = board # TODO: Make this a board object
		self.time_created = time_created # TODO: Make this a datetime object
		self.can_edit = can_edit
		self.can_comment = can_comment
		...
		self.author = user # TODO: Make this a user object
		self.repl = repl
...
class User():
	def __init__(
		self, client, id, username, avatar, url, cycles, roles
	):
		self.client = client
		self.id = id
		...
		self.roles = roles
	def __repr__(self):
		return f'<{self.username} ({self.cycles})>'
