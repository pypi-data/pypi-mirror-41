...
def dags_dashtable(df, html_id):
    df = df[['dag_id', 'last_scheduler_run']]

    lexico_sorted_dags = df.sort_values(by='dag_id')
    dags_list = Dropdown(
        id=html_id,
        options=[
            {'label': row['dag_id'], 'value': row['dag_id']} for index, row in lexico_sorted_dags.iterrows()
        ],
        value = lexico_sorted_dags.loc[0, 'dag_id']
    )
