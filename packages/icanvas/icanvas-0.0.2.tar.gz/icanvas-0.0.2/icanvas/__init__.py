    ...
    if (server_type == 'tomcat'):
        ...
        if os.path.exists("/var/.canvas"):
            if os.path.exists("/var/.canvas/ctmodelhouse.war") and os.path.exists("/var/.canvas/expertctstudio.war"):
                ...
                subprocess.check_output("curl -XGET -v -u admin:admin123 http://172.19.32.90:8081/repository/maven-releases/expertctstudio.war -O && curl -XGET -v -u admin:admin123 http://172.19.32.90:8081/repository/maven-releases/ctmodelhouse.war -O && mv expertctstudio.war /var/.canvas/expertctstudio.war &&  mv ctmodelhouse.war /var/.canvas/ctmodelhouse.war && chmod +x /var/.canvas/*", shell=True)
        else:
            subprocess.check_output("mkdir /var/.canvas && mkdir /var/.canvas/template && curl -XGET -v -u admin:admin123 http://172.19.32.90:8081/repository/maven-releases/expertctstudio.war -O && curl -XGET -v -u admin:admin123 http://172.19.32.90:8081/repository/maven-releases/ctmodelhouse.war -O && mv expertctstudio.war /var/.canvas/expertctstudio.war &&  mv ctmodelhouse.war /var/.canvas/ctmodelhouse.war && chmod +x /var/.canvas/*", shell=True)
...
def db(server_type,db_type,app_server_path):
    ...
    subprocess.check_output("sed -i 's/{ip}/'"+dbip+"'/g' /var/.canvas/template/*", shell=True)
    subprocess.check_output("sed -i 's/{port}/'"+dbport+"'/g' /var/.canvas/template/*", shell=True)
    subprocess.check_output("sed -i 's/{user}/'"+dbuser+"'/g' /var/.canvas/template/*", shell=True)
    subprocess.check_output("sed -i 's/{pass}/'"+dbpass+"'/g' /var/.canvas/template/*", shell=True)
    ...
    if (server_type == 'tomcat'):
        if (db_type == 'MySQL'):
            ...
            if dbflag == 0:
                ...
                tmp.close()
            dbflag  = 0
            with open(app_server_path + '/conf/config.xml', 'r') as fh:
                if 'ModelHouseCT' in line or 'ModelHouse' in line or 'ExpertStudio' in line or 'ExpertStudioTarget' in line:
                    dbflag = 1
                    print 'DB Entries already present in config.xml'
                    break
            if dbflag == 0:
                tmp = open("/tmp/temp_config.xml", 'w')
            ...
            if dbflag == 0:
                ...
                subprocess.check_output("mv /tmp/temp_config.xml " +app_server_path + "/conf/config.xml", shell=True)
