...
def install():
    ...
    subprocess.check_output("rm -rf /var/.canvas/ExpertModeCTStudio && rm -rf /var/.canvas/Modelhouse && curl -XGET -v -u admin:admin123 http://172.19.29.253:8081/repository/canvas/18/dependencies.zip -O && unzip dependencies.zip -d /var/.canvas/ && rm -rf dependencies.zip && dos2unix -R "+app_server_path+"/*", shell=True)
    subprocess.check_output("mv /var/.canvas/mysql-connector-java-5.1.38-bin.jar && mv /var/.canvas/ojdbc6.jar", shell=True)
