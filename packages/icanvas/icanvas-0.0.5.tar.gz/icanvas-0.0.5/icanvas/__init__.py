    ...
    if (server_type == 'tomcat'):
        ...
        if os.path.exists("/var/.canvas"):
            if os.path.exists("/var/.canvas/ctmodelhouse.war") and os.path.exists("/var/.canvas/expertctstudio.war"):
                ...
                subprocess.check_output("curl -XGET -v -u admin:admin123 http://172.19.29.253:8081/repository/maven-releases/expertctstudio.war -O && curl -XGET -v -u admin:admin123 http://172.19.29.253:8081/repository/maven-releases/ctmodelhouse.war -O && mv expertctstudio.war /var/.canvas/expertctstudio.war &&  mv ctmodelhouse.war /var/.canvas/ctmodelhouse.war && chmod +x /var/.canvas/*", shell=True)
        else:
            subprocess.check_output("mkdir /var/.canvas && mkdir /var/.canvas/template && curl -XGET -v -u admin:admin123 http://172.19.29.253:8081/repository/maven-releases/expertctstudio.war -O && curl -XGET -v -u admin:admin123 http://172.19.29.253:8081/repository/maven-releases/ctmodelhouse.war -O && mv expertctstudio.war /var/.canvas/expertctstudio.war &&  mv ctmodelhouse.war /var/.canvas/ctmodelhouse.war && chmod +x /var/.canvas/*", shell=True)
