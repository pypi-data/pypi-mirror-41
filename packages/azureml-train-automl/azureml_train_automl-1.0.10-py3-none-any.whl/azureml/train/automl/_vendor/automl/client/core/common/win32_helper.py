...
class JobObjectInfoClass(object):
...
class Win32Helper(object):
    ...
    def __init__(self):
        ...
        if not self.is_process_in_job():
            ...
            job_object = windll.kernel32.CreateJobObjectW(None, 'automl-{}'.format(self.get_process_id()))
    ...
    def get_process_id(self):
        ...
        """
        return windll.kernel32.GetProcessId(self.current_process)
    ...
    def open_process(self, pid=None):
        ...
        handle = windll.kernel32.OpenProcess(PROCESS_ALL_ACCESS, False, pid)
        if handle == 0:
    ...
    def assign_process_to_job_object(self, job, pid=None):
        """
        ...
        :param pid: ID of the process we want to assign to the job. If None, assigns the current process to the job.
        """
        handle = self.open_process(pid)
    ...
    def query_information_job_object(self, job_object_info_class, job_info_struct):
        """
    ...
    def get_process_times(self, pid=None):
        """
        Get process time wrapper for the Win32 GetProcessTimes function.

        Retrieves timing information for the specified process.
        Note: The Win32 API returns times in 100ns increments, but this function converts to seconds for convenience.
        ...
        :param pid: The PID of the process to get timing information for. If None, gets info for the current process.
    ...
    def get_children_memory_usage(self):
        """
        ...
        for info in mem_info:
            child_mem_usage[0] += info.WorkingSetSize
            child_mem_usage[1] += info.PrivateUsage
        return tuple(child_mem_usage)

    def get_job_accounting_info(self):
        """
        ...
        return self.query_information_job_object(JobObjectInfoClass.JobObjectBasicAndIoAccountingInformation, struct)

    def get_job_process_list(self):
        """
        Get list of pids of all processes associated with the job this process is in.

        :return: a list of all pids
        """
        captured_all_processes = False
        max_processes = 10
        struct = None
        while not captured_all_processes:
            ...
            if struct.NumberOfProcessIdsInList > struct.NumberOfAssignedProcesses:
                # Not enough space to capture all pids, resize array and try again
                max_processes *= 2
            else:
                captured_all_processes = True
        return list(struct.ProcessIdList[:struct.NumberOfProcessIdsInList])

    def get_child_process_times(self):
        """
        ...
        child_timing_info = [0, 0, 0, 0]
        for info in timing_info:
            child_timing_info[0] += info[0]
            child_timing_info[1] += info[1]
            child_timing_info[2] += info[2]
            child_timing_info[3] += info[3]
        return tuple(child_timing_info)
    ...
    def get_error(api_name=None):
        ...
        """
        if api_name is None:
            return RuntimeError('Win32 API call failed: [WinError {}] {}'.format(
                ctypes.GetLastError(), ctypes.FormatError()
            ))
        else:
            return RuntimeError('Win32 {} API call failed: [WinError {}] {}'.format(
                api_name, ctypes.GetLastError(), ctypes.FormatError()
            ))
