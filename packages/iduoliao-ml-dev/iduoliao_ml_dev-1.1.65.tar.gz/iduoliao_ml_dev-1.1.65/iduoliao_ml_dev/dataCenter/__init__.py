...
def toHive():
	...
	while start < (time.time() - 40 * 60) * 1000:
		...
		for hit in es.scrollSearch('clientlog_read', 'watch', {"size": 10000, "query": {"range": {"create_time": {"gte": start,"lt": end}}}}):
			source = hit['_source']
			values.append("('" + "', '".join([esTimeToHiveTime(source['create_time']), str(source['uid']), esTimeToHiveTime(source['starttime']), esTimeToHiveTime(source['endtime']), data['body'], source['type']]) + "')")
		execute('insert into log_client partition (date_time="' + timeToPartitionTime(start / 1000) + '") values ' + ','.join(values))
		writeEndTime(end / 1000)
		print('endTime: ' + str(end) + '(' + es.timeToEsLocalTime(end / 1000) + ')')
		start = end
		time.sleep(2)
		gc.collect()
		break
