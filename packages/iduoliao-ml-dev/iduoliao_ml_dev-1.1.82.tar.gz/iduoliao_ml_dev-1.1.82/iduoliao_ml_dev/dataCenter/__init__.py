...
def toHive():
	...
	while start < (time.time() - 5 * 60) * 1000:
		...
		for hit in es.scrollSearch('clientlog_read', 'watch', {"size": 10000, "query": {"range": {"create_time": {"gte": start,"lt": end}}}}):
			...
			values.append('insert into log_client partition (date_time="' + timeToPartitionTime(start / 1000) + '") values ' + "('" + "', '".join([esTimeToHiveTime(source['create_time']), str(source['uid']), esTimeToHiveTime(source['starttime']), esTimeToHiveTime(source['endtime']), body, source['type']]) + "')")
