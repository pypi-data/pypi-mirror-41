...
def toHive():
	...
	while start < (time.time() - 75 * 60) * 1000:
		begin = time.time()
		...
		for hit in es.scrollSearch('clientlog_read', 'watch', {"size": 10000, "query": {"range": {"create_time": {"gte": start,"lt": end}}}}):
			...
			values.append("\t".join([esTimeToHiveTime(source['create_time']), str(source['uid']), esTimeToHiveTime(source['starttime']), esTimeToHiveTime(source['endtime']), '' if body == None else body, source['type']]))
