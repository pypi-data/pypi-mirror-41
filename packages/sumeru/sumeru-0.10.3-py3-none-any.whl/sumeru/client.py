...
from sumeru.dask.base import tokenize, normalize_token, collections_to_dsk
from sumeru.dask.core import flatten, get_dependencies
from sumeru.dask.compatibility import apply, unicode
try:
    from cytoolz import first, groupby, merge, valmap, keymap
except ImportError:
    from toolz import first, groupby, merge, valmap, keymap
try:
    from sumeru.dask.delayed import single_key
except ImportError:
    single_key = first
...
class Task(object):
    def __init__(self, task_id, status = None, client=None, msg = None):
        self.status = status
    ...
    def running_status(self):
        return self.client.task_status(self.task_id)

    def stop(self):
        return self.client.stop_task(self.task_id)
...
class Future(WrappedKey):
    ...
    def result(self, timeout=None):
        ...
        ``dask.sumeru.TimeoutError`` is raised.
        """
...
class Client(Node):
    """ Connect to and drive computation on a sumeru Dask cluster

    The Client connects users to a dask.sumeru compute cluster.  It
    ...
    address: string, or Cluster
        ...
        ``'127.0.0.1:7328'`` or a cluster object like ``LocalCluster()``
    timeout: int
    ...
    >>> client = Client('127.0.0.1:7328')  # doctest: +SKIP
    ...
    See Also
    --------
    sumeru.scheduler.Scheduler: Internal scheduler
    """
    def __init__(self, address=None, loop=None, timeout=no_default,
        ...
        if timeout == no_default:
            timeout = dask.config.get('sumeru.comm.timeouts.connect')
        ...
        if heartbeat_interval is None:
            heartbeat_interval = dask.config.get('sumeru.client.heartbeat')
        ...
        if set_as_default:
            ...
            dask.config.set(scheduler='dask.sumeru')
        ...
        from sumeru.recreate_exceptions import ReplayExceptionClient
    ...
    def add_task(self, **kwargs):
        task = self.sync(self.scheduler.add_task, **kwargs)
        status = task.get('status')
        task_id = task.get('task_id')
        msg = task.get('msg')
        task = Task(task_id = task_id, status = status, client = self, msg = msg)
        return task

    def task_status(self, task_id):
        return self.sync(self.scheduler.get_status, task_id = task_id)

    def stop_task(self, task_id):
        return self.sync(self.scheduler.stop_task, task_id = task_id)
    ...
    def _repr_html_(self):
        ...
        if info and 'bokeh' in info['services']:
            ...
            if protocol == 'inproc':
                ...
                host = rest.split(':')[0]
            template = dask.config.get('sumeru.dashboard.link')
    ...
    def _release_key(self, key):
        """ Release key from sumeru memory """
    ...
    def gather(self, futures, errors='raise', maxsize=0, direct=None,
               asynchronous=None):
        """ Gather futures from sumeru memory
    ...
    def scatter(self, data, workers=None, broadcast=False, direct=None,
        ...
        """ Scatter data into sumeru memory

        This moves data from the local client process into the workers of the
        sumeru scheduler.  Note that it is often better to submit jobs to
    ...
    def compute(self, collections, sync=False, optimize_graph=True,
        ...
        **kwargs:
        ...
        Examples
        --------
        >>> from sumeru.dask import delayed
    ...
    def _upload_environment(self, zipfile):
        ...
        def unzip(dask_worker=None):
            from sumeru.utils import log_errors
            import zipfile
            import shutil
            with log_errors():
    ...
    def retire_workers(self, workers=None, close_workers=True, **kwargs):
        ...
        See dask.sumeru.Scheduler.retire_workers for the full docstring.
        ...
        See Also
        --------
        dask.sumeru.Scheduler.retire_workers
        """
    ...
    def get_task_stream(self, start=None, stop=None, count=None, plot=False,
        ...
        >>> from sumeru.dask.sumeru import get_task_stream
...
def wait(fs, timeout=None, return_when='ALL_COMPLETED'):
    ...
    timeout: number, optional
        Time in seconds after which to raise a ``dask.sumeru.TimeoutError``
    -------
...
def default_client(c=None):
    ...
    if c:
        ...
        raise ValueError("No clients found\n"
                         ...
                         "  client = Client('ip-addr-of-scheduler:7328')\n")
...
def redict_collection(c, dsk):
    from sumeru.dask.delayed import Delayed
