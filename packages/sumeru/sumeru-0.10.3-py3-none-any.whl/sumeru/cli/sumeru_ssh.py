...
from sumeru.deploy.ssh import SSHCluster
import click

from sumeru.cli.utils import check_python_3


@click.command(help="""Launch a sumeru cluster over SSH. A 'dask-scheduler' process will run on the
...
@click.option('--scheduler-port', default=7328, type=int,
              help="Specify scheduler port number.  Defaults to port 7328.")
...
def main(ctx, scheduler, scheduler_port, hostnames, hostfile, nthreads, nprocs,
    ...
    import sumeru
    print('\n---------------------------------------------------------------')
    print('                 Dask.sumeru v{version}\n'.format(version=sumeru.__version__))
