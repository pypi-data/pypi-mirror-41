...
import click
from sumeru import Nanny, Worker
from sumeru.config import config
from sumeru.utils import get_ip_interface, parse_timedelta
from sumeru.worker import _ncores
from sumeru.security import Security
from sumeru.cli.utils import (check_python_3, uri_from_host_port,
                                   install_signal_handlers)
from sumeru.comm import get_address_host_port
from sumeru.preloading import validate_preload_argv
from sumeru.proctitle import (enable_proctitle_on_children,
...
logger = logging.getLogger('sumeru.dask_worker')
...
def main(scheduler, host, worker_port, listen_address, contact_address,
    ...
    #if bokeh:
    #    try:
    #        from sumeru.bokeh.worker import BokehWorker
    #    except ImportError:
    #        pass
    #    else:
    #        if bokeh_prefix:
    ...
    if not scheduler and not scheduler_file and 'scheduler-address' not in config:
        raise ValueError("Need to provide scheduler address like\n"
                         "sumeru-worker SCHEDULER_ADDRESS:7328")
