...
from sumeru.metrics import time
from sumeru.utils import sync, tmpfile
from sumeru.utils_test import (popen, slow, terminate_process,
                                    wait_for_port)
from sumeru.utils_test import loop  # noqa: F401
...
def test_memory_limit(loop):
    with popen(['dask-scheduler', '--no-bokeh']) as sched:
        with popen(['dask-worker', '127.0.0.1:7328', '--memory-limit', '2e3MB',
                    '--no-bokeh']) as worker:
            with Client('127.0.0.1:7328', loop=loop) as c:
                while not c.ncores():
                    sleep(0.1)
...
def test_no_nanny(loop):
    with popen(['dask-scheduler', '--no-bokeh']) as sched:
        with popen(['dask-worker', '127.0.0.1:7328', '--no-nanny',
                    '--no-bokeh']) as worker:
...
def test_no_reconnect(nanny, loop):
    with popen(['dask-scheduler', '--no-bokeh']) as sched:
        wait_for_port(('127.0.0.1', 7328))
        with popen(['dask-worker', 'tcp://127.0.0.1:7328', '--no-reconnect', nanny,
                    '--no-bokeh']) as worker:
            sleep(2)
...
def test_resources(loop):
    with popen(['dask-scheduler', '--no-bokeh']) as sched:
        with popen(['dask-worker', 'tcp://127.0.0.1:7328', '--no-bokeh',
...
def test_local_directory(loop, nanny):
    with tmpfile() as fn:
        with popen(['dask-scheduler', '--no-bokeh']) as sched:
            with popen(['dask-worker', '127.0.0.1:7328', nanny,
                        '--no-bokeh', '--local-directory', fn]) as worker:
                with Client('127.0.0.1:7328', loop=loop, timeout=10) as c:
                    start = time()
                    while not c.scheduler_info()['workers']:
                        sleep(0.1)
                        assert time() < start + 8
...
def test_nprocs_requires_nanny(loop):
    with popen(['dask-scheduler', '--no-bokeh']) as sched:
        with popen(['dask-worker', '127.0.0.1:7328', '--nprocs=2',
                    '--no-nanny']) as worker:
...
def test_nprocs_expands_name(loop):
    with popen(['dask-scheduler', '--no-bokeh']) as sched:
        with popen(['dask-worker', '127.0.0.1:7328', '--nprocs', '2',
                    '--name', 'foo']) as worker:
            with popen(['dask-worker', '127.0.0.1:7328', '--nprocs', '2']) as worker:
                with Client('tcp://127.0.0.1:7328', loop=loop) as c:
                    start = time()
