...
from sumeru.utils_test import (popen, get_cert, new_config_file,
...
def test_basic(loop):
    with popen(['dask-scheduler', '--no-bokeh'] + tls_args) as s:
        with popen(['dask-worker', '--no-bokeh', 'tls://127.0.0.1:7328'] + tls_args) as w:
            with Client('tls://127.0.0.1:7328', loop=loop,
...
def test_nanny(loop):
    with popen(['dask-scheduler', '--no-bokeh'] + tls_args) as s:
        with popen(['dask-worker', '--no-bokeh', '--nanny', 'tls://127.0.0.1:7328'] + tls_args) as w:
            with Client('tls://127.0.0.1:7328', loop=loop,
                        security=tls_security()) as c:
                wait_for_cores(c)
...
def test_separate_key_cert(loop):
    with popen(['dask-scheduler', '--no-bokeh'] + tls_args_2) as s:
        with popen(['dask-worker', '--no-bokeh', 'tls://127.0.0.1:7328'] + tls_args_2) as w:
            with Client('tls://127.0.0.1:7328', loop=loop,
                        security=tls_security()) as c:
                wait_for_cores(c)
...
def test_use_config_file(loop):
    with new_config_file(tls_only_config()):
        with popen(['dask-scheduler', '--no-bokeh', '--host', 'tls://']) as s:
            with popen(['dask-worker', '--no-bokeh', 'tls://127.0.0.1:7328']) as w:
                with Client('tls://127.0.0.1:7328', loop=loop,
                            security=tls_security()) as c:
                    wait_for_cores(c)
