...
from sumeru import Scheduler, Client
from sumeru.utils import get_ip, get_ip_interface, tmpfile
from sumeru.utils_test import (popen,
...
def test_defaults(loop):
    with popen(['dask-scheduler', '--no-bokeh']) as proc:
        ...
        def f():
            ...
            yield [
                assert_can_connect_from_everywhere_4_6(7328, 5.0),  # main port
            ]
...
def test_multiple_workers(loop):
    with popen(['dask-scheduler', '--no-bokeh']) as s:
        with popen(['dask-worker', 'localhost:7328', '--no-bokeh']) as a:
            with popen(['dask-worker', 'localhost:7328', '--no-bokeh']) as b:
                with Client('127.0.0.1:%d' % Scheduler.default_port, loop=loop) as c:
                    start = time()
...
def test_interface(loop):
    ...
    with popen(['dask-scheduler', '--no-bokeh', '--interface', if_name]) as s:
        with popen(['dask-worker', '127.0.0.1:7328', '--no-bokeh', '--interface', if_name]) as a:
...
def test_pid_file(loop):
    ...
    with tmpfile() as s:
        ...
        with tmpfile() as w:
            with popen(['dask-worker', '127.0.0.1:7328', '--pid-file', w,
                        '--no-bokeh']) as worker:
