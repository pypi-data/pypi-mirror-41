...
import sumeru.dask as dask
from sumeru.dask.core import istask
from sumeru.dask.compatibility import apply
try:
...
import shutil
import zipfile
from sumeru.manage.plugin_manager import PluginManager
from multiprocessing import Queue
import queue
from .manage.conf import refresh_config
refresh_config('scheduler')
refresh_config('agent')
from .manage.libs.sysinfo import get_sys_info
from .manage.libs.util import get_plugins
########## add end ###
...
LOG_PDB = dask.config.get('sumeru.admin.pdb-on-err')
...
class WorkerBase(ServerNode):
    def __init__(self, scheduler_ip=None, scheduler_port=None,
        ...
        #### Add ###
        self.agent_id = __sumeru__.AGENT_ID
        self.token = __sumeru__.TOKEN
        self.register = False

        self.data_queue = Queue()
        self.task_queue = Queue()
        self.running_tasks = Queue()
        self.plugin_manager = PluginManager(self.data_queue, self.task_queue, self.running_tasks, logger)
        self.plugin_manager.manager_start()
        #### Add end ###
        ...
        if 'memory_target_fraction' in kwargs:
            ...
            self.memory_target_fraction = dask.config.get('sumeru.worker.memory.target')
        if 'memory_spill_fraction' in kwargs:
            self.memory_spill_fraction = kwargs.pop('memory_spill_fraction')
        else:
            self.memory_spill_fraction = dask.config.get('sumeru.worker.memory.spill')
        if 'memory_pause_fraction' in kwargs:
            self.memory_pause_fraction = kwargs.pop('memory_pause_fraction')
        else:
            self.memory_pause_fraction = dask.config.get('sumeru.worker.memory.pause')
        ...
        pc = PeriodicCallback(self.heartbeat, 5000, io_loop=self.io_loop)
    ...
    def _setup_logging(self):
        self._deque_handler = DequeHandler(n=dask.config.get('sumeru.admin.log-length'))
        self._deque_handler.setFormatter(logging.Formatter(dask.config.get('sumeru.admin.log-format')))
    ...
    def heartbeat(self):
        try:
            if not self.register:
                b, msg = yield self.scheduler.register_agent(agent_id = self.agent_id, token = self.token,
                                            address = self.contact_address)
                if b == 1:
                    self.agent_id = msg
                    __sumeru__.SET_AGENT_ID(self.agent_id)
                    __sumeru__.SET_TOKEN("")
                    self.token = ""
                elif b == 2:
                    self.token = msg
                    self.register = True
                    __sumeru__.SET_TOKEN(self.token)
                else:
                    __sumeru__.SET_TOKEN("")
            ...
            try:
                running_tasks = {}
                while True:
                    running_tasks = self.running_tasks.get_nowait()
            except queue.Empty:
                pass
            ...
            plugins_remove = response.get('plugins_remove')
            plugins_data = response.get('plugins_data')

            for item in plugins_remove:
                plugin_path = os.sep.join([__sumeru__.PLUGIN_PATH, 'plugins', item])
            ...
            for item in plugins_data:
                ...
                plugin_path = os.sep.join([__sumeru__.PLUGIN_PATH, 'plugins', folder])
            ...
            try:
                while True:
                    ...
                    datas.append(msg)
                    if len(datas) == 5:
                        yield self.scheduler.task_result(data = datas)
                        datas = []
            except queue.Empty:
                pass

            task_result_ret = yield self.scheduler.task_result(data = datas)

            tasks = yield self.scheduler.task_pull(agent_id = self.agent_id, address = self.contact_address)
            if tasks:
                for task in tasks:
                    logger.info(task)
    ...
    def _register_with_scheduler(self):
        ...
        while True:
            ...
            try:
                ...
                self.status = 'running'
                logger.info('Connect to scheduler: %26s', self.scheduler.address)
                break
...
class Worker(WorkerBase):
    """ Worker node in a Dask sumeru cluster
    ...
    * **resource_restrictions**: ``{key: {str: number}}``
    ...
    Use the command line to start a worker::
        ...
        $ dask-scheduler
        Start scheduler at 127.0.0.1:7328

        $ dask-worker 127.0.0.1:7328
        Start worker at:               127.0.0.1:1234
        Registered with scheduler at:  127.0.0.1:7328

    See Also
    --------
    sumeru.scheduler.Scheduler
    sumeru.nanny.Nanny
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.total_out_connections = dask.config.get('sumeru.worker.connections.outgoing')
        self.total_in_connections = dask.config.get('sumeru.worker.connections.incoming')
        ...
        self.recent_messages_log = deque(maxlen=dask.config.get('sumeru.comm.recent-messages-log-length'))
        ...
        profile_cycle_interval = kwargs.pop('profile_cycle_interval',
                                            dask.config.get('sumeru.worker.profile.cycle'))
        ...
        pc = PeriodicCallback(
                ...
                parse_timedelta(dask.config.get('sumeru.worker.profile.interval'), default='ms') * 1000,
                io_loop=self.io_loop
        )
    ...
    def get_current_task(self):
        ...
        Examples
        --------
        >>> from sumeru.dask.sumeru import get_worker
        >>> def f():
