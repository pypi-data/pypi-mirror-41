...
from sumeru.protocol import (loads, dumps, msgpack, maybe_compress,
                                  to_serialize)
from sumeru.protocol.compression import compressions
from sumeru.protocol.serialize import (Serialize, Serialized,
                                            serialize, deserialize)
from sumeru.utils import nbytes
from sumeru.utils_test import slow
...
def test_maybe_compress():
    ...
    with dask.config.set({'sumeru.comm.compression': None}):
    ...
    for compression in try_compressions:
        ...
        with dask.config.set({'sumeru.comm.compression': compression}):
            for f in try_converters:
                payload = b'123'
                assert maybe_compress(f(payload)) == (None, payload)
