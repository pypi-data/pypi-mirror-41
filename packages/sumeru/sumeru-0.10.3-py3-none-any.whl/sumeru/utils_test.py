...
try:
    import sumeru.dask as dask.array  # register config
except ImportError:
    pass
...
def client_contract_script(tmpdir_factory):
    local_file = tmpdir_factory.mktemp('data').join('sumeru_script.py')
    lines = ("from sumeru import Client", "e = Client('127.0.0.1:8989')",
             'print(e)')
    local_file.write('\n'.join(lines))
    return local_file
...
def mock_ipython():
    import mock
    from sumeru._ipython_utils import remote_magic
    ip = mock.Mock()
    ip.user_ns = {}
    ip.kernel = None
    ...
    with mock.patch('IPython.get_ipython', get_ip), \
            mock.patch('sumeru._ipython_utils.get_ipython', get_ip):
        yield ip
...
def gen_cluster(ncores=[('127.0.0.1', 1), ('127.0.0.1', 2)],
    ...
    def _(func):
        ...
        def test_func():
            ...
            dask.config.set({'sumeru.comm.timeouts.connect': '5s'})
...
def assert_can_connect(addr, timeout=None, connection_args=None):
    """
    Check that it is possible to connect to the sumeru *addr*
...
def assert_cannot_connect(addr, timeout=None, connection_args=None, exception_class=EnvironmentError):
    """
    Check that it is impossible to connect to the sumeru *addr*
    within the given *timeout*.
    """
    if timeout is None:
        timeout = 0.5
