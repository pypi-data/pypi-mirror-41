...
if dask.config.get('sumeru.dashboard.export-tool'):
    from .export_tool import ExportTool
else:
    ExportTool = None


profile_interval = dask.config.get('sumeru.worker.profile.interval')
profile_interval = parse_timedelta(profile_interval, default='ms')


class DashboardComponent(object):
    """ Base class for Dask.sumeru UI dashboard components.
    ...
    *  update: a method that consumes the messages dictionary found in
    ...
    def update(self, messages):
        """ Reads from bokeh.sumeru.messages and updates self.source """
