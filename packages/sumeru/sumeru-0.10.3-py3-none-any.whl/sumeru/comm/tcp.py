...
def set_tcp_timeout(stream):
    ...
    timeout = dask.config.get('sumeru.comm.timeouts.tcp')
...
class TCP(Comm):
    ...
    def conv_object(self, d):
        if isinstance(d, (bytes,)):
            ...
            return [self.conv_object(x) for x in d]
        elif isinstance(d, dict):
            return dict([(self.conv_object(k), self.conv_object(v)) for k, v in d.items()])
        else:
            return d
...
class BaseTCPListener(Listener, RequireEncryptionMixin):
    ...
    def start(self):
        ...
        backlog = int(dask.config.get('sumeru.comm.socket-backlog'))
        for i in range(5):
            try:
