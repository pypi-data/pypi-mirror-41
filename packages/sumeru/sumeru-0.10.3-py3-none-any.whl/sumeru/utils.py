...
from sumeru.dask import istask
import toolz
import tornado
...
try:
    from sumeru.dask.context import thread_state
except ImportError:
...
def _initialize_mp_context():
    if PY3 and not sys.platform.startswith('win') and 'PyPy' not in sys.version:
        method = dask.config.get('sumeru.worker.multiprocessing-method')
...
def silence_logging(level, root='sumeru'):
    """
...
def get_traceback():
    ...
    bad = [os.path.join('sumeru', 'worker'),
           os.path.join('sumeru', 'scheduler'),
...
def reset_logger_locks():
    ...
    https://github.com/dask/sumeru/issues/1491
    """
