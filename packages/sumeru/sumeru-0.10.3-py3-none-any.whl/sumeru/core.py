...
tick_maximum_delay = parse_timedelta(dask.config.get('sumeru.admin.tick.limit'), default='ms')
...
class Server(object):
    ...
    Superclass for endpoints in a sumeru cluster, such as Worker
    ...
    def __init__(self, handlers, stream_handlers=None, connection_limit=512,
        ...
        if not hasattr(self.io_loop, 'profile'):
            ...
            self.io_loop.profile = profile.watch(
                    ...
                    interval=dask.config.get('sumeru.worker.profile.interval'),
                    cycle=dask.config.get('sumeru.worker.profile.cycle'),
                    stop=stop,
            )
        ...
        pc = PeriodicCallback(
                self._measure_tick,
                parse_timedelta(dask.config.get('sumeru.admin.tick.interval'), default='ms') * 1000,
...
class ConnectionPool(object):
    ...
    This provides a connect method that mirrors the normal sumeru.connect
