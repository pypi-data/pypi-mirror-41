...
class DaskMethodsMixin(object):
    ...
    def persist(self, **kwargs):
        ...
        such as is the case of the dask.sumeru scheduler, then persist
        ...
        This function is particularly useful when using sumeru systems,
        because the results will be kept in sumeru memory, rather than
...
def visualize(*args, **kwargs):
    ...
    **kwargs
    ...
    https://docs.dask.org/en/latest/optimize.html
    """
    from sumeru.dask.dot import dot_graph
...
def persist(*args, **kwargs):
    ...
    background on a cluster (in the sumeru case).

    This function operates differently if a ``dask.sumeru.Client`` exists
    and is connected to a sumeru scheduler.  In this case this function
    ...
    >>> from sumeru.dask import persist  # use persist function on multiple collections
    ...
    **kwargs
    ...
    if inspect.ismethod(schedule):
        try:
            from sumeru.client import default_client
        except ImportError:
            pass
        else:
            try:
...
normalize_token.register((int, long, float, str, unicode, bytes, type(None), type, slice, complex, type(Ellipsis)), identity)
...
try:
    from sumeru.dask import multiprocessing as dask_multiprocessing
except ImportError:
    pass
else:
...
def get_scheduler(get=None, scheduler=None, collections=None, cls=None):
    ...
    if scheduler is not None:
        ...
        elif scheduler.lower() in ('dask.sumeru', 'sumeru'):
            from sumeru.worker import get_client
            return get_client().get
        else:
            raise ValueError("Expected one of [sumeru, %s]" % ', '.join(sorted(named_schedulers)))
    ...
    if getattr(thread_state, 'key', False):
        from sumeru.worker import get_worker
