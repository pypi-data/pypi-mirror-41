...
from sumeru.manage.libs.models import *
load_settings()
from .manage.libs.crython.tab import default_tab as tab
from .manage.libs.crython_api import *
from .manage.libs.mq_util import RedisMQ, HA_ETCD, HA_ETCD_SYNC
from .manage.libs.util import *
from .manage.libs.models import *
from .manage.plugin_data_transfer import data_transfer
#### add end ####
...
BANDWIDTH = dask.config.get('sumeru.scheduler.bandwidth')
ALLOWED_FAILURES = dask.config.get('sumeru.scheduler.allowed-failures')

LOG_PDB = dask.config.get('sumeru.admin.pdb-on-err')
DEFAULT_DATA_SIZE = dask.config.get('sumeru.scheduler.default-data-size')
...
if dask.config.get('sumeru.scheduler.work-stealing'):
...
class TaskState(object):
    ...
    .. attribute:: dependencies: {TaskState}
       ...
       :attr:`has_lost_dependencies`  is set.
...
class Scheduler(ServerNode):
    ...
    executable::
         ...
         $ sumeru-scheduler
         Scheduler started at 127.0.0.1:7328
    ...
    * **coroutines:** ``[Futures]``:
    ...
    def __init__(
        ...
        worker_ttl = worker_ttl or dask.config.get('sumeru.scheduler.worker-ttl')
        ...
        ############# add client ##
        self.register_crontask = False
        self.agent = {}
        self.mq = RedisMQ()
        self.HA = HA_ETCD()
        self.HA_SYNC = HA_ETCD_SYNC()
        self.tab = tab
        self.tab.start()
        self.decode_map = {}
        ...
        self.extensions = {}
        self.plugins = []
        self.transition_log = deque(maxlen=dask.config.get('sumeru.scheduler.transition-log-length'))
        self.log = deque(maxlen=dask.config.get('sumeru.scheduler.transition-log-length'))
        self.worker_setups = []
        ...
        self.handlers = {
            ...
            'task_pull': self.task_pull,
            'result_pull': self.result_pull,
        ...
        self.periodic_callbacks['daemon_time_loop'] = \
                PeriodicCallback(self.daemon_time_loop, 10000, io_loop=loop)

        self.periodic_callbacks['hearbeat'] = \
                PeriodicCallback(self.hearbeat, 1000, io_loop=loop)
        ...
        setproctitle("sumeru-scheduler [not started]")
    ...
    def start(self, addr_or_port=7328, start_queues=True):
        ...
        setproctitle("sumeru-scheduler [%s]" % (self.address,))
    ...
    def close(self, comm=None, fast=False):
        ...
        setproctitle("sumeru-scheduler [closing]")
        ...
        setproctitle("sumeru-scheduler [closed]")
    ...
    @database_connection
    @gen.coroutine
    def register_agent(self, comm = None, agent_id = None, token = None, address = None):
        if not agent_id:
            agent_id = make_hash()
            DNode(node_id = agent_id, node_addr = str(address)).save()
            return 1, agent_id
        else:
            agent = DNode.get_or_none(DNode.node_id == agent_id)
            ...
            if agent.node_status == 0:
                return False, 'FAIL NOT AUTH'

            if agent.node_token and agent.node_token == token:
                return 2, token

            if not token and agent.node_status == 1:
                token = make_token(agent_id, __sumeru__.SECRET_KEY)
                DNode.update(node_token = token, node_status = 2).where(DNode.node_id == agent_id).execute()
                return 2, token

            logger.debug('FAIL CHECK AUTH {} {} {}'.format(agent_id, token, address))
            return False, 'FAIL NOT AUTH'

    @database_connection
    @gen.coroutine
    def heartbeat_worker(self, comm=None, address=None, resolve_address=True,
                          now=None, resources=None, host_info=None, metrics=None, agent_id = None, token = None, 
                          sysinfo = {}, task_list = {}, plugins = []):
            ...
            DNode.update(heartbeat_time = time(), node_addr = get_address_host(comm.peer_address)).where(DNode.node_id == agent_id).execute()

            if agent_id not in self.agent:
                self.agent[agent_id] = {}

                t = threading.Thread(target=send_email, args = ("{} ONLINE".format(get_address_host(comm.peer_address)), '[Sumeru][NODE] ONLINE {}'.format(get_address_host(comm.peer_address))))
                t.daemon = True
                t.start()

            self.agent[agent_id]['address'] = address
            self.agent[agent_id]['heartbeat_time'] = time()
            self.agent[agent_id]['sysinfo'] = sysinfo or {}
            self.agent[agent_id]['task_list'] = list(task_list.values())

            ######

            for task in self.agent[agent_id]['task_list']:
                task_id = task['TASK_ID']
                current_task = yield self.HA.read('/task_id/{}'.format(task_id))
                if not current_task: continue
                current_task = json.loads(current_task.value)

                root_task = None
                try:
                    yield self.get_root_task(current_task)
                except Exception as e:
                    root_task = json.loads(e.args[0])

                if not root_task: continue

                root_task_id = root_task['task_id']
                yield self.HA.write('/running_tasks/{}'.format(root_task_id), time())
                daemon_task_id = yield self.HA.read("/daemon_task/{}".format(root_task_id))
                if not daemon_task_id: continue
                yield self.HA.write("/daemon_task_heartbeat/{}".format(root_task_id), time())
            ######

            node = DNode.get_or_none(DNode.node_id == agent_id)
            plugins_data = []
            plugins_remove = []
            if node:
                g = DNodeGroup.select().where(DNodeGroup.dnode_id == node.id)
                gids = [_g.group_id for _g in g]
                groups = Group.select().where(Group.id.in_(gids))

                remote_plugins = []
                _ = [g.plugins.split(",") for g in groups if g.plugins]
                for _i in _:
                    remote_plugins.extend(_i)

                remote_plugins = PluginStore().select().where(PluginStore.plugin_name.in_(remote_plugins), PluginStore.is_used == 1)
                remote_plugins = [_p.filename.split(".")[0] for _p in remote_plugins]

                plugins_remove = list(set(plugins) - set(remote_plugins))
                plugins_add = list(set(remote_plugins) - set(plugins))
                plugins_data = []
                for pa in plugins_add:
                    plugin_path = os.sep.join([__sumeru__.WORKSPACE_PATH, 'plugins_store', '{}.zip'.format(pa)])

                    if not os.path.exists(plugin_path): continue
                    p_data = {'filename': '{}.zip'.format(pa)}
    ...
    def trans_task(self, _task):
        task_id = _task.get('task_id')
        parent_task_id = _task.get('parent')

        p_task = self.HA_SYNC.read('/task_id/{}'.format(parent_task_id))
        p_task = json.loads(p_task.value)
        if task_id in p_task['finish_child']: return

        p_task['finish_child'].append(task_id)
        self.HA_SYNC.write('/task_id/{}'.format(parent_task_id), json.dumps(p_task))

        if p_task['parent'] and (len(p_task['child']) == len(set(p_task['finish_child']))):
            self.trans_task(p_task)

    @gen.coroutine
    def get_root_task(self, _task):
        parent_task_id = _task.get('parent')

        p_task = yield __sumeru__.scheduler.HA.read('/task_id/{}'.format(parent_task_id))
        p_task = json.loads(p_task.value)

        if p_task['parent'] == 0:
            raise Exception(json.dumps(p_task))

        yield self.get_root_task(p_task)


    @gen.coroutine
    def get_tasks(self, task_id, r = []):
        task = yield self.HA.read('/task_id/{}'.format(task_id))
        if not task: return
        task = json.loads(task.value)
        r.append(task)
        childs = task['child']
        for child in childs:
            yield self.get_tasks(child, r = r)

    @gen.coroutine
    def check_task_status(self, root_task_id):
        tasks = []
        yield self.get_tasks(root_task_id, tasks)
        status = [item['status']  for item in tasks if item['parent'] != 0]
    ...
    @database_connection
    @gen.coroutine
    def task_result(self, comm, data):
        ...
        for data in datas:
            ...
            # read current task from HA
            current_task = self.HA_SYNC.read('/task_id/{}'.format(task_id))
            if not current_task: return
            current_task = json.loads(current_task.value)

            # save log to mysql
            InsTimeLine(task_id = task_id, \
                    root_task_id = current_task['root_task_id'], \
                    data = text_data[:1024 * 64 - 1]).save()

            if msg_type == 'TASK_DATA':
                current_component = self.HA_SYNC.read("/component_id/{}".format(current_task['component_id']))
                current_component = json.loads(current_component.value)
                childs = current_component['child']
                for child in childs:
                    component = self.HA_SYNC.read('/component_id/{}'.format(child))
                    component = json.loads(component.value)
                    
                    task_data = {'PARAM': data_transfer(current_component['name'], component['name'], data['data']), 'RUN_METHOD': current_task['run_method']}
                    run_component(current_task, component, task_data)

                # push exec data to redis
                redis_data = dict(task_id = current_task['root_task_id'], \
                                layout_id = current_task['layout_id'], \
                                msg_type = msg_type, data = data['data'])

                try:
                    plugin_name = current_component.get('name')
                    ins = getattr(self.decode_map.get(plugin_name), 'Decoder')
                    ins().decode(data.get('data'))
                except Exception as e:
                    pass

            elif msg_type == 'TASK_STATUS':
                # Refresh current task status to HA
                current_task['status'] = running_status
                if running_status in ['SUCCESS', 'FAILURE', 'CANCELED']:
                    current_task['finish_time'] = time()

                self.HA_SYNC.write('/task_id/{}'.format(task_id), json.dumps(current_task))
                redis_data = dict(task_id = current_task['root_task_id'], \
                                layout_id = current_task['layout_id'], \
                                msg_type = msg_type, \
                                running_status = 'RUNNING', unix_time = time())

                if running_status in ['SUCCESS', 'FAILURE', 'CANCELED']:
                    if not current_task['child']:
                        self.trans_task(current_task)

                    # read root task from HA
                    root_task = self.HA_SYNC.read('/task_id/{}'.format(current_task['root_task_id']))
                    if not root_task: return
                    root_task = json.loads(root_task.value)

                    logger.info('root_task: {} {} {}'.format(root_task['task_id'], len(root_task['child']), len(root_task['finish_child'])))

                    if len(root_task['child']) == len(root_task['finish_child']):
                        redis_data['running_status'] = running_status

                        root_task_status = running_status
                        crontask_id = root_task.get('crontask_id')
                        crontask = CronTask.get_or_none(CronTask.id == crontask_id)
                        CronTask.update(count = CronTask.count + 1).\
                                        where(CronTask.id == crontask_id).execute()

                        root_task['finish_time'] = time()
                        self.HA_SYNC.write('/task_id/{}'.format(root_task['task_id']), json.dumps(root_task))

                        TaskIns.update(running_status = root_task_status, finish_time = time()).\
                                        where(TaskIns.task_id == root_task.get('task_id')).execute()

                        if crontask.is_sendmail:
                            t = threading.Thread(target=send_email, args = ('TASK {} SUCCESS'.format(crontask.task_name), '[Sumeru][TASK] {}'.format(root_task_status)))
                            t.daemon = True
                            t.start()
            # push exec data to redis
            try:
                redis_data = json.dumps(redis_data)
                self.mq.push('RESULT_TASK_QUEUE', redis_data)
            except Exception as e:
                logger.error(redis_data)


            logger.info('handle time {}'.format(time() - t1))

        return b'success'

    def task_pull(self, comm, agent_id, address = None):
        '''
        pull a task requests
        '''
        datas = []
        while True:
            data = self.mq.pop(agent_id)
            if not data: break
            datas.append(data)

        return datas

    def result_pull(self, comm):
        '''
        pull a result requests
        '''
        return self.mq.pop("RESULT_TASK_QUEUE") or None

    @gen.coroutine
    def hearbeat(self):
        bytes_sent = psutil.net_io_counters().bytes_sent
        bytes_recv = psutil.net_io_counters().bytes_recv
        self.uplink_speed = (bytes_sent - self.bytes_sent)/1024.0/5
        self.downlink_speed = (bytes_recv - self.bytes_recv)/1024.0/5
        self.bytes_sent = bytes_sent
        self.bytes_recv = bytes_recv

    @database_connection
    @gen.coroutine
    def daemon_time_loop(self):
        if not getattr(__sumeru__, "DB_DRIVER") or \
                not hasattr(__sumeru__, "ETCD_DRIVER") or \
                not hasattr(__sumeru__, "REDIS_DRIVER"):
            return 

        if not getattr(__sumeru__, "DB_DRIVER") or \
                not getattr(__sumeru__, "ETCD_DRIVER") or \
                not getattr(__sumeru__, "REDIS_DRIVER"):
            return 

        task_count = sum([len(agent.get('task_list', [])) for agent_id, agent in self.agent.items()])
        max_task_count = self.mq.get('max_task_count')
        max_task_count = max(task_count, int(max_task_count)) if max_task_count else 0
        self.mq.set('task_count', task_count)
        self.mq.set('max_task_count', max_task_count)
            
        for agent_id in list(self.agent.keys()):
            agent_info = self.agent[agent_id]
            if time() - agent_info['heartbeat_time'] > 60:
                del(self.agent[agent_id])

        if not self.register_crontask:
            self.register_crontask = True
            for ct in CronTask.select().where(CronTask.task_type == 1, \
                                                CronTask.status == 1):
                register_cron_job(ct.id, ct.cron)

        # plugins check
        plugin_local = get_plugins(folder = 'scheduler_plugins')
        store_plugins = PluginStore().select().where(PluginStore.is_used==1)
        store_plugins = [_p.filename.split(".")[0] for _p in store_plugins]

        plugins_remove = list(set(plugin_local) - set(store_plugins))
        plugins_add = list(set(store_plugins) - set(plugin_local))
        plugins_data = []
        for item in plugins_add:
            zip_file = os.sep.join([__sumeru__.WORKSPACE_PATH, 'plugins_store', '{}.zip'.format(item)])
            if not os.path.exists(zip_file): continue

            ex_path = os.sep.join([__sumeru__.PLUGIN_PATH, 'scheduler_plugins', item])
        ...
        for item in plugins_remove:
            ...
            if os.path.exists(zip_file):
                os.remove(zip_file)
            plugin_path = os.sep.join([__sumeru__.PLUGIN_PATH, 'scheduler_plugins', item])
            if os.path.exists(plugin_path):
                shutil.rmtree(plugin_path)
            
        self.decode_map = {}
        scheduler_plugins_dir = os.sep.join([__sumeru__.PLUGIN_PATH, 'scheduler_plugins'])

        if not sys.path.count(scheduler_plugins_dir):
            sys.path.append(scheduler_plugins_dir)

        for plugin_dir in os.listdir(scheduler_plugins_dir):
            p = os.sep.join([scheduler_plugins_dir, plugin_dir])
            if not os.path.isdir(p): continue

            f_config = os.sep.join([p, 'config.json'])
            if not os.path.exists(f_config): continue
            decode_file = os.sep.join([p, 'decoder.py'])
            if not os.path.exists(decode_file): continue

            config = load_json(f_config)
            plugin_name = config.get('plugin_name')

            try:
                module = __import__('{}.decoder'.format(plugin_dir), fromlist = ['Decoder'])
                self.decode_map[plugin_name] = module
            except Exception as e:
                pass

        # daemon task check
        daemon_task = yield self.HA.read('/daemon_task')
        if daemon_task and daemon_task.children:
            for t in daemon_task.children:
                if not t.value: continue
                root_task_id = t.value
                root_task = yield self.HA.read('/task_id/{}'.format(root_task_id))
                if not root_task: continue
                root_task = json.loads(root_task.value)
                crontask = CronTask.get_or_none(CronTask.id == root_task['crontask_id'])
                if not crontask or not crontask.is_daemon:
                    yield self.HA.delete(t.key)
                    continue

                root_task_status = yield self.check_task_status(root_task['task_id'])
                last_time = yield self.HA.read('/daemon_task_heartbeat/{}'.format(root_task['task_id']))

                if last_time:
                    last_time = float(last_time.value)

                    logger.info('daemon_task: {} {} {}'.format(root_task_id, root_task_status, time() - last_time))
                else:
                    logger.info('daemon_task: {} {}'.format(root_task_id, root_task_status))

                if root_task_status in ['FINISH', 'FAILURE', 'CANCELED']:
                    run_cron_job(root_task['crontask_id'])
                    TaskIns.update(running_status = root_task_status).where(TaskIns.task_id == root_task['task_id']).execute()
                    yield self.HA.delete(t.key)

                    if crontask and crontask.get('is_alarm'):
                        t = threading.Thread(target=send_email, \
                                args = ("DAEMON TASK {} {}".format(crontask.task_name, root_task_status), '[Sumeru][TASK] DOWN'))
                        t.daemon = True
                        t.start()
                else:
                    if last_time:
                        if time() - last_time > 10 * 60:
                            run_cron_job(root_task['crontask_id'])
                            TaskIns.update(running_status = 'FAILURE').where(TaskIns.task_id == root_task['task_id']).execute()
                            yield self.HA.delete(t.key)

        # running tasks check
        running_tasks = yield self.HA.read('/running_tasks')
        if running_tasks and running_tasks.children:
            for t in running_tasks.children:
                if not t.value: continue

                root_task_id = t.key.split('/')[-1]
                last_time = float(t.value)
                ...
                if len(root_task['child']) == len(root_task['finish_child']):
                    ...
                    logger.info('running_tasks: {}, {}'.format(root_task_id, time() - last_time))
                    if time() - last_time < 30 * 60: continue
                    yield self.HA.write('/running_tasks/{}'.format(root_task_id), time())

                    tasks = []
                    yield self.get_tasks(root_task_id, tasks)
                    for task in tasks:
                        if not task['parent']: continue
                        if task['status'] not in ['RUNNING', 'PENDING']: continue
                        run_task(task['task_id'], task['group_id'], task['params'])
    ...
    def add_task(self, comm, **kwargs):
        """
        args:
            task_name:  task name
            group_id:
            layout_id:
            cron: cron_expr: eg: @secondly, @dayly ... or ''
            task_code: base64.b64decode("task={'PARAM': {}}")
            task_desc: task description
            run_method: 1 thread; 2 process
        """
        kwargs['group_id'] = kwargs.get('group_id') or 1
        ct = CronTask(**kwargs)
        ct.save()
        return run_cron_job(ct.id)

    @gen.coroutine
    def stop_task(self, comm, task_id):
        tasks = []
        yield self.get_tasks(task_id, r = tasks)
        for task in tasks:
    ...
    @gen.coroutine
    def get_status(self, comm, task_id):
        task_status = yield self.check_task_status(task_id)
        return task_status

    @gen.coroutine
    def get_result(self, comm, task_id):
        task_status = yield self.check_task_status(task_id)
        return task_status
        ...
        with log_errors():
            ...
            for agent_id in list(self.agent.keys()):
                try:
                    ...
                    t = threading.Thread(target=send_email, args = ("{} OFFLINE".format(get_address_host(comm.peer_address)), '[Sumeru][NODE] OFFLINE {}'.format(get_address_host(comm.peer_address))))
                    t.daemon = True
                    t.start()
                except Exception as e:
                    pass
    ...
    def add_plugin(self, plugin=None, idempotent=True, **kwargs):
        ...
        See https://sumeru.readthedocs.io/en/latest/plugins.html
        """
    ...
    def workers_to_close(self, memory_ratio=None, n=None, key=None,
        ...
        This is for use with systems like ``sumeru.deploy.adaptive``.
    ...
    def get_profile_metadata(self, comm=None, workers=None, merge_workers=True,
        ...
        dt = profile_cycle_interval or dask.config.get('sumeru.worker.profile.cycle')
