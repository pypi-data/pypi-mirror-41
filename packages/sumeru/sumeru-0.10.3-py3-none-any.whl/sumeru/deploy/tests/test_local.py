...
from sumeru import Client, Worker, Nanny
from sumeru.deploy.local import LocalCluster
from sumeru.metrics import time
from sumeru.utils_test import (inc, gen_test, slowinc,
...
from sumeru.utils import sync
from sumeru.worker import TOTAL_MEMORY

from sumeru.deploy.utils_test import ClusterTest
...
def test_transports():
    ...
    with LocalCluster(1, processes=False, scheduler_port=7328, silence_logs=False,
                      diagnostics_port=None) as c:

        assert c.scheduler_address == 'tcp://127.0.0.1:7328'
        assert c.workers[0].address.startswith('tcp://')
        with Client(c.scheduler.address) as e:
            assert e.submit(inc, 4).result() == 5
...
@pytest.mark.skipif('sys.version_info[0] == 2', reason='fork issues')
def test_defaults():
    from sumeru.worker import _ncores
...
def test_silent_startup():
    code = """if 1:
        from time import sleep
        from sumeru import LocalCluster
...
def test_local_tls(loop):
    from sumeru.utils_test import tls_only_security
    security = tls_only_security()
    with LocalCluster(scheduler_port=7328, silence_logs=False, security=security,
...
if sys.version_info >= (3, 5):
    from sumeru.deploy.tests.py3_test_deploy import *  # noqa F401
