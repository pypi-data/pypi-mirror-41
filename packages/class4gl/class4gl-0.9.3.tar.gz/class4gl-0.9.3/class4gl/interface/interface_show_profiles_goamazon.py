...
'''
print('Adding python library:','/user/home/gent/vsc422/vsc42247/software/class4gl/class4gl')
...
if (select[0]) > 0:
    c4gldata[0].next_record( (select[0]) - sel)
...
for j,i in enumerate(select):
    ...
    # if i in [12,13]:
    #     profiles_noon_datetime.append(c4gldata[1].frames['profiles']['record_yaml_end_obs'].pars.ldatetime)
    #     profiles_noon_obs.append(c4gldata[1].frames['profiles']['record_yaml_end_obs'].air_ap)
    #     profiles_noon_obs_raw.append(c4gldata[1].frames['profiles']['record_yaml_end_obs'].air_balloon)
    #     profiles_noon_end_mod.append(c4gldata[1].frames['profiles']['record_yaml_end_mod'].air_ap)
    #     profiles_noon_obs[-1].theta = np.nan
    #     profiles_noon_obs_raw[-1].theta = np.nan
    #     profiles_noon_end_mod[-1].theta = np.nan
    #     profiles_noon_obs[-1].q = np.nan
    #     profiles_noon_obs_raw[-1].q = np.nan
    #     profiles_noon_end_mod[-1].q = np.nan



    profiles_afternoon_datetime.append(c4gldata[0].frames['profiles']['record_yaml_end_obs'].pars.ldatetime)
    profiles_afternoon_obs.append( c4gldata[0].frames['profiles']['record_yaml_end_obs'].air_ap)
    profiles_afternoon_obs_raw.append(c4gldata[0].frames['profiles']['record_yaml_end_obs'].air_balloon)
    profiles_afternoon_end_mod.append( c4gldata[0].frames['profiles']['record_yaml_end_mod'].air_ap)


    if j+1 < len(select):
        c4gldata[0].next_record(select[j+1] - sel)


            #axes[varkey].set_title(latex['d'+varkey+'dt']+' '+units_final,fontsize=12.)                                     


numprofiles = len(profiles_afternoon_end_mod)
...
for var in VARS:
    ...
    for i in range(numprofiles):
        ...
        plt.plot(profiles_afternoon_end_mod[i][var].values*fac, profiles_afternoon_end_mod[i].z.values,'r-',label=label)
