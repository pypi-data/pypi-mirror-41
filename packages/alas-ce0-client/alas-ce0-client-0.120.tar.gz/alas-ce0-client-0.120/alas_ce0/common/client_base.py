...
class ApiClientBase(object):
    def __init__(self, **kwargs):
        ...
        if self.username and self.password:
            self.headers['Authorization'] = \
                'Basic ' + base64.encodestring(self.username + ':' + self.password).replace(os.linesep, '')
            self.requests_auth = HTTPBasicAuth(self.username, self.password)
        else:
    ...
    def http_get_json(self, url):
        if self.for_app_engine:
            ...
            response = urlfetch.fetch(self.base_url + url, method=urlfetch.GET, headers=self.headers)
            # response = requests.get(self.base_url + url, auth=self.requests_auth)
    ...
    def http_put_json(self, url, obj):
        if self.for_app_engine:
            ...
            response = urlfetch.fetch(self.base_url + url, method=urlfetch.PUT, headers=self.headers, payload=obj)
            # response = requests.put(self.base_url + url, auth=self.requests_auth, json=obj)
            return self._process_response(response, response.content)
        else:
            with self._get_client() as _client:
    ...
    def http_post_json(self, url, obj):
        if self.for_app_engine:
            ...
            response = urlfetch.fetch(self.base_url + url, method=urlfetch.POST, headers=self.headers, payload=obj)
            # response = requests.post(self.base_url + url, auth=self.requests_auth, json=obj)
            return self._process_response(response, response.content)
        else:
            with self._get_client() as _client:
    ...
    def http_post(self, url, data):
        if self.for_app_engine:
            ...
            response = urlfetch.fetch(self.base_url + url, method=urlfetch.POST, headers=self.headers, payload=data)
            # response = requests.post(self.base_url + url, auth=self.requests_auth, data=data)
    ...
    def http_delete(self, url, data=None):
        if self.for_app_engine:
            ...
            response = urlfetch.fetch(self.base_url + url, method=urlfetch.DELETE, headers=self.headers, payload=data)
            # response = requests.delete(self.base_url + url, auth=self.requests_auth, data=data)
            return self._process_response(response, response.content)
        else:
            with self._get_client() as _client:
