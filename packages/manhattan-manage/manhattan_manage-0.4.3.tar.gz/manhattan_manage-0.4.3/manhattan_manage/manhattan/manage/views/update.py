...
# Define the links
update_chains.set_link(factories.config(form_cls=None, projection=None))
...
def update_document(state):
    ...
    if hasattr(state.manage_config.frame_cls, 'logged_update'):
        ...
        document.logged_update(state.manage_user, form_data)
        ...
        document.update(list(form_data.keys()))
