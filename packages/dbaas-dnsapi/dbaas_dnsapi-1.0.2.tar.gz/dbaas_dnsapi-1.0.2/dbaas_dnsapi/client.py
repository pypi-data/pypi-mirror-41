...
class DNSAPI(object):
    ...
    def create_record(self, name, content, record_type="A", domain_id=None, create_reverse=False):
        if FAKE:
            return {}
        if domain_id is not None:
    ...
    def delete_record_by_name(self, host_name, ip, domain='globoi.com', delete_reverse=False, dns_type='A'):
        '''Dado o hostname e dominio, removo a entrada no DNS e seu reverso'''
        if FAKE:
            return None
        default_domain_id = self.domain_id_discover(domain)
        record_id = self.get_record_by_name(host_name, domain_id=default_domain_id, record_type=dns_type)
        if delete_reverse:
    ...
    def update_record_ttl(self, record_id, name, content, ttl=None, record_type="A"):
        if FAKE:
            return {}
        self.__request(PUT, '/records/%d.json' % record_id,
                       {"record": {"name": name, "type": record_type, "content": content, "ttl": ttl}})
