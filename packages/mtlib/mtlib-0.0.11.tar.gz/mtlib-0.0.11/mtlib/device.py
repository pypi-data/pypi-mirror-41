...
class Device(object):
    ...
    async def async_get_state(self):
        ...
        except pycoap.exceptions.ProtocolError as error:
            raise ProtocolError(str(error)) from error
        except pycoap.exceptions.MessageTimeoutError as error:
            raise CoapRequestTimedOut(str(error)) from error
        except pycoap.exceptions.NameResolutionError as error:
            raise NameNotResolved(str(error)) from error
    ...
    async def async_observe_refresh(self):
        await self._observation.register()
