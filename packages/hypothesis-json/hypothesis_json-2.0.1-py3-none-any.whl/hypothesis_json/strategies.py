...
from ._utils import ensure_not_integer, validate_bool
...
def _integers(
    ...
    # TODO assert min_value and max_value are not out of JSON bounds
    return hs.integers(min_value, max_value)
...
def _numbers(
    *, infs: bool = True, nan: bool = True, integers: bool = True
    ...
    if integers:
        ...
        return hs.one_of(
            ints,
            hs.builds(float, ints),
            hs.floats(allow_infinity=infs, allow_nan=nan),
        )
    fints = hs.builds(float, _integers(-(2 ** 52) + 1, +(2 ** 52) - 1))
    return hs.one_of(
        hs.builds(next_down, fints),
        hs.builds(next_up, fints),
        hs.builds(
            ensure_not_integer, hs.floats(allow_infinity=infs, allow_nan=nan)
        ),
    )
