...
class Element(object):
    ...
    def __init__(self, response, **options):
        ...
        bgColour = options.get('bgcolour')
        if bgColour is not None:
            self.add_custom_css({'background-color':bgColour})
            self._backgroundColour = bgColour
...
#        return result
    ...
    def toInlinedHTML(self):
        ...
        self.add_custom_css({'font-size':'12px','white-space':'nowrap'})
...
class Dashboard(Element):
    ...
    def _set_style(self, style):
        ...
        self._borderColour = style.getBorderColour()
        if 'background-color' not in self.customCss:
