...
from .models import SoupstarsCache
...
class SqlalchemyCache(BaseCache):
    """
    A database backed cache. Stories items as pickled objects.
    """

    def __init__(self, url=None):
        self.url = url or os.environ['DATABASE_URL']
        self.engine = sqlalchemy.engine.create_engine(self.url)
        self.Session = sqlalchemy.orm.sessionmaker(bind=self.engine)

    def get(self, key):
        sess = self.Session()
        val = sess.query(SoupstarsCache).get(key)
        if val is None:
            return
        else:
            return pickle.loads(val.blob)

    def set(self, key, value):
        blob = pickle.dumps(value)
        val = SoupstarsCache(id=key, blob=blob)
        sess = self.Session()
        sess.merge(val)
        sess.commit()
