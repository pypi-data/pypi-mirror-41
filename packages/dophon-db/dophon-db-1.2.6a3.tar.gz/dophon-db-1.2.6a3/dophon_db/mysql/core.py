...
class curObj:
    ...
    def __init__(self, path: str, poolFlag: bool, debug: bool, db=None):
        """
        初始化结果集对象
    ...
    def __getattr__(self, item):
        return CallableObject(self.exe_sql, item)
    ...
    def close(self):
        ...
        if self._poolFlag:
            # 为连接池定义
            self._pool.closeConn(self._conn)
            # 归还连接后清除指针
            self._cursor = None
            self._db = None
            self._conn = None
...
class CallableObject:
    ...
    def __init__(self, callable_entity, execution_body: str):
