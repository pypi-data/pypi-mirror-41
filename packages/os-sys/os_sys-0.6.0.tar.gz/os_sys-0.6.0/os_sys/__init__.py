...
def all_dict(dicto, maps, files, print_or_return, exception=None):
    import os
    data = []
    string_data = ''
    print_ = True if print_or_return == 'print' or print or True else False
...
all_dict('C:\\mattie', True, True, False, exception='pygame')
try:
    from . import fail, modules, system, wifi, programs, test, os_sys, errors
except Exception:
    try:
        from os_sys import fail, modules, system, wifi, programs, test, os_sys, errors
    except Exception:
        import fail, modules, system, wifi, programs, test, os_sys, errors
fail = fail
modules = modules
system = system
wifi = wifi
