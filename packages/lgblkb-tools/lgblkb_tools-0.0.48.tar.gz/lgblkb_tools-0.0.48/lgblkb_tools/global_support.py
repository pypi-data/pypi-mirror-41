...
def try_to_do(func,*args,check_success=None,max_attempts=5,max_wait_seconds=0,**kwargs):
	if check_success is None: check_success=lambda x:x
...
class ScheduledTask:
	...
	def __init__(self,parent_task=None,filename=None,**log_opts):
		if parent_task is not None: parent_task.child_tasks.append(self)
		self.parent_task=parent_task
		self.child_tasks=list()
		self.filename=filename
		if not self.filename is None: self.create_log_file(level=logsup.INFORM,**log_opts)
