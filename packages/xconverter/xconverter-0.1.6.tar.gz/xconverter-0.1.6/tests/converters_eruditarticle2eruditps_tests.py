...
class TestEruditArticle2EruditPS(unittest.TestCase):
    ...
    def test_journal_id_erudit(self):
        ...
        result = self.xml.find("front/journal-meta/journal-id[@journal-id-type='erudit']").text
    ...
    def test_article_meta_title_group_title(self):
        ...
        self.maxDiff = None
        expected = '<article-title xml:lang="fr">« Chantez au Seigneur un chant nouveau… » (Ps.95.1) : le portrait de la musique <italic>rock</italic> chrétienne</article-title>'
        self.assertEqual(
    ...
    def test_article_meta_title_group_subtitle(self):
        ...
        self.assertEqual(
            etree.tostring(result, encoding="utf-8").decode('utf-8'),
            '<subtitle xml:lang="fr">Article <italic>Subtitle</italic></subtitle>'
        )
    ...
    def test_article_meta_title_group_title_1(self):
        ...
        expected = '<article-title xml:lang="fr">Titre Ref Biblio</article-title>'
        self.assertEqual(
            etree.tostring(result, encoding="utf-8").decode('utf-8'),
            expected
        )
    ...
    def test_article_meta_title_group_title_2(self):
        ...
        custom_xml = self.conv.transform(lxml_etree=True)
        result = custom_xml.find("front/article-meta/title-group/article-title[@{http://www.w3.org/XML/1998/namespace}lang='fr']")
        expected = '<article-title xml:lang="fr">« Chantez au Seigneur un chant nouveau… » (Ps.95.1) : le portrait de la musique <italic>rock</italic> chrétienne (Titre Ref Biblio)</article-title>'
        self.assertEqual(
            etree.tostring(result, encoding="utf-8").decode('utf-8'),
            expected
        )
    ...
    def test_article_meta_title_group_translated_title(self):
        ...
        expected = [
            b"""<trans-title>Translated title in <italic>english</italic></trans-title>""",
            b"""<trans-title>Translated title in <italic>portuguese</italic></trans-title>"""
        ]
    ...
    def test_article_meta_title_group_translated_subtitle(self):
        ...
        result = [etree.tostring(i) for i in result]
        expected = [
            b"""<trans-subtitle>Translated subtitle in <italic>english</italic></trans-subtitle>""",
            b"""<trans-subtitle>Translated subtitle in <italic>portuguese</italic></trans-subtitle>"""
        ]
        self.assertEqual(
            sorted(result),
            sorted(expected)
    ...
    def test_article_meta_contrib_group_content(self):
        ...
        self.assertEqual(result, ['Marie-Chantal', 'Stéphane Marie'])
    ...
    def test_trans_abstract(self):
        ...
        expected = b"""<trans-abstract xml:lang="en"><title>Abstract</title><p>Abstract translation sample.</p></trans-abstract>"""
