...
class IMessageCE(IMessageHandler):
    ...
    def init_message_ce(self,site,projectCode,code) -> ResultVO:
        ...
        # 启动消息队列的监听
        t = MessageListener(self,f"{site}.{projectCode}.{code}",sourceFd.target, sourceFd.connection)
    ...
    def check_and_init(self):
        ...
        if not self.check_ficus_online(ec):
            log.error("启动时,没有可用的ficus服务,忽略初始化消息监听")
            return
        ...
        if registry is None or len(registry) == 0 or "computeexecutions" not in registry:
            # 说明没有yml文件,不进行注册处理
            log.warn("MessageCE开始初始化消息队列,registry.yml中没有配置ce 不自动初始化.请使用调度触发")
            return
        ...
        for computeexecution in registry["computeexecutions"]:
            ...
            if task_handler_name == computeexecution["handler"]:
                ...
                log.info("MessageCE开始初始化消息队列,初始化CE:"+site+"_"+dataproject+"_"+code+" 完成,结果:"+stringResultVO.msg)
    ...
    def __init__(self):
        ...
        # 这里启动一个线程来检测是否启动了
        mthread = threading.Thread(target=self.whileCheck)
        mthread.start()
...
class MessageListener(threading.Thread):
    ...
    def __init__(self,messageCe:IMessageCE,group_id,topic,brokers):
        ...
        self.consumer = Consumer(config,logger=log)     # TODO 这里的入参好像有问题
