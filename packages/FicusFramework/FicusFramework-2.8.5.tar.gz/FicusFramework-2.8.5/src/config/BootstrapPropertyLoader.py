...
def load_properties_after_started(app, eureka_client):
    ...
    try:
        ...
        if len(r.content) > 0:
            ...
            import yaml
            # 合并本地配置文件和云端配置文件
            annotation.deep_search_and_merge(annotation.REMOTE_YML_CONFIG,yaml.load(r.content))
            log.info(f"完成配置中心配置读取{config.config_name}-{config.config_profile}")
        else:
...
def init_from_environ_property():
    ...
    if "server.port" in environs:
        config.server_port = environs["server.port"]
    ...
    if "spring.cloud.config.fail-fast" in environs:
        config.config_fail_fast = environs["spring.cloud.config.fail-fast"]
