...
def prepare_to_log(trigger_date: datetime, log_id: int):
    from schedule.utils.log.LogFileHolder import LogFileHolder
    __logFileHolder = LogFileHolder(_make_log_file_name(trigger_date, log_id)).open()
    # __context_holder.key = _make_log_file_name(trigger_date, log_id)


def end_log():
    if __logFileHolder is not None:
        __logFileHolder.close()
    # __context_holder.key = None


def get_log_file_name() -> str:
    return __logFileHolder.current.filePath
    # return __context_holder.key


def get_log_file_path(trigger_date: datetime, log_id: int) -> str:
    return os.path.join(get_log_path(), trigger_date.strftime("%Y-%m-%d"), (str(log_id) if log_id is not None else "0")+".log")


def _make_log_file_name(trigger_date: datetime, log_id: int):
...
def read_log(trigger_date: datetime, log_id: int, from_line_num: int, to_line_num: int)->Munch:
    """
    读取文件内容
    :param trigger_date:
    :param log_id:
    ...
    if not os.path.exists(log_file_path):
        ...
        munch.content = f"读取日志失败,日志文件:{log_file_path} 不存在"
        munch.end = True
        return munch
