...
from sdhpy.utils.utils import get_dataset_and_resource_id
...
def get_pandas_dataframe(**kwargs):
    ...
    if not tokens:
        if store_apikey:
            tokens = get_store_user(store_apikey)["tokens"]
        else:
            raise ValueError("Please provide eihter a (list of) token(s) or SDH store API Key")

    # Dataset
    dataset, resource_id = get_dataset_and_resource_id(store_apikey, resource_url=resource_url, db_and_tbl=db_and_tbl,
                                                       dataset=dataset, dataset_id=dataset_id, resource_id=resource_id)
    if not dataset:
        raise DatasetNotProvidedError("Dataset was not provied. Please provide it either using the URL, table name (database_name.table_name), Dataset ID and Resource ID or Dataset object and Resource ID")
    ds = PandasDataset(tokens, dataset, resource_id)
    return ds
...
class SdhPandas(object):
    def __init__(self, **kwargs):
        ...
        self._store_apikey = kwargs.get("store_apikey", None)
