...
function DjangoTable(dom_element) {
    ...
    this.modal = new DjangoTableModal(this.css_prefix + '__modal', this.get_elem('modal'));
}
...
DjangoTable.prototype.draw_rows = function(rows) {
    ...
    if (rows.length == 0) {
        var text = 'cannot find any data';
        tbody.appendChild(document.createTextNode(text));
    }
...
DjangoTable.prototype.register_events = function() {
    ...
    this.dom_element.querySelector('tbody').addEventListener('change', function(event) {
        if (event.target.classList.contains(self.css_prefix + '__action')) {
            var select = event.target;
            var option = select.options[select.selectedIndex];
            option.selected = false;
            var pks = JSON.stringify([Number(select.dataset.item_pk)]);
            self.open_action(option.value, option.innerHTML, pks);
        }
    });
}

DjangoTable.prototype.open_action = function(action_id, action_name, pks){
    var self = this;
    ...
    fetch(url, {method: 'get'}).then(function(response) {
        ...
        self.render_action(response, action_id, pks);
    }).catch(err => console.log(err));
}

DjangoTable.prototype.render_action = function(html, action_id, pks) {
    var form = document.createElement('form');
    form.innerHTML = html;
    this.modal.set_body(form);
    this.modal.set_footer('');
    this.modal.add_button('Save', () => this.save_action(action_id, pks));
    this.modal.add_button('Abort', () => this.modal.close(), 'secondary');
}

DjangoTable.prototype.save_action = function(action_id, pks) {
    var self = this;
    ...
    fetch(self.urls.action, {
        ...
        if (content) {
            self.render_action(content, action_id, pks);
        } else {
            self.modal.close();
        }
        self.get_data();
...
function DjangoTableModal(css_prefix, dom_element) {
    var self = this;
