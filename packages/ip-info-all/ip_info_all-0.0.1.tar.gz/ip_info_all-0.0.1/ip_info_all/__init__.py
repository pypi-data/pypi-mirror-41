...
class IPInfo(object):
    ...
    def all_ipv4_ip_info_dict(self):
        ...
        for item, info in  self.ip_info_list:
            ...
            if ipv4_info:
                info_dict[item] = self.info_tuple(addr=ipv4_info[0]["addr"], netmask=ipv4_info[0]["netmask"],broadcast=ipv4_info[0]["broadcast"])
        return info_dict
    ...
    def all_ipv6_ip_info_dict(self):
        ...
        for item, info in  self.ip_info_list:
            ...
            if ipv6_info:
                info_dict[item] = self.info_tuple(addr=ipv6_info[0]["addr"], netmask=ipv6_info[0]["netmask"],broadcast=ipv6_info[0]["broadcast"])
        return info_dict

    def get_ipv6_ip_by_encap(self, encap_name, param="ip"):
