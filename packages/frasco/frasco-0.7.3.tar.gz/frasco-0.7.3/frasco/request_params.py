...
from .utils import unknown_value, FlagContextStack
    ...
    def process(self, request_data=None):
        try:
            ...
            if self.required and self.default is unknown_value:
                current_app.logger.debug('Missing required parameter: %s' % ", ".join(self.names))
                abort(400)
        ...
        if not self.validate(*values):
            current_app.logger.debug('Invalid parameter: %s' % ", ".join(self.names))
            abort(400)
...
auto_inject_request_params = FlagContextStack()
...
def wrap_request_param_func(func):
    ...
    def wrapper(*args, **kwargs):
        if auto_inject_request_params.active() and has_request_context():
