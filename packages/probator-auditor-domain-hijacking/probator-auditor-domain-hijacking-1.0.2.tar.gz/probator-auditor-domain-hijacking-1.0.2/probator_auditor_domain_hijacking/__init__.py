...
from probator.constants import RGX_BUCKET, RGX_BUCKET_WEBSITE, RGX_INSTANCE_DNS, ConfigOption
from probator.database import db
from probator.plugins import BaseAuditor
from probator.plugins.types.issues import DomainHijackIssue
from probator.plugins.types.resources import EC2Instance, S3Bucket, CloudFrontDist, DNSZone, BeanStalk
        ...
        for name in [x.strip('.').lower() for x in record.value]:
            if name not in self.beanstalks:
                if dns_record_exists(name):
                    issues.append({
                        'key': record.name,
                        'value': 'Elastic Beanstalk {} exist but not found on any known account. Referenced by {}/{} on {})'.format(
                            name,
                            zone.name,
                            record.name,
                            zone.source
                    ...
                    issues.append({
                        'key': record.name,
                        'value': 'Elastic Beanstalk {} doesnt exist, takeover possible. Referenced by {}/{} on {})'.format(
                            name,
                            zone.name,
                            record.name,
                            zone.source
    ...
    def audit(self, record, zone):
        ...
        if type(record.value) in (tuple, list):
            for name in [x.lower() for x in record.value]:
                ...
                bucket_name, region = parse_bucket_info(name)
            ...
            bucket_name, region = parse_bucket_info(record.value)
    ...
    def audit(self, record, zone):
        ...
        if name not in self.buckets:
            return [
                {
                    'key': record.name,
                    'value': 'Assumed S3Bucket {} (ALIAS w/o endpoint) doesnt exist on any known account. Referenced by {}/{} on {}'.format(
                        name,
                        zone.name,
                        record.name,
                        zone.source
        ...
        if record.type.lower() in ('cname', 'alias'):
            for name in [x.lower() for x in record.value]:
                ...
                if ip not in self.ec2_public_ips:
                    error = 'EC2 Instance with public DNS {} does not exist on any known account. Referenced by {}/{} on {}'.format(
                        name,
                        zone.name,
                        record.name,
                        zone.source
