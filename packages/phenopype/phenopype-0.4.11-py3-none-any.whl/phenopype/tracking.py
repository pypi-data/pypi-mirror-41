...
class fish_module(object):
    def __init__(self, frame, fgmask, shadows,blur_kern, blur_thresh, min_length):
...
class video_info(object):
    def __init__(self, video):      
        ...
        length = str(int(( nframes / fps)/60)).zfill(2) + ":" +str(int((((nframes / fps)/60)-int((nframes / fps)/60))*60)).zfill(2)
