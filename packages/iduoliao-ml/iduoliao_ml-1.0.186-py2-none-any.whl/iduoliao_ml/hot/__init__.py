...
def updateHotList(subjectHistory, worksHistory, oriWorksDict, startTime, config):
	...
	newHits = es.searchStatisticsDataByBody("works_actions", {"query": {"bool": {"must": [{"range": {"createTime": {"gte": startTime, "lt": endTime}}}, {"terms": {"action": [1, 2, 3, 5, 12, 13, 14, 15, 16, 17]}}]}}, "size": 10000})
	for hit in newHits:
		action = hit['_source']
		vid = action['vid']
	...
	worksHots = calHotV2(subjectHistory, worksHistory, worksDict.values(), 0, dbConfig['aidWeights'], dbConfig['vidWeights'])
	...
	ilog.info('update works hot: ' + ('success ' if es.updateStatisticsData('works_hot_simulation', worksHots) else 'fail ') + str(time.time() - start))
