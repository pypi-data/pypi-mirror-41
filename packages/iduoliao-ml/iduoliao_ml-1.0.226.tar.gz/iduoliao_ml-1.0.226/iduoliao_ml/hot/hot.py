...
def calWorksScore(worksList, dauFactor=10):
	...
	for works in worksList:
		resShowTrust = min(works.resShow, Trust['resShow']) / Trust['resShow']
		playTrust = min(works.play, Trust['play']) / Trust['play']
		timeCountTrust = min(works.timeCount, Trust['timeCount']) / Trust['timeCount']
		...
		longPlayRatio = 0 if works.timeCount <= 0 and works.duration <= 0 else works.timeTotal * 1.0 / works.timeCount / works.duration * getPlayCompleteWeight(works.duration) * timeCountTrust
		subRatio = 0 if works.play <= 0 else works.sub * 1.0 / works.play * playTrust
		worksArgs.append([conv, likeRatio, shareRatio, longPlayRatio, subRatio])
	worksArgsMat = mat(worksArgs)
	worksArgsMat[worksArgsMat < 0] = 0
	worksMat = imatrix.inOneDivOffStray(worksArgsMat)
...
def calHotV2(subjectHistory, worksHistory, worksList, dauFactor, aidWeights, vidWeights):
	...
	for index, works in enumerate(worksList):
		vid = works.vid
		#aid = works.aid
		#actionHot = works.score * 0.9 + (worksHistory[vid]['score'] if worksHistory.has_key(vid) else 0) * 0.1
		...
		hotData['isNew'] = works.publishTime < 72 and works.resShow < 200
		hotList[vid] = hotData
