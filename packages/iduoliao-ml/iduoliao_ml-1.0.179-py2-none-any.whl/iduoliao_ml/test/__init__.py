...
def hotGrow():
	...
	for vid in ["8133878fd33659cd57fac322e6e08e53", "b9ee6b64790042a3de02b430edd359c1", "2cdef127c294230a2d86438a79c4db3f", "54423e3437395068981e503ec646e682", "c9c2ad4ab7c4c3680e7d0ef5b0c909c9", "ab68fbda808b6c5ab1c84e43c3a1aedc", "a39f8db3de774426e5500b468adcd3b0"]:
		for i in range(0, 200):
			endTime = 1547568000 + i * 60 * 60
			print(vid, endTime)
			works = dataDispose.createWorks(vid)
			works.updateActions(endTime)
			data = works.toSaveEsData()
			data['conv'] = 0 if works.resShow < 200 else works.resShowPlay * 1.0 / works.resShow
			data['likeRatio'] = 0 if works.play < 20 else works.like * 1.0 / works.play
			data['shareRatio'] = 0 if works.play < 20 else works.share * 1.0 / works.play
			data['longPlayRatio'] = 0 if works.play < 20 else works.longPlay * 1.0 / works.play
			data['subRatio'] = 0 if works.play < 20 else works.sub * 1.0 / works.play
