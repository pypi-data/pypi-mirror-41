...
def recordEsToRedis():
	...
	while True:
		...
		for nextUid in nextUids:
			...
			hits = es.scrollSearch('statistics_works_hot_read', 'recommendviewrecordsearch', {"query":{"bool":{"must":[{"range":{"time":{"lte":1548644400000}}},{"term":{"uid":{"value":nextUid}}}]}},"size":10000})
			for hit in hits:
				i = es.getIntByVid(hit['_parent'])
				if i == None:
					print(i, hit['_parent'], hit)
