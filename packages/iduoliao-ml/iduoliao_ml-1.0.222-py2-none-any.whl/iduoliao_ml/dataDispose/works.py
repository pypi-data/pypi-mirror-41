...
class Works(object):
	...
	def toSaveEsData(self):
		return {"vid": self.vid, "score": self.score, "time": self.endTime, "aid": self.aid, "title": self.title, "play": self.play, "share": self.share, "sub": self.sub, "like": self.like, "longPlay": self.longPlay, "resShow": self.resShow, "resShowPlay": self.resShowPlay, "videourl": self.videourl, "timeCount": self.timeCount, "timeTotal": self.timeTotal}
	...
	def updateActions(self, endTime, startTime=0, needCreateLimit=False):
		...
		play, share, like, sub, longPlay, resShow, resShowPlay, timeCount, timeTotal = self.getDataByVid(self.vid, startTime, endTime, needCreateLimit)
		self.play = play
		self.share = share
		self.like = like
		self.sub = sub
	...
	def getDataByVid(self, vid, startTime, endTime, needCreateLimit):
		...
		body = baseBody
		body['query']['bool']['must'].append({"terms": {"action": [1, 2, 3, 5, 12, 13, 14, 15, 16, 17]}})
		...
		return counts[1], counts[12] + counts[13], counts[16] - counts[17], counts[14] - counts[15], counts[2], counts[3], counts[5], timeCount, timeTotal
