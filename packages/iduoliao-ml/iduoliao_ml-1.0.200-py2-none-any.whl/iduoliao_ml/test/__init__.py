...
def test():
	print(es.getAllUids(0))
	#recordEsToRedis()
	#hotGrow()

def recordEsToRedis():
	currentUid = readEndUid()
	nextUid = es.scrollSearch('statistics_works_hot_read', 'recommendviewrecordsearch', {"query":{"bool":{"must":[{"range":{"time":{"lte":1548644400000}}},{"range":{"uid":{"gt":currentUid}}}]}},"size":1,"sort":[{"uid":{"order":"asc"}}]})[0]['_source']['uid']
	existInts = es.getIntsByUid(nextUid)
	hitInts = []
	for hit in es.scrollSearch('statistics_works_hot_read', 'recommendviewrecordsearch', {"query":{"bool":{"must":[{"range":{"time":{"lte":1548644400000}}},{"term":{"uid":{"value":nextUid}}}]}},"size":10000}):
		i = es.getIntByVid(hit['_parent'])
		if i not in existInts:
			hitInts.append(i)
	print(hitInts)
