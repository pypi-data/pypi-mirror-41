...
def calHotV2(subjectHistory, worksHistory, worksList, dauFactor, aidWeights, vidWeights):
	...
	for index, works in enumerate(worksList):
		...
		if vidWeights.has_key(vid):
			...
			weight = 1
		hotList[vid] = {'vid': vid, 'aid': aid, 'play': (worksHistory[vid]['play'] if worksHistory.has_key(vid) else 0) + works.play, 'title': works.title, 'baseHot': baseHotMat[index, 0], 'actionHot': actionHot, 'totalHot': totalHot, 'hot': totalHot * getDecayRatio(works.publishTime) * weight, 'isNew': works.publishTime < 72}
