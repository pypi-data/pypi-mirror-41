...
def getIntByVid(vid):
	global vidDict
	if vidDict.has_key(vid):
		return vidDict[vid]
	else:
		i = getIntByVidInRedis(vid)
		vidDict[vid] = i
		return i

recommendRedisPool = redis.ConnectionPool(host='r-bp160ebab6e97594.redis.rds.aliyuncs.com', port='6379', password='flrjovlOJEROIJ324')

def getIntByVidInRedis(vid):
	recommendRedis = redis.Redis(connection_pool=recommendRedisPool, db=0)
	#return recommendRedis.get(vid)
	print(recommendRedis.get(vid))
	#print(recommendRedis.lrange('recommendcache:videouserrecord:uid;177701952', 0, sys.maxint))
	        ...
	        },
	        {
	          "range": {
	            "create_time": {
	              ...
	              "lte": timeToEsTime(min(lastPersonasTime + 12 * 60 * 60, int(time.time()) - 90))
	            }
	          }
	        }
	      ]
...
def getEsclient(fullIndex=None):
	return testEsclient if isTest else relEsclient if fullIndex == 'works_video_read' else localEsclient if fullIndex in ['statistics_works_hot_grow_write', 'statistics_works_hot_simulation_write', 'statistics_works_history_read', 'statistics_works_history_write'] else dataEsclient
