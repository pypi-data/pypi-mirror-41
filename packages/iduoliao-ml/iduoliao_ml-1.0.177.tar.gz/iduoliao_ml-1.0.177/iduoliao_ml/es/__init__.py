...
def updateStatisticsDocResolve(index, doc):
	...
	success = esUpdateStatisticsDocResolve(getEsclient(), index, doc)
...
def getEsclient(fullIndex=None):
	return testEsclient if isTest else relEsclient if fullIndex == 'works_video_read' else localEsclient if fullIndex == 'statistics_works_hot_grow_write' else dataEsclient
