...
def getWorksHistory():
	worksHistory = {}
	for hit in es.searchStatisticsData('works_history_simulation'):
		history = hit['_source']
...
def getWorksList():
	...
	for hit in es.scrollSearch('works_video_read', 'video', {"query": {"bool": {"must": [{"term": {"state": {"value": 0}}}, {"term": {"authorstate": {"value": 0}}}, {"range": {"publishtime": {"gte": getDayBeforeTime(3)}}}]}}, "size":1000}):
...
def updateHistoryActions():
	...
	zeroTimeMonthBefore = zeroTime - 60 * 24 * 60 * 60
	...
	updateWorksHistoryActions(worksList, zeroTime + 15 * 60 * 60, zeroTimeMonthBefore, True)

	worksHistory = {}
	for works in worksList:
		worksHistory[works.vid] = works.toSaveEsData()

	return es.coverStatisticsDataReal('works_history_simulation', worksHistory)
...
9: 推荐分享
'''
normalHotMap = {'2': 4, '3': 5, '4': 9, '10': 6, '11': 7}

def addHotAction(hotActions, _id, createTime, actionId, _time, uid, items):
	if actionId == '9' and items[4] == '3':
		vid  = items[3]
		action = 1
		value = 1
	elif actionId == '7' and items[1] == 'pages/common/videoplay/videoplay' and items[3] == '1' and len(items) >= 6 and items[5] == '4':
		vid = items[4]
		action = 2
		value = 1
	elif actionId == '1' and len(items) >= 8 and not bool(items[7]):
		vid = items[4]
		action = 3
		value = 1
	elif normalHotMap.has_key(actionId) and len(items) >= 6 and items[5] == '1':
		vid = items[4 if actionId in ['10', '11'] else 3]
		action = normalHotMap[actionId]
		value = 1
	elif actionId == '6' and len(items) >= 8 and items[7] == '1':
		vid = items[3]
		action = 8
		value = items[4]
	elif actionId == '5' and len(items) >= 6 and items[3] == '0' and items[5] == '1':
		vid = items[4]
		action = 9
		value = 1
	else:
