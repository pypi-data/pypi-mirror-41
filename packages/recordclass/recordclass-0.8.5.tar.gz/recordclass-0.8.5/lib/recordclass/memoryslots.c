...
#define b ((PyMemorySlotsObject *)bb)
    ...
    np = (PyMemorySlotsObject *) PyMemorySlots_New(size);
    if (np == NULL) {
        return NULL;
    }
...
#define aa ((PyMemorySlotsObject*)a)
    ...
    if (Py_TYPE(a) == &PyMemorySlots_Type)
        np = (PyMemorySlotsObject*)PyMemorySlots_New(len);
    else    
...
memoryslots_repeat(PyMemorySlotsObject *a, Py_ssize_t n)
{
    ...
    if (n > PY_SSIZE_T_MAX / Py_SIZE(a))
        return PyErr_NoMemory();
    size = Py_SIZE(a);
    np = (PyMemorySlotsObject *) PyMemorySlots_New(Py_SIZE(a) * n);
    if (np == NULL)
        return NULL;
...
// static Py_hash_t
// memoryslots_hash(PyMemorySlotsObject *v)
// {
...
//     if (PyType_Ready(&PyMemorySlotsRO_Type) < 0)
//          Py_FatalError("Can't initialize memoryslotsro type");
    
    Py_INCREF(&PyMemorySlots_Type);
    PyModule_AddObject(m, "memoryslots", (PyObject *)&PyMemorySlots_Type);

//     Py_INCREF(&PyMemorySlotsRO_Type);
//     PyModule_AddObject(m, "memoryslotsro", (PyObject *)&PyMemorySlotsRO_Type);
