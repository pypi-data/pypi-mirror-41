...
def recordclass(typename, fields, 
                rename=False, defaults=None, readonly=False, hashable=True, module=None):
...
"""
    ...
    def _make(_cls, iterable):
        ...
        if len(ob) != num_fields:
            raise TypeError('Expected %s arguments, got %s' % (num_fields, len(ob)))
        return ob
    ...
    for index, name in enumerate(field_names):
        try:
            itemgetset_object = cache[index]
        except KeyError:
            itemgetset_object = itemgetset(index)
            #doc = 'Alias for field number ' + str(index)
            cache[index] = itemgetset_object
        class_namespace[name] = itemgetset_object
    ...
    _result = type(typename, (baseclass,), class_namespace)
