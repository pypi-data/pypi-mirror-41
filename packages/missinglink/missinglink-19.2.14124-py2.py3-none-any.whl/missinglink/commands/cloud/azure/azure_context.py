...
class AzureContext(BackendContext, CloudConnector):
    ...
    def _assign_role_key_vault_perms(self):
        ...
        storage_accs = storage_client.storage_accounts.list()
        storage_resource_groups = {self.storage_rg_name}
        for storage_account in storage_accs:
            rg_name = storage_account.id.split('/')[4]
            storage_resource_groups.add(rg_name)
        for rg_name in storage_resource_groups:
            self._grant_access_to_storage_rg(role_identity, rg_name)

        click.echo('Done')

    def _grant_access_to_storage_rg(self, role_identity, rg_name):
        auth_client = get_client_from_cli_profile(AuthorizationManagementClient)
        roles = list(auth_client.role_assignments.list_for_resource_group(rg_name,
                                                                          filter='principalId eq \'%s\'' % role_identity.principal_id))
        scope = '/subscriptions/%s/resourceGroups/%s' % (auth_client.config.subscription_id, rg_name)
        ...
        self._create_role_if_missing(auth_client, role_definition_id, roles, role_identity.principal_id, scope)
