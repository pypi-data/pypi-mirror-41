...
class HideAzure(click.Group):
    hidden_cmds = {'azure', 'restore_aws_template', 'restore_azure_template'}

    def list_commands(self, ctx):
        commands = super(HideAzure, self).list_commands(ctx)
        return [c for c in commands if c not in self.hidden_cmds]
...
@click.option('--capacity', required=False, default=1)
def install_rm(ctx, org, ssh_key_path, force, resource_token, capacity=None, **kwargs):
    ...
    docker_tools.validate_local_config(org=org, force=force, ssh_key_path=ssh_key_path, token=resource_token, capacity=capacity)
...
def apply_azure_template(
    ...
    run_docker(ctx, ml_backend, ml_data, prefix, rm_config_volume, rm_container_name, rm_manager_image,
               rm_socket_server, ssh_key, token, env={'ML_INSTANCE_ROLE': role_id})


def run_docker(ctx, ml_backend, ml_data, prefix, rm_config_volume, rm_container_name, rm_manager_image,
               rm_socket_server, ssh_key, token, env=None):
    ...
    docker_tools.setup_rms_volume(ssh_key, token, prefix=prefix, ml_data=ml_data, env=env, force=True)
...
@click.argument('name', required=True, nargs=1)
@click.pass_context
def manage_queue(ctx, org=None, name=None):
