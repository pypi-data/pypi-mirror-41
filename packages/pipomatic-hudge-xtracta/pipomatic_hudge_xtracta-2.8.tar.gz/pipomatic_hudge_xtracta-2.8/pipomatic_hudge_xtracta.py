...
def email_to_dict(filename):
    json_stem = filename.name.split("-")[0]
    json_file = (filename.parent / json_stem).with_suffix(".json")
    if json_file.exists():
        email_json = json.loads(json_file.read_text())
...
def get_email_xml(email_dict):
    fields = {
        "field": [
            {"@name": "email_date", "#text": email_dict["Subject"]},
