...
insurer_line_workflow = os.getenv("INSURER_LINE_ID")

pw = os.getenv("TWINE_PASSWORD")
...
def build_full_output(doc_list, po_lines="", include_lines=False):
    no_po = p / "no_po"
    no_po.mkdir(parents=True, exist_ok=True)
    op = p / "output"
    ...
    for doc in doc_list:
        ...
        if document:
            ...
            if not output["header"]["po_number"]:
                ...
                filename_stem = f"{output['header']['po_number']}_{output['header']['invoice_number']}"
                write_json_simple((no_po / filename_stem).with_suffix(".json"), output)
                response = requests.get(output["document_url"])
                (no_po / filename_stem).with_suffix(".pdf").write_bytes(
                    response.content
                )
            elif include_lines:
