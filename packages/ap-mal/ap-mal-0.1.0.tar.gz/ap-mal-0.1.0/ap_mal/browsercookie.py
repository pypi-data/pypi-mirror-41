# This is a modified version of the original browsercookie package
# by Richard Penman. Unfortunately, the original package depends
# on pycrypto, which a) does not install properly on Windows and
# b) is ancient and unmaintained. There is a drop-in replacement,
# pycryptodome, however how dependencies are resolved for most tools
# prevent a nice replacement of the dependency.
# https://bitbucket.org/richardpenman/browsercookie/issues/31/
#
                ...
                if 'json_data' in locals():
                    ...
                    for window in json_data.get('windows', []):
                        for cookie in window.get('cookies', []):
                            yield create_cookie(cookie.get('host', ''), cookie.get('path', ''),
                                                False, expires, cookie.get('name', ''),
                                                cookie.get('value', ''))
                else:
...
def create_cookie(host, path, secure, expires, name, value):
    ...
    return cookielib.Cookie(0, name, value, None, False, host, host.startswith('.'),
                            host.startswith('.'), path, True, secure, expires, False, None, None,
                            {})
