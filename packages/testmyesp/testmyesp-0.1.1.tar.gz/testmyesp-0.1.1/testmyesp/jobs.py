...
def run_job(job):
    job.logger.debug('flashing started')
    job.state = constants.JOB_STATE_FLASHING
    ...
    espctl.power_on()
    espctl.reset()  # TODO this is no longer needed

    flashimages.write_flash_images(job)
...
async def exec_loop():
    while True:
        try:
            ...
            except taskqueue.TimeoutException as e:
                job.logger.error(e)
                job_error = str(e)
