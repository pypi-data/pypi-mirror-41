...
class HTTPRequestException(InstructionException):
    pass


class UnexpectedResponseStatus(InstructionException):
...
class UnexpectedResponseHeader(InstructionException):
...
class UnexpectedResponseBody(InstructionException):
...
class UnexpectedJSONResponseBody(UnexpectedResponseBody):
    def __str__(self):
...
class InvalidJSONResponseBody(InstructionException):
...
class JSONResponseBodySchemaValidationError(InstructionException):
...
class HTTPRequest(BaseInstruction):
    NAME = 'http-request'
    ...
    def execute(self):
        ...
        if not ip_address:
            raise HTTPRequestException('cannot find device IP address')
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.request(self.method, url, headers=self.headers, data=self.body) as response:
                ...
                if response.status != self.expected_status:
                    raise UnexpectedResponseStatus(self.expected_status, response.status, received_body)

                if self.expected_headers:
                    for name, value in self.expected_headers.items():
                        if response.headers.get(name) != value:
                            raise UnexpectedResponseHeader(name, value, response.headers.get(name), received_body)
    ...
    def verify_received_body(self, received_body):
        if (self.expected_body is not None) and (received_body != self.expected_body):
            raise UnexpectedResponseBody(self.expected_body, received_body)
...
class HTTPJSONRequest(HTTPRequest):
    ...
    def parse_received_body(self, body):
        try:
            ...
            raise InvalidJSONResponseBody(body)

        if body.strip() == '':  # empty body treated as null
            return None
    ...
    def verify_received_body(self, received_body):
        if (self.expected_body is not None) and (received_body != self.expected_body):
            raise UnexpectedJSONResponseBody(self.expected_body, received_body)
        ...
        if self.expected_body_schema is not None:
            try:
                ...
                raise JSONResponseBodySchemaValidationError(self.expected_body, received_body, e)

    class Schema(HTTPRequest.Schema):
