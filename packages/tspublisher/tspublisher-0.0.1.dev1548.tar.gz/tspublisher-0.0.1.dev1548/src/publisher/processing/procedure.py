...
def build_procedure_object(procedure_code):
    proc_data = get_production_csv_data('procedure')
    mod_data = get_production_csv_data('module')

    procedure = Procedure()
    procedure.code = procedure_code
    procedure.name, procedure.specialties, procedure.channel, vbs = get_procedure_details(procedure_code, proc_data)
    procedure.vbs = is_vbs(vbs)

    phases_data = get_procedure_phase_list(procedure_code, mod_data)
    sorted_phase_data = sorted(phases_data, key=lambda phase: phase["order"])
    procedure.phases = [build_phase_object(p) for p in sorted_phase_data]
...
def build_single_phase(phase_code, graphics=True, pip_graphics=True, widget_graphics=True, thumbnails=True,
    ...
    phase_data = get_phase_data(phase_code, get_production_csv_data())[0]
    _, _, _, vbs = get_procedure_details(phase_data['procedure_code'], get_production_csv_data('procedure'))
