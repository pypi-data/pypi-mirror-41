...
#thread_id,从1000开始,最小是1000
#我们让最小tid的线程,只跑一半的loop_count,从而让它先退出.
def thread_main(thread_id, thread_jobs, thread_params, thread_results):
    try: 
        ...
        #我们让最小tid的线程,只跑一半的loop_count,从而让它先退出.其它线程看见它退出了，也会很快退出.
        if thread_id == 1000:
            loop_count = int(loop_count/2)+1
        for loopc in range(loop_count):
            try:
...
def wccqcc_combasic():
    try:
        ...
        start_time = int(time.time())
        #loop_count要设的稍微大一点，因为我们依赖快的线程来决定提前结束,如果太小,很多线程都要等待某个线程.
        thread_params = {"loop_count":200}
        """
        获取数据库连接
        """
        env_dict = os.environ
