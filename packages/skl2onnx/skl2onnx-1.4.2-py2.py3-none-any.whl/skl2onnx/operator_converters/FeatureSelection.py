...
def convert_sklearn_feature_selection(scope, operator, container):
    ...
    if index.any():
        ...
        container.add_node('ArrayFeatureExtractor', [operator.inputs[0].full_name, column_indices_name],
                           output_name, name=scope.get_unique_operator_name('ArrayFeatureExtractor'),
                           op_domain='ai.onnx.ml')
    else:
        container.add_node('ConstantOfShape', operator.inputs[0].full_name, output_name,
                           op_version=9)

    apply_cast(scope, output_name, operator.outputs[0].full_name, container,
