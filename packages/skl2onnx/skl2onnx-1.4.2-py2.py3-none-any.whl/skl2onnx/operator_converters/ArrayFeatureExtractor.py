...
def convert_sklearn_array_feature_extractor(scope, operator, container):
    ...
    for i, ind in enumerate(operator.column_indices):
        if not isinstance(ind, int):
            raise RuntimeError(("Column {0}:'{1}' indices must be specified as integers. "
                                "This error may happen when column names are used to "
                                "define a ColumnTransformer. Column name in input data "
                                "do not necessarily match input variables defined for "
                                "the ONNX model.").format(i, ind))
