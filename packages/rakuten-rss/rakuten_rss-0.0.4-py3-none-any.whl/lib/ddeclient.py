...
class DDE(object):
    ...
    Initialize = get_winfunc("user32", "DdeInitializeW", UINT,
                             (LPDWORD, DDECALLBACK, DWORD, DWORD))  # Unicode version of DDE initialize
    ...
    QueryString = get_winfunc("user32", "DdeQueryStringA", DWORD,
                              (DWORD, HSZ, LPSTR, DWORD, c_int))  # ANSI version of QueryString
...
class DDEClient(object):
    ...
    def execute(self, command):
        ...
        hDdeData = DDE.ClientTransaction(pData, cbData, self._hConv, HSZ(), CF_TEXT, XTYP_EXECUTE, TIMEOUT_ASYNC,
                                         LPDWORD())
        if not hDdeData:
    ...
    def request(self, item, timeout=5000):
        ...
        pdwResult = DWORD(0)
        hDdeData = DDE.ClientTransaction(LPBYTE(), 0, self._hConv, hszItem, CF_TEXT, XTYP_REQUEST, timeout,
                                         byref(pdwResult))
        DDE.FreeStringHandle(self._idInst, hszItem)
        if not hDdeData:
        ...
        if timeout != TIMEOUT_ASYNC:
            ...
            pData = None
        DDE.FreeDataHandle(hDdeData)
        DDE.UnaccessData(hDdeData)  # test fix( by zaq)
    ...
    def poke(self, item, data, timeout=5000):
        ...
        hDdeData = DDE.ClientTransaction(pData, cbData, self._hConv, hszItem, CF_TEXT, XTYP_POKE, timeout,
                                         byref(pdwResult))
        DDE.FreeStringHandle(self._idInst, hszItem)
