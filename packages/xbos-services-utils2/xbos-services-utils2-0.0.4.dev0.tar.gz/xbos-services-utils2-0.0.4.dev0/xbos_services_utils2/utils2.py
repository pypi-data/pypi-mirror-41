...
import os
XBOS_SERVICES_UTILS_CONFIG_PATH = os.environ["XBOS_SERVICES_UTILS_CONFIG_PATH"]
...
def get_zones(bldg):
    ...
    path = XBOS_SERVICES_UTILS_CONFIG_PATH + "/Buildings" + "/all_zones.yml"
...
def get_buildings():
    ...
    path = XBOS_SERVICES_UTILS_CONFIG_PATH + "/Buildings/building_names"
    with open(path, "r") as f:
        data = yaml.load(f)
    return data
...
def smart_resample(data, start, end, window, method):
    """
    Groups and data into intervals according to the method used.
