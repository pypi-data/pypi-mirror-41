...
class RamseyModel:
    ...
    def simulate(self, c0=None):
        ...
        for t in range(self.T):
            ...
            # iv. disconvergence
            if (
                self.sim["c"][t] > self.c_upper_fac * self.c_ss
                or self.sim["c"][t] < self.c_lower_fac * self.c_ss
            ):
                self.sim["k"][t:] = np.nan
                self.sim["c"][t:] = np.nan
                break
    ...
    def optimal_initial_consumption_level(self):
        ...
        for t in range(1, self.T):  # t = 1,2,3,...
            ...
            c_fac[t] = (
                c_fac[t - 1]
                * ((1 + r_path[t]) * self.beta) ** (1 / self.sigma)
                / (1 + r_path[t])
            )
            R_fac /= 1 + r_path[t]
...
def phase_diagram(beta, sigma, delta, alpha, k0):
    ...
    # a. solve model
    model = RamseyModel(
        beta=beta, sigma=sigma, delta=delta, alpha=alpha, k0=k0, do_print=False
    )
