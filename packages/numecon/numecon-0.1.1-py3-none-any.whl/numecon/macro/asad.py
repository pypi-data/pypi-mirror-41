...
def simulate(
    a=0.4, gamma=0.1, phi=0.9, delta=0.8, omega=0.15, sigma_x=1, sigma_c=0.2, T=100
):
    ...
    widgets.interact(
        ...
        sigma_x=widgets.FloatSlider(
            description="$\\sigma_{x}$", min=0.00, max=2.0, step=0.01, value=sigma_x
        ),
        sigma_c=widgets.FloatSlider(
            description="$\\sigma_{c}$", min=0.00, max=2.0, step=0.01, value=sigma_c
        ),
        T=widgets.fixed(T),
    )
...
def simulate_(a, phi, gamma, delta, omega, sigma_x, sigma_c, T):
    ...
    # b. function
    y_hat_func = (
        lambda y_hat_lag, z, z_lag, s, s_lag: b * y_hat_lag
        + beta * (z - z_lag)
        - a * beta * s
        + a * beta * phi * s_lag
    )
    pi_hat_func = (
        lambda pi_lag, z, z_lag, s, s_lag: b * pi_lag
        + beta * gamma * z
        - beta * phi * gamma * z_lag
        + beta * s
        - beta * phi * s_lag
    )
