...
class EdgeworthModel:
    ...
    def pareto_figure(self, xAs):
        ...
        # box
        ax_A.plot(
            [0, self.e1bar, self.e1bar, 0, 0],
            [0, 0, self.e2bar, self.e2bar, 0],
            lw="2",
            color="black",
        )
    ...
    def walras_figure(self, xAs):
        ...
        self.consumer_A.plot_indifference_curves(
            ax_A, u0s=[self.consumer_A.u_ast], color=colors[0]
        )
        ...
        self.consumer_B.plot_indifference_curves(
            ax_B, u0s=[self.consumer_B.u_ast], color=colors[1]
        )
...
def _interactive_walras(p1, omega_1, omega_2, alpha_A, beta_A, alpha_B, beta_B, par):
    ...
    model = EdgeworthModel(
        consumer_A,
        consumer_B,
        p1=p1,
        p2=par["p2"],
        eA=(omega_1, omega_2),
        eB=(1 - omega_1, 1 - omega_2),
    )
...
def interactive_walras(
    ...
    widgets.interact(
        ...
        omega_1=widgets.FloatSlider(
            description="$\\omega_1$",
            min=0.05,
            max=0.99,
            step=0.05,
            value=kwargs["omega_1"],
        ),
        omega_2=widgets.FloatSlider(
            ...
            max=0.99,
            step=0.05,
            value=kwargs["omega_2"],
        ),
        alpha_A=widgets.FloatSlider(
            description="$\\alpha^A$",
            min=kwargs["alpha_A_min"],
            max=kwargs["alpha_A_max"],
            step=kwargs["alpha_A_step"],
            value=kwargs["alpha_A"],
        ),
        beta_A=widgets.FloatSlider(
            description="$\\beta^A$",
            min=kwargs["beta_A_min"],
            max=kwargs["beta_A_max"],
            step=kwargs["beta_A_step"],
            value=kwargs["beta_A"],
        ),
        alpha_B=widgets.FloatSlider(
            description="$\\alpha^B$",
            min=kwargs["alpha_B_min"],
            max=kwargs["alpha_B_max"],
            step=kwargs["alpha_B_step"],
            value=kwargs["alpha_B"],
        ),
        beta_B=widgets.FloatSlider(
            description="$\\beta^B$",
            min=kwargs["beta_B_min"],
            max=kwargs["beta_B_max"],
            step=kwargs["beta_B_step"],
            value=kwargs["beta_B"],
        ),
