...
class ConsumerClass():
    ...
    def calculations(self):
        ...
        if self.preferences == 'cobb_douglas':
            ...
            self.utility_func = lambda x1,x2,alpha,beta: (alpha*x1**(-beta)+(1-alpha)*x2**(-beta))**(-1/beta) if beta != 0 else x1**alpha*x2**(1-alpha)
            self.indiff_func_x1 = lambda u0,x1,alpha,beta: ((u0**(-beta)-alpha*x1**(-beta))/(1-alpha))**(-1/beta) if beta != 0 else (u0/x1**alpha)**(1/(1-alpha))
            self.indiff_func_x2 = lambda u0,x2,alpha,beta: ((u0**(-beta)-(1-alpha)*x2**(-beta))/alpha)**(-1/beta) if beta != 0 else (u0/x2*(1-alpha))**(1/alpha)
            ...
            self.utility_func = lambda x1,x2,alpha,beta: np.fmin(alpha*x1,beta*x2) 
            self.indifference_curves_method = self.preferences           
            self.cost_min_method = 'numerical'
        elif self.preferences == 'quasi_log':
            self.utility_func = lambda x1,x2,alpha,beta: alpha*np.log(x1)+x2*beta            
            ...
            self.utility_func = lambda x1,x2,alpha,beta: alpha*np.sqrt(x1)+x2*beta            
            self.indiff_func_x1 = lambda u0,x1,alpha,beta: (u0-alpha*np.sqrt(x1))/beta
            self.indiff_func_x2 = lambda u0,x2,alpha,beta: ((np.fmax(u0-x2*beta,0))/alpha)**2
            self.cost_min_method = 'numerical'
        elif self.preferences == 'concave':
            self.utility_func = lambda x1,x2,alpha,beta: alpha*x1**2+beta*x2**2            
    ...
    def minimize_cost(self,u,numerical=False,**kwargs):
        ...
        if self.cost_min_method == 'numerical' or numerical:
            ...
            def target_2d(x):
                ...
                return self.p1*x1+self.p2*x2 + 1000*udiff + 1000*np.max([-x[0],-x[1],0])
    ...
    def plot_budgetline(self,ax,**kwargs):
        ...
        if self.budgetsettype == 'kinked':
            ...
            x = [0,self.kink_point,self.kink_point+(self.I-self.p1*self.kink_point)/(self.p1-self.kink_slope)]
    ...
    def plot_budgetset(self,ax,**kwargs):
        ...
        if self.budgetsettype == 'kinked':
            ...
            x = [0,0,self.kink_point,self.kink_point+(self.I-self.p1*self.kink_point)/(self.p1-self.kink_slope)]
    ...
    def find_indifference_curve_numerical(self,u0=None):
        ...
        for x1_now in x1_x1:
            ...
            x_B,_infodict_B,ier_B,_mesg_B = optimize.fsolve(target_for_x2, self.x2max, full_output=True)        
        ...
        for x2_now in x2_x2:
            ...
            x_B,_infodict_B,ier_B,_mesg_B = optimize.fsolve(target_for_x1, self.x1max, full_output=True)        
    ...
    def find_indifference_curve(self,u0=None,numerical=False):
        ...
        if self.indifference_curves_method == 'numerical' or self.indifference_curves_method == 'saturated' or numerical:
    ...
    def plot_indifference_curves(self,ax,u0s=[],do_label=True,only_dots=False,numerical=False,**kwargs):
        ...
        for i,u0 in enumerate(u0s):
            ...
            if self.preferences == 'saturated':
                radius = np.sqrt(-u0)
                circle = plt.Circle((self.alpha,self.beta),radius,fill=False,color=colors[i],**kwargs)
    ...
    def plot_decomposition_exogenous(self,ax,p1_old,p1_new,p2=1,steps=3):
        ...
        self.plot_indifference_curves(ax,[u_ast],do_label=False,ls='--',color=colors[0])
        ...
        if steps > 1:
            ...
            self.plot_indifference_curves(ax,[u_ast_new],do_label=False,ls='--',color=colors[1])
...
def _interactive_budgetset_exogenous(p1,p2,I,par):
    ...
    consumer = ConsumerClass(p1=p1,p2=p2,I=I,budgetsettype='exogenous',x1max=par['x1max'],x2max=par['x2max'])
...
def _interactive_budgetset_endogenous(p1,p2,e1,e2,par):
    ...
    consumer = ConsumerClass(p1=p1,p2=p2,e1=e1,e2=e2,budgetsettype='endogenous',x1max=par['x1max'],x2max=par['x2max'])
    _fig,ax = consumer.figure()
...
def _interactive_budgetset_kink(p1,kink_point,kink_slope,p2,I,par):
    ...
    consumer = ConsumerClass(p1=p1,p2=p2,I=I,budgetsettype='kinked',kink_point=kink_point,kink_slope=kink_slope,
        x1max=par['x1max'],x2max=par['x2max'])
    _fig,ax = consumer.figure()

    if show_warning:
        ax.text(0.65*consumer.x1max,0.95*consumer.x2max,f'warning: $p_1-\\Delta_1 <= 0$',backgroundcolor='white')
...
def interactive_budgetset(budgetsettype,**kwargs):
    ...
    if budgetsettype == 'exogenous':
        ...
        widgets.interact(_interactive_budgetset_exogenous,        
        ...
        widgets.interact(_interactive_budgetset_endogenous,        
            p1=widgets.FloatSlider(description='$p_1$',min=0.1, max=5, step=0.05, value=2), 
            p2=widgets.FloatSlider(description='$p_2$',min=0.1, max=5, step=0.05, value=1),
        ...
        widgets.interact(_interactive_budgetset_kink,        
            p1=widgets.FloatSlider(description='$p_1$',min=0.1, max=5, step=0.05, value=1), 
            kink_point=widgets.FloatSlider(description='$\\overline{x}_1$',min=0.1, max=5, step=0.05, value=2),                   
            kink_slope=widgets.FloatSlider(description='$\\Delta_1$',min=-5, max=5, step=0.10, value=-1),
            p2=widgets.FloatSlider(description='$p_2$',min=0.1, max=5, step=0.05, value=1), 
...
def _interactive_indifference_curves(alpha,beta,par):
    ...
    # 45 degrees 
    ax.plot([0,consumer.x1max],[0,consumer.x2max],'--',color="black",zorder=1,alpha=0.1)
...
def interactive_indifference_curves(preferences='cobb_douglas',**kwargs):
    ...
    widgets.interact(_interactive_indifference_curves,
        alpha=widgets.FloatSlider(description='$\\alpha$',
            min=kwargs['alpha_min'],max=kwargs['alpha_max'],step=kwargs['alpha_step'],value=kwargs['alpha']),
        beta=widgets.FloatSlider(description='$\\beta$',
            min=kwargs['beta_min'],max=kwargs['beta_max'],step=kwargs['beta_step'],value=kwargs['beta']),
        par=widgets.fixed(kwargs),
    )
...
def interactive_utility_max(preferences='cobb_douglas',**kwargs):
    ...
    widgets.interact(_interactive_utility_max,
        p1=widgets.FloatSlider(description='$p_1$',
            min=kwargs['p1_min'], max=kwargs['p1_max'], step=kwargs['p1_step'], value=kwargs['p1']),
        p2=widgets.FloatSlider(description='$p_2$',
            min=kwargs['p2_min'], max=kwargs['p2_max'], step=kwargs['p2_step'], value=kwargs['p2']),
        I=widgets.FloatSlider(description='$I$',
            min=kwargs['I_min'], max=kwargs['I_max'], step=kwargs['I_step'], value=kwargs['I']),
        alpha=widgets.FloatSlider(description='$\\alpha$',
            min=kwargs['alpha_min'], max=kwargs['alpha_max'], step=kwargs['alpha_step'], value=kwargs['alpha']),
        beta=widgets.FloatSlider(description='$\\beta$',
            min=kwargs['beta_min'], max=kwargs['beta_max'], step=kwargs['beta_step'], value=kwargs['beta']),
...
def interactive_slutsky_exogenous(preferences='cobb_douglas',**kwargs):
    ...
    widgets.interact(_interactive_slutsky_exogenous,
        par=widgets.fixed(kwargs), 
        steps=widgets.IntSlider(description='steps',
            min=1, max=3, step=1, value=1),
        p1_old=widgets.FloatSlider(description='$p_1$',
            min=kwargs['p1_min'], max=kwargs['p1_max'], step=kwargs['p1_step'], value=kwargs['p1']),
        p1_new=widgets.FloatSlider(description='$p_1^{\\prime}$',
            min=kwargs['p1_min'], max=kwargs['p1_max'], step=kwargs['p1_step'], value=kwargs['p1_new']),        
        alpha=widgets.FloatSlider(description='$\\alpha$',
            min=kwargs['alpha_min'], max=kwargs['alpha_max'], step=kwargs['alpha_step'], value=kwargs['alpha']),
        beta=widgets.FloatSlider(description='$\\beta$',
            min=kwargs['beta_min'], max=kwargs['beta_max'], step=kwargs['beta_step'], value=kwargs['beta']))
...
def interactive_slutsky_endogenous(preferences='cobb_douglas',**kwargs):
    ...
    widgets.interact(_interactive_slutsky_endogenous,
        par=widgets.fixed(kwargs), 
        steps=widgets.IntSlider(description='steps',
        ...
        e1=widgets.FloatSlider(description='$e_1$',
            min=kwargs['e1_min'], max=kwargs['e1_max'], step=kwargs['e1_step'], value=kwargs['e1']),
        e2=widgets.FloatSlider(description='$e_2$',
            min=kwargs['e2_min'], max=kwargs['e2_max'], step=kwargs['e2_step'], value=kwargs['e2']),
        alpha=widgets.FloatSlider(description='$\\alpha$',
            min=kwargs['alpha_min'], max=kwargs['alpha_max'], step=kwargs['alpha_step'], value=kwargs['alpha']),
        beta=widgets.FloatSlider(description='$\\beta$',
            min=kwargs['beta_min'], max=kwargs['beta_max'], step=kwargs['beta_step'], value=kwargs['beta']))
