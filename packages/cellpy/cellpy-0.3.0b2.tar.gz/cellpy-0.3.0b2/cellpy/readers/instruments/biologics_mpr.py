...
from cellpy.readers.instruments.biologic_file_format import bl_dtypes, \
    hdr_dtype, mpr_label, bl_log_pos_dtype, bl_flags
...
class MprLoader(Loader):
    ...
    def _parse_mpr_log_data(self):
        ...
        # converting dates
        date_datetime = ole2datetime(
            self.mpr_log["Acquisition started on"]
        )
    ...
    def _load_mpr_data(self, filename, bad_steps):
        ...
        if not p == (len(main_data) / n_data_points):
            self.logger.info(
                ...
                "but it seems it should be %i" % (
                    p,
                    len(main_data) / n_data_points
                )
            )
        bulk = main_data
    ...
    def _create_summary_data(self):
        ...
        warnings.warn(
            "Creating summary data for biologics mpr-files"
