...
def open3d_visualize(pointcloud, color_by_class=False):
    ...
    if color_by_class == False:
        ...
        o3pc = o3.PointCloud()
        o3pc.points = o3.Vector3dVector(pointcloud[['X','Y','Z']].values)
        colors = pointcloud['class'].values.astype('float')
        ...
        rgb_classes = [colorsys.hsv_to_rgb(c, 0.8, 0.8) for c in classes]
        rgb_classes = np.array(rgb_classes)

        color_arr = np.array([rgb_classes[np.where(c==unique_classes)] for c in colors]).reshape(-1, 3)
