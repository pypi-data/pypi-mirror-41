...
class DaskDataSet(object):
	...
	def __init__(self, train=True, filelist=None, num_workers=4, blocksize=25e6, load_scene=True,
					   num_points=1000, columns=None, header=None, dtype=None, labels_weights_dir=None,
					   load_labels_weights=True, z_scalar=1., box_size=10, client=None):
	...
	def _load_model_simple(self, filepath, sep=',', header=None):

		""" Simple method to load a model from disk. Assumes models are .csv files with ',' seperator """
	...
	def _load_model(self, filepath, sorted=True, sep=',', header=None, compute=False):
		...
		print 'getting scene extent...'
		self._scene_extent()
		print 'generating batch list'
		self._generate_batch_list()

		print (len(self._curr_batch_list))

		# Re-compute the z-order scales used for indexing
		self._x_scale = self._z_scalar / (self._curr_scene_extent[3] - self._curr_scene_extent[0])
		self._y_scale = self._z_scalar / (self._curr_scene_extent[4] - self._curr_scene_extent[1])
	...
	def generate_labels_weights_file(self, save_dir):
		...
		for scene_file in self._filelist:
			self._load_model_simple(scene_file)
	...
	def _scene_extent(self):
		...
		self._curr_scene_extent = [xmin, ymin, zmin, xmax, ymax, zmax]
	...
	def _generate_batch_list(self, dims=2, bpd=32):
		...
		if dims == 2:
			self._encoder = pf.ZEncoder(2, dims*bpd)
			num_xstrides = int(np.floor((self._curr_scene_extent[3]-self._curr_scene_extent[0]) / self._box_size))
			num_ystrides = int(np.floor((self._curr_scene_extent[4]-self._curr_scene_extent[1]) / self._box_size))
			for x_stride in range(0, num_xstrides):
				for y_stride in range(0, num_ystrides):
					batch_list.append([self._curr_scene_extent[0]+(x_stride*self._box_size),
									   self._curr_scene_extent[1]+(y_stride*self._box_size),
									   self._curr_scene_extent[0]+((x_stride*self._box_size)+self._box_size),
									   self._curr_scene_extent[1]+((y_stride*self._box_size)+self._box_size)])
	...
	def get_next_batch(self, sample_method='uniform'):
		...
		while valid_batch == False:
			...
			"""
			idx1 = pf.encode_pt_2d([batch_bbox[0], batch_bbox[1]], self._z_scalar, self._encoder, self._curr_scene_extent, self._bpd)
			idx2 = pf.encode_pt_2d([batch_bbox[2], batch_bbox[3]], self._z_scalar, self._encoder, self._curr_scene_extent, self._bpd)
			tmp_batch = self._curr_scene.loc[min(idx1, idx2):max(idx1, idx2)]
			tmp_batch = tmp_batch.compute()
			"""
			select_points = dask.delayed(pf.select_points)
			tmp_batch = select_points(self._curr_scene, batch_bbox, z_order=False)
			tmp_batch = tmp_batch.compute()
			# Check if current batch has enough points
			if tmp_batch.values.shape[0] < self._num_points:
				print 'miss: ', self._batch_iterator
				pass
