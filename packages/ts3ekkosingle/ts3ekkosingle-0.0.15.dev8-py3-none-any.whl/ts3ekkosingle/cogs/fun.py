...
try:
    ...
    from ts3ekkosingle.argparser import NonExitParser, UtilityParser
except ImportError:
    from ..event_type import EventType
    from ..errors import EkkoUnsuitedCommand
    from ..argparser import NonExitParser,  UtilityParser
    ...
    def cmd_dice(self, event):
        ...
        if self.ekko_bot.check_cmd_suitability(f'^{cmd_prefix}', event[0]['msg']):
            if self.ekko_bot.can(FunPermission.DICE, event):
                force, string = self.ekko_bot.parse(UtilityParser.parse_force, event, cmd_prefix, event[0]['msg'])
                if not force:
