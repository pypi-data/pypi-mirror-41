...
class JSCB(RuleCallback):
        ...
        if name in ("pre", "post", "failing"):
            if name not in self.callbacks:
                ...
                raise AttributeError(
                    err_msg.format(self.__class__.__name__, name)
                )
    ...
    def execute(self, name, *args, **kwargs):
        ...
        return pool.with_context(
            lambda ctx: self._execute(ctx, name, self.arguments[name], *args, **kwargs))

    def _execute(self, js_ctx, name, arguments, original, return_value, args, kwargs):
        ...
        # Cleanup the JS CTX after X calls, else it will start to leak memory
        ...
        for _ in range(100):
            ...
            try:
                ...
                result = js_ctx.call(
                    callee, *resolved_args, encoder=CustomJSONEncoder
                )
            ...
            if key == "%hash_val_include":
                ...
                for value in values:
                    if not isinstance(value, dict):
                        continue
                    for v in cls.iter_hash_val_include_values(
                        value, depth - 1
                    ):
                        yield v
