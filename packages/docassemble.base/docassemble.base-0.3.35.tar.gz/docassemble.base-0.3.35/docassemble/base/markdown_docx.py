...
class SoupParser(object):
    ...
    def new_paragraph(self):
        if self.still_new:
            logmessage("new_paragraph is still new and style is " + self.style + " and indentation is " + unicode(self.indentation))
            self.current_paragraph['params']['style'] = self.style
            self.current_paragraph['params']['indentation'] = self.indentation
            return
        logmessage("new_paragraph where style is " + self.style + " and indentation is " + unicode(self.indentation))
    ...
    def __unicode__(self):
        ...
        for para in self.paragraphs:
            logmessage("Got a paragraph where style is " + para['params']['style'] + " and indentation is " + unicode(para['params']['indentation']))
            output += '<w:p><w:pPr><w:pStyle w:val="Normal"/>'
            if para['params']['style'] in ('ul', 'ol', 'blockquote'):
                output += '<w:ind w:left="' + unicode(36*para['params']['indentation']) + '" w:right="0" w:hanging="0"/>'
            output += '<w:rPr></w:rPr></w:pPr>'
            if para['params']['style'] == 'ul':
                output += unicode(RichText("•\t"))
            if para['params']['style'] == 'ol':
                output += unicode(RichText(unicode(list_number) + ".\t"))
                list_number += 1
            else:
                list_number = 1
    ...
    def traverse(self, elem):
        for part in elem.contents:
            if isinstance(part, NavigableString):
                self.run.add(unicode(part), italic=self.italic, bold=self.bold, underline=self.underline, strike=self.strike, size=self.size)
                self.still_new = False
            elif isinstance(part, Tag):
                logmessage("Part name is " + unicode(part.name))
                if part.name == 'p':
                    self.new_paragraph()
                    self.traverse(part)
                elif part.name == 'li':
...
def markdown_to_docx(text, tpl):
    ...
    #add_to_rt(tpl, None, html_linear_parse(soup))
    return output
