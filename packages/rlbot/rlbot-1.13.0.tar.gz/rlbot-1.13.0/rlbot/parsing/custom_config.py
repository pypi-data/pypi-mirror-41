...
class ConfigObject:
    ...
    def parse_file(self, config: Union[Path, str, RawConfigParser, 'ConfigObject'], max_index=None, config_directory=None):
        ...
        for section in self.raw_config_parser.sections():
            if section not in self.headers:
                self.add_header_name(section)
...
class ConfigHeader:
    ...
    def add_value(self, name, value_type, default=None, description="", value=None):
        """
