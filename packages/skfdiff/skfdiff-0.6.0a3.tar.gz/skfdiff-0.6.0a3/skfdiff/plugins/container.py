...
def _reduce_container(_nbuffer, _mode, _metadata, save, _cached_data, path):
...
class Container:
    def __init__(
        self, path=None, mode="a", *, save="all", metadata={}, force=False, nbuffer=50
    ):
    ...
    def connect(self, stream):
        ...
        (accumulation_stream.partition(self._nbuffer).sink(self._collector.flush))
...
{data}""".format(
    ...
    def retrieve(path: Path, isel: Union[str, dict, int] = "all", lazy: bool = False):
        ...
        try:
            ...
            data = open_mfdataset(path / "data*.nc", concat_dim="t").sortby("t")
        try:
        ...
        if not lazy:
            return FieldsData(data=data.load(), metadata=AttrDict(**metadata))
    ...
    def get_last(path):
        ...
        return Container.retrieve(path, isel=[-1], lazy=False)
    ...
    def get_all(path):
        ...
        return Container.retrieve(path, isel="all", lazy=False)
    ...
    def merge_datafiles(path, override=False):
        ...
        split_data = open_mfdataset(path / "data*.nc", concat_dim="t").sortby("t")
