...
@click.pass_context
def cli(ctx, validate, logfile, config, loop_every, issue_to_send):
    """CLI for Jira2Vsts"""
    logger = __get_logger(logfile)
    while True:
        try:
            _main(ctx, validate, logger, config, issue_to_send)
        except:
            logger.error(traceback.format_exc())
        if loop_every:
...
def __process_project(logger, config, data, old_client, wit_client, jira, jira_project, PROJECTS, STATES, JIRA_BROWSE,
    ...
    if vsts_destination_issue and vsts_destination_issue != vsts_project:
        logger.warning('the destination project is not the same [%s] vs [%s]', vsts_destination_issue, vsts_project)
        return
    nbr_issues = 0
    ...
    if issue_to_send:
        ...
        logger.info('search for issues updated after %s project=%s', last_sync_formatted, jira_project)
        _query = 'project=%s AND updated > "%s" ORDER BY updated ASC, createdDate ASC' % (jira_project, last_sync_formatted)
    ...
    for jira_item in jira_issues:
        ...
        if not issue_to_send and issue.fields.updated <= LAST_SYNC:
            logger.warning('%s - skip because is already sent', jira_item)
            continue
        ...
        if LAST_UPDATED and parse(LAST_UPDATED).strftime("%Y%m%d") < parse(issue.fields.updated).strftime("%Y%m%d"):
            ...
            logger.info("set last sync project=%s value=%s", jira_project, LAST_UPDATED)
    ...
    {f.description}
        ...
        try:
            ...
            if not vsts_id:
                ...
                logger.info('%s - create a new work item with state=%s', jira_item,
                ...
                vsts_id = workitem.id
                logger.info('%s - workitem is created id=%s', jira_item, vsts_id)
            else:
                workitem = wit_client.get_work_item(id=vsts_id)
                if html2text(workitem.fields['System.Description']) != html2text(issue_description):
                    logger.info('%s - update work item with vsts_id=%s, state=%s', jira_item, vsts_id,
                                STATES.get(issue.fields.status.name), )
                    workitem = wit_client.update_work_item(id=vsts_id, document=document)
                    logger.info('%s - workitem is updated vsts_id=%s', jira_item, vsts_id)
            ...
            for attachment in issue.fields.attachment:
                ...
                if attachment_filename in exists_attachments:
                    continue
                logger.info('%s - try to get the attachment [%s] attachment_id=%s', jira_item, attachment_filename,
                            attachment.id)
                ...
                logger.info('%s - the temporary file is saved to [%s', jira_item, tmpfile)
                logger.info('%s - try to send the attachment [%s] vsts_id=%s', jira_item, attachment_filename, vsts_id)
                with open(tmpfile, 'rb') as f:
                    try:
                        ...
                        if not SKIP_LARGE_ATTACHMENTS:
                            ...
                            logger.warning('%s - the attachgment [%s] is skipped', jira_item, attachment_filename)
                            continue
                logger.info('%s - the attachment [%s] is uploaded url=%s', jira_item, attachment_filename,
                            created_attachment.url)
                try:
                    os.remove(tmpfile)
                except:
                    pass
                ...
                logger.info('%s - the  attachment [%s] is created id=%s', jira_item, attachment_filename, vsts_id)
            ...
            if issue.fields.status.name in STATES:
                ...
                logger.warning('%s - the status [%s] is not mapped', jira_item, issue.fields.status.name)
        except:
            logger.error(traceback.format_exc())
            raise click.Abort()
    ...
    PROJECTS[jira_project]['last_sync'] = LAST_SYNC
    __update_config_file(config, data)


def _main(ctx, validate, logger, config, issue_to_send):
    """CLI for Jira2Vsts"""
    start = time.time()
