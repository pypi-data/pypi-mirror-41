...
class CANPIDController:
    ...
    def setReference(self, value: float, ctrl: ControlType, pidSlot: int = 0, arbFeedforward: float = 0) -> CANError:
        ...
        retval = CANError.kOK
        self._setpointCommand(value, ctrl, pidSlot, arbFeedforward)
        return retval
...
class CANSparkMaxLowLevel:
    ...
    def setControlFramePeriod(self, periodMs: int) -> None:
        ...
        """
        assert isinstance(periodMs, int)
        self._hal_data["control_period_ms"] = periodMs

    def getControlFramePeriod(self) -> int:
        """Return the rate of transmission for setpoint frame

        :returns: Control frame perioid in milliseconds
        """
        retval = self._hal_data["control_period_ms"]
        assert isinstance(retval, int)
        return retval
