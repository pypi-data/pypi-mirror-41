...
def build_po_lines_dataframe(stock_po_csv):
    stock_pos = pd.read_csv(stock_po_csv)
    po_column_mapping = set_po_column_mapping()
    ...
    stock_pos = stock_pos.sort_values(
        by=["po_date", "po_number", "line_number"], ascending=False
    )
...
def build_service_po_header_dataframe(service_po_excel):
    service_pos = pd.read_excel(service_po_excel)
...
def build_master_vendor_dataframe(supplier_master_excel):
