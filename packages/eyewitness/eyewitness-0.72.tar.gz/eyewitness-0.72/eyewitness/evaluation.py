...
class BboxMAPEvaluator(Evaluator):
    ...
    """
    def __init__(self, iou_threshold=0.5, test_set_only=True):
        logging.basicConfig()
    ...
    def _evaluate(self, detector, dataset):
        ...
        for image_obj in dataset.image_obj_iterator(self.test_set_only):
        ...
        # collect all ground truth from dataset.
        gt_objs = defaultdict(lambda: defaultdict(list))
        gt_label_count = defaultdict(int)
        for image_gt_objs in dataset.ground_truth_iterator(self.test_set_only):
            image_id = image_gt_objs[0]
            image_gt_objs = [image_gt_obj for image_gt_obj in
                             image_gt_objs[1] if image_gt_obj.label in valid_labels]
            for image_gt_obj in image_gt_objs:
                gt_objs[image_id][image_gt_obj.label].append(image_gt_obj)
    ...
    def calculate_label_ap(self, valid_labels, detected_objs, gt_objs, gt_label_count):
        ...
        for label in valid_labels:  # loop each label
            ...
            self.logger.info('ap calculated for %s: %s', label, ap_stat[label]['AP'])
        return ap_stat
