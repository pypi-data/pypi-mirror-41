...
class KerasCallback(BaseCallback, ModelHashInterface, GradCAMInterface, VisualBackPropInterface):
    ...
    def _on_test_end(self, phase):
        ml_prefixed_metrics_dict = {self.__prefix_metric(name, is_test=True): value for name, value in phase.metrics.items()}
    ...
    def __wrap_test_results(self, phase, steps, caller):
        ...
        for metrics_name, val in zip(metrics_names, actual_results):
            phase.add_metric(metrics_name, val)
    ...
    def __ml_test_loop_wrap(self, phase, is_method_bounded, call_test_loop):
        def _ml_test_loop(model, f, ins, batch_size=None, verbose=0, steps=None):
            temp_steps = self.__calc_steps_if_possible(ins, batch_size, steps)

            return self.__wrap_test_results(phase, temp_steps, lambda: call_test_loop(model, f, ins, batch_size, verbose, steps))

        def _ml_test_loop_bounded(f, ins, batch_size=None, verbose=0, steps=None):
            temp_steps = self.__calc_steps_if_possible(ins, batch_size, steps)

            return self.__wrap_test_results(phase, temp_steps, lambda: call_test_loop(f, ins, batch_size, verbose, steps))
