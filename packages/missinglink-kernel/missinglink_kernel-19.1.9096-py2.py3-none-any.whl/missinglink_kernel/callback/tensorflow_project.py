...
class TensorFlowProject(BaseCallback):
    ...
    def create_experiment(self,
        ...
        try:
            ...
            context_validator.enter(Context.EXPERIMENT)

            yield experiment

            context_validator.exit(Context.EXPERIMENT)
...
class ExperimentCounters(object):
    ...
    def get_loop(self, generator, post_data):
    ...
    def get_epoch_loop(self, generator, post_data):
        for result in generator:
            self._epoch += 1
            self._batch = 0
    ...
    def get_batch_loop(self, generator, post_data):
        for result in generator:
            self._is_iteration_with_validation = False
            self._iteration += 1
            self._batch += 1
...
class Experiment(ModelHashInterface, GradCAMInterface, VisualBackPropInterface):
    ...
    def loop_context(self, max_iterations=None, condition=None, iterable=None, epoch_size=None):
        ...
        def post_data():
            ...
            self.callback.batch_end(
                self._batch, self._epoch, self._latest_metrics, iteration=self._iteration,
        ...
        try:
            yield self._experiment_counters.get_loop(generator, post_data)
            post_data()
    ...
    def epoch_loop_context(self, epochs=None, condition=None, iterable=None):
        ...
        yield self._experiment_counters.get_epoch_loop(generator, post_data)
    ...
    def batch_loop_context(self, batches=None, condition=None, iterable=None):
        ...
        def post_data():
            ...
            self.callback.batch_end(
                self._batch, self._epoch, self._latest_metrics,
                iteration=self._iteration, is_test=self._is_iteration_with_validation,
                weights_hash=weights_hash)

        try:
            yield self._experiment_counters.get_batch_loop(generator, post_data)
            post_data()
        except GeneratorDone:
            pass
    ...
    def _on_test_end(self):
        # noinspection PyProtectedMember
        self.callback._test_iteration_end([], [], [], is_finished=True)

    def _on_test_begin(self, steps, model):
        # noinspection PyProtectedMember
        weights_hash = self.get_weights_hash(model)
        # noinspection PyProtectedMember
        self.callback._test_begin(steps, weights_hash)
    ...
    @contextmanager
    def test(self, expected=None, predicted=None, monitored_metrics=None, custom_metrics=None, session_instance=None):
        """
        ...
        steps = -1  # calc test steps if possible

        self._on_test_begin(steps, session_instance or self._train_session)
    ...
    def _is_iteration_with_validation(self):
        return self._experiment_counters._is_iteration_with_validation

    @_is_iteration_with_validation.setter
    def _is_iteration_with_validation(self, value):
        self._experiment_counters._is_iteration_with_validation = value
