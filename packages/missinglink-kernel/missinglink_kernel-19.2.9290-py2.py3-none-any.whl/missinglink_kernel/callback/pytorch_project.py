...
from .phase import Phase, ExperimentCounters, GeneratorDone
...
class PyTorchProject(BaseCallback, ModelHashInterface):
    ...
    def create_experiment(self, model,
        ...
        try:
            experiment = PyTorchExperiment(self, model, metrics, self._context_validator)
...
class PyTorchExperiment(object):
    def __init__(self, project, model, metrics, context_validator, every_n_secs=None, every_n_iter=None):
    ...
    def _on_train_ended(self):
        # noinspection PyProtectedMember
        self._callback._train_end(iterations=self._iteration, metricData=self.get_average_metrics())
    ...
    def _update_metric_data(self, metric_name, metric_data):
        self._phase.add_metric(metric_name, metric_data)
    ...
    def _wrap(self, key, base):
        def wrapped(*args, **kwargs):
            ...
            self._update_metric_data(key, metric_data)
    ...
    def _wrap_metrics_list(self, metrics_list):
        ...
        for base in metrics_list:
            ...
            self._metrics[key] = wrapped_function
        ...
        with self._context_validator.context(Context.LOOP):
            ...
            def post_data():
                ...
                if is_epoch_iteration:
                    ...
                    self._callback.epoch_end(self._epoch, self.get_average_metrics(), weights_hash=weights_hash)
    ...
    def batch_loop_context(self, batches=None, condition=None, iterable=None):
        ...
        with self._context_validator.context(Context.BATCH_LOOP):
            ...
            def post_data():
                weights_hash = self._callback.get_weights_hash(
