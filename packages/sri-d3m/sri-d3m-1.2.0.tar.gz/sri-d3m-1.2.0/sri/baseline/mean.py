...
class MeanBaseline(pi_unsupervised_learning.UnsupervisedLearnerPrimitiveBase[Inputs, Outputs, MeanBaselineParams, MeanBaselineHyperparams]):
    ...
    def _validate_training_input(self, inputs: Inputs):
        ...
        num_data_elements = int(inputs.metadata.query([])['dimension']['length'])
        for data_element_raw in range(num_data_elements):
            ...
            for i in range(numCols):
                ...
                if ('https://metadata.datadrivendiscovery.org/types/SuggestedTarget' not in column_info['semantic_types']):
                    continue
