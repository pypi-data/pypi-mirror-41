...
from trytond.modules.employee_timetracking.const import WF_EVALUATION_CREATED, \
    WF_EVALUATION_LOCK, WF_EVALUATION_ACTIVE
...
class Evaluation(Workflow, ModelSQL, ModelView):
    ...
    worktime_actual_str = fields.Function(fields.Char(string=u'Actual', 
            help=u'Actual working time per month', readonly=True), 'get_worktime_info')
    ...
    def on_change_with_bal_prev_month(self, name=None):
        ...
        ev_lst = EvalItem.search([
                ('evaluation', '=', self.id),
                ('account', '=', self.employee.tariff.main_timeaccount)
            ])
        if len(ev_lst) == 0:
    ...
    def get_worktime_info(cls, evaluations, names):
        ...
        tab_sumact = DayOfEval.get_eval_sumact_sql()
        cursor = Transaction().connection.cursor()

        res = {'worktime_target': {}, 'worktime_target_str': {}, 
                'worktime_actual': {}, 'worktime_actual_str': {}, 
        ...
        qu2 = qu1.select(qu1.evaluation,
                Sum(qu1.targettime).as_('targwt'),
                Sum(qu1.holiday).as_('holidays'),
        ...
        for i in l1:
            (id1, targ, holidays) = i
            res['worktime_target'][id1] = targ
            res['holidays'][id1] = holidays

        # actual/diff working time
        qu1 = tab_sumact.select(tab_sumact.evaluation,
                Sum(tab_sumact.sumact).as_('sumact'),
                where=tab_sumact.evaluation.in_(id_lst),
                group_by=[tab_sumact.evaluation],
            )
        cursor.execute(*qu1)
        l1 = cursor.fetchall()
        for i in l1:
            (id1, sumact) = i
            res['worktime_actual'][id1] = sumact
    ...
    def updt_calc_evaluation(cls, evaluation):
        ...
        for i in evaluation.evalitem:
            ...
            # apply factor to duration of time-account-item
            for k in ta_lst:
                baldiff += k.duration * float(k.accountrule.factor)

            # reduce main-time-account by breaktime
            if i.account == evaluation.employee.tariff.main_timeaccount:
                date_cache = []
...
class EvaluationBreakTime(ModelSQL):
    ...
    @classmethod
    def get_reduced_worktime(cls, evaluation, worktime):
        """ calculates the reduced working hours based on the rules
            evaluation: employee_timetracking.evaluation,
            worktime: timedelta
        """
        if isinstance(evaluation, type(None)) or isinstance(worktime, type(None)):
            return (None, None)

        # select all breaktime-rules of current evaluation, order by mintime
        rl1 = cls.search([
                    ('evaluation', '=', evaluation), 
                ], order=[('mintime', 'ASC')])

        fnd1 = False
        worktime_ori = worktime
        if len(rl1) > 0:
            # walk through list, find match
            last_itm = None
            for i in rl1:
                if (i.mintime <= worktime <= i.maxtime):
                    fnd1 = True
                    # dont reduce working time below mintime
                    if (worktime - i.deduction) < i.mintime:
                        if isinstance(last_itm, type(None)):
                            worktime = i.mintime
                        else :
                            beg1 = i.mintime - last_itm.deduction
                            worktime -= i.deduction
                            if worktime < beg1:
                                worktime = beg1
                    else :
                        worktime -= i.deduction
        ...
        if fnd1 == False:
            ...
            rl1 = cls.search([
                        ('evaluation', '=', evaluation),
                        ('maxtime', '<=', worktime),
                    ], order=[('maxtime', 'DESC')], limit=1)
            if len(rl1) == 1:
                worktime -= rl1[0].deduction
                if worktime < rl1[0].mintime:
                    worktime = rl1[0].mintime

        # (<reduced working time>, <used break time>)
        return (worktime, worktime_ori - worktime)
