...
class TariffModelTestCase(ModuleTestCase):
    ...
    def test_tariffmodel_reduce_worktime(self):
        ...
        # 4:50 --> 4:20
        self.assertEqual(
            str(TariffM.get_reduced_worktime(tarobj1, timedelta(seconds=4*60*60 + 50*60))),
            '4:20:00'
            )
        # 6:30 --> 5:30
        self.assertEqual(
            str(TariffM.get_reduced_worktime(tarobj1, timedelta(seconds=6*60*60 + 30*60))),
            '5:30:00'
            )
        # 6:58 --> 5:58
        self.assertEqual(
            str(TariffM.get_reduced_worktime(tarobj1, timedelta(seconds=6*60*60 + 58*60))),
            '5:58:00'
            )
        # 7:10 --> 6:10
        self.assertEqual(
            str(TariffM.get_reduced_worktime(tarobj1, timedelta(seconds=7*60*60 + 10*60))),
            '6:10:00'
            )
