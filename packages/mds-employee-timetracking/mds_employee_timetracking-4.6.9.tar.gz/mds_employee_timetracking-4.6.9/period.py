...
class Period(Workflow, ModelSQL, ModelView):
    ...
    @classmethod
    def get_overlaps(cls, id_employee, period, startpos, endpos):
        """ get ids of overlapping period items
            period = current item to edit (if None: startpos+endpos must set)
        ...
        if not isinstance(period, type(None)):
            if isinstance(startpos, type(None)):
                startpos = period.startpos
            if isinstance(endpos, type(None)):
                endpos = period.endpos
        ...
        for items, values in zip(actions, actions):
            for i in items:
                ...
                if ('startpos' in values.keys()) or ('endpos' in values.keys()):
                    l1 = cls.get_overlaps(i.employee.id, i, values.get('startpos', None), values.get('endpos', None))
                    if len(l1) > 0:
                        obj1 = Period(l1[0])
                        
                        if isinstance(obj1.startpos, type(None)):
                            t_start_d = '--.--.----'
                            t_start_t = '--:--'
                        else :
                            t_start_d = obj1.startpos.strftime('%d.%m.%Y')
                            t_start_t = obj1.startpos.strftime('%H:%M')
                            
                        if isinstance(obj1.endpos, type(None)):
                            t_end_t = '--:--'
                        else :
                            t_end_t = obj1.endpos.strftime('%H:%M')
                        t1 = '%s: %s - %s' % (t_start_d, t_start_t, t_end_t)
                        cls.raise_user_error('overlap_period', (t1))
    ...
    def create(cls, vlist):
        ...
        for values in vlist:
            ...
            if ('startpos' in values.keys()) and ('endpos' in values.keys()):
                l1 = cls.get_overlaps(values['employee'], None, values['startpos'], values['endpos'])
                if len(l1) > 0:
                    obj1 = Period(l1[0])
