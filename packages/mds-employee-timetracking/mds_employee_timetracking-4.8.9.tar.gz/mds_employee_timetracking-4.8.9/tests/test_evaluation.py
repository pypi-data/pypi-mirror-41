...
from trytond.modules.employee_timetracking.tests.testlib import create_employee, \
    ...
    create_tariff, create_breaktime
...
class EvaluationTestCase(ModuleTestCase):
    ...
    def test_evaluation_employee_dayofyear(self):
        ...
        with set_company(tarobj1.company):            
            ...
            self.assertEqual(p1.name, '08:30 - 12:05, 2018-04-18 [W]')  # CEST
            ...
            self.assertEqual(emp_lst[0].worktime_diff, -timedelta(seconds=20*60 + 122*60*60))
            self.assertEqual(emp_lst[0].worktime_wobreaks, timedelta(seconds=10*60 + 25*60*60))
            self.assertEqual(emp_lst[0].holidays, 4)
            ...
            for i in [
                ...
                (17, '2018-04-18', '3', 16, '2018-04-18 06:30:00', '2018-04-18 10:05:42', '7:18:05', '8:00:00', '3:35:42',  '-1 day, 22:48:05', '6:48:05',  '0:30:00', False, None, 'wd', str(l2[2]), '()', 'Break:00h30'),
                (18, '2018-04-18', '3', 16, '2018-04-18 11:00:00', '2018-04-18 14:42:23', '7:18:05', '8:00:00', '3:42:23',  '-1 day, 22:48:05', '6:48:05',  '0:30:00', False, None, 'wd', str(l2[3]), '()', 'Break:00h30'),
    ...
    def test_daysofevaluation_reduced_workingtime(self):
        ...
        with set_company(tarobj1.company):
            ...
            (wtime, btime) = EvalBtRule.get_reduced_worktime(evobj, None)
            self.assertEqual(str(wtime), 'None')
            self.assertEqual(str(btime), 'None')
            (wtime, btime) = EvalBtRule.get_reduced_worktime(None, timedelta(seconds=0))
            self.assertEqual(str(wtime), 'None')
            self.assertEqual(str(btime), 'None')
            (wtime, btime) = EvalBtRule.get_reduced_worktime(evobj, timedelta(seconds=3*60*60 + 30*60))
            self.assertEqual(str(wtime), '3:30:00')
            self.assertEqual(str(btime), '0:00:00')
            (wtime, btime) = EvalBtRule.get_reduced_worktime(evobj, timedelta(seconds=4*60*60 + 20*60))
            self.assertEqual(str(wtime), '4:00:00')
            self.assertEqual(str(btime), '0:20:00')
            (wtime, btime) = EvalBtRule.get_reduced_worktime(evobj, timedelta(seconds=4*60*60 + 50*60))
            self.assertEqual(str(wtime), '4:20:00')
            self.assertEqual(str(btime), '0:30:00')
            (wtime, btime) = EvalBtRule.get_reduced_worktime(evobj, timedelta(seconds=5*60*60 + 50*60))
            self.assertEqual(str(wtime), '5:20:00')
            self.assertEqual(str(btime), '0:30:00')
            (wtime, btime) = EvalBtRule.get_reduced_worktime(evobj, timedelta(seconds=6*60*60))
            self.assertEqual(str(wtime), '5:30:00')
            self.assertEqual(str(btime), '0:30:00')
            (wtime, btime) = EvalBtRule.get_reduced_worktime(evobj, timedelta(seconds=6*60*60 + 10*60))
            self.assertEqual(str(wtime), '5:30:00')
            self.assertEqual(str(btime), '0:40:00')
            (wtime, btime) = EvalBtRule.get_reduced_worktime(evobj, timedelta(seconds=6*60*60 + 15*60))
            self.assertEqual(str(wtime), '5:30:00')
            self.assertEqual(str(btime), '0:45:00')
            (wtime, btime) = EvalBtRule.get_reduced_worktime(evobj, timedelta(seconds=6*60*60 + 20*60))
            self.assertEqual(str(wtime), '5:35:00')
            self.assertEqual(str(btime), '0:45:00')
            (wtime, btime) = EvalBtRule.get_reduced_worktime(evobj, timedelta(seconds=6*60*60 + 30*60))
            self.assertEqual(str(wtime), '5:45:00')
            self.assertEqual(str(btime), '0:45:00')
            (wtime, btime) = EvalBtRule.get_reduced_worktime(evobj, timedelta(seconds=6*60*60 + 58*60))
            self.assertEqual(str(wtime), '6:13:00')
            self.assertEqual(str(btime), '0:45:00')
            (wtime, btime) = EvalBtRule.get_reduced_worktime(evobj, timedelta(seconds=7*60*60 + 10*60))
            self.assertEqual(str(wtime), '6:25:00')
            self.assertEqual(str(btime), '0:45:00')
            (wtime, btime) = EvalBtRule.get_reduced_worktime(evobj, timedelta(seconds=7*60*60 + 58*60))
            self.assertEqual(str(wtime), '7:13:00')
            self.assertEqual(str(btime), '0:45:00')
            (wtime, btime) = EvalBtRule.get_reduced_worktime(evobj, timedelta(seconds=8*60*60))
            self.assertEqual(str(wtime), '7:15:00')
            self.assertEqual(str(btime), '0:45:00')
            (wtime, btime) = EvalBtRule.get_reduced_worktime(evobj, timedelta(seconds=8*60*60 + 14*60))
            self.assertEqual(str(wtime), '7:15:00')
            self.assertEqual(str(btime), '0:59:00')
            (wtime, btime) = EvalBtRule.get_reduced_worktime(evobj, timedelta(seconds=8*60*60 + 16*60))
            self.assertEqual(str(wtime), '7:16:00')
            self.assertEqual(str(btime), '1:00:00')
            (wtime, btime) = EvalBtRule.get_reduced_worktime(evobj, timedelta(seconds=8*60*60 + 45*60))
            self.assertEqual(str(wtime), '7:45:00')
            self.assertEqual(str(btime), '1:00:00')
            (wtime, btime) = EvalBtRule.get_reduced_worktime(evobj, timedelta(seconds=10*60*60 + 30*60))
