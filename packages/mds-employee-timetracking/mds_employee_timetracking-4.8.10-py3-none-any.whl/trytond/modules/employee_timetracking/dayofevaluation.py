...
from trytond.modules.employee_timetracking.const import sel_weekday2, \
    WF_BREAKPERIOD_EXAMINE, WF_BREAKPERIOD_LOCK
from .tools import fmttimedelta, get_translation
...
class DaysOfEvaluation(ModelSQL, ModelView):
    ...
    startpos = fields.Function(fields.DateTime(string=u'Start', readonly=True), 'get_day_data')
    endpos = fields.Function(fields.DateTime(string=u'End', readonly=True), 'get_day_data')
    ...
    def get_working_days_sql(cls):
        ...
        qu1 = tab_days.join(tab_worktime, 
                ...
                tab_days.weekday,
                tab_days.week,
                tab_days.holiday_ena, 
                tab_days.holidayname,
                tab_days.daytype,
                Case(
                    ((tab_days.holiday_ena == True) & (tab_days.halfday == True), 
                     Coalesce(tab_worktime.targtime, timedelta(seconds=0)) / 2),
                    ((tab_days.holiday_ena == True) & (tab_days.halfday == False), 
                     timedelta(seconds=0)),
    ...
    def get_data_sql(cls):
        ...
        tab_qu1 = tab_workdays.join(tab_sumact,
                ...
                type_='LEFT OUTER'
            ).select(tab_workdays.id_date.as_('id'),
                tab_workdays.evaluation,
                tab_workdays.date,
                tab_workdays.weekday,
                tab_workdays.week,
                tab_workdays.holiday_ena,
                tab_workdays.holidayname,
                tab_workdays.daytype,
                tab_workdays.targettime,
                tab_sumact.sumact,
                tab_tacc.id.as_('accountitem'),
                tab_tacc.period,
                tab_tacc.startpos,
                tab_tacc.endpos,
                (tab_tacc.endpos - tab_tacc.startpos).as_('duration'),
                Coalesce(tab_breaks.sumbreaks, timedelta(seconds=0)).as_('sumbreaks')
            )
        return tab_qu1
    ...
    def get_day_data(cls, days, names):
        ...
        l1 = cursor.fetchall()
        for i in l1:
            (id1, p1, wd1, dt1, w1, sp1, ep1, du1, su1, he1, hn1, tt1, sb1) = i
            r1['period'][id1] = p1
            r1['weekday'][id1] = wd1
            r1['daytype'][id1] = dt1
            r1['week'][id1] = w1
    ...
    @fields.depends('daytype', 'holidayname', 'accountitems_nonmain', 'breaktime')
