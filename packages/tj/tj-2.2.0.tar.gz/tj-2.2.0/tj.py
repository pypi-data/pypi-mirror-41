...
Updated on: 29rd January 8:50 PM
''' % ( __version__)
...
def pinput(prompt='',symbol='*' ,flag=True):
    '''Args->
    ...
    * by default. You can change it to # or $ or x or even
    ...
    flag-> This is an optional argument set to false. Turning
    to True, pinput() will clear the screen and print just the
    prompt. If flag is false, it will not clear the screen. The
    if you don't care about the data already printed on the screen,
    then turn this flag to True, which will enable backspace ability
    in this password input field. Turning flag to false will not allow
    users to use backspace.

    pinput() (pssword-input) is a function, which makes entering
    sensitive data secure, by hiding the characters.
'''
    if flag:os.system('cls')
    print(prompt, end='')
    s = ''
    sys.stdout.flush()
    while True:
