...
try:
    ...
    class ScoopExecutor(ExecutorLike): # Stub
        """Executor that encapsulate scoop concurrency functions. Need the scoop package to be installed and importable."""
        ...
        def shutdown(self, wait: bool = True) -> None:
            raise NotImplementedError("`ScoopExecutor` needs the scoop package to be installed and importable.")
    _scoop_as_completed: Optional[Callable] = None
    _scoop_future_class: Optional[Any] = None
...
def generic_as_completed(fs: Sequence[FutureLike], timeout: Optional[int]=None) -> FutureLike:
    ...
    if isinstance(fs[0], SequentialFuture):
        return fs[0]
    if _scoop_future_class is not None and isinstance(fs[0], _scoop_future_class):
    ...
    if hasattr(m, "as_completed"):
        return next(m.as_completed(fs, timeout))
    else:
...
class CreatableFromConfig(object):
    ...
    def from_config(cls, config: Mapping[str, Any], **kwargs: Any) -> Any:
        ...
        """
        #def get_params(names: Sequence[str]) -> Mapping[str, Any]:
        #    d = {}
        #    for n in names:
        #        d[n] = config.get(n, None)
        #    return d
        #params: Sequence[str] = list(set(inspect.signature(cls.__init__).parameters) - set(['self', 'args', 'kwargs']))
        #kwargs_from_config: Mapping[str, Any] = get_params(params)
        final_kwargs = {**config, **kwargs}
        #print("DEBUG from_config2", final_kwargs)
...
class Factory(dict):
    ...
    def _build_internal(self, config: MutableMapping[str, Any], default_params: Mapping[str, Any] = {}, **kwargs: Any) -> Any:
        ...
        if "type" in config:
            ...
            name: str = self._get_name(obj, None, config)
    ...
    def build(self, config: Optional[Mapping[str, Any]] = None, default_params: Mapping[str, Any] = {}, **kwargs: Any) -> Any:
