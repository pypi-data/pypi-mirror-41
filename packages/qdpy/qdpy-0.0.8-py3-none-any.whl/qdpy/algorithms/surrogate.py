...
class SurrogateAssistedAlg(AlgWithBatchSuggestions):
    ...
    def __init__(self,
        ...
        super().__init__(illumination_algo.container, illumination_algo.budget,
                ...
                base_ind_gen=illumination_algo.base_ind_gen, **kwargs2)
    ...
    @property
    def prediction_algo(self) -> QDAlgorithm:
        return self._prediction_algo


    @abc.abstractmethod
    def _compute_acquisition(self) -> Sequence[IndividualLike]:
    ...
    def _select_suggestions_from_acquisition(self, acquisition_suggestions: Sequence[IndividualLike],
            nb_suggestions: int) -> Sequence[IndividualLike]:
    ...
    def _generate_batch_suggestions(self) -> None:
        ...
        acquisition_suggestions: Sequence[IndividualLike] = self._compute_acquisition() if len(self.container) > 0 else []
        # Select suggestions from the acquisition container
        self._batch_inds.extend(self._select_suggestions_from_acquisition(acquisition_suggestions, self._batch_size))
        # If not enough suggestions were obtained from acquisition, ask `self._illumination_algo` for suggestions.
        nb_not_suggested_by_surrogate = int(min(self.budget_not_suggested_by_surrogate, self._batch_size - len(self._batch_inds)))
