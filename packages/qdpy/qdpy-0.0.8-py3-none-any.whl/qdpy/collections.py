...
class Container(Sequence, Summarisable, Copyable, CreatableFromConfig):
    ...
    def fitness_extrema(self) -> Optional[Sequence[DomainLike]]:
        ...
        if len(self) == 0:
            return None
        maxima = np.array(self[0].fitness.values)
        minima = np.array(self[0].fitness.values)
        for ind in self:
    ...
    def features_extrema(self) -> Optional[Sequence[DomainLike]]:
        ...
        if len(self) == 0:
            return None
        maxima = np.array(self[0].features)
        minima = np.array(self[0].features)
        for ind in self:
            ind_f = ind.features
            for i in range(len(maxima)):
                if ind_f[i] > maxima[i]:
...
#        return reversed(self.items)
    ...
    def _add_to_collection(self, collection: BackendLike[IndividualLike], individual: IndividualLike) -> Tuple[bool,int]:
        ...
        if isinstance(collection, MutableSet) and isinstance(collection, Sequence):
    ...
    # TODO : option to store or not individual with nul fitness
    ...
    def to_grid(self, shape: Union[ShapeLike, int],
            ...
            capacity: Optional[float] = None, **kwargs: Any) -> None:
        ...
        if not 'features_domain' in kwargs:
            kwargs['features_domain'] = self.fitness_domain if self.fitness_domain is not None else self.features_extrema
...
class Grid(Container):
    ...
    _items_per_bin: GridItemsPerBinLike
    ...
    def _init_grid(self) -> None:
        ...
        self._items_per_bin = np.zeros(self._shape, dtype=float) #{x: 0 for x in self._index_grid_iterator()}
    ...
    @property
    def items_per_bin(self) -> GridItemsPerBinLike:
        """Return the number of items stored in each bin of the grid."""
        return self._items_per_bin
    ...
    def _update_quality(self, ig: GridIndexLike) -> None:
        ...
        if self._items_per_bin[ig] == 0:
            val: Optional[FitnessLike] = None
        elif self._items_per_bin[ig] == 1:
    ...
    def add(self, individual: IndividualLike, raise_if_not_added_to_depot: bool = False) -> Optional[int]:
        ...
        if self._items_per_bin[ig] < self.max_items_per_bin:
            can_be_added = True
        else:
            worst_idx = 0
            worst: IndividualLike = self.solutions[ig][worst_idx]
            if self._items_per_bin[ig] > 1:
                for i in range(1, self._items_per_bin[ig]):
