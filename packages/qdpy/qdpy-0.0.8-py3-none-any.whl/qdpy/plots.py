...
def default_plots_grid(logger, output_dir=".", to_grid_parameters={}):
    ...
    plot_evals(logger, os.path.join(output_dir, "./evals_fitnessmax.pdf"), "max", ylabel="Fitness")
    plot_evals(logger, os.path.join(output_dir, "./evals_contsize.pdf"), "cont_size", ylim=(0, container.capacity), ylabel="Container size")
    plot_iterations(logger, os.path.join(output_dir, "./iterations_nbupdated.pdf"), "nb_updated", ylabel="Number of updated bins")

    if isinstance(container, collections.Grid):
        grid = container
    else:
        if 'nb_bins' not in to_grid_parameters:
            to_grid_parameters['nb_bins'] = (32,) * len(container.features_domain)
