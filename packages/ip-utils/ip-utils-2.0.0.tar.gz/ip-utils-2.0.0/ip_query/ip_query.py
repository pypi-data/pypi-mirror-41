...
def ip_query(requests_proxies: dict = None, timeout: int = TIMEOUT, geo: bool = False):
    """
    Get IP data, include: ip address, country, country_code, asn, aso

    :param requests_proxies: ...
    :param timeout: ...
    :param geo: with Geo info?
    :return: dict or None
    """
...
def requests_get_json(url: str, requests_proxies: dict = None, timeout: int = TIMEOUT):
    ...
    try:
        ...
        if 200 == resp.status_code:
            ...
            xp.error('[ip-query] requests.status_code: {}'.format(resp.status_code))

    except Exception as e:
        xp.error('[ip-query] {}'.format(e))
        return None
...
def geoip(ip_address):
    ...
    try:
        ...
        xp.error('[ip-query] geoip.error: {}'.format(e))
        return None
