...
async def async_annotate(
    ...
    annotations_list = await sdk.call_with_retry(
        sdk.annotations.get_annotations,
        batch=csra_batch,
...
def add_annotation(
    ...
    if annotations_list and is_call_or_variant(call):
        csra = to_csra(call)
        annotations = obj_to_dict(
            fields, annotations_list.get_annotation(csra)
        )

        ann_bundle_version = annotations.get("annotation_bundle_version")

        # if the annotation_bundle does not match, raise an exception
        if annotation_bundle_version != ann_bundle_version:
            msg = (
                "Annotations Process Terminated due to "
                "annotations_bundle_version "
                "changing during a run. \n"
                "Original annnotation_bundle_version: {original} \n"
                "New annotation_bundle_version: {field_value}".format(
                    original=annotation_bundle_version,
                    field_value=ann_bundle_version,
                )
            )
...
def up_build(call: dict):
    hgvs_parts = call.get("hgvs_g").split("|")
    hgvs_parts[-1] = call.get("build")
    call.update(hgvs_g="|".join(hgvs_parts))
    return call
