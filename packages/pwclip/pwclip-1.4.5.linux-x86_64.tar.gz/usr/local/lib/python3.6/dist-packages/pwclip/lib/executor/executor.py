...
class Command(object):
	...
	def __init__(self, *args, **kwargs):
		...
		if self.dbg:
			print('\033[1;30m%s\033[0m'%str(Command.__mro__))
			print(' ', '\033[1;30m%s\033[0m'%str(self.__dict__))
	...
	def run(self, *commands):
		...
		return Popen(
            commands, stdin=PIPE, stdout=DEVNULL,
            stderr=DEVNULL, shell=self.sh_).pid
	...
	def call(self, *commands, stdout=True, stderr=True, inputs=None, b2s=None):
		...
		return int(run(
            commands, shell=self.sh_, stdin=PIPE, stdout=stdout,
	...
	def stde(self, *commands, inputs=None, b2s=True):
		...
		prc = Popen(commands, stdin=PIPE, \
            stdout=PIPE, stderr=PIPE, shell=self.sh_)
	...
	def erno(self, *commands, inputs=None, b2s=True):
		...
		prc = Popen(commands, stdin=PIPE, \
            stdout=DEVNULL, stderr=DEVNULL, shell=self.sh_)
