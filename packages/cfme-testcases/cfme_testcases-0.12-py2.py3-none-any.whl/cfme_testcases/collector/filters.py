    ...
    def get_filtered_xmls(self):
        ...
        if self.missing:
            if self.testcases_root is not None:
                xmls_container.missing_testcases = self.get_missing_testcases()
            if self.testsuites_root is not None:
                xmls_container.missing_testsuites = self.get_missing_testsuites()

        if self.testcases_root is not None:
            xmls_container.updated_testcases = self.get_updated_testcases()
