...
class ParamParser:
    ...
    def __init__(self, paramstyle):
        ...
        logger.info("Queries will use paramstyle = {0}".format(paramstyle))
...
class SqlClient:
    ...
    def _log_query(self, query, parameters):
        ...
        if self.humour == 'bad':
            ...
            except Exception:  # yes it's broad but i'm really not going to ever care that those two lines didn't run correctly
    ...
    @staticmethod
    def _archive_query(query, parameters, file):
        logger.info('saving query to file: {}'.format(file))
    ...
    def _run_and_log_sql(self, command, parameters, pandas=False, archive_query=False, dry_run=False):
        ...
        if command != '':
            self._log_query(command, params)
            if archive_query:
                self._archive_query(sql, params, archive_query)
