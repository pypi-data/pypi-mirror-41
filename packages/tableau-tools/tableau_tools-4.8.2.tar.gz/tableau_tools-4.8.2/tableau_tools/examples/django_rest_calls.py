...
t.signin()
master_session_token = deepcopy(t.token)
...
def switch_user(request, username):
    ...
    t.token = deepcopy(master_session_token)
    if username in living_user_sessions:
        t.token = deepcopy(living_user_sessions[username])
    else:
        user_luid = t.query_user_luid(username)
        # Create new user session
        t.token = None
        t.signin(user_luid)
        living_user_sessions[username] = deepcopy(t.token)
