class QualityCheck:
            ...
            if check_result in conditions[self.metric_name]:
                ...
                if condition_type == 'gt':
                    if self.metric_value > condition_value:
                        self.failed_check = '> {0}'.format(condition_value)
                ...
                # less than and greater than
                    ...
                    if (self.metric_value < condition_value[0] or self.metric_value > condition_value[1]):
                        self.failed_check = '< {0} and/or > {1}'.format(condition_value[0], condition_value[1])
                        self.result = check_result.upper()
                        return
                # equal to
                elif condition_type == 'eq':
                    if self.metric_value == condition_value:
                        self.failed_check = 'Equals {0}'.format(condition_value)
                        self.result = check_result.upper()
                        return
                # less than and greater than
                elif condition_type == 'ne':
                    if self.metric_value != condition_value:
                        self.failed_check = 'Does not equal {0}'.format(condition_value)
                        self.result = check_result.upper()
                        return
                # any
                elif condition_type == 'any':
                    if self.metric_value in condition_value:
                        self.failed_check = 'One of {0}'.format(', '.join(condition_value))
                        self.result = check_result.upper()
                        return
        self.result = 'PASS'
        return
    ...
    def to_tuple(self):
        return (self.metric_name, self.metric_value, self.failed_check, self.result)