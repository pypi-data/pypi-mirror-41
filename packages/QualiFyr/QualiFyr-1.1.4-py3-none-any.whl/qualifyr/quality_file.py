...
class QualityFile:
    ...
    def quality_checks_to_dict(self, only_failed_checks = True):
        quality_checks_dict = { self.type_and_id : [] }
        ...
        for quality_check in included_quality_checks:
             quality_checks_dict[self.type_and_id].append(OrderedDict([
                ('metric_name' , quality_check.metric_name),
                ('metric_value' , quality_check.metric_value),
                ('check' , quality_check.failed_check),
        ...
        if output_format == 'tsv':
            checks_string = '\n'.join(
                    [ '\t'.join([
                        ...
                        str(quality_check.failed_check),
            ...
            return json.dumps(self.quality_checks_to_dict(), indent = 2 )
    ...
    def multiple_qc_result_string(sample_name, quality_files, output_format = 'tsv', only_failed_checks = True):
        ...
        if output_format == 'tsv':
            ...
            body_string = '\n'.join([quality_file.output_quality_checks_string(output_format, only_failed_checks) for quality_file in quality_files])
