...
class Asset(object):
    def __init__(self, assetId):
        self.assetId='' if assetId == 'NEEL' else assetId
    ...
    def status(self):
        if self.assetId!='NEEL':
            try:
                req = pyneel.wrapper('/transactions/info/%s' % self.assetId)
                if req['type'] == 3:
...
class AssetPair(object):
    def __init__(self, asset1, asset2):
        self.asset1 = asset1
        self.asset2 = asset2
        self.a1 = 'NEEL' if self.asset1.assetId == '' else self.asset1.assetId
        self.a2 = 'NEEL' if self.asset2.assetId == '' else self.asset2.assetId
    ...
    def orderbook(self):
        req = pyneel.wrapper('/matcher/orderbook/%s/%s' % (self.a1, self.a2), host=pyneel.MATCHER)
        return req

    def ticker(self):
        return pyneel.wrapper('/api/ticker/%s/%s' % (self.a1, self.a2), host=pyneel.DATAFEED)
    ...
    def _getMarketData(self, method, params):
        return pyneel.wrapper('%s/%s/%s/%s' % (method, self.a1, self.a2, params), host=pyneel.DATAFEED)

    def trades(self, *args):
        if len(args)==1:
            limit = args[0]
            if limit > 0 and limit <= pyneel.MAX_WDF_REQUEST:
    ...
    def candles(self, *args):
        if len(args)==2:
            timeframe = args[0]
            limit = args[1]
            if timeframe not in pyneel.VALID_TIMEFRAMES:
                msg = 'Invalid timeframe'
                pyneel.throw_error(msg)
                return logging.error(msg)
            elif limit > 0 and limit <= pyneel.MAX_WDF_REQUEST:
