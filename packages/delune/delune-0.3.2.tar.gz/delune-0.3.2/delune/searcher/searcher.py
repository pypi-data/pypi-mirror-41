...
class Searcher:
	...
	def maintern (self):
		# maintern index
		# another process reading file
		# there is 2 cases when merging and duplicating		
		if time.time () - self.maintern_time > self.MAINTERN_INTERVAL and self.ismainternable ():
			self.maintern_time = time.time ()
			self.queue and self.queue.maintern ()
			if self.need_refresh ():
				self.si.log ("refreshing `%s'" % self.si.getAlias ())				
				self.do_refresh ()
			else:
				#self.si.log ("reloading deldoc `%s'" % self.si.getAlias ())
				self.segments.reload_deleted () # reload del files
	...
	def do_delete (self, qs, lang = "en", analyze = 1, commit = True):
		try:
			try:
				plock = self.si.plock
				if plock is not None: plock.acquire ()  # process lock			
			...
			self.si.trace ()
			#return [500, 0, 0, ""]
			return {"code":500, "err": "Deletion Error"}
		
		#return [200, 0, result [2], result [3]]
		return {"code":200, "total": result ["total"], "time": result ["time"]}		
		
	def do_query (self, qs, offset = 0, fetch = 10, sort = "", summary = 30, returns = "", nthdoc = 0, lang = "un", analyze = 1, data = 1, limit = 1, *arg, **karg):
		try:
			offset = int (offset)
			fetch = int (fetch)
			summary = int (summary)
		...
		try:
			...
			if data:
				...
				#result = [200, q.duration, q.totalcount, query.makeHighLightRE (q.highlights)] + fetched
			else:
				result ["result"] = q.bucket  [offset:offset + fetch]				
				#result = [200, [q.sortindex, q.sortorder], q.totalcount, query.makeHighLightRE (q.highlights)] + fetched
	...
	def delete (self, *arg, **karg):
		return self.single_job (self.do_delete, 0, *arg, **karg)
	
	def commit (self, *arg, **karg):
		return self.single_job (self.do_commit, 0, *arg, **karg)
