...
class Reaction(object):
    ...
    @ureg.check(None, None, 'molar', 'kelvin')
    def transform(
        ...
        """
        ddg_over_rt = 0.0 * ureg('dimensionless')
        for compound, coeff in self.items():
            if compound == Reaction.PROTON_COMPOUND:
                continue  # H+ is ignored in the Legendre transform
            ddg_over_rt += coeff * compound.transform(p_h, ionic_strength,
                                                      temperature)
        return ddg_over_rt
    ...
    def dg_correction(
        ...
        for compound in self.keys_without_water_and_protons():
            if compound in compound_to_conc:
                assert compound_to_conc[compound].check(ureg.molar)
    ...
    def physiological_dg_correction(self) -> float:
        ...
        """
        return self._GetSumCoeff() * np.log(physiological_concentration /
                                            standard_concentration)
