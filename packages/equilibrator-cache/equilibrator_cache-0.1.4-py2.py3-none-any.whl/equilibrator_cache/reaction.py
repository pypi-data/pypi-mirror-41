...
from .thermodynamic_constants import POSSIBLE_REACTION_ARROWS
...
class Reaction(object):
    ...
    def keys_without_water_and_protons(self):
        return (set(self.keys()) - {Reaction.WATER_COMPOUND,
    ...
    def get_transform_ddG0(self, pH, ionic_strength, T):
        ...
        for compound, coeff in self.items():
            ...
            ddG0_forward += coeff * compound.transform(pH, ionic_strength, T)
        return ddG0_forward
    ...
    def dG_correction(self, compound_to_conc: Dict[Compound, float]) -> float:
        """
            ...
            Returns:
                dG_correction - in units of RT
        """
        compounds = set(self.keys_without_water_and_protons())
        compounds = compounds.intersection(compound_to_conc.keys())
        dG_correction = sum([self.get_coeff(c) * np.log(compound_to_conc[c])
                             for c in compounds])
    ...
    def _GetSumCoeff(self) -> float:
        ...
        """
        sum_coeff = sum(map(self.get_coeff,
    ...
    def dGm_correction(self) -> float:
        """
            Calculate the dG' in typical physiological concentrations (1 mM)
        """
        return self._GetSumCoeff() * np.log(1e-3)
...
def create_stoichiometric_matrix_from_reactions(
    ...
    S = pd.DataFrame.from_records(
        map(lambda r: r.sparse, reactions)
    ).fillna(0).T
