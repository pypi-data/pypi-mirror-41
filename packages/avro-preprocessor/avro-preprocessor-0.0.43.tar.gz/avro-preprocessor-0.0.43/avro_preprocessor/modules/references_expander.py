...
class ReferencesExpander(PreprocessorModule):
    ...
    def expand_ref_in_record(self, node: Avro.Node, parent: Avro.ParentNode, key: Avro.Key) -> bool:
        """
        Expands references in an Avro Node.
        :param node: The node
        :param parent: The parent node of node
        :param key: The key of node in parent
        """
        if parent is not None and key is not None and isinstance(node, str):
            original_value = parent[key]
            modified_value = self.do_expand(node)

            if modified_value != original_value:
                parent[key] = modified_value
                return True
