...
class DocumentationChecker(PreprocessorModule):
    ...
    def check_documentation(self, schema: Avro.Node, _: Avro.ParentNode, __: Avro.Key) -> None:
        """
        ...
        if isinstance(schema, OrderedDict):
            ...
            if Avro.Doc in schema:
                if Avro.Name not in schema and Avro.Protocol not in schema:
                ...
                if Avro.Name in schema or Avro.Protocol in schema:
                    self.failure_due_to_naming = True
                    print(
