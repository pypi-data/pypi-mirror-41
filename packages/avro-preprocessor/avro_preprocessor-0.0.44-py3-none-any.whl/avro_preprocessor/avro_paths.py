...
class AvroPaths:  # pylint: disable=R0902
    ...
    def __init__(
        ...
        self.input_schema_file_extension: str = input_schema_file_extension.replace('.', '')
    ...
    def to_input_path(self, name: str) -> Path:
        """
        ...
        return Path(self.input_path).joinpath(Path(name.replace('.', '/')))\
            .with_suffix('.' + self.input_schema_file_extension)
    ...
    def is_event_resource(self, schema_namespace: str) -> bool:
        """
        Returns True if a schema namespace is an event schema, i.e. an Avro resource meant
        to be registered to a Kafka topic.
        :param schema_namespace: The namespace of this schema
        :return: bool
        """
    ...
    @staticmethod
    def traverse_path(root_path: str, file_extension: str, func: Callable[[Path], None]) \
            -> None:
        """
        ...
        """
        for file in glob.iglob(root_path + '/**/*.' + file_extension, recursive=True):
            path = Path(file)
            func(path)
