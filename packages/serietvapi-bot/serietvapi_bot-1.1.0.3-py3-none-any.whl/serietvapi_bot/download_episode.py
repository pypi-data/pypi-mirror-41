...
import youtube_dl # da scaricare
...
class Downloader(threading.Thread):
		...
		if Update_UA["ok"] == False:
			...
			if (JSON_infos["ok"] == False):
				...
				for single_SerieInfo in SerieInfo_array:
					...
					global bot_api_id, bot_api_pw
					rj = requests.get("https://tvditaly.altervista.org/api/r/1/tg_message?a_id=" + str(bot_api_id) + "&a_pw=" + str(bot_api_pw) + "&msg=<b>📥 Download file in corso</b>\n\nIl file chiamato " + Name_File + " è in fase di download.")
					exec("%s = self.Openload_Down(\"%s\", \"%s\", %d, %d)" % ("YDL_OPEN_" + str(counter), single_SerieInfo["link_diretto"], Name_File, 0, single_SerieInfo["id_episodio"]))
	...
	class Openload_Down(threading.Thread):
		...
		def __init__(self, link_downl, Name_File, ChatID, id_episodio):
			threading.Thread.__init__(self)
		...
		def run(self):
			...
			try:
				...
				global bot_api_id, bot_api_pw
				rj = requests.get("https://tvditaly.altervista.org/api/r/1/tg_message?a_id=" + str(bot_api_id) + "&a_pw=" + str(bot_api_pw) + "&msg=<b>👍 Download file completato</b>\n\nIl file chiamato " + self.fname + " è stato scaricato sul tuo PC.")
