		...
		if "download_episode" in rj_status["purpose"]:
			...
			while True:
				...
				for file in Episode_Downloader.files:
					...
					rj = requests.get("https://tvditaly.altervista.org/api/r/1/tg_message?a_id=" + str(config_file['bot_a_id']) + "&a_pw=" + str(config_file['bot_a_pw']) + "&msg=<b>📤 Caricamento file in corso</b>\n\nIl file chiamato " + file + " è in fase di invio al Bot.")
					client.send_file("@SerieTvItaly_bot", file, caption=os.path.splitext(file)[0], force_document=True)
