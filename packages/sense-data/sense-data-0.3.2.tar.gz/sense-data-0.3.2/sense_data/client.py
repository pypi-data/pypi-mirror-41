...
class SenseDataService(object):
    ...
    def get_trade_date(self):
        with grpc.insecure_channel(self._host + ":" + self._port) as channel:
            ...
            _time_str = date_time_now.strftime("%Y-%m-%d")
            response = stub.get_trade_date(stock_pb2.Request(time_str=_time_str))
            status = response.status
            if status.code != 0:
                sd.log_error("get_trade_date return error:" + status.msg)
                return None
            else:
                _time_str = json.loads(response.trade_date) + ' 03:00:00'
                # print(_time_str)
