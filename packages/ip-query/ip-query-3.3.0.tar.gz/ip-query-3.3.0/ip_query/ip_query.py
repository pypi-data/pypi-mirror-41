...
def geoip(ip_address):
    ...
    try:
        # country
        with geoip2.database.Reader(os.path.join(script_dir, 'GeoLite2-Country.mmdb')) as reader:
            resp = reader.country(ip_address)
            data['country'] = resp.country.name
            data['country_code'] = resp.country.iso_code

        # asn
        with geoip2.database.Reader(os.path.join(script_dir, 'GeoLite2-ASN.mmdb')) as reader:
