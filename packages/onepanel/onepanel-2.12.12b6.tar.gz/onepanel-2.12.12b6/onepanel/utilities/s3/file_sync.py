...
class FileSynchronizer:
    ...
    def _synchronize_local_master(self, file_difference):
        if file_difference.state == FileDifference.State.NEW:
            ...
            self.s3_wrapper.move_file(file_difference.original_destination_path, file_difference.destination_path)

    def _synchronize_remote_master(self, file_difference):
        if file_difference.state == FileDifference.State.NEW:
            self.s3_wrapper.download_file(file_difference.source_path, file_difference.destination_path)
        elif file_difference.state == FileDifference.State.MODIFIED:
...
class ThreadedFileSynchronizer:
    ...
    def synchronize_single(self, file_difference):
        # self.executor.submit(self.synchronizer.synchronize_single, file_difference)
        self.synchronizer.synchronize_single(file_difference)
