...
from onepanel.utilities.s3.file_sync import ThreadedFileSynchronizer
...
@datasets.command('sync', help='Synchronizes the dataset from your machine to onepanel datasets')
@click.option(
    '-y',
    '--skip_check',
    type=bool,
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help='Use this flag to indicate that local files to be synced according to remote.')
@click.option(
    '-t',
    '--threads',
    type=int,
    help='Number of threads to devote to synchronization.')
@click.pass_context
@login_required
def sync(ctx, skip_check, remote_master, threads):
    ctx.obj['vc'] = DatasetViewController(ctx.obj['connection'])
    vc = ctx.obj['vc']
    vc.init_credentials_retrieval()
    vc.init_endpoint()
    ...
    synchronizer.synchronize(differences.values(), print_status(cwd_length, master))
    synchronizer.shutdown()

    vc.mark_version_dirty(current_version['data']['version']['version'])


def watch_sync(ctx, threads=10):
    ...
    observer.schedule(FileWatchSynchronizerEventHandler(cwd, dataset_path, synchronizer, print_status(cwd_length)),
...
def print_status(cwd_length, master=FileSynchronizer.LOCAL):
    ...
    """
    def print_status_internal(file_difference):
        ...
        action = 'Uploading'
        if master == FileSynchronizer.REMOTE:
            action = 'Downloading'
