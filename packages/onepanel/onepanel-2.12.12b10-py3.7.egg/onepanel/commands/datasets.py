...
def _watch_sync(ctx, threads=10):
    ...
    synchronizer = ThreadedFileSynchronizer(synchronizer, threads, print_status(cwd_length))

    observer = Observer()

    observer.schedule(FileWatchSynchronizerEventHandler(cwd, dataset_path, synchronizer, ignore_patterns=['.*', '*.swx', '*.swpx', '*.swp', '*~']),
