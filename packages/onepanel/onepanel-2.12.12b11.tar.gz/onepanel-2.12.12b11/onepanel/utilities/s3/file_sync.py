...
class FileSynchronizer:
    ...
    def synchronize(self, file_differences, hook=None):
        for difference in file_differences:
            self.synchronize_single(difference, hook)

    def synchronize_single(self, file_difference, hook=None):
        """
        ...
        """
        if hook is not None:
            hook(FileEvent(FileEvent.START, file_difference))
        try:
            if self.master == FileSynchronizer.LOCAL:
            ...
            if hook is not None:
                hook(FileEvent(FileEvent.FINISHED, file_difference, result))
        except BaseException as exception:
            if hook is not None:
                hook(FileEvent(FileEvent.FAILED, file_difference, exception))
...
class ThreadedFileSynchronizer:
    def __init__(self, synchronizer, max_workers=10, hook=None):
    ...
    def synchronize_single(self, file_difference):
        ...
        :return:
        """
        # Check if there's a conflict or dependency
        if file_difference.state == FileDifference.State.MODIFIED or file_difference.state == FileDifference.State.MOVED:
            for current_task in self.active_items.values():
                if (current_task.state == FileDifference.State.NEW or
                    current_task.state == FileDifference.State.MODIFIED) and \
                        current_task.original_source_path == file_difference.original_source_path:
    ...
    def _execute_item(self, file_difference):
        task = self.executor.submit(self.synchronizer.synchronize_single, file_difference, self.hook)
    ...
    def _add_to_backlog(self, file_difference):
        ...
        if key in self.backlog:
            self.backlog[key].push(file_difference)
            return
    ...
    def _get_backlog(self, file_difference):
        ...
        for item in todo:
            # We do all modifications before move for consistency of file data
            if item.state == FileDifference.State.MODIFIED:
                selected_item = item

        # No files modified, so they are all some other state and we don't distinguish between these
        if selected_item is None:
            selected_item = todo[0]

        todo.remove(selected_item)
