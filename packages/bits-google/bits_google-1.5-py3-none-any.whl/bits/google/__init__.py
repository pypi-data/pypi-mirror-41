...
class Google(object):
    ...
    # Helper Functions
    #
    def check_billing_labels(self, projects, projects2, billing_accounts, credentials):
        ...
        for projectId in sorted(projects):
            ...
            if billing_label != billing_account:
                ...
                # get project
                # project = self.crm().get_project(projectId, credentials)
                ...
                # update project
                self.crm().update_project(projectId, project, credentials)

    def check_costobject_labels(self, projects, projects2, billing_accounts, credentials):
    ...
    def check_project_folders(self, projects, projects2, folders, billing_accounts, credentials):
        ...
        for projectId in sorted(projects):
            ...
            if move and folder:
                ...
                self.crm().update_project(projectId, p, credentials)
    ...
    def getBillingAccounts(self, projects=False, iampolicy=False):
        ...
        credentials = self.auth_service_account(self.scopes, self.subject)
        billing_accounts = self.billing().get_billing_accounts_dict(credentials)
    ...
    def getBillingAccountProjects(self, name):
        ...
        credentials = self.auth_service_account(self.scopes, self.subject)
        return self.billing().get_billing_account_projects(name, credentials)

    def getCalendar(self, calendarId):
        """Return a calendar."""
        credentials = self.auth_service_account(self.scopes, self.subject)
        return self.calendar().get_calendar(calendarId, credentials)
    ...
    def getAllCalendars(self):
        ...
        for uid in sorted(
            ...
            credentials = self.auth_service_account(self.scopes, email)
            try:
                user_calendars = self.calendar().get_calendar_list(credentials)
            except Exception:
                user_calendars = []
    ...
    def getAllServiceAccounts(self):
        ...
        for p in sorted(self.projects):
            ...
            if project['lifecycleState'] == 'DELETE_REQUESTED':
                continue
            accounts = self.iam().get_service_accounts_dict(p, credentials)
    ...
    def addDriveFileToFolder(self, userKey, fileId, folderId):
        ...
        credentials = self.auth_service_account(self.scopes, userKey)
        return self.drive().add_parents(fileId, folderId, credentials)

    def changeDriveFileOwner(self, userKey, fileId, owner):
        """Change the owner of a drive file."""
        credentials = self.auth_service_account(self.scopes, userKey)
        return self.drive().change_owner(fileId, owner, credentials)

    def createDriveFolder(self, userKey, folder):
        """Create a drive folder."""
        credentials = self.auth_service_account(self.scopes, userKey)
        return self.drive().create_folder(folder, credentials)

    def getDriveAbout(self, userKey, fields=None):
        """Return the Google Drive About information for a user."""
        credentials = self.auth_service_account(self.scopes, userKey)
        return self.drive().get_about(credentials, fields)

    def getDriveFiles(self, userKey, fields=None, orderBy=None, q=None):
        """Get the list of files for a user query."""
        credentials = self.auth_service_account(self.scopes, userKey)
        return self.drive().get_files(credentials, fields, orderBy, q)
    ...
    def getGroup(self, groupKey):
        ...
        credentials = self.auth_service_account(self.scopes, self.subject)
        group = self.directory().get_group(groupKey, credentials)
        members = self.directory().get_members(groupKey, credentials)
    ...
    def getGroups(self, memberKey=None):
        ...
        credentials = self.auth_service_account(self.scopes, self.subject)
        self.groups = self.directory().get_groups_dict(credentials, userKey=memberKey)
        return self.groups
    ...
    def getAllGroupsSettings(self):
        ...
        credentials = self.auth_service_account(self.scopes, self.subject)
        groups = self.directory().get_groups_dict(credentials)
        groups_settings = {}
        # if self.verbose:
    ...
    def getGroupSettings(self, groupKey):
        ...
        credentials = self.auth_service_account(self.scopes, self.subject)
        return self.groupssettings().get_group_settings(groupKey, credentials)
    ...
    def addMember(self, groupKey, memberKey):
        """Add a group member."""
        credentials = self.auth_service_account(self.scopes, self.subject)
        return self.directory().add_member(groupKey, memberKey, credentials)
    ...
    def getMember(self, groupKey, memberKey):
        """Return a group member."""
        credentials = self.auth_service_account(self.scopes, self.subject)
        return self.directory().get_member(groupKey, memberKey, credentials)

    def getMembers(self, groupKey):
        """Return members of a group."""
        credentials = self.auth_service_account(self.scopes, self.subject)
        return self.directory().get_members(groupKey, credentials)
    ...
    def removeMember(self, groupKey, memberKey):
        ...
        credentials = self.auth_service_account(self.scopes, self.subject)
        try:
            return self.directory().remove_member(groupKey, memberKey, credentials)
        except Exception as e:
            print(e)
            return None
    ...
    def createUser(self, user, password, show_error=True):
        ...
        try:
            return self.directory().create_user(body, credentials)
        except Exception:
            if show_error:
    ...
    def deleteUser(self, username, show_error=True):
        ...
        credentials = self.auth_service_account(self.scopes, self.subject)
        try:
            return self.directory().delete_user(username, credentials)
        except Exception:
            if show_error:
    ...
    def getUser(self, username, show_error=True):
        ...
        credentials = self.auth_service_account(self.scopes, self.subject)
        try:
            return self.directory().get_user(username, credentials)
        except Exception:
            if show_error:
    ...
    def patchUser(self, username, body={}):
        """Patch a user."""
        credentials = self.auth_service_account(self.scopes, self.subject)
        return self.directory().patch_user(username, credentials, body)

    def suspendUser(self, username):
        """Suspend a user."""
        credentials = self.auth_service_account(self.scopes, self.subject)
        return self.directory().suspend_user(username, credentials)

    def unsuspendUser(self, username):
        """Unsuspend a user."""
        credentials = self.auth_service_account(self.scopes, self.subject)
        return self.directory().unsuspend_user(username, credentials)
