...
class Prosumer(Consumer, Producer):
    ...
    **Description**
    ...
    def maximize_conso_prod_match(self,
        ...
        """
        final_operated_consumption_unit_list = self._check_operated_unit_list(
            obj_operated_consumption_unit_list)
        final_operated_production_unit_list = self._check_operated_unit_list(
        ...
        print(self.calc_conso_prod_match.exp)
    ...
    def maximize_selfconsumption_rate(self, time,
        ...
        of the prosumer's units (all or part of them) with maximizing the
        load matching
        selfconsummed production is calculated with the export nodes
        ...
        production
        exports from nodes on which the objective will be applied. Might be
        empty.
        :param obj_operated_selfconsummed_production_unit_list: List of
        production units on which the objective will be applied. Might be empty.
        :param weight: Weight coefficient for the objective
        """
        ...
        self.conso_prod_match_SC_rate = Quantity(
            name='conso_prod_match_SC_rate', description='conso_prod '
                                                         'difference to '
                                                         'maximize '
                                                         'selfconsumption rate',
            vlen=time.LEN, parent=self)

        self.calc_conso_prod_match_SC_rate = DynamicConstraint(
            name='calc_conso_prod_match_SC_rate',
            exp_t='{0}_conso_prod_match_SC_rate[t] == ({1} + {2} - ( {3} '
                  '))'.format(self.name, sc_production_export_p_string,
                              sc_production_unit_p_string, production_p_string),
            t_range='for t in time.I', parent=self)

        self.conso_prod_match_SC_rate_abs = def_abs_value(
            self.conso_prod_match_SC_rate, q_min=-1e+5, q_max=1e+5)

        self.max_conso_prod_match_SC_rate_pos = Objective(
            name='max_conso_prod_match_SC_rate_pos',
            exp='lpSum({}_conso_prod_match_SC_rate_pos[t] for t in time.I)'
                .format(self.name),
            weight=weight, parent=self)

        self.max_conso_prod_match_SC_rate_neg = Objective(
            name='max_conso_prod_match_SC_rate_neg',
            exp='lpSum({}_conso_prod_match_SC_rate_neg[t] for t in time.I)'
                .format(self.name),
            weight=weight, parent=self)
    ...
    def maximize_selfproduction_rate(self, time,
        ...
        load matching
        selfproduced consumption may required export nodes
        ...
        production exports from nodes on which the objective will be applied.
        Might be empty.
        ...
        self.conso_prod_match_sp_rate = Quantity(
            name='conso_prod_match_sp_rate', description='conso_prod '
                                                         'difference to '
                                                         'maximize '
                                                         'selfconsumption rate',
            vlen=time.LEN, parent=self)

        self.calc_conso_prod_match_sp_rate = DynamicConstraint(
            name='calc_conso_prod_match_sp_rate',
            exp_t='{0}_conso_prod_match_sp_rate[t] == ({1} + {2} - ( {3} '
                  '))'.format(
                self.name, sp_consumption_export_p_string,
                sp_consumption_unit_p_string, consumption_p_string),
            t_range='for t in time.I', parent=self)

        self.conso_prod_match_sp_rate_abs = def_abs_value(
            self.conso_prod_match_sp_rate, q_min=-1e+5, q_max=1e+5)

        self.max_conso_prod_match_sp_rate_pos = Objective(
            name='max_conso_prod_match_sp_rate_pos',
            exp='lpSum({}_conso_prod_match_sp_rate_pos[t] for t in time.I)'
                .format(self.name),
            weight=weight, parent=self)

        self.max_conso_prod_match_sp_rate_neg = Objective(
            name='max_conso_prod_match_sp_rate_neg',
            exp='lpSum({}_conso_prod_match_sp_rate_neg[t] for t in time.I)'
                .format(self.name),
            weight=weight, parent=self)
