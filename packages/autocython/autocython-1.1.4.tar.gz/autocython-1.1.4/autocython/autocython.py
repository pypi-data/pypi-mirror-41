...
if PY2:
    PLATFORM_SUFFIX = b'_py27_{}_{}'.format(sys.platform, platform.architecture()[0])
else:
    PLATFORM_SUFFIX = ''

COMPILE_STATE = 'autocython_compile_state.json'
...
def get_last_compile_state(folder):
    try:
        with open(os.path.join(folder, COMPILE_STATE), 'r') as f:
            return json.load(f)
...
def save_compile_state(folder, compile_state):
    with open(os.path.join(folder, COMPILE_STATE), 'w', newline='\n') as f:
