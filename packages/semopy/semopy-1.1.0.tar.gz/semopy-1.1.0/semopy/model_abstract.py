...
class ModelABC(ABC):
    ...
    def load_dataset(self, data: DataFrame, center=True, ordcor=True):
        ...
        for v in self.vars['IndsObs']:
            ...
            if num_uniqs < data.shape[0] / 3:
                if num_uniqs == 1:
                    s = "Warning: variable {} attains only one value".format(v)
                    print(s)                   
                self.vars['Categorical'].append(v)
        if not self.vars['Categorical'] or not ordcor:
            if center:
                data -= data.mean()
