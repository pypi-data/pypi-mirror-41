...
class BaseHandler(RequestHandler):
    ...
    async def spawn_single_user(self, user, server_name='', selectedimage=None, options=None):
        ...
        spawn_future = user.spawn(server_name, selectedimage, options)
...
class UserImageHandler(BaseHandler):
    ...
    def _render(self, username):
        user = self.user_from_username(username)
        image_url = url_path_join(
                        user.settings.get('base_url', '/'), 'hub', 'image', 'user', user.escaped_name) + '/'
    ...
    async def post(self, user_name, user_path):
        ...
        self.log.warning("data is: %s", data)
        ...
        if should_spawn:
            ...
            if status is not None:
                if spawner.options_form:
                    ...
                    await self.spawn_single_user(user, selectedimage=selectedimage)
