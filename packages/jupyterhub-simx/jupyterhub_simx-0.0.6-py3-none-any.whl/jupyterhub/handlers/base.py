...
class BaseHandler(RequestHandler):
    ...
    def get_next_url(self, user=None, select_image=False):
            ...
            if user and self.redirect_to_server:
                if select_image:
                    next_url = url_path_join(
                        user.settings.get('base_url', '/'), 'image', 'user', user.escaped_name) + '/'
                else:
                    next_url = user.url
            else:
    ...
    async def spawn_single_user(self, user, server_name='', options=None, select_image=None):
        self.log.warning("select image is " + select_image)
        ...
        spawn_future = user.spawn(server_name, options, select_image)
...
class UserImageHandler(BaseHandler):
    ...
    def _render(self, username):
        user = self.user_from_username(username)
        image_url = url_path_join(user.settings.get('base_url', '/'), 'image', 'user', user.escaped_name) + '/'
        return self.render_template('image.html',
    ...
    async def get(self, user_name, user_path):
        ...
        self.finish(self._render(username=user_name))

    # async def post(self, user_name, user_path):
    #     # parse the arguments dict
    #     data = {}
    #     for arg in self.request.arguments:
    #         data[arg] = self.get_argument(arg, strip=False)
    #
    #     select_image = data['select_image']
    #
    #     if not user_path:
    #         user_path = '/'
    ...
    #                 return
    #             else:
    #                 self.log.warning("begin to spawn single user server!!!!!!!!!!!")
    #                 await self.spawn_single_user(user, select_image=select_image)
    #
