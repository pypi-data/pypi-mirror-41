...
class BaseHandler(RequestHandler):
    ...
    def get_next_url(self, user=None, select_image= False):
        ...
        if not next_url:
            ...
            if user and self.redirect_to_server:
                if select_image:
                    next_url = url_path_join(user.url, "image")
                else:
                    next_url = user.url
    ...
    async def spawn_single_user(self, user, server_name='', select_image=None,  options=None):
        ...
        spawn_future = user.spawn(server_name, select_image, options, handler=self)
...
class UserImageHandler(BaseHandler):
    ...
    def _render(self, username):
        ...
        return self.render_template('image.html',
                                    ...
                                    image_url=url_path_join(user.url, "image"),
                                    )
...
default_handlers = [
    ...
    (r'/user/(?P<user_name>[^/]+)(/.*)?/image)?', UserImageHandler),
