...
ISBN10_RE = r'(?P<isbn10>(\d(-| )?){9}(x|X|\d))'
ISBN10_NRE = r'(\d(-| )?){9}(x|X|\d)'

ISBN13_RE = r'(?P<isbn13>(\d(-| )?){12}(\d))'
ISBN13_NRE = r'(\d(-| )?){12}(\d)'


def _normalize(str):
    if not re.match(r'^(\d(-| )?){9}(x|X|\d|(\d(-| )?){3}\d)$', str):
        raise ISBNError("Invalid ISBN format: {}".format(str))

    return re.sub(r'[^0-9X]', '', str.upper())
