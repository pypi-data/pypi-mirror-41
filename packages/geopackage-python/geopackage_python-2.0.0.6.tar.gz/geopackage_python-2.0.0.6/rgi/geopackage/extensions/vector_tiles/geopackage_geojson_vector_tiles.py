...
from rgi.geopackage.extensions.vector_tiles.geopackage_vector_tiles import GeoPackageVectorTiles
from rgi.geopackage.extensions.vector_tiles.vector_fields.geopackage_vector_fields import GeoPackageVectorFields
from rgi.geopackage.extensions.vector_tiles.vector_fields.vector_fields_entry import VectorFieldsEntry, VectorFieldType
from rgi.geopackage.extensions.vector_tiles.vector_layers.geopackage_vector_layers import GeoPackageVectorLayers
...
class GeoPackageGeoJSONVectorTiles(GeoPackageVectorTiles):
    ...
    def insert_vector_layers_and_fields_from_tile_data(cursor,
        ...
        for feature in data['features']:
            for json_property in feature['properties']:
                ...
                vector_field_entry = VectorFieldsEntry(layer_id=layer_entry.id,
                                                       name=json_property,
                                                       field_type=VectorFieldType.convert_string_value_to_vector_field_type(value))

                GeoPackageVectorFields.insert_or_update_vector_tile_field_entry(cursor=cursor,
                                                                                vector_tiles_field_entry=vector_field_entry)
                vector_fields_entries.append(vector_field_entry)
    ...
    def create_pyramid_user_data_table(cls,
        ...
        super(GeoPackageGeoJSONVectorTiles, cls).create_pyramid_user_data_table(cursor=cursor,
                                                                                tiles_content=tiles_content)

        GeoPackageExtensions.insert_or_update_extensions_row(cursor=cursor,
                                                             extension=
                                                             GeoPackageGeoJSONVectorTiles(tiles_content.table_name))
