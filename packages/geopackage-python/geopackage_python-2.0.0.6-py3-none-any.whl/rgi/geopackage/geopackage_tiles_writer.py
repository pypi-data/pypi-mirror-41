...
from rgi.geopackage.extensions.geopackage_extensions import GeoPackageExtensions
...
from rgi.geopackage.tiles.geopackage_tiles import GeoPackageAbstractTiles
...
from rgi.geopackage.utility.sql_utility import get_database_connection, table_exists, synchronized
...
class GeoPackageTilesWriter(object):
    ...
    def clean(self):
        ...
        """
        self.__database_connection().cursor().execute("VACUUM")
    ...
    def __init__(self,
        ...
        if not path.exists(gpkg_file_path):
            ...
            if not path.exists(dirname(gpkg_file_path)):
        ...
        existing_entry = GeoPackageCore.get_content_entry_by_identifier(cursor=self.__database_connection().cursor(),
    ...
    def add_tiles(self,
        ...
        contents_tile_type = self.__get_tile_type_by_contents_entry(self.__database_connection(), self.tile_table_name)
    ...
    def remove_tile(self,
        ...
        gpkg_tiles_creator = self.__get_tiles_creator_by_contents_entry(self.__database_connection(),
                                                                        self.tile_table_name)
        # have to know what type of tile_data is the tile, and if it exists
        gpkg_tile_data = gpkg_tiles_creator.get_tile_data(cursor=self.__database_connection().cursor(),
                                                          table_name=self.tile_table_name,
                                                          zoom_level=tile.zoom_level,
                                                          tile_column=tile.tile_column,
                                                          tile_row=tile.tile_row)
        # no tile to delete
        if gpkg_tile_data is None:
            return

        gpkg_tiles_creator.delete_tile_data(cursor=self.__database_connection().cursor(),
                                            table_name=self.tile_table_name,
                                            zoom_level=tile.zoom_level,
                                            tile_row=tile.tile_row,
                                            tile_column=tile.tile_column)
    ...
    def remove_zoom_levels(self,
        ...
        for zoom_level in zoom_levels:
            gpkg_tiles_creator.delete_zoom_level(cursor=self.__database_connection().cursor(),
                                                 tile_table_name=self.tile_table_name,
    ...
    def get_all_tiles(self,
        ...
        gpkg_tiles_creator = self.__get_tiles_creator_by_contents_entry(self.__database_connection(),
                                                                        self.tile_table_name)
        return gpkg_tiles_creator.get_all_tile_data(cursor=self.__database_connection().cursor(),
                                                    tile_table_name=self.tile_table_name,
                                                    tiles=tiles)
    ...
    def get_gpkg_contents_entry(self):
        ...
        """
        return GeoPackageCore.get_content_entry_by_table_name(cursor=self.__database_connection().cursor(),
                                                              table_name=self.tile_table_name)
    ...
    def get_srs_entry_by_id(self,
        ...
        """
        return GeoPackageCore.get_spatial_reference_system_by_srs_id(cursor=self.__database_connection().cursor(),
                                                                     srs_id=srs_id)
    ...
    def does_tiles_table_exist(self):
        ...
        return table_exists(self.__database_connection().cursor(),
                            self.tile_table_name)
    ...
    def __add_default_tables(self,
        ...
        if GeoPackageCore.get_content_entry_by_table_name(cursor=self.__database_connection().cursor(),
            ...
            GeoPackageCore.insert_or_update_content(cursor=self.__database_connection().cursor(),
                                                    content=tiles_content)

        # if the tiles table does not have a an entry in the gpkg_tile_matrix_set table, create an entry
        if gpkg_tiles_creator.get_tile_matrix_set_entry_by_table_name(cursor=self.__database_connection().cursor(),
    ...
    def __get_tiles_creator_by_contents_entry(cls, database_connection, tile_table_name):
        ...
        # check if vector-tiles- geojson
            ...
            gpkg_tiles_creator = GeoPackageGeoJSONVectorTiles(vector_tiles_table_name=tile_table_name)
        # check if vector-tiles- Mapbox
        elif tile_type == TileType.MAPBOX:

            gpkg_tiles_creator = GeoPackageMapBoxVectorTiles(vector_tiles_table_name=tile_table_name)
    ...
    def __get_tile_type_by_contents_entry(cls, database_connection, tile_table_name):
        ...
        :rtype: TileType, None
        """
        content_entry = GeoPackageCore.get_content_entry_by_table_name(cursor=database_connection.cursor(),
                                                                       table_name=tile_table_name)
        ...
        # check if vector-tiles- geojson
        elif GeoPackageExtensions.has_extension(cursor=database_connection.cursor(),
                                                extension=GeoPackageGeoJSONVectorTiles(
                                                    vector_tiles_table_name=tile_table_name)):

            return TileType.GEOJSON
        # check if vector-tiles- Mapbox
        elif GeoPackageExtensions.has_extension(cursor=database_connection.cursor(),
                                                extension=GeoPackageMapBoxVectorTiles(
                                                    vector_tiles_table_name=tile_table_name)):
            return TileType.MAPBOX
        else:
            raise ValueError("The existing entry in the contents table has an unsupported tiles type.")
