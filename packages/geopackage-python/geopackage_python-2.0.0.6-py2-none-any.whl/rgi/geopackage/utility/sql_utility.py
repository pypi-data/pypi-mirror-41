...
def delete_row(cursor,
               ...
               sql_columns_list):
    """
    Deletes a single row from the table given.  If more than one row is returned or if no rows match the where condition
    ...
    # check to make sure only one row is returned to delete
    if existing_row is None or len(existing_row) == 0:
        raise ValueError("No rows exist with those conditions")
    elif len(existing_row) > 1:
        raise ValueError("More than one row exists with those conditions.")
    else:
        # if only one row is returned, delete that row
...
def delete_rows(cursor,
                ...
                sql_columns_list):
    """
    Deletes all rows from the table given that matches the where condition. If no rows match the where condition an
    ...
    where_dictionary = {column.column_name: column.column_value
                        ...
                        in sql_columns_list
                        if column.include_in_where_clause}
    # check which rows match the where condition
    existing_rows = select_query(cursor=cursor,
                                 table_name=table_name,
                                 select_columns=[column.column_name
                                                 for column
                                                 in sql_columns_list
    ...
    # check to make sure at least one row is returned
    if existing_rows is None or len(existing_rows) == 0:
        raise ValueError("No rows exist with those conditions")
    else:
