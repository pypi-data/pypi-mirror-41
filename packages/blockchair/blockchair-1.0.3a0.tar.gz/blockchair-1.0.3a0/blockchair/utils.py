...
DATA_TIME_CHARS_RE = re.compile(
    r'^((20|21)\d\d[-](0[1-9]|1[012])[-](0[1-9]|[12][0-9]|3[01])\s(([0,1][0-9])|(2[0-3])):[0-5][0-9]:[0-5][0-9])$|^((20|21)\d\d[-](0[1-9]|1[012])[-](0[1-9]|[12][0-9]|3[01]))$|^((20|21)\d\d[-](0[1-9]|1[012]))$'
)
...
QUERY_BLOCK_LIST_BCH_BSV = SORT_BLOCK_LIST_BCH_BSV + \
    ['coinbase_data_bin', 'coinbase_data_hex']
...
def is_valid_chain(chain):
    if not (chain is BTC or chain is BCH or chain is LTC or chain is BSV or
            chain is ETH):
        raise FormatError(
            "Incorrect chain value. Currently supports BTC, BCH, BSV, LTC, ETH."
        )
