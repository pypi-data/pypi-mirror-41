...
if TYPE_CHECKING:
    ...
    # noinspection PyUnresolvedReferences
    from ..span import SpanGeneral  # pragma: no cover
...
PointContext.__new__.__defaults__ = (None,) * 10  # type: ignore
...
StageToSections = Mapping[
    Stage,
    Sequence[Union[DestinationSection, OriginSection]]
]
"""Настройка автоматического отнесения экстрактора к той или иной фазе
если хотя бы одна из секций (источника или назначения) настройки экстрактора
относится к указанным здесь, то извлечение работает на этом этапе, при этом
POST секция пользуется преимуществом
"""
...
"""
Pipeline = Tuple[Union[Callable, PipeCommand], ...]
...
stage
    необязательное явное указание фазы, в которой будет работать данная
    настройка
destination
...
Rule.__new__.__defaults__ = (None, [],)  # type: ignore
"""Настройки по умолчанию для назначения извлекаемых данных
"""
