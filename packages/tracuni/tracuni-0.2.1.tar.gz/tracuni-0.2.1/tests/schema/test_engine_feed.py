...
# @pytest.mark.skip(reason="get out of the way of the best boosted beasts")
def test__detect_stage(simple_engine, auto_stage_rules, stage_to_sections):
    p = simple_engine
    assert (
        p.schema_feed.stage_to_sections == stage_to_sections
    ), "Используются нужные подставные данные по отношению секций к фазам"
    for stage, schema in auto_stage_rules.items():
        for rule in schema:
            assert (
                p.schema_feed._detect_stage(rule) is stage
            ), "Верное отнесение правила к фазе"
