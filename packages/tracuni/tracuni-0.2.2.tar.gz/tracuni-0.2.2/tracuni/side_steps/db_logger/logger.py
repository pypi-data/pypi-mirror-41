...
from tracuni.schema.builtin.pipe import pipe_mask_secret
...
class Logger(object):
    ...
    async def log(self, log_data):
        ...
        while True:
            try:
                transport, protocol = await aioamqp.connect(**self.config['logging'])
                break
            except (OSError, aioamqp.AmqpClosedConnection) as e:
                to_wait = round(min(30, (to_wait ** 1.5)), 2)
                logging.info("[x] Failed to connect to RabbitMQ: %s. Waiting %s seconds...", e, to_wait)
                await asyncio.sleep(to_wait)
                time_waited += to_wait

        channel = await protocol.channel()
        await channel.publish(
            pipe_mask_secret(json.dumps(log_data)),
