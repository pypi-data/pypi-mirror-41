...
def ext_http_in_tornado_request(arguments: namedtuple) -> Dict[str, Any]:
    """Получить данные входящего HTTP запроса для Tonado
...
def pipe_head(seq: Sequence[Any]) -> Any:
    ...
    """
    return pick_items.do(seq, indexes=0)


def pipe_check_skip(data: Tuple[str, 'TracerWrapperRef']):
    ...
    """
    path, tracer_wrap = data
    flag = tracer_wrap.should_not_trace(path)
    return flag
...
def pipe_catch_essential(data):
    found = catch_essential.do(data)
    return PipelineTee(
        main_story=data,
        side_story={
            DestinationSection.SPAN_TAGS: found
        },  # type: TeeDescriptor
    )


def pipe_mask_secret(data):
...
def pipe_dump(data):
    if isinstance(data, (dict, tuple, list)):
        return json_dumps_with_dt(data, indent=4)
    else:
        return data
...
def pipe_cut(data, cut_limit=CUT_LIMIT, cut_line=CUT_LINE):
    if isinstance(data, (dict, tuple, list)):
        prepared_data = json_dumps_with_dt(data, indent=4)
    else:
...
def ext_tracer_path(track, length=3):
    track_short_list = tuple(islice(
        (el.get('short_path') for el in track),
        0,
        length
    ))
    return (' > '.join(
...
def ext_sql_args(arguments):
    res = dict((k, v) for k, v in arguments._asdict().items() if k not in ('self', 'sql'))
...
def ext_sql_response(res):
    if Record and isinstance(res, Record):
        return dict(res)
    return res
