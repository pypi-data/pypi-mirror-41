...
class Garage(object):
    ...
    def _bottleNeckRes(self, sc, inputs, shallow, deep, scope, name, down_sample=False, pre_activate=True):
        ...
        if down_sample:
            ...
            stride = 1
            if pre_activate:
                idt_path = (COMP.DUPLICATE(name=basename(input), input=sc.layout[input]))
            else:
                idt_path = (COMP.CONV(name=name + '_conv_idt', input=sc.layout[input], kernel=[1, 1], out_chs=deep))

        res_path = self._residual((sc.layout[input], sc.layout[is_train]), shallow, deep, stride, name)
    ...
    def _bottleNeckSE(self, sc, inputs, shallow, deep, scope, name, down_sample=False, pre_activate=True):
        ...
        res_path = sc.assemble(self._residual((sc.layout[input], sc.layout[is_train]),
                                               shallow, deep, stride, name), sub_scope=scope)
        se_path = (COMP.DUPLICATE(name=basename(res_path), input=sc.layout[res_path])
                   * self._squeezeExicitation(sc.layout[res_path], shallow//4, deep, name))

        return sc.assemble(idt_path + se_path, sub_scope=scope)

    def _squeezeExicitation(self, input, shallow, deep, name):
        COMP = Component()
        se_block = (COMP.AVG_POOL(name=name+'_se_avg_pool', input=input, padding='valid', if_global=True)
    ...
    def se_resnet_v2_50(self, sc, inputs, scope='SE_Res_50'):
        return self._se_resnet_v2(sc, inputs, scope, [3, 4, 6, 3])

    def se_resnet_v2_101(self, sc, inputs, scope='SE_Res_101'):
        return self._se_resnet_v2(sc, inputs, scope, [3, 4, 23, 3])

    def se_resnet_v2_152(self, sc, inputs, scope='SE_Res_152'):
        return self._se_resnet_v2(sc, inputs, scope, [3, 8, 36, 3])

    def _se_resnet_v2(self, sc, inputs, scope, blocks):
        COMP = Component()
        input, is_train = inputs
        net = sc.assemble(COMP.CONV(name='conv0', input=sc.layout[input],
                                    kernel=[7, 7], stride=[2, 2], out_chs=64)
