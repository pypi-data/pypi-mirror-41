...
# XXX US23291: Add ad: domain-type support
VALID_DOMAIN_TYPES = ('local', 'world', 'ldap_user', 'ldap_group')
...
def posix_uid_to_sid(uid):
    return '{}-{}'.format(POSIX_USER_SID_PREFIX, uid)

def posix_gid_to_sid(uid):
    return '{}-{}'.format(POSIX_GROUP_SID_PREFIX, uid)


class ShareAceTranslator(AceTranslator):
    def parse_trustee(self, trustee, ace):
        '''
        Users and groups can be specified through SID, UID, GID, auth_id, or
        possibly ambiguous (exists in multiple domain types, e.g. cluster-local,
        LDAP, etc.) name.
        Names can exist in a disambiguated form narrowed down by domain-type
        and/or domain prefix (e.g. ldap_user:joe, MyDomain\joe).
        Domain-type prefixes end with a colon (:), while domain prefixes end
        with a backslash (\). A user may opt to not use a domain-type and just
        provide a name: prefix and let QFSD search various sources to resolve it
        '''
        # Attempt to split by domain- or id-type (e.g. local:, uid:)
        trustee = [i.strip() for i in trustee.split(":")]
        if len(trustee) > 2:
            raise ValueError("Trustee may not have more than one ':'")
        if len(trustee) == 1:
            # Trustee was not prefixed by type so it must be a SID or name
            trustee_capitalized = trustee[0].capitalize()
            if SID_REGEXP.match(trustee_capitalized):
                ace['trustee'] = {'sid': trustee_capitalized}
                return
            else:
                ace['trustee'] = {'name': trustee[0]}
                return

        trustee_type, trustee = trustee
        trustee_type = trustee_type.lower()
        if trustee_type in ("uid", "gid"):
            ace['trustee'] = {trustee_type: int(trustee)}
        elif trustee_type == 'auth_id':
            # NB: validate int, and allow hex if desired:
            ace['trustee'] = {trustee_type: str(int(trustee, base=0))}
        elif trustee_type in ('sid', 'name'):
            ace['trustee'] = {trustee_type: trustee}
        elif trustee_type == 'world':
            if trustee.lower() != EVERYONE_NAME.lower():
                raise ValueError('Trustee type "world" may only be used with '
                    'the Everyone trustee (i.e. world:Everyone')

            ace['trustee'] = { 'domain': WORLD_DOMAIN, 'name': EVERYONE_NAME }
        elif trustee_type == 'local':
            ace['trustee'] = { 'domain': LOCAL_DOMAIN, 'name': trustee }
        elif trustee_type == 'ldap_user':
            ace['trustee'] = { 'domain': POSIX_USER_DOMAIN, 'name': trustee }
        elif trustee_type == 'ldap_group':
            ace['trustee'] = { 'domain': POSIX_GROUP_DOMAIN, 'name': trustee }
        else:
            raise ValueError('Trustee type must be ' +
                ', '.join(['"%s"' % v for v in VALID_TRUSTEE_TYPES[:-1]]) +
                ', or "%s"' % VALID_TRUSTEE_TYPES[-1])

    def pretty_trustee(self, ace):
        # Pick an attribute.  Check in descending order of "user friendliness"
        if ace['trustee'].get('auth_id') == EVERYONE_AUTH_ID:
            return EVERYONE_NAME
        if ace['trustee'].get('auth_id') == GUEST_AUTH_ID:
            return GUEST_NAME
        if ace['trustee'].get('name') is not None:
            return ace['trustee']['name']
        if ace['trustee'].get('uid') is not None:
            return 'uid:{}'.format(ace['trustee']['uid'])
        if ace['trustee'].get('gid') is not None:
            return 'gid:{}'.format(ace['trustee']['gid'])
        if ace['trustee'].get('domain') == AD_DOMAIN:
            if 'sid' in ace['trustee']:
                return ace['trustee']['sid']
        if ace['trustee'].get('auth_id') is not None:
            return 'auth_id:{}'.format(ace['trustee']['auth_id'])
    ...
    def ace_trustee_equal(self, ace, trustee):
        ...
        parsed_ace = {}
        self.parse_trustee(trustee, parsed_ace)
        arg_trustee = parsed_ace['trustee']

        # Note that there is an invariant that each api_trustee attribute
        # uniquely identifies a fully expanded api_trustee value, so given a
        # partial trustee (parsed from the command line) you only need one
        # common attribute to determine equality.
        common = set(arg_trustee.keys()).intersection(ace['trustee'].keys())
        comp = [arg_trustee[k] == ace['trustee'][k] for k in common]
