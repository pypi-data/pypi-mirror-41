...
class FindUserCommand(qumulo.lib.opts.Subcommand):
    ...
    DESCRIPTION = 'Find all representations of a given identifier'
    ...
    def options(parser):
        ...
        parser.add_argument(
            '--auth-id', type=int, required=False,
            help='The identifier qsfs uses internally.')
        parser.add_argument(
            '--uid', type=int, required=False, help='A posix UID')
        parser.add_argument(
            '--gid', type=int, required=False, help='A posix GID')
        parser.add_argument('--sid', required=False, help='A SID')
        parser.add_argument('--name', required=False,
            help='A name of a cluster-local, AD, or LDAP user.  May be an '
            'unqualified login name, qualified with netbios name (e.g. '
            'DOMAIN\user), a universal principal name (e.g. '
            'user@domain.example.com), or an LDAP distinguished name (e.g '
