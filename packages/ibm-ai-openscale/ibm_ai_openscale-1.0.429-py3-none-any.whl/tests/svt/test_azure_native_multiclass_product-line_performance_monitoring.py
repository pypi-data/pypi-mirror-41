...
class TestAIOpenScaleClient(unittest.TestCase):
    ...
    def test_12_stats_on_payload_logging_table(self):
        ...
        self.assertIn('Scored Probabilities', str(payload_table_content))
        self.assertIn('AGE', str(payload_table_content))
        self.assertIn('PRODUCT_LINE', str(payload_table_content))
        self.assertIn('GENDER', str(payload_table_content))
        self.assertIn('MARITAL_STATUS', str(payload_table_content))
        self.assertIn('PROFESSION', str(payload_table_content))
