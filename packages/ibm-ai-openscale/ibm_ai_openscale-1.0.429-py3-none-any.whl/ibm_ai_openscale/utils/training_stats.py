...
class TrainingStats():
    ...
    def __validate_training_data_info(self):
        ...
        if self.model_type not in accepted_model_types:
            raise Exception(
                "Invalid/Missing model type. Accepted values are:" + str(accepted_model_types))
    ...
    def __validate_fairness_inputs(self):
        ...
        if fairness_attributes is None or len(fairness_attributes)==0:
            raise Exception("'fairness_attributes' attribute is either missing or has empty value under fairness_inputs")
        ...
        if self.model_type != "regression":
            ...
            if (favourable_class is not None):
                ...
                if isinstance(favourable_class[0],(list,tuple)):
                    raise Exception("'favourable_class' in fairness_inputs should not be a list of lists. It should be a list of values.")
            else:
            ...
            if (unfavourable_class is not None):
                ...
                if isinstance(unfavourable_class[0],(list,tuple)):
                    raise Exception("'unfavourable_class' in fairness_inputs should not be a list of lists. It should be a list of values.")
            else:
            ...
            if(favourable_class is None):
                raise Exception("'favourable_class' attribute is required in fairness_inputs in case of regression model.")
            else:
            ...
            if(unfavourable_class is None):
                raise Exception("'unfavourable_class' attribute is required in fairness_inputs in case of regression model.")
            else:
    ...
    def __validate_maj_min(self, feature_name, maj_min,type):
        if maj_min is None or maj_min == '' or maj_min == []:
            error_msg = "Missing required field: You haven't specified {0} value for the feature '{1}'.".format(type, feature_name)
            raise Exception(error_msg)
        if not isinstance(maj_min, list):
            error_msg = "Invalid syntax: The {0} value for feature '{1}' must be specified as a list of categorical values or numerical ranges.".format(type, feature_name)
            raise Exception(error_msg)
    ...
    def __getDistribution(self, dataset,inputs): 
        ...
        for protected_attribute in inputs['fairness_attributes']: 
            distribution_map[protected_attribute]=self.__modify_distribution(attributes_dataset,protected_attribute,class_dataset)
    ...
    def __computeTrainingDataDistribution(self):
        ...
        try:    
            ...
            class_label = self.training_data_info.get("class_label")
            ...
            if self.model_type is not None and self.model_type=="regression":
                ...
                distribution_data = self.__compute_regression_training_distribution(data,  fairness_attributes_list, favourable_unfavourable_class, feature_data_types)
            else:
                distribution_data =  self.__compute_training_distribution(data_frame, data, training_data_info, fairness_attributes, feature_data_types ) 
            ...
            if self.model_type is None or self.model_type!="regression":
    ...
    def __compute_regression_training_distribution(self, data, fairness_attributes, favourable_classes,feature_data_types):
        distribution_data = []    
        distinct_data = {}
    ...
    def __get_boundaries(self, majority, minority, data_type,bucket_size):
        values = {}
        boundaries = []
        if data_type is not None:    
            ...
            if not data_type is None and data_type=="int":    
                ...
                range = self.__truncate(range_cnt,5)
            ...
            if not data_type is None and data_type=="int":
                ...
                next_start = self.__truncate(temp,5)
            ...
            while temp <= max:
                ...
                if not data_type is None and data_type=="int":
                    ...
                    start = next_start  
                    end = self.__truncate(temp+range,5)
    
                    if (idx<(length_sorted_values)):
                        sorted_value = self.__truncate(sorted_values[idx],5)
    
                        if(sorted_value == end):
                            idx+=1
                            next_start = self.__truncate(temp+range,5)
                            boundary = [start,end]
                            boundaries.append(boundary)
                            temp = temp + range
    ...
    def __compute_training_distribution(self, payload_df,data, request_payload , feature_attributs, feature_data_types):  
        ...
        for feature in feature_attributs[:]:
            ...
            if data_type is not None:
                ...
                if is_numeric and len(key_values)<bucket_size:
                    ...
                    boundaries = self.__get_boundaries(majority, minority, data_type, bucket_size)    
