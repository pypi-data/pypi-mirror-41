...
class Bindings:
    ...
    def get_asset_deployment_details(self):
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            clients = [(client) for client in clients]
            clients_artifacts = executor.map(lambda client: (client, client.get_artifacts()), clients)
            clients_artifacts = [(pair[0], el) for pair in clients_artifacts for el in pair[1]]
            asset_deployments = executor.map(lambda client_and_artifact: (client_and_artifact[1], client_and_artifact[0].get_deployments(client_and_artifact[1])), clients_artifacts)
