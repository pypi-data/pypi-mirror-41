...
class FeedbackLogging(TableFromRestApiViewer):
    ...
    def store(self, feedback_data, fields=None, feedback_format=FeedbackFormat.WML, data_header=False, data_delimiter=','):
        """
            ...
            >>> subscription.feedback_logging.store("Personal Accessories,F,20,Single,Other", feedback_format=FeedbackFormat.TEXT)
        """

        if feedback_format == FeedbackFormat.TEXT:
            validate_type(feedback_data, "feedback_data", str, True)
        ...
        elif feedback_format == FeedbackFormat.TEXT:

            query_params = '?header=' + json.dumps(data_header) + '&delimiter=' + quote(data_delimiter, safe='') + '&on_error=continue'
            response = requests_session.post(
