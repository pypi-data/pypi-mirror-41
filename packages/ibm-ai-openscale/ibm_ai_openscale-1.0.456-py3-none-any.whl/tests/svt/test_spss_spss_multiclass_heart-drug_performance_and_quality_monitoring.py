...
@unittest.skipIf(get_env() != 'ICP_SPSS_CNDS', "Please run this test only on ICP_SPSS_CNDS env")
class TestAIOpenScaleClient(unittest.TestCase):
    ...
    def test_10_stats_on_payload_logging_table(self):
        ...
        self.assertTrue('$N-Drug' in str(payload_table_content))
    ...
    def test_13_send_feedback_data(self):
        ...
        with open(file='datasets/BestHeartDrug/drug_feedback_data_no_header.csv') as file:
            feedback_data = file.read()
        self.assertIsNotNone(feedback_data, msg="Unable to load feedback data from file.")

        TestAIOpenScaleClient.subscription.feedback_logging.store(feedback_data=feedback_data, feedback_format=FeedbackFormat.TEXT, data_header=False, data_delimiter=';')
