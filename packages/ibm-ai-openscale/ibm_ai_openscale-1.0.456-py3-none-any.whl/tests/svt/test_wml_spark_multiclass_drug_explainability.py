...
class TestAIOpenScaleClient(unittest.TestCase):
    ...
    def test_06_subscribe(self):
        ...
        print("Subscription details: {}".format(str(subscription.get_details())))
    ...
    def test_09_setup_explainability(self):
    ...
    def test_11_score(self):
        ...
        scores = self.wml_client.deployments.score(scoring_endpoint, payload_scoring,
                                              transaction_id=TestAIOpenScaleClient.transaction_id)

        print('Scoring result: {}'.format(scores))
        print("Waiting for payload propagation...")
        time.sleep(60)

        TestAIOpenScaleClient.subscription.payload_logging.print_table_schema()
        TestAIOpenScaleClient.subscription.payload_logging.show_table()

    def test_12_run_explainability(self):
        ...
        TestAIOpenScaleClient.status = TestAIOpenScaleClient.subscription.explainability.run(
            transaction_id="{}-1".format(TestAIOpenScaleClient.transaction_id), background_mode=False)['entity']['status']['state']
        print("Status: {}".format(TestAIOpenScaleClient.status))

        self.assertTrue("finished" in str(TestAIOpenScaleClient.status))
