...
class Bindings:
    ...
    def list_asset_deployments(self, **kwargs):
        """
              List available asset deployments. Available asset deployments are these which wasn't added yet as AI subscription:

              >>> client.data_mart.subscriptions.add(asset_uid) # adding all deployments of asset to your AI Openscale instance
              >>> client.data_mart.subscriptions.add(asset_uid, deployment_uids=['123']) # subscribing specific deployment `123`
        ...
        records = [[c['deployment_id'], c['name'], c['created_at'], c['deployment_type'],
                    c['parent_asset_uid'], c['is_subscribed']] for c in self.get_asset_deployment_details()]
    ...
    def get_asset_details(self):
        ...
        def safe_get_artifacts(client):
            try:
                ...
                print("Warning: Failure during getting artifacts for binding: {}.".format(client.binding_uid))
                return []
    ...
    def get_asset_deployment_details(self):
        """
              Get details of available asset deployments (for models and functions). Available asset deployments are these which wasn't added yet as AI subscription:
        ...
        def safe_get_artifacts(client):
            try:
                return client, client.get_artifacts()
            except Exception as e:
                print("Warning: Failure during getting artifacts for binding: {}.".format(client.binding_uid))
                return client, []

        def safe_get_deployments(client_and_artifact):
            try:
                return client_and_artifact[1], client_and_artifact[0].get_deployments(client_and_artifact[1])
            except Exception as e:
                print("Warning: Failure during getting deployments for binding: {}.".format(client_and_artifact[0].binding_uid))
                return client_and_artifact[1], []

        with ThreadPoolExecutor(max_workers=10) as executor:
            clients = [(client) for client in clients]
            clients_artifacts = executor.map(safe_get_artifacts, clients)
            clients_artifacts = [(pair[0], el) for pair in clients_artifacts for el in pair[1]]
            asset_deployments = executor.map(safe_get_deployments, clients_artifacts)
            asset_deployment_list = [[l[0], d] for l in asset_deployments for d in l[1]]

        return [get_deployment_json(asset_deployment[1], asset_deployment[0].source_uid, asset_deployment[1].guid in subscribed_deployments) for asset_deployment in asset_deployment_list]
