    ...
    def __init__(self, apiKey, currency):
        def getFlights(sourceAirport, dateFrom, dateTo):
            params = {
                ...
                "apikey": self.apiKey}
            response = requests.get("http://apigateway.ryanair.com/" +
                                    "pub/v1/farefinder/3/oneWayFares",
                                    params=params).json()['fares']
            flights = []
            for flight in response:
                ...
                flights.append(Flight(
                    ...
                    originFull=flight['departureAirport']['name'] +
                    ", " + flight['departureAirport']['countryName'],
                    destination=flight['arrivalAirport']['iataCode'],
                    destinationFull=flight['arrivalAirport']['name'] +
        ...
        def getReturnFlights(sourceAirport, dateFrom, dateTo,
            ...
            params = {
                ...
                "apikey": self.apiKey}
            response = requests.get("http://apigateway.ryanair.com/" +
                                    "pub/v1/farefinder/3/roundTripFares",
                                    params=params).json()['fares']
            trips = []
            for trip in response:
                ...
                trips.append(Trip(outbound=outbound, inbound=inbound,
                                  totalPrice=round(outbound.price +
                                                   inbound.price, 2)))
            return trips

        Flight = namedtuple("Flight", ("origin", "originFull",
