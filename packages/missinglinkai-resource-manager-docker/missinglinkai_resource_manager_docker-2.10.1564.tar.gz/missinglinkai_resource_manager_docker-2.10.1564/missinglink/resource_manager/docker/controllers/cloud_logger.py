...
class CloudLogger:
    ...
    async def _send_logs_internal(self):
        ...
        await self.session.post(url=self.endpoint, json=self.transit_logs)
    ...
    async def on_log(self, line, step_name=None, log_level=None):
        ...
        log_level_guess = 'INFO' if step_name == 'Run Code' else 'DEBUG'
        log_level = log_level or log_level_guess
