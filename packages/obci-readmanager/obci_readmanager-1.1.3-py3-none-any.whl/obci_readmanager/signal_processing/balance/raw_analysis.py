...
def plot_COP(signal, fs):
    """
    Plot real values of COP positions: COPx(time), COPy(time) and COPy(COPx).
...
def calculate_distance(position_1, position_2):
    """Return distance between two points or 1-dim and 2-dim vectors.
...
def max_sway_AP_ML(signal):
    """Return maximal sway values for mediolateral (x) and anterioposterior (y) directions.
...
def mean_COP_sway_AP_ML(signal):
    """Return mean sway values for mediolateral (x) and anterioposterior (y) directions.
...
def get_percentages_being(signal, fs, grid=0.1, plot=True):
    ...
    percentages_being, xedges, yedges = np.histogram2d(signal[0], signal[1], bins=[grid_x.shape[0], grid_y.shape[0]],
