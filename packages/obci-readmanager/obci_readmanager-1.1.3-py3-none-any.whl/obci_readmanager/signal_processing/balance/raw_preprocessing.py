...
def raw_downsample_signal(signal, factor):
    """Return downsampled signal values (according to factor)."""
    ch_num = len(signal)
    ch_len = len(signal[0])
    ret_sig_len = 0
    i = 0
...
def raw_filter_signal(signal, fs, cutoff_upper, order, use_filtfilt=False):
    """Return signal values filtered with Butterworth lowpass filter."""
...
def raw_fix_sampling(signal):
    """Return raw signal values with fixed sampling rate."""
    return
