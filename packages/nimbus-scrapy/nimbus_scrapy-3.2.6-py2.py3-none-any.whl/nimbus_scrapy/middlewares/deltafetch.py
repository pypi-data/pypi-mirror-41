...
from ..item import FlexItem, Item
...
class DeltaFetch(object):
    ...
    def _get_key(self, request, item=None):
        key = request.meta.get('deltafetch_key') or self._get_item_id(item) or self._get_fingerprint(request) or request_fingerprint(request)
