$(document).ready(function() {
    ...
    var beeConfig = {
        ...
        onSave: function(jsonFile, htmlFile) {
            $.confirm({
                title: 'Confirm!',
                content: 'Are you sure you want to save web template design!!',
                buttons: {
                    confirm: function () {
                        var template_name = $('input[name=name]').val();
                        var campaign = $('#id_campaign').val();
                        if(!template_name){
                            $.alert({
                                title: 'Alert!',
                                content: 'Please provide template name',
                            });
                        } else if(!campaign){
                            $.alert({
                                title: 'Alert!',
                                content: 'Please select a campaign',
                            });
                        } else {
                            $('.loader, .overlay').show();
