let keywords = new Vue({
    ...
    methods: {
        loadKeywords (url_) {
            let url = url_ ? url_ : '/api/marketing/campaigns/keywords/'
            HTTPClient.get(url)
                .then(response => {
        ...
        addKeyword () {
            ...
            let payload = {
                campaign: this.currentKeyword.campaign,
                keywords: keywords.filter(word => word !== ' ')
            }

            HTTPClient.post('/api/marketing/campaigns/keywords/', payload)
                .then(response => {
                    this.loadKeywords()
                    this.currentKeyword = {
                        campaign: '',
        ...
        deleteKeyword (indexes) {
            ...
            HTTPClient.delete('/api/marketing/campaigns/keywords/' + keyword.id + '/')
                .then(response => {
