...
import cv2
from math import pi, cos as cosinus, sin as sinus, log, tan as tangent, factorial, atan
...
def angle_between_points(p1, p2):
    """
    Action: calculates the slope between 2 points
    :param p1: the first point (2d)
    :param p2: the second point  (2d)
    :return: the angle
    """
    slope = 0 if p1[0] == p2[0] else ((float(p1[1]) - float(p2[1])) / (float(p1[0]) - float(p2[0])))
    return radians2degrees(atan(slope))
