...
from .Color import MultiColor, Color
from .Vision import Vision
import cv2
import json
from .General import print_pipe
from .Filters import area_filter
...
class Calibration(object):
    ...
    def __init__(self, vision, class_amount, filter_min_ratio=0.9, min_area=100):
        """
        Calibration object is an object that preforms a Calibration function and runs during setup.
        ...
        if type(vision) is str:
            with open(vision, 'r') as json_f:
                self.vision = Vision.json_deserialize_secure(json_f.read())
                if self.vision is False:
                    raise ValueError('Failed to open the vision object from the json file.')
                self.color = vision.color
        elif type(vision) is Vision:
            self.vision = vision
            self.color = vision.color
        else:
            raise TypeError('Vision must be a vision object ')

        for idx, filter_function in enumerate(self.vision.filters):
            if filter_function.__name__.endswith('sort'):
                del self.vision.filters[idx]
            elif filter_function is area_filter:
                del self.vision.filters[idx]
    ...
    def brute_force(self, img, color, start_v=25, start_s=13, s_buffer=20, v_buffer=20):
        ...
        final_list = []
        for s in xrange(start_s, s_values + 1):
            for v in xrange(start_v, v_values + 1):
                ...
                for filter_ratio in filter_ratio_product:
                    for idx, r in enumerate(filter_ratio):
                        ratings[idx] *= ContourRating(self.vision.contours[idx], r)
                ...
                if len(ratings) >= self.vision.target_amount:
                    ...
                    for contour_rating in ratings[0: self.vision.target_amount]:
                        if type(contour_rating) is ContourRating:
        ...
        if type(color) is Color:
            ...
            for img in img_list:
                ...
                for c in color.colors:
                    tmp = get_data_from_color(c, img, mean)
                    tmp_saturation_data = tmp[0]
                    tmp_saturation_classes = tmp[1]
                    tmp_value_data = tmp[2]
                    tmp_value_class = tmp[3]
    ...
    def json_serialize(self, file_name=None):
        """
        Action: Turns a Trained Calibration object to a Json File for use in a Vision program
                And saves it as a json file
        :return: The Translation of the Object to Python
        """
        # All the information needed for the Calibration for the saturation machine learning
        s_wc1, s_wc2 = self.s_clf.coef_[0]
        s_w_intercept = self.s_clf.intercept_
        s_mean = self.s_clf.means_

        # All the information needed for the Calibration for the value machine learning
        v_wc1, v_wc2 = self.v_clf.coef_[0]
        v_w_intercept = self.v_clf.intercept_
        v_mean = self.v_clf.means_

        # The Json Conversion of the 2 classifiers
        translation = {'swv': {'co1': s_wc1, 'co2': s_wc2, 'inter': s_w_intercept, 'mean': s_mean},
                       'vwv': {'co1': v_wc1, 'co2': v_wc2, 'inter': v_w_intercept, 'mean': v_mean}}
        if file_name is not None:
            if not file_name.endswith('.json'):
                print_pipe(None, 'Target file is not a json file')
