...
class Circuit(CircuitBase['Circuit', Encoder, Decoder, MeasurementResultsPyquil]):
    """
    Class for building and running a :py:class:`CircuitBase` for the Forest environment.

    :Example:
        Create a `Circuit` that entangles two qubits as an EPR pair. First, using a pyquil Program:

        >>> quantum_program = pyquil.quil.Program()
        >>> quantum_program.inst(pyquil.gates.H(0))
        >>> quantum_program.inst(pyquil.gates.CNOT(0, 1))
        >>> epr_circuit = Circuit(quantum_transforms=quantum_program)
        ...
        >>> epr_circuit = Circuit(quantum_transforms=transforms)
    ...
    def __init__(self, quantum_transforms: Union[Tuple[QuantumTransform, ...], Program],
        ...
        super(Circuit, self).__init__(quantum_transforms=quantum_transforms)
    ...
    def run_program(self, input_data: Sequence[Any], encoder: Encoder,
        ...
        if isinstance(self._backend, QuantumComputer):
            ...
            measurement_results = Circuit._qvm_to_qc_results(qvm_results)
        else:
