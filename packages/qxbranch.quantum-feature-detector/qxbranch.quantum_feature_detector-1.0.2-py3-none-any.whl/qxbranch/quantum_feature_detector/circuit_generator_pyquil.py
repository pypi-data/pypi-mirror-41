...
class CircuitGeneratorPyquil(CircuitGeneratorBase[Union[QuantumComputer, str], Circuit]):
    """
    Factory class for generating random :py:class:`CircuitPyquil` objects for
    use in :py:class:`QuantumFeatureDetector` objects.

    :Example:
        Create a factory for generating :py:class:`CircuitPyquil` objects, each with 5 qubits and 8
        gates, and generate 5 such circuits.

        >>> circuit_generator = CircuitGeneratorPyquil(qubit_count=5, gate_count=8)
    ...
    def __init__(self, qubit_count: int, gate_count: int,
        ...
        super(CircuitGeneratorPyquil, self).__init__(qubit_count=qubit_count, gate_count=gate_count, backend=backend,
