...
datafiles = []
for dir in datadir:
    datafiles.extend([(d, [os.path.join(d, f) for f in files])
                      for d, folders, files in os.walk(dir)])
...
class PostInstallCommand(install):
    ...
    def run(self):
        ...
        for filepath in self.get_outputs():
            if 'lb-describe-platform' in filepath:
                os.chmod(filepath, mode)


setup(name='LbPlatformUtils',
      version='0.0.1',
      description='LHCb Platform Utils',
      long_description='LHCb Platform Utils',
      url='https://gitlab.cern.ch/lhcb-core/LbPlatformUtils',

      author='Ben Couturier',
      author_email='ben.couturier@cern.ch',
      license='GPL',
      ...
      # platform.
      scripts=['scripts/lb-describe-platform'])
