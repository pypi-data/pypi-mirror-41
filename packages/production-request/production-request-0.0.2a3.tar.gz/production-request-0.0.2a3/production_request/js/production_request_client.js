    ...
    try {
        ...
        if (req_uuid && req_started) {
            ...
            if (addToLog) {
                ...
                record['tr_total'] = (record['c_total'] - record['s_total']).toFixed(3);
...
function startLogging(clientLogUrl) {
    function sendProductionRequestStats() {
        if (productionRequestLogStore.length) {
            ...
            Ext3.Ajax.request({
                ...
                method: 'POST',
                params: {
                    logs: Ext3.util.JSON.encode(log_part)
                },
    ...
    setInterval(sendProductionRequestStats, 10000);
}