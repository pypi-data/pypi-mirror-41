...
OFFLINE = False
NODE = 'https://nodestestnet.acrylplatform.com'
...
MATCHER = 'https://matcher.acrylplatform.com'
MATCHER_PUBLICKEY = 'nHzTy18GY53w2NRnGLkHpjdVpLbWVfajVctA2DCqpvt'
...
def setChain(chain = CHAIN, chain_id = None):
    global CHAIN, CHAIN_ID, NODE

    if chain_id is not None:
        CHAIN = chain
        CHAIN_ID = chain_id
    else:
        if chain.lower()=='mainnet' or chain.lower()=='a':
...
def wrapper(api, postData='', host='', headers='', timeout=20):
    global OFFLINE
    if OFFLINE:
        offlineTx = {}
    ...
    if postData:
        req = requests.post(
            '%s%s' % (host, api), data=postData, headers={'content-type': 'application/json'}, timeout=timeout
        ).json()
    else:
...
WAVES = Asset('')
BTC = Asset('68X83bfvSSQauL81fcfqA3Vne9s4y2BRAG36DGRcv4V2')
