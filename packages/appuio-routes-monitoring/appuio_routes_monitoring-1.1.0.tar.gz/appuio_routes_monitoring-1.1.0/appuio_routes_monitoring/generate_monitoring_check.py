...
def get_appuio_object(cmd):
    """get a object from APPUiO and return them as yaml"""
...
def main():
    ...
    if args.projects == 'all':
        cmd = ['oc', 'get', 'ns', '-o', 'yaml']
        namespaces = get_appuio_object(cmd)
        for namespace in namespaces:
            cmd = ['oc', '-n', namespace['metadata']['name'],
                   'get', 'routes', '-o', 'yaml']
            routes += get_appuio_object(cmd)
    else:
    ...
    for route in routes:
        ...
        if 'annotations' in route['metadata']:
            ...
            if 'monitoring/alert_vshn' in annotations and \
                ...
                production_level = 4
            if 'monitoring/path' in annotations:
                monitoring_path = annotations['monitoring/path']
