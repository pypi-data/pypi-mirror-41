...
class DatasetSchemasTestCase(SdkIntegrationTestCase):
    ...
    def test_cannot_register_schema_with_invalid_payload(self):
        ...
        with self.assertRaises(InvalidPayloadException):
            self.client.register_schema(self.dataset_id, version='1a', valid_as_of='2018-10-31', fields=[{'random': 'keys'}])
            
        # type cannot be None or empty
        with self.assertRaises(InvalidPayloadException):
            self.client.register_schema(self.dataset_id, version='1a', valid_as_of='2018-10-31', type='', fields=my_schema_fields)
        with self.assertRaises(InvalidPayloadException):
            self.client.register_schema(self.dataset_id, version='1a', valid_as_of='2018-10-31', type=None, fields=my_schema_fields)
        
        # valid_as_of should be date in ISO format
        with self.assertRaises(InvalidPayloadException):
            self.client.register_schema(self.dataset_id, version='1a', valid_as_of='31/10/2018', fields=my_schema_fields)
        with self.assertRaises(InvalidPayloadException):
        ...
        with self.assertRaises(InvalidPayloadException):
            self.client.register_schema(self.dataset_id, version='1a', valid_as_of='2018-10-31', fields=my_schema_fields, partitions=[])
        with self.assertRaises(InvalidPayloadException):
            self.client.register_schema(self.dataset_id, version='1a', valid_as_of='2018-10-31', fields=my_schema_fields, partitions=[{'random': 'keys'}])
    ...
    def test_get_schemas_raises_error_for_invalid_count(self):
        self.assert_page_count_is_valid_for_paginated_resource_actions(lambda c: self.client.get_schemas("some_dataset", count=c))
