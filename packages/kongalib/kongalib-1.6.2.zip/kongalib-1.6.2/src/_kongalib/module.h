 ...
 *
 *		$Revision: 30038 $
 *		$Date: 2018-12-06 10:04:57 +0100 (Thu, 06 Dec 2018) $
 *		$Author: lillo $
 *
 */
...
typedef struct MODULE_STATE
{
	...
	CL_Mutex						fTimerLock;
	CL_RecursiveMutex				fInterpreterLock;
	std::list<DeferredObject *>		fTimerList;
	std::list<MGA_Client *>			fClientList;
	std::list<MGA_Client *>			fFreeClientsList;
	std::list<InterpreterObject *>	fInterpreterList;
...
#define GET_STATE()					(GET_STATE_EX(PyState_FindModule(MGA::gModuleDefPtr)))
