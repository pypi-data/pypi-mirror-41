...
class GalaxyFit(fitter.DataFitterStack):
    ...
    def fast_likelihood(cls, galaxy_datas, galaxy):
        ...
        chi_squareds_ = fitting_util.chi_squared_map_from_residual_map_mask_and_noise_map(residuals_, [galaxy_datas[0].noise_map_])
        chi_squared_terms = fitting_util.chi_squared_from_chi_squared_map(chi_squareds_)
        noise_terms = fitting_util.noise_normalization_from_mask_and_noise_map([galaxy_datas[0].noise_map_])
        return sum(fitting_util.likelihood_from_chi_squared_and_noise_normalization(chi_squared_terms, noise_terms))


class GalaxyFitDeflections(fitter.DataFitterStack):
    ...
    def fast_likelihood(cls, galaxy_datas, model_galaxy):
        ...
        chi_squared_terms = fitting_util.chi_squared_from_chi_squared_map(chi_squareds_)
        noise_terms = fitting_util.noise_normalization_from_mask_and_noise_map([galaxy_datas[0].noise_map_, galaxy_datas[0].noise_map_])
        return sum(fitting_util.likelihood_from_chi_squared_and_noise_normalization(chi_squared_terms, noise_terms))