...
class TestGalaxyFit:
    ...
    class TestLikelihood:
        ...
        def test__1x1_image__light_profile_fits_data_perfectly__lh_is_noise_term(self):
            ...
            fit = galaxy_fit.GalaxyFitDeflections(galaxy_datas=[g_data_y, g_data_x], model_galaxy=g0)
            assert fit.model_galaxy == g0
        ...
        def test__1x2_image__noise_not_1__all_terms_correct(self):
            ...
            g_data = galaxy_data.GalaxyDataIntensities(array=array, noise_map=noise_map, mask=mask, sub_grid_size=1)
            fit = galaxy_fit.fit_galaxy_data_with_galaxy(galaxy_data=[g_data], model_galaxy=g0)
            assert fit.model_galaxy == g0
            assert fit.chi_squared_term == (25.0 / 4.0)
            assert fit.reduced_chi_squared == (25.0 / 4.0) / 2.0
            assert fit.figure_of_merit == -0.5 * ((25.0 / 4.0) + 2.0 * np.log(2 * np.pi * 2.0 ** 2))
            ...
            g_data_y = galaxy_data.GalaxyDataDeflectionsY(array=array, noise_map=noise_map, mask=mask, sub_grid_size=1)
            g_data_x = galaxy_data.GalaxyDataDeflectionsX(array=array, noise_map=noise_map, mask=mask, sub_grid_size=1)
            fit = galaxy_fit.fit_galaxy_data_with_galaxy(galaxy_data=[g_data_y, g_data_x], model_galaxy=g0)
            assert fit.model_galaxy == g0
        ...
        def test__same_as_above_but_fast_likelihood(self):
            ...
            g_data = galaxy_data.GalaxyDataIntensities(array=array, noise_map=noise_map, mask=mask, sub_grid_size=1)
            assert galaxy_fit.fast_likelihood_from_galaxy_data_and_galaxy(galaxy_datas=[g_data], model_galaxy=g0) == \
                   -0.5 * (25.0 / 4.0 + 2.0*np.log(2 * np.pi * 2.0**2))
            assert galaxy_fit.GalaxyFit.fast_fit(galaxy_datas=[g_data], galaxy=g0) == \
                   -0.5 * (25.0 / 4.0 + 2.0*np.log(2 * np.pi * 2.0**2))
            ...
            g_data_y = galaxy_data.GalaxyDataDeflectionsY(array=array, noise_map=noise_map, mask=mask, sub_grid_size=1)
            g_data_x = galaxy_data.GalaxyDataDeflectionsX(array=array, noise_map=noise_map, mask=mask, sub_grid_size=1)
            assert galaxy_fit.fast_likelihood_from_galaxy_data_and_galaxy(galaxy_datas=[g_data_y, g_data_x], model_galaxy=g0) == \
    ...
    class TestCompareToManual:
        ...
        def test__intensities(self):
            ...
            fit = galaxy_fit.fit_galaxy_data_with_galaxy(galaxy_data=[g_data], model_galaxy=galaxy)

            assert fit.model_galaxy == galaxy

            model_data = galaxy.intensities_from_grid(grid=g_data.grid_stack.sub)
            model_datas = [g_data.grid_stack.sub.sub_data_to_regular_data(sub_array=model_data)]
            residuals = fitting_util.residual_map_from_data_mask_and_model_data([g_data[:]], model_datas)
            chi_squareds = fitting_util.chi_squared_map_from_residual_map_mask_and_noise_map(residuals, [g_data.noise_map_1d])

            assert g_data.grid_stack.regular.scaled_array_from_array_1d(g_data.noise_map_1d) == \
                   pytest.approx(fit.noise_map, 1e-4)
            assert g_data.grid_stack.regular.scaled_array_from_array_1d(model_datas[0]) == \
                   pytest.approx(fit.model_data, 1e-4)
            assert g_data.grid_stack.regular.scaled_array_from_array_1d(residuals[0]) == \
                   pytest.approx(fit.residual, 1e-4)
            assert g_data.grid_stack.regular.scaled_array_from_array_1d(chi_squareds[0]) == \
                   pytest.approx(fit.chi_squared, 1e-4)

            chi_squared_terms = fitting_util.chi_squared_from_chi_squared_map(chi_squareds)
            noise_terms = fitting_util.noise_normalization_from_mask_and_noise_map([g_data.noise_map_1d])
            likelihoods = fitting_util.likelihood_from_chi_squared_and_noise_normalization(chi_squared_terms, noise_terms)

            assert likelihoods[0] == pytest.approx(fit.figure_of_merit, 1e-4)

            fast_likelihood = galaxy_fit.fast_likelihood_from_galaxy_data_and_galaxy(galaxy_datas=[g_data],
                                                                                     model_galaxy=galaxy)
            assert fast_likelihood == pytest.approx(fit.figure_of_merit)
        ...
        def test__surface_density(self):
            ...
            model_data = galaxy.surface_density_from_grid(grid=g_data.grid_stack.sub)
            model_datas = [g_data.grid_stack.sub.sub_data_to_regular_data(sub_array=model_data)]
            residuals = fitting_util.residual_map_from_data_mask_and_model_data([g_data[:]], model_datas)
            chi_squareds = fitting_util.chi_squared_map_from_residual_map_mask_and_noise_map(residuals, [g_data.noise_map_1d])
        ...
        def test__potential(self):
            ...
            model_data = galaxy.potential_from_grid(grid=g_data.grid_stack.sub)
            model_datas = [g_data.grid_stack.sub.sub_data_to_regular_data(sub_array=model_data)]
            residuals = fitting_util.residual_map_from_data_mask_and_model_data([g_data[:]], model_datas)
            chi_squareds = fitting_util.chi_squared_map_from_residual_map_mask_and_noise_map(residuals, [g_data.noise_map_1d])
        ...
        def test__deflections(self):
            ...
            fit = galaxy_fit.fit_galaxy_data_with_galaxy(galaxy_data=[g_data_y, g_data_x], model_galaxy=galaxy)

            assert fit.model_galaxy == galaxy

            model_data_y = galaxy.deflections_from_grid(grid=g_data_y.grid_stack.sub)
            model_data_y = g_data_y.grid_stack.sub.sub_data_to_regular_data(sub_array=model_data_y[:, 0])
            model_data_x = galaxy.deflections_from_grid(grid=g_data_y.grid_stack.sub)
            model_data_x = g_data_y.grid_stack.sub.sub_data_to_regular_data(sub_array=model_data_x[:, 1])
            ...
            chi_squareds = fitting_util.chi_squared_map_from_residual_map_mask_and_noise_map(residuals, [g_data_y.noise_map_1d,
                                                                                                         g_data_x.noise_map_1d])

            assert g_data_y.grid_stack.regular.scaled_array_from_array_1d(g_data_y.noise_map_1d) == \
                   pytest.approx(fit.noise_maps[0], 1e-4)
            assert g_data_y.grid_stack.regular.scaled_array_from_array_1d(model_data_y) == \
                   pytest.approx(fit.model_datas[0], 1e-4)
            assert g_data_y.grid_stack.regular.scaled_array_from_array_1d(residuals[0]) == \
                   pytest.approx(fit.residuals[0], 1e-4)
            assert g_data_y.grid_stack.regular.scaled_array_from_array_1d(chi_squareds[0]) == \
                   pytest.approx(fit.chi_squareds[0], 1e-4)

            assert g_data_y.grid_stack.regular.scaled_array_from_array_1d(g_data_x.noise_map_1d) == \
                   pytest.approx(fit.noise_maps[1], 1e-4)
            assert g_data_y.grid_stack.regular.scaled_array_from_array_1d(model_data_x) == \
                   pytest.approx(fit.model_datas[1], 1e-4)
            assert g_data_y.grid_stack.regular.scaled_array_from_array_1d(residuals[1]) == \
                   pytest.approx(fit.residuals[1], 1e-4)
            assert g_data_y.grid_stack.regular.scaled_array_from_array_1d(chi_squareds[1]) == \
                   pytest.approx(fit.chi_squareds[1], 1e-4)

            chi_squared_terms = fitting_util.chi_squared_from_chi_squared_map(chi_squareds)
            noise_terms = fitting_util.noise_normalization_from_mask_and_noise_map([g_data_y.noise_map_1d, g_data_x.noise_map_1d])
            likelihoods = fitting_util.likelihood_from_chi_squared_and_noise_normalization(chi_squared_terms, noise_terms)

            assert likelihoods[0] == pytest.approx(fit.likelihoods[0], 1e-4)
            assert likelihoods[1] == pytest.approx(fit.likelihoods[1], 1e-4)
            assert likelihoods[0] + likelihoods[1] == pytest.approx(fit.figure_of_merit, 1e-4)

            fast_likelihood = galaxy_fit.fast_likelihood_from_galaxy_data_and_galaxy(
