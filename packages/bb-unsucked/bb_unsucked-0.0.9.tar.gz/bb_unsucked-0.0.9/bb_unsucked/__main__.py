...
bb-unsucked version 0.0.9, Copyright (C) 2019 Jeffrey McAteer <jeffrey.p.mcateer@outlook.com>
    ...
    if sidebar_element_name.lower() in link["text"].lower():
      ...
      if len(downloadable_files) < 1:
        ...
        if show_flag:
          ...
          now_s = int(time.time())
          file_name = f"/tmp/{now_s}.{extension}"
      ...
      if nothing_printed and "grades" in sidebar_element_name.lower():
        ...
        for row_elm in soup.findAll("div", {"class": "row"}):
          ...
          #print(f"{name} {activity} {grade} {status}")
