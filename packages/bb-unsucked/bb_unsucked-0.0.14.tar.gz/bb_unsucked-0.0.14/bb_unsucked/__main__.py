...
def print_help():
  ...
  bb-unsucked ftp-server [ip [port]]
    Host an FTP server on 127.0.0.1:2121 or ip:port if specified.
...
bb-unsucked version 0.0.14, Copyright (C) 2019 Jeffrey McAteer <jeffrey.p.mcateer@outlook.com>
...
if cmd == "ls":
  ...
  if not bb.verify_logged_in():
    ...
    print("bb-unsucked build-cache path/to/recent_session.har")
    sys.exit(1)
  ip_port = ("127.0.0.1", 2121)
  if len(sys.argv) > 2:
    ip_port = (sys.argv[2], 2121)
    if len(sys.argv) > 3:
      ip_port = (sys.argv[2], int(sys.argv[3]))
  print("Starting FTP service on {}:{}...".format(ip_port[0], ip_port[1]))
  bb.ftp_mount(addr_port_tuple=ip_port)
