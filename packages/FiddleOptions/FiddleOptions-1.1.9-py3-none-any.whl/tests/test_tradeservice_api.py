...
class TestTradeserviceApi(unittest.TestCase):
    ...
    def _create_complex_position_v2(self):
        ...
        trades.append(self._create_single_option())
    ...
    def _create_butterfly_position(self):
        """
        :return:
        """
        put_contracts = self._put_chain.contracts
        first_leg = OptionLeg(contract=put_contracts[0], quantity=1)
        mid_leg = OptionLeg(contract=put_contracts[int(len(put_contracts) / 2)],
                            quantity=-2)
        third_leg = OptionLeg(contract=put_contracts[-1], quantity=1)
    ...
    def _create_single_option(self):
        contracts = self._call_chain.contracts
        return OptionSingle(symbol=self.equity,
                            quantity=1,
                            opening_date=self.open_date,
                            option_contract=contracts[0])

    def _create_single_option_position(self):
        option_single = self._create_single_option()
