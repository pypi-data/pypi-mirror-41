...
class RequisitionReport(Report):
    ...
    def __init__(self, appointment=None, selected_panel_names=None,
    ...
    def render(self, **kwargs):
        ...
        story.append(
            Paragraph(
                "PATIENT SPECIMEN REQUISITION / MANIFEST", self.styles["line_label_center"]))

        data = [[Paragraph('DATE', self.styles["line_label"]),
                 Paragraph(
                     'REFERENCE',
                     self.styles["line_label"])],
                [Paragraph(
                    get_utcnow().strftime('%Y-%m-%d'),
                    self.styles["line_data_largest"]),
                 Paragraph(make_human_readable(timestamp),
                           self.styles["line_data_largest"]),
                 ]]
        t = Table(data)
        ...
        data = [[Paragraph('FROM (complete name and address)', self.styles["line_label"]),
        ...
        data = [
            ...
            [Paragraph(
                self.shipper.email or '', self.styles["line_data_large"]), '']]
        t1 = Table(data)
        ...
        story.append(
            Table([[Paragraph('I DECLARE THE INFORMATION TO BE TRUE AND CORRECT',
                              self.styles["line_label"])]]))
        ...
        data = [
            [Paragraph(self.contact_name, self.styles["line_data_large"]), '',
             Paragraph(
                ...
                self.styles["line_data_large"])],
            [Paragraph('SIGNATURE (sign next to your name.)',
                       self.styles["line_label"]), '',
        ...
        data = [
            [Paragraph('', self.styles["line_data_large"]), '',
             Paragraph('', self.styles["line_data_large"])],
            [Paragraph('SIGNATURE OF LABORATORY TECHNICIAN (Write name and sign.)',
                       self.styles["line_label"]), '',
             Paragraph('DATE', self.styles["line_label"])]]
            ...
            for count in range(1, item_count + 1):
                table_data.append([
                    ...
                    Paragraph(f'{requisition.panel_object.aliquot_type}',
                              self.styles['row_data']),
                    Paragraph(requisition.drawn_datetime.strftime(
