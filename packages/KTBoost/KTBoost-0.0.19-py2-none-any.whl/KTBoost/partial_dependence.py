"""Partial dependence plots for boosted trees. """

# Authors: Peter Prettenhofer, adapted to KTBoost by Fabio Sigrist
...
from .KTBoost import BaseBoosting
...
def partial_dependence(model, target_variables, grid=None, X=None,
...
def plot_partial_dependence(model, X, features, feature_names=None,
    ...
    label : object
        ...
        Only if model is a multi-class model. Must be in ``model.classes_``.
    n_cols : int
    ...
    **fig_kw : dict
    ...
    >>> fig, axs = plot_partial_dependence(model,Xtrain,features,
                                           percentiles=(0,1),figsize=(8,6))
    ...
    # set label_idx for multi-class model
    if hasattr(model, 'classes_') and np.size(model.classes_) > 2:
        if label is None:
            raise ValueError('label is not given for multi-class PDP')
        label_idx = np.searchsorted(model.classes_, label)
        if model.classes_[label_idx] != label:
            raise ValueError('label %s not in ``model.classes_``' % str(label))
    else:
    ...
    if model.n_features_ != X.shape[1]:
        raise ValueError('X.shape[1] does not match model.n_features_')
    ...
    if feature_names is None:
        ...
        feature_names = [str(i) for i in range(model.n_features_)]
    ...
    pd_result = Parallel(n_jobs=n_jobs, verbose=verbose)(
        delayed(partial_dependence)(model, fxs, X=X,
