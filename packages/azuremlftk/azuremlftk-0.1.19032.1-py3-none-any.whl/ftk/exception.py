...
Each of these classes correspond to a specific error handling needed for forecasting package.
"""
#------------------------------------------------------------------------
#
...
class TransformValueException(TransformException):
    """
    Exception raised for value errors in a transform class 
    in the AzureML Forecasting SDK.
...
class TransformTypeException(TransformException):
    """
    Exception raised for type errors in a transform class 
    in the AzureML Forecasting SDK.
...
class ComputeTypeException(AzureMLForecastException):
    """ComputeTypeException is thrown if a non ComputeBase derived class is attempted to be used
    as a compute option for processing in the Forecasting Toolkit"""
    def __init__(self, message):
        super().__init__("Compute object must inherit ftk.compute.ComputeBase class.{0}".format(message))

class ComputeException(AzureMLForecastException):
    """ComputeException is generic exception raised within a concrete ComputeBase class"""
    def __init__(self, message):
        super().__init__("ComputeException: {0}".format(message))

class MultiSeriesFitNotSupportedException(AzureMLForecastException):
    """MultiSeriesFitNotSupportedException is generic exception raised when a learner does not supported
    multi-series fit"""
    def __init__(self, message):
