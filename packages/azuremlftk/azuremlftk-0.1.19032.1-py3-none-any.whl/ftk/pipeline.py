"""
A module that contains definition of AzureMLForecastPipeline class which is used to create a pipeline capable of 
supporting the operations like train, transform and predict.
"""
#------------------------------------------------------------------------
#
# Copyright (c) Microsoft Corporation.  All rights reserved.
#
# Pipeline class - Azure ML Forecast toolkit pipeline
# Design considerations:
#   a. Composes scikit-learn Pipeline instead of inheriting it. 
#   http://blog.thedigitalcatonline.com/blog/2014/08/20/python-3-oop-part-3-delegation-composition-and-inheritance/#.WgIFX7pFyfA
#
#------------------------------------------------------------------------
...
from ftk.exception import AzureMLForecastException, InvalidEstimatorTypeException
from ftk.verify import Messages
from ftk.time_series_data_frame import TimeSeriesDataFrame
from ftk.exception import PipelineException
from ftk.constants import *
from builtins import isinstance
from uuid import uuid4

_can_use_core_telemetry = True
try:
    from ftk.time_series_logging import TimeSeriesLogger
except ImportError:
    warnings.warn('Unable to import TimeSeriesLogger. '
                  + 'Logging/Telemetry will be disabled.',
                  UserWarning)
...
class AzureMLForecastPipeline(object):
    """
    .. py:class:: AzureMLForecastPipeline
    Pipeline class for Azure Machine Learning Package For Forecasting
    (AMLPF). Encapsulates the sklearn pipeline and exposes methods to execute
    pipeline steps and and retrieve execution metrics.
    ...
    :param steps:
        ...
        Transformers must be a subclass of :class:`ftk.transforms.sklearn_transformer_wrapper.AzureMLForecastTransformerBase`,
    ...
    def add_pipeline_step(self, name, step, prepend=False):
        ...
        :type step:
            :class:`ftk.transforms.sklearn_transformer_wrapper.AzureMLForecastTransformerBase`, :class:`ftk.models.regression_forecaster.RegressionForecaster`, or a subclass of :class:`ftk.models.recursive_forecaster.RecursiveForecaster`.
    ...
    def execute_pipeline_op(self, execution_type, X, y=None, sample_weight=None, **fit_params):
        ...
        :type X: :class:`ftk.time_series_data_frame.TimeSeriesDataFrame`, or :class:`ftk.forecast_data_frame.ForecastDataFrame`
        ...
        if self._logging_on:
            ...
            telemetry_values[TELEMETRY_RUN_ID] = run_id
        ...
        if self._logging_on:
            telemetry_values = self._get_telemetry_values(execution_type, X, sample_weight, **fit_params)
            logger.log_end(activity, program_name, start_time, telemetry_values)

        return res

    def __execute_pipeline__preprocess_transforms(self, X):
        """
        Private method that runs transform on all but last step in the
        pipeline.
        """
        Xt = X
    ...
    def predict_proba(self, X, **predict_params):
        ...
        sklearn.pipeline.Pipeline.predict_proba: http://scikit-learn.org/stable/modules/generated/sklearn.pipeline.Pipeline.html#sklearn.pipeline.Pipeline.predict_proba
    ...
    def predict_log_proba(self, X, **predict_params):
        ...
        sklearn.pipeline.Pipeline.predict_log_proba: http://scikit-learn.org/stable/modules/generated/sklearn.pipeline.Pipeline.html#sklearn.pipeline.Pipeline.predict_log_proba
    ...
    def score(self, X, y=None, sample_weight=None):
        ...
        sklearn.pipeline.Pipeline.score: http://scikit-learn.org/stable/modules/generated/sklearn.pipeline.Pipeline.html#sklearn.pipeline.Pipeline.score
    ...
    def _get_telemetry_values(self, execution_type, X, sample_weight, **fit_params):
        telemetry_values = {}
        telemetry_values[TELEMETRY_COMPONENT] = 'forecastingPipeline'
        telemetry_values[TELEMETRY_FUNCION] = execution_type.name
        telemetry_values[TELEMETRY_PIPELINE_STEPS] = [step[0] for step in self._steps]
        telemetry_values[TELEMETRY_PIPELINE_STEP_NUM] = len(self._steps)
        telemetry_values[TELEMETRY_PIPELINE_SAMPLE_WEIGHT] = sample_weight
        telemetry_values[TELEMETRY_PIPELINE_ID] = self._pipeline_id
        if not X is None:
            telemetry_values[TELEMETRY_NUM_ROWS] = X.shape[0]
        if isinstance(X, TimeSeriesDataFrame):
            telemetry_values[TELEMETRY_TIME_COLUMN] = X.time_colname
            if X.grain_colnames:
                telemetry_values[TELEMETRY_GRAIN_COLUMNS] = ', '.join(X.grain_colnames)
            if X.origin_time_colname:
                telemetry_values[TELEMETRY_ORIGIN_COLUMN] = X.origin_time_colname
            if X.ts_value_colname:
                telemetry_values[TELEMETRY_TARGET_COLUMN] = X.ts_value_colname
            if X.group_colnames:
                telemetry_values[TELEMETRY_GROUP_COLUMNS] = ', '.join(X.group_colnames)
            telemetry_values[TELEMETRY_DATA_COLUMNS] = ', '.join([X.time_colname]+list(X.columns.values))
