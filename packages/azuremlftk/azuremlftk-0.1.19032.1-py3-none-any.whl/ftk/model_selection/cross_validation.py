"""
The :mod:`~ftk.model_selection.cross_validation` module includes classes and
functions to validate the model using Rolling Origin Cross-Validation
evaluation strategy.
"""
...
class RollingOriginValidator(BaseCrossValidator):
    """
    A subclass of sklearn.BaseCrossValidator_ which creates temporal
    splits in rows of data in a :class:`ftk.time_series_data_frame.TimeSeriesDataFrame`.
    ...
    def n_step(self, val):
        if isinstance(val, int):
            if val < 1:
                raise ValueError('n_step must be a positive int.')
            self.__n_step = val
        elif val is None:
            self.__n_step = val
        else:
    ...
    def split(self, X, y=None, groups=None):
        ...
        if not isinstance(X, TimeSeriesDataFrame):
            raise NotTimeSeriesDataFrameException(
                "X must be a TimeSeriesDataFrame or a ForecastDataFrame.")
        ...
        if X.infer_single_freq() is None:
            raise DataFrameTypeException(
                "Could not infer a single frequency for all grains.")
        ...
        if (n_splits - 1) * n_step_temp + 1 + num_horizon > n_samples:
            raise ValueError(
                ("Cannot have (n_splits - 1)*n_step + 1 + num_horizon = {0} greater"
                 " than the number of samples: {1}.").format((n_splits - 1) * n_step_temp + 1 + num_horizon,
                                                             n_samples))
        if has_origin:
