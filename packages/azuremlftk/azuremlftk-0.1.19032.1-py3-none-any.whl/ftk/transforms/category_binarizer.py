...
from ftk.exception import (EstimatorTypeException, EstimatorValueException,
                           DataFrameMissingColumnException, TransformException)


class CategoryBinarizer(AzureMLForecastTransformerBase):
    """
    .. py:class:: CategoryBinarizer
    
    This transform produces binary/indicator columns from
    categorical columns in an input data frame.
    Also known as "One-Hot Encoding" or "Dummy Coding."

    .. _pandas.get_dummies: http://pandas.pydata.org/pandas-docs/version
                            /0.19.0/generated/pandas.get_dummies.html

    Internally, this class uses pandas.get_dummies to
    generate the coding.
    ...
    def _detect_columns(self, X):
        ...
        if self.encode_all_categoricals:
            not_in_cat_list = set(X_cat.columns) - \
                set(self._detected_cols)
    ...
    def fit(self, X, y=None):
        ...
        Save lists of categorical columns
        and categories present in the
    ...
    def transform(self, X, y=None):
        ...
        If any of X's categorical columns contain
        categories not present at fit, these entries
        will be coded as NA and a warning will be printed.
