...
from ftk.utils import get_period_offsets_from_dates
...
class OriginTimeMixin(object):
        ...
        if isinstance(max_horizon, dict):
            ...
            if len(not_positive) > 0:
                raise TransformValueException(
                    ...
                    'The following unsupposed values were present in ' +
            ...
            if max_horizon <= 0:
                raise TransformValueException(
                    ('All max_horizon values must be greater than zero. ' +
                    'The following unsupposed value was present in ' +
    ...
    def max_horizon_from_key_safe(self, key, max_horizon, 
                                  default_horizon=1):
        """
        Safely return a maximum horizon in the case when it *might* be 
    ...
    def detect_max_horizons_by_grain(self, X, freq=None):
        """
        Detect a dictionary of maximum horizons for each grain in an input
        data frame with a time index and origin times.
