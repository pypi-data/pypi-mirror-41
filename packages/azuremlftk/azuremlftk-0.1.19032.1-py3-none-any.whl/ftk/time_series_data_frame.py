...
from ftk.utils import grain_level_to_dict, make_groupby_map
...
import ftk.verify as verify
from ftk.ts_utils import datetime_is_date
from ftk.utils import (tick_formatter,
                       convert_to_list,
                       get_period_offsets_from_dates,
                       import_plot_and_formatter)
from ftk.verify import (ALLOWED_TIME_COLUMN_TYPES, is_datetime_like, 
                        is_collection)

path_to_script = os.path.realpath(os.path.dirname(__file__))
path_to_dataframets = os.path.join(path_to_script, '../../lib')
sys.path.append(path_to_dataframets)
...
def _infer_freq_single_grain_special_cases(time_index_column):
    ...
    to infer the frequency, e.g the time index have only 2 values or the time
    index have some gaps in the data however under some scenarios the
    frequency can be properly inffered.
    ...
    if datetime_is_date(time_index_column):
        ...
        # weekly
            ...
            return Week(weekday=weekday, n=n)
...
def _return_freq(freq, return_freq_string=False):
    """
    Helper function to return DateOffset object itself or DateOffset alias
    string based on the provided return_freq_string argument.
...
class TimeSeriesDataFrame(pd.DataFrame):
    ...
    def identity_grain_level(cls):
        """
        Placeholder name when a TimeSeriesDataFrame doesn't
    ...
    def _convert_int64_to_datetime(self, cols):
        """
        Convert columns to datetime[ns]. If column is of type int64 assume that it contains time in 
        ...
        for col in cols:
            if not col is None and col in list(self.columns.values) and \
    ...
    def origin_time_colname(self):
        """
        Name of column that contains origin_time values. For every observation,
    ...
    def frequency(self):
        """
        Frequency of the TimeSeriesDataFrame, as inferred by
        pandas.infer_freq. If the data frame contains multiple grains with
    ...
    def _get_index_by_names(self, index_names):
        """
        Returns any requested subset of the TSDF index.
        ...
        if col in self.index.names:
            if not self.index.get_level_values(col).dtype == 'datetime64[ns]':
                if not all(self._verify_datetime_like(d)
                    ...
                    raise (DataFrameTypeException(
                        print('The type of time column must be '
                              'one of the following:\n{0}'.
                              format('\n'.join(
                                  str(t) for t in ALLOWED_TIME_COLUMN_TYPES)))))
    ...
    def _check_column_equal_across_origin(self, colname=None):
        """
        This function checks whether the column value is consistent across
        different origin times when the grain and time_index are the same.
    ...
    def _reset_tsindex(self):
        """
        _reset_tsindex() is called to reassign the index of a 
        TimeSeriesDataFrame so it conforms to the prescribed design.
    ...
    def reindex(self, *args, **kwargs):
        """
        Override of pandas.reindex that respects
        TimeSeriesDataFrame metadata.
    ...
    def reset_index(self, level=None, inplace=False, **kwargs):
        ...
        .. _pandas.reset_index: https://pandas.pydata.org/pandas-docs/stable/generated/pandas.DataFrame.reset_index.html
    ...
    def deduplicate_index(self, inplace=False, **kwargs):
        ...
        index_frequency = defaultdict(list)
    ...
    def set_index(self, *args, **kwargs):
        ...
        if 'time_colname' in kwargs:
            ...
            if not 'append' in kwargs or not kwargs['append']:
        ...
        if 'keys' in kwargs:
            ...
            if isinstance(args[0], str):
                args[0] = [args[0]] + \
                    keys_to_add  # pylint: disable=unsupported-assignment-operation
            elif verify.is_iterable_but_not_string(args[0]):
                args[0].extend(keys_to_add)
        else:
            kwargs['keys'] = keys_to_add

        self = super(TimeSeriesDataFrame, self).set_index(*args, **kwargs)
    ...
    def groupby_group_and_horizon(self, horizon_colname='horizon', 
        ...
        """
        Group the TimeSeriesDataFrame by the columns in the `group_colnames`
        property and the `horizon_colname` passed to this function.
    ...
    def infer_single_freq(self, return_freq_string=False):
        """
        This method will return a frequency for TimeSeriesDataFrame where a
        single uniform frequency across all time series could be inferred.
    ...
    def fill_datetime_gap(self, freq=None, origin=None, end=None):
        ...
        >>> data2 = pd.DataFrame({'store': ['a', 'a', 'a', 'b', 'b'],
        ...                       'brand': ['a', 'a', 'a', 'b', 'b'],
        ...                       'date': pd.PeriodIndex(['2012-01', '2012-03', '2012-04', '2012-02', '2012-03'],
        ...                                              dtype = 'period[M]', freq = 'M'),
        ...                       'sales':[1, np.nan, 5, 2, np.nan],
        ...                       'price':[np.nan, 2, 3, np.nan, 4]})
        >>> df2 = TimeSeriesDataFrame(data2, grain_colnames=['store', 'brand'], time_colname='date', ts_value_colname='sales')
        >>> df2
          brand    date  price  sales store
        0     a 2012-01    NaN    1.0     a
        1     a 2012-03    2.0    NaN     a
        2     a 2012-04    3.0    5.0     a
        3     b 2012-02    NaN    2.0     b
        4     b 2012-03    4.0    NaN     b
        >>> df2.fill_datetime_gap(freq='M')
          brand    date  price  sales store
        0     a 2012-01    NaN    1.0     a
        1     a 2012-02    NaN    NaN     a
        2     a 2012-03    2.0    NaN     a
        3     a 2012-04    3.0    5.0     a
        4     b 2012-02    NaN    2.0     b
        5     b 2012-03    4.0    NaN     b
    ...
    def merge(left, right, how='inner', on=None, left_on=None, right_on=None,
        ...
        """
        Overwrites the pandas merge function to
    ...
    def ts_summary(self, show_plots=True, acf_lags=32,
                   show_general_data_summary=True,
                   show_time_series_summary=True,
                   show_value_summary=True,
                   show_STL_decomposition_plots=True):
        """
        This method generates a summary of a TimeSeriesDataFrame. 

        In addition to general data statistics provided by pandas.info() 
        and pandas.describe(), the following statistics are computed:

        * Minimum and maximum timestamps
        * Inferred frequency of each time series
        * Inferred seasonality of each time series
        * Detailed value column statistics including missing value 
          percentage, zero value percentage, coefficients of variation

        This method also generates a few overview plots 
        of the TimeSeriesDataFrame, see param show_plots for details.

        :param show_plots: 
            If show_plots=True(default), the following plots are generated:

            * Averaged value of the 'ts_value_colname' column over time
            * STL decomposition plots of the averaged time series values
            * Histogram of the 'ts_value_colname' column
            * Time series length distribution plot
        :type show_plots: bool

        :param acf_lags: 
            Number of lags to plot in the Autocorrelation Function 
            and Partial Autocorrelation Function plots
        :type acf_lags: Integer

        :param show_general_data_summary:
            If True, print a data overview generated by pandas.DataFrame.info(),
            numerical variable summary and non-numerical data summary generated 
            by pandas.DataFrame.describe(). Default: True
        :type show_general_data_summary: bool

        :param show_time_series_summary:
            If True, print a time series summary including number of time series,
            min and max time stamps, inferred frequencies and detected 
            seasonalities. If show_plots is also True, plot the average of the 
            value column over time, time series length distribution. 
            If STL_decomposition is also True, plot the STL decomposition. 
            Default: True
        :type show_time_series_summary: bool

        :param show_value_summary:
            If True, print a summary of the value column, 
            including percentage of missing and zero values, and the 
            distribution of coefficients of variation.
            If show_plots is also true, plot the histogram and Box Plot 
            of the value column. Default: True
        :type show_value_summary: bool

        :param show_STL_decomposition_plots:
            If True, plot the STL decomposition of the 
            average of all time series values. Default: True
        :type show_STL_decomposition_plots: bool

        :return: None
        :rtype: NoneType

        """
        if show_general_data_summary:
            print(
                '--------------------------------  Data Overview  '
                '---------------------------------')
            self.info()
            ...
            if self.select_dtypes(exclude=[np.number]).shape[1] != 0:
                print(
                    '------------------------  Non-Numerical Variable Summary  '
                    '-----------------------')
                print(self.describe(exclude=[np.number]))

        if show_time_series_summary:
            _ = self.compute_ts_summary()
        if show_value_summary:
            _ = self.compute_ts_value_summary()

        # Some overview plots, optional, default is True.
        if show_plots:
            if show_time_series_summary:
                _, _ = self.plot_ts_agg()

            if show_STL_decomposition_plots:
                _, _ = self.plot_stl()

            # Histogram and box plot of the target column
            if show_value_summary:
                _, _ = self.plot_ts_value_distribution()

            if show_time_series_summary:

                ts_count = self.count_ts()
                if ts_count > 1:
                    # Bar chart of time series length distribution
                    _, _ = self.plot_ts_length_distribution()

                # Auto-correlation and partial auto-correlation functions
                _, _ = self.plot_acf_pacf(acf_lags)

    def ts_report(self, show_plots=True, acf_lags=32,
                  show_general_data_summary=True,
                  show_time_series_summary=True,
                  show_value_summary=True, show_STL_decomposition_plots=True,
                  show_categorical_plots=True, infer_categorical=True,
                  correlation_matrix=True):
        """
        This method generates a comprehensive report of the TimeSeriesDataFrame
        by calling the ts_summary, plot_categorical, and plot_numerical methods.

        :param show_plots: If True, generate plots. Default: True
        :type show_plots: bool
        ...
        :param show_categorical_plots:
            If True, plot the distribution of categorical variables and the
            target value variation for each unique value of each
            categorical variable. Default: True
        :type show_categorical_plots: bool

        :param infer_categorical:
            If True, infer categorical variables from numerical type variables.
            Default: True
        :type infer_categorical: bool

        :param correlation_matrix:
            If True, print the correlation matrix of numerical variables.
            If show_plots is also True, plot the correlation matrix heat map.
            Default: True
        :type correlation_matrix: bool

        :return: None
        :rtype: NoneType

        """
        self.ts_summary(show_plots, acf_lags, show_general_data_summary,
                        show_time_series_summary, show_value_summary,
                        show_STL_decomposition_plots)
        if show_categorical_plots:
            _, _ = self.plot_categorical(show_plots, infer_categorical)
        if correlation_matrix:
            self.plot_numerical(show_plots)
    ...
    def compute_ts_summary(self):
        """
        Compute time series related statistics, including number of time
        series, minimum timestamp, maximum timestamp,  frequency,
        and seasonality.

        :return: A summary of time series related statistics.
        :rtype: int
        """
        ts_summary_dict = {}
        ts_count = self.count_ts()
        ts_summary_dict['ts_count'] = ts_count
        print(
            '------------------------------  Time Series Summary  '
            '-----------------------------')
        print('Number of time series                 {0}'.format(ts_count))
        if self.time_colname:
            min_time = self.time_index.min()
            max_time = self.time_index.max()
            print('Minimum time                    {0}'.format(min_time))
            print('Maximum time                    {0}'.format(max_time))
            ts_summary_dict['min_time'] = min_time
            ts_summary_dict['max_time'] = max_time
        else:
            warn('The time_colname property of the TimeSeriesDataFrame '
                 + 'is not set. No frequency is inferred')

        # Time series frequency and length
        freq_by_grain_dict = self.get_frequency_dict()
        ts_summary_dict['freq_by_grain_dict'] = freq_by_grain_dict
        if ts_count > 1 and self.time_colname:
            print('')
            print('Inferred frequencies')
            frequency_count = []
            for freq, grains in freq_by_grain_dict.items():
                frequency_count.append((freq, len(grains)))

            frequency_count = sorted(
                frequency_count, key=lambda x: x[1], reverse=True)

            for freq, count in frequency_count:
                print('Number of {0}s with frequency {1:10}{2}'.format(
                    self.grain_colnames, str(freq), count))

            print(('Use get_frequency_dict() method to explore {0}s '
                   + 'with unusual frequency and clean data.')
                  .format(self.grain_colnames))

        elif self.time_colname:
            print('')
            for freq, grains in freq_by_grain_dict.items():
                print(
                    'Inferred frequency                  {0}'.format(freq))

            print('')
            tslen = len(self)
            print('Time series length                  {0}'.format(tslen))
            ts_summary_dict['tslen'] = tslen

        # Seasonality detection
        seasonality_by_grain_dict = self.get_seasonality_dict()
        ts_summary_dict['seasonality_by_grain_dict'] = \
            seasonality_by_grain_dict
        if self.grain_colnames and self.ts_value_colname:
            print('')
            print('Detected seasonalities')
            seasonality_count = []
            for seasonality, grains in seasonality_by_grain_dict.items():
                seasonality_count.append((seasonality, len(grains)))
            seasonality_count = sorted(
                seasonality_count, key=lambda x: x[1], reverse=True)

            for seasonality, count in seasonality_count:
                if np.isnan(seasonality):
                    print(
                        'Number of {0}s with seasonality {1:10}{2}'.format(
                            self.grain_colnames, 'nan', count))
                else:
                    print(
                        'Number of {0}s with seasonality {1:10}{2}'.format(
                            self.grain_colnames, str(int(seasonality)),
                            count))
            print(
                'Use get_seasonality_dict() method to explore {0}s with '
                'unusual seasonality and clean data.'
                .format(self.grain_colnames))
        elif self.ts_value_colname:
            print('')
            for seasonality, grains in seasonality_by_grain_dict.items():
                print('Detected seasonality:               {0}'.format(
                    seasonality))
        return ts_summary_dict

    def compute_ts_value_summary(self):
        """
        Compute summary statistics of the target column, i.e. column
        specified by ts_value_colname. Statistics computed include
    ...
    def plot_ts_agg(self, agg_func='mean'):
        """
        Plot the target column values aggregated across all grains over
        time.

        :param: agg_func: Function to use for aggregating the data.
        :type: agg_func: str, function

        :return: A figure object and an axes object for further customization.
        :rtype: tuple
        """
        ...
        if self.ts_value_colname and self.time_colname:
            agg_value_by_time = self.groupby(level=self.time_colname)[
                self.ts_value_colname].agg(agg_func)
            fig_ts, ax_ts = plt.subplots(figsize=(15, 10))
            agg_value_by_time.plot(
                title='{0} of {1} Over Time'.format(
                    function_name, self.ts_value_colname))
            ax_ts.set(ylabel=self.ts_value_colname)
    ...
    def plot_stl(self):
        """
        Plot the STL decomposition of the time series values averaged across
        grains.
        ...
        formatter = FuncFormatter(tick_formatter)
        if self.ts_value_colname and self.time_colname:
            mean_value_by_group = self.groupby(level=self.time_colname)[
                self.ts_value_colname].mean()
            try:
                decomposition = seasonal_decompose(mean_value_by_group)
                ...
                fig_stl, stl_axes = plt.subplots(
                    4, 1, figsize=(15, 10))
                stl_axes[0].yaxis.set_major_formatter(formatter)
                stl_axes[1].yaxis.set_major_formatter(formatter)
                stl_axes[2].yaxis.set_major_formatter(formatter)
                stl_axes[3].yaxis.set_major_formatter(formatter)
                plt.subplots_adjust(hspace=0.5)
                plt.suptitle('STL Decomposition', fontsize=15)
                mean_value_by_group.plot(kind='line', ax=stl_axes[0])
                stl_axes[0].legend(['Average {0}'.format(
                    self.ts_value_colname)], loc='upper left')
                trend.plot(kind='line', ax=stl_axes[1])
                stl_axes[1].legend(['Trend'], loc='upper left')
                seasonal.plot(kind='line', ax=stl_axes[2])
                stl_axes[2].legend(['Seasonality'], loc='upper left')
                residual.plot(kind='line', ax=stl_axes[3])
                stl_axes[3].legend(['Residual'], loc='upper left')

                return fig_stl, stl_axes
            except ValueError as err:
                warn(
                    'STL decomposition failed due to ValueError: {0}'
                    .format(err))

    def plot_ts_value_distribution(self):
        """
        Plot the histogram and box plot of the target column.

        :return: A figure object and two axes objects for further
        customization.
        :rtype: tuple
        """
        plt, FuncFormatter = import_plot_and_formatter()
        formatter = FuncFormatter(tick_formatter)
        if self.ts_value_colname:
            fig_value, (ax_hist, ax_box) = plt.subplots(
                1, 2, figsize=(10, 10))
            ax_hist.xaxis.set_major_formatter(formatter)
            ax_hist.yaxis.set_major_formatter(formatter)
            ax_box.yaxis.set_major_formatter(formatter)
            self[self.ts_value_colname].plot(
                kind='hist', ax=ax_hist,
                title='Histogram of {0}'.format(self.ts_value_colname))
            self[self.ts_value_colname].plot(
                kind='box', ax=ax_box,
                title='Box Plot of {0}'.format(self.ts_value_colname))
            return fig_value, (ax_hist, ax_box)

    def plot_ts_length_distribution(self):
        """
        ...
        plt, FuncFormatter = import_plot_and_formatter()
        formatter = FuncFormatter(tick_formatter)
        tslen_freq = self.groupby_grain().size().value_counts()
        fig_tslen, ax_tslen = plt.subplots(figsize=(10, 10))
        ax_tslen.xaxis.set_major_formatter(formatter)
        ax_tslen.yaxis.set_major_formatter(formatter)
        tslen_freq.plot(
            kind='bar', title='Time Series Length Distribution')
        ax_tslen.set(xlabel='Time Series Length',
                     ylabel='Number of {0}'.format(self.grain_colnames))
        return fig_tslen, ax_tslen

    def plot_acf_pacf(self, acf_lags=32):
        """
        Plot the auto correlation function and the partial auto correlation
        function.

        :param acf_lags: Number of lags to plot
        :type acf_lags: int
        ...
        if self.time_colname and self.ts_value_colname:
            ts_count = self.count_ts()
            if ts_count > 1:
                ts_values = self.groupby(level=self.time_colname)[
                    self.ts_value_colname].mean()
            else:
                ts_values = self[self.ts_value_colname]
            fig_acf, (ax_acf, ax_pacf) = plt.subplots(
                2, 1, figsize=(10, 10))
            plot_acf(ts_values, lags=acf_lags,
                     title='Auto Correlation Function', ax=ax_acf)
            ax_acf.set(xlabel='Lag', ylabel='Auto Correlation')
            plot_pacf(ts_values, lags=acf_lags,
                      title='Partial Auto Correlation Function', ax=ax_pacf)
            ax_pacf.set(xlabel='Lag', ylabel='Partial Auto Correlation')
            return fig_acf, (ax_acf, ax_pacf)

    def plot_categorical(self, show_plots=True, infer_categorical=True):
        """
        This method generates bar charts and target variable distribution plots 
        of categorical variables and variables that have categorical properties. 

        To find variables not of the type 'categorical', 
        but "look" categorical, a value count is first done on each unique 
        value of the variable. If the top 10 unique values contain 
        more than 80% of the data, we infer that it may be a categorical 
        variable.
        ...
        :return: 
            cat_stats, like_cat_stats. 
            cat_stats is a pandas.DataFrame containing the unique value 
            counts of each categorical variable. 
            like_cat_stats is a pandas DataFrame containing the unique value 
            counts of each inferred categorical variable.
        :rtype: pandas.DataFrame

        """
        plt, FuncFormatter = import_plot_and_formatter()
        # Compute statistics of categorical columns and potential categorical columns
        # Compute statistics of explict categorical columns
        cat_columns = list(self.select_dtypes(include=['category']).columns)
        cat_stats = []
        if len(cat_columns) > 0:
            for col in cat_columns:
                value_counts = self[col].value_counts().sort_index()
                cat_stats.append(
                    pd.DataFrame({'CategoricalColumn': col,
                                  'UniqueValue': value_counts.index,
                                  'ValueCount': value_counts.values}))
            cat_stats = pd.concat(cat_stats)

        # Infer categorical columns
        like_cat_columns = []
        like_cat_stats = []
        if infer_categorical:
            non_cat_columns = list(self.select_dtypes(
                exclude=['category']).columns)
            for col in non_cat_columns:
                value_counts = self[col].value_counts(normalize=True)
                if sum(value_counts.head(10)) > 0.8:
                    like_cat_columns.append(col)
                    value_counts_raw = self[col].value_counts()
                    like_cat_stats.append(
                        pd.DataFrame({'InferredCategoricalColumn': col,
                                      'UniqueValue': value_counts_raw.index,
                                      'ValueCount': value_counts_raw.values}))
        if len(like_cat_columns) > 0:
            like_cat_stats = pd.concat(like_cat_stats)

        if len(cat_columns) == 0 and len(like_cat_columns) == 0:
            print('There are no categorical columns or columns that look '
                  + 'categorical in the data')

        if show_plots:
            formatter = FuncFormatter(tick_formatter)
            total_num_rows = len(self)
            if len(cat_columns) > 0:
                for col in cat_stats['CategoricalColumn'].unique():
                    col_stats = \
                        cat_stats[cat_stats['CategoricalColumn'] == col][[
                            'UniqueValue', 'ValueCount']]
                    col_stats = col_stats.sort_values(by='UniqueValue')
                    if self.ts_value_colname:
                        fig_col, (ax_count, ax_value) = plt.subplots(
                            1, 2, figsize=(10, 10))
                        ax_count.yaxis.set_major_formatter(formatter)
                        ax_value.yaxis.set_major_formatter(formatter)

                        fig_col.suptitle('Categorical Column: {0}'.format(
                            col), fontsize=12, fontweight='bold')

                        # Count of each unique value
                        col_stats.plot(x='UniqueValue', y='ValueCount',
                                       kind='bar', ax=ax_count, legend=False)
                        ax_count.set(
                            xlabel=col, ylabel='Frequency', title='Frequency')
                        self. _label_bar_percentage(ax_count, total_num_rows)

                        # Average (+ variance) target value for each unique
                        # value (box plot)
                        col_value_stats = self[[col, self.ts_value_colname]]\
                            .groupby(col).agg(['mean', 'std'])
                        col_value_stats[self.ts_value_colname]['mean'].plot(
                            kind='bar', ax=ax_value, yerr=col_value_stats[
                                self.ts_value_colname]['std'])
                        ax_value.set(ylabel='Mean and Std of {0}'.format(
                            self.ts_value_colname), title='Mean and Std of {0}'
                            .format(self.ts_value_colname))

                    else:
                        fig_col, ax_count = plt.subplots(figsize=(10, 10))
                        fig_col.suptitle('Categorical Column: {0}'.format(
                            col), fontsize=12, fontweight='bold')
            ...
            if len(like_cat_columns) > 0:
                for col in like_cat_stats['InferredCategoricalColumn'].unique():
                    col_stats = like_cat_stats[
                        like_cat_stats['InferredCategoricalColumn'] == col][
                        ['UniqueValue', 'ValueCount']]
                    col_stats = col_stats.sort_values(by='UniqueValue')
                    if self.ts_value_colname:
                        fig_col, (ax_count, ax_value) = plt.subplots(
                        ...
                        ax_value.yaxis.set_major_formatter(formatter)
                        ax_count.yaxis.set_major_formatter(formatter)
                        fig_col.suptitle('Inferred Categorical Column: {0}'
                                         .format(col), fontsize=12, fontweight='bold')
                        ...
                        # Average (+ variance) target value for each unique value (box plot)
                        col_value_stats = self[[col, self.ts_value_colname]]\
                            .groupby(col).agg(['mean', 'std'])
                        col_value_stats[self.ts_value_colname]['mean'].plot(
                            kind='bar', ax=ax_value, yerr=col_value_stats
        ...
        return cat_stats, like_cat_stats

    def plot_numerical(self, show_plots=True):
        """
        This method computes and prints the correlation matrix 
        of the numerical variables in the data. It also generates a heat map 
        figure of the correlation matrix when show_plots=True. 
        To get a scatter matrix of the numerical variables, 
        use the method plot_scatter_matrix.

        :param show_plots: 
            When show_plots=True(default), a heat map figure of the correlation
            matrix is generated.
        :type show_plots: bool
        ...
        """
        from matplotlib import pyplot as plt
        numeric_df = self.select_dtypes([np.number])

        if not numeric_df.empty:
            corr = numeric_df.corr()

            if show_plots:
                # Correlation matrix heat map between
                #   each pair of continuous variables,
                fig = plt.figure(figsize=(10, 10))
                plt.imshow(corr)
                plt.colorbar()
                plt.xticks(range(len(corr)), corr.columns, rotation='vertical')
                plt.yticks(range(len(corr)), corr.columns)
                plt.suptitle('Numerical Variable Correlation Matrix Heat Map',
            ...
            print(
                ...
                '------------------------------')
            print(corr)
            print('You may call TimeSeriesDataFrame.plot_scatter_matrix() to '
                  'get a correlation matrix plot. However, this\n'
                  'is not recommended if your data is big.')
        else:
            print('There is no numerical columns in the data.')

    def plot_scatter_matrix(self):
        """
        This method plots a scatter matrix of the numerical variables 
        in the data.
        ...
        if not numeric_df.empty:
            ...
            ax_scatter = pd.plotting.scatter_matrix(
                numeric_df, figsize=(10, 10))
            plt.suptitle('Numerical Variable Correlation Scatter Matrix',
                         fontsize=15, fontweight='bold')
            corr_matrix = corr.as_matrix()

            for i, j in zip(*plt.np.triu_indices_from(ax_scatter, k=1)):
                ax_scatter[i, j].annotate("{0:.2f}".format(corr_matrix[i, j]),
                                          (0.9, 0.9), xycoords='axes fraction',
                                          ha='center', va='center')
        else:
            print('There is no numerical columns in the data.')
    ...
    def get_frequency_dict(self):
        """
        This method generates a dictionary of frequencies present 
        in the TimeSeriesDataFrame.

        :return: 
            Dictionary where each key is an inferred frequency
            and each value is a list of grains that have the inferred
            frequency.
        :rtype: dict

        """
        # Are we sure this is the sort of frequency dictionary we want?
        # I think it makes more sense to use the grains as the keys...

        freq_by_grain_dict = defaultdict(list)
        if self.grain_colnames is not None and self.time_colname is not None:
            for grain_level, df in self.groupby_grain():
                freq_key = _infer_freq_single_grain(df.time_index,
                                                    return_freq_string=True)
                if freq_key is None:
                    freq_key = FREQ_NONE_VALUE_STRING

                freq_by_grain_dict[freq_key].append(grain_level)

        elif self.time_colname is not None:
            # Make this more general after we fix groupby_grain bug
            freq_key = _infer_freq_single_grain(self.time_index,
                                                return_freq_string=True)
            freq_by_grain_dict[freq_key].append('__identity_grain')

        return dict(freq_by_grain_dict)

    def get_seasonality_dict(self):
        """
        This method generates a dictionary of seasonalities present 
        in the TimeSeriesDataFrame.

        :return: 
            Dictionary where each key is an inferred seasonality
            and each value is a list of grains that have the inferred
            seasonality.
        :rtype: dict
        ...
        if self.grain_colnames and self.ts_value_colname:
            seasonality_all = self.groupby_grain().apply(
                lambda g: detect_seasonality(
                    g[self.ts_value_colname].sort_index().values))
            for seasonality in seasonality_all.unique():
                if np.isnan(seasonality):
                    seasonality_by_grain_dict[seasonality] = list(
                        seasonality_all.loc[seasonality_all.isnull()].index)
                else:
                    seasonality_by_grain_dict[seasonality] = list(
                        seasonality_all.loc[seasonality_all ==
                                            seasonality].index)

        elif self.ts_value_colname:
            seasonality_all = detect_seasonality(
                self[self.ts_value_colname].sort_index().values)
            seasonality_by_grain_dict[seasonality_all] = 1

        return seasonality_by_grain_dict
    ...
    def construct_from_json(cls, json_str):
        ...
        if isinstance(json_dict, str):
            ...
            if not isinstance(json_dict, dict):
                raise ('Input of type {} can not be converted json '
    ...
    def _get_floor_freq(self):
        """
        The first non-all-zero time part will be inferred as the floor
        frequency.
        """
    ...
    def get_period_end_time_index(self, is_start_of_period=True, freq=None,
        ...
        This method infers the period end time implied by 
        data frame timestamps given information about the time series frequency.
    ...
    def _get_metadata_dict(self):
        """
        This helper function is used internally to get the dictionary of
    ...
    def subset_by_grains(self, grains):
        """
        This helper function returns a subset of data specified by "grains".
    ...
    def subset_by_origin_time(self, origin_time=None):
        """
        This helper function returns a subset of data frame that corresponds 
        to a certain value of `origin_time` or a range of origin times. Primary
        use case is to get the "latest" slice of the predicted data in a 
    ...
    def plot_column_by_grain(self, colname=None, max_grains_to_plot=10,
                             plot_columns=2, *args, **kwargs):
        """
        .. py:method:: plot_column_by_grain

        Plot column values against time by grain.

        :param colname:
            Name of the column in the TimeSeriesDataFrame whose values should
            be plotted. Defaults to `ts_value_colname`.
        :type err_fun: str

        :param max_grains_to_plot:
            Maximum number of grains in the input TimeSeriesDataFrame to plot.
            If actual number of distinct grains exceeds this value, only the
            first `max_grains_to_plot` grains will be plotted and a warning
            will be printed. Plotting many grains can take a while. 
            Default value is `10`.
        :type err_name: int

        :param plot_columns:
            How many columns to create in the grid of plots by grain. Defaults
            to `2` to save screen space, increase if you have a wide screen.
        :type by_col: int

        :param **kwargs: 
            Additional keyword arguments that are passed to plt.subplots.
        ...
        if colname is None:
            ...
            if colname not in self.columns:
                missing_colname_err = ("Column named {} is not found in the "
                                       + "input TimeSeriesDataFrame!").format(
                    colname)
                raise ValueError(missing_colname_err)

        clone_tsdf = self.copy()
        all_grains = clone_tsdf.grain_index.unique()
        num_grains = len(all_grains)
        if max_grains_to_plot is None:
            max_grains_to_plot = num_grains
        if num_grains > max_grains_to_plot:
            too_many_grains_wrn = ("Input TimeSeriesDataFrame has {0} unique "
                                   + "grains, while max_grains_to_plot is {1}. "
                                   + "Plotting only the first {1} grains. "
                                   + "Set max_grains_to_plot to higher value "
                                   + "or to None if you want all grains "
                                   + "plotted. Note that plotting more than "
                                   + "30-ish grains can take a while!").format(
                num_grains, max_grains_to_plot)
            warn(too_many_grains_wrn, UserWarning)
            clone_tsdf = clone_tsdf.loc[clone_tsdf.grain_index.isin(
                all_grains[:max_grains_to_plot])]

        plot_rows = max_grains_to_plot // plot_columns + \
                        max_grains_to_plot % plot_columns

        # go over lists of  kwargs to override the defaults
        if 'nrows' in kwargs:
            plot_rows = kwargs['nrows']
            kwargs.pop('nrows')
        if 'ncols' in kwargs:
            plot_columns = kwargs['ncols']    
            kwargs.pop('ncols')
        sharex = True
        if 'sharex' in kwargs:
            sharex = kwargs['sharex']
            kwargs.pop('sharex')
        figure_size = (6 * plot_columns, 5 * plot_rows) 
        if 'figsize' in kwargs:
            figure_size = kwargs['figsize']
            kwargs.pop('figsize')

        fig, ax = plt.subplots(nrows=plot_rows, ncols=plot_columns, 
                               sharex=sharex, figsize=figure_size, **kwargs)
        formatter = FuncFormatter(tick_formatter)
        clone_df = clone_tsdf.sort_index()._extract_time_series(colname)
        
        for i in range(max_grains_to_plot):
            r, c = i // plot_columns, i % plot_columns
            grain = all_grains[i]
            temp_df = clone_df.loc[pd.IndexSlice[:, grain], :]
            x_values = temp_df.index.get_level_values(self.time_colname)
            y_values = temp_df[colname].values
            ax[r, c].set_xlim(left=min(x_values), right=max(x_values))
            ax[r, c].yaxis.set_major_formatter(formatter)
            ax[r, c].set_title(grain)
            ax[r, c].plot(x_values, y_values, label=colname)
            ax[r, c].legend()
