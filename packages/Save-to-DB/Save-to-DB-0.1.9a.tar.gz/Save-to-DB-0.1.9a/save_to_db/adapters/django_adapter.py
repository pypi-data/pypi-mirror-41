...
class DjangoAdapter(AdapterBase):
    ...
    def add_related_models(cls, model, fkey, related_models):
        the_field = getattr(model, fkey)
        for related_model in related_models:
            the_field.add(related_model)
    ...
    @classmethod
    def related_x_to_many_contains(cls, model, field_name, child_models,
                                   adapter_settings):
        if not child_models:
            return []
        ...
        contained_models = getattr(model, field_name).filter(**{
    ...
    @classmethod
    def get_related_x_to_many(cls, model, field_name):
        return list(getattr(model, field_name).all())