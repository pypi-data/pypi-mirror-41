...
.. ifconfig:: calcephapi in ('Mex')
    ...
    It's not safe for two threads to call the functions with the same object of type :mat:class:`CalcephBin`. But it's safe for two threads to access simultaneously to the same ephemeris file with two different objects of type :mat:class:`CalcephBin`.  In this case, each thread must open the same file. 
