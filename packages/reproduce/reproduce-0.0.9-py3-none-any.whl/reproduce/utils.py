...
def google_bucket_fetch_and_validate(
        bucket_id, blob_id, iam_token_path, target_path):
    """Create a function to download a Google Blob to a given path and hash.
...
def generate_download_code(
        bucket_id_to_filename_map, iam_token_path, target_dir):
    """Generate Python code to download and validate the specified blobs.

    Parameters:
        bucket_id_to_filename_map (dict): maps a filename to a Google
            bucket/blob id pair.
        iam_token_path (str): path to Google Cloud private key generated by
        https://cloud.google.com/iam/docs/creating-managing-service-account-keys
        target_dir (str): path to target directory to save files.

    Returns:
        a string that is Python source code that can be used to download and
        validate the blobs specified in the `bucket_to_id_filename_map.
    """
    source_code = 'import reproduce.utils\n\n'

    for bucket_id, blob_id in bucket_id_to_filename_map.values():
        source_code += (
            f'''reproduce.utils.google_bucket_fetch_and_validate(
    r'{bucket_id}', r'{blob_id}', r'{iam_token_path}', r'{
        os.path.join(target_dir, blob_id)}')\n''')
