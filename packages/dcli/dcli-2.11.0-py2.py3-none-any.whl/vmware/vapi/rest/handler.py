...
from .rules import http_method_map, RoutingRuleGenerator
...
class MetadataStore(object):
    ...
    def _create_operation_summary(self, operation_info):
        ...
        if verb_info is not None:
            query_variables_map = {
                param_info.metadata[RestAnnotations.QUERY_VARIABLE].elements[
                    RestAnnotations.NAME_ELEMENT].string_value: param_name
                for param_name, param_info in six.iteritems(param_info_map)
                if RestAnnotations.QUERY_VARIABLE in param_info.metadata
            }

            header_variables_map = {
                param_info.metadata[RestAnnotations.HEADER_ELEMENT].elements[
                    RestAnnotations.NAME_ELEMENT].string_value: param_name
                for param_name, param_info in six.iteritems(param_info_map)
...
class DataValueDeserializer(object):
        ...
        try:
            ...
            if native_type == six.text_type:
                ...
                if isinstance(json_value, bool):
                    ...
                    and builtin_type == Type.BuiltinType.DYNAMIC_STRUCTURE):
    ...
    def map_query_params(self, request, service_id, operation_id):
        ...
        query_variables_map = operation.query_variables_map

        query_parameters = URLValueDeserializer.deserialize(
                request.query_string)
        return {query_variables_map[key]: value
                for key, value in six.iteritems(query_parameters)
                if key != TASK_REST_QUERY_PARAM and key in query_variables_map}
    ...
    def map_header_params(self, request, service_id, operation_id):
        ...
        header_variables_map = normalize(operation.header_variables_map)

        return {header_variables_map[key]: value
                for key, value in six.iteritems(dict(request.headers))
                if key in header_variables_map}
...
class RESTHandler(object):
    ...
    def invoke(self, request, endpoint, uri_parameters):
        ...
        if mapping_type == RestAnnotationType.VERB:
            input_data.update(self.data_value_deserializer.map_query_params(
                request, service_id, operation_id))

            input_data.update(self.data_value_deserializer.map_header_params(
                request, service_id, operation_id))
