...
class IntegrationBase(abc.ABC):
  def __init__(self, rasa_api_attrs: RasaApiAttributes, field_mapping: FieldMapping, log_func):
    ...
    self.rasa_active_person_id_cust_id_mapping = {}
  ...
  def __get_rasa_api_token(self):
    response = post(self.get_rasa_api_tokens_endpoint(),
      ...
      data=json.dumps({KEY: self.rasa_api_attrs.key}))
  ...
  def __create_or_update_persons_in_rasa(self):
    mapped_persons = self.map_customer_pesons_to_rasa_persons(self.get_customer_active_persons())
    for mapped_person in mapped_persons:
      ...
      if not rasa_person:
        ...
        if response.status_code not in(200, 201):
          ...
          self.rasa_active_person_id_cust_id_mapping[self.__get_rasa_id_from_create_response(response)] = mapped_person[EXTERNAL_ID]
      else:
        self.rasa_active_person_id_cust_id_mapping[rasa_person[ID]] = mapped_person[EXTERNAL_ID]
  ...
  def __unsubscribe_customers_inactive_persons_in_rasa(self):
    mapped_persons = self.map_customer_pesons_to_rasa_persons(self.get_customer_inactive_persons())
    if mapped_persons:
      for mapped_person in mapped_persons:
        rasa_person = self.__get_rasa_person_by_mapped_person(mapped_person)
  ...
  def __unsubscribe_rasa_persons_for_oneway_sync(self):
    for rasa_person in self.rasa_active_persons:
      if not self.rasa_active_person_id_cust_id_mapping.get(rasa_person[ID]):
        self.__unsubscribe_rasa_person(rasa_person)

  def map_customer_pesons_to_rasa_persons(self, customer_persons):
    mapped_persons = []
  ...
  def map_rasa_persons_to_customer_pesons(self, rasa_persons):
    mapped_persons = []
    field_mapping_json = json.loads(self.field_mapping.to_json()) # pylint: disable=E1101
