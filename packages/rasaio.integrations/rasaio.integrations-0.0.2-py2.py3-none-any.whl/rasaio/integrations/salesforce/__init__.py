...
class SalesforceIntegration(IntegrationBase):
  ...
  def __create_or_update_rasa_active_persons(self):
    mapped_persons = self.map_rasa_persons_to_customer_pesons(self.rasa_active_persons)
        ...
        if not sf_person:
          ...
          if mapped_person[SALESFORCE_DATE_UPDATED] > parse_date_from(sf_person[SALESFORCE_DATE_UPDATED]):
            ...
            if changes:
              ...
              self.log("No change found for person record - id = {}, email = {}".format(mapped_person[SALESFORCE_ID], mapped_person[SALESFORCE_EMAIL]))
          else:
            self.log("Ignoring person record update in Salesforce. Salesforce record is more recent - id = {}, email = {}".format(mapped_person[SALESFORCE_ID], mapped_person[SALESFORCE_EMAIL]))
  ...
  def __unsubscribe_rasa_inactive_persons(self):
    mapped_persons = self.map_rasa_persons_to_customer_pesons(self.rasa_inactive_persons)
    for mapped_person in mapped_persons:
      sf_person = None
      if mapped_person.get(SALESFORCE_ID):
        sf_person = self.rasa_salesforce.get_person_by_id(mapped_person[SALESFORCE_ID])
