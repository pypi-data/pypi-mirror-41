...
parser_common.add_argument('--dry-run', required=False, action='store_true', help='Don\'t run kubectl commands')
parser_common.add_argument('--sync-mode', action='store_true', required=False, default=False,
                           help='Turn on sync mode and wait deployment ending')
parser_common.add_argument('--tries', type=int, required=False, default=360,
                           help='Count of tries to check deployment status')
parser_common.add_argument('--retry-delay', type=int, required=False, default=5,
                           help='Sleep between tries in seconds')
parser_common.add_argument('--strict', action='store_true', required=False,
                           help='Check existence of all env variables in config.yaml and stop if var is not set')
parser_common.add_argument('--use-kubeconfig', action='store_true', required=False,
                           help='Try to use kube config')
parser_common.add_argument('--k8s-handle-debug', action='store_true', required=False,
                           help='Show K8S client debug messages')

arguments_connection = parser_common.add_argument_group()
...
parser_provision = subparsers.add_parser('provision', parents=[parser_common, ],
                                         help='Sub command for provision app')
parser_provision.add_argument('file', type=str, nargs='+', help='Pass yaml files here')
parser_provision.add_argument('--show-logs', action='store_true', required=False, default=False, help='Show logs for jobs')
parser_provision.add_argument('--tail-lines', type=int, required=False, help='Lines of recent log file to display')


parser_deploy = subparsers.add_parser('deploy', parents=[parser_common, parser_target],
...
parser_destroy = subparsers.add_parser('destroy', parents=[parser_common, parser_target],
