...
#            cprint('\u2191\u2191\u2191'+wrap_text('found')+'\u2191\u2191\u2191')
    ...
    if len(text)<60:
        text+=' '*(60 - len(text))
    text += ' --- {} times {:.2f} s'.format(try_times,(t1-t0))
    cprint( text )
...
def generate_batch(df,batch_size,out_file_path,name_start,debug_mode=False,store_method='byTime',**parms):
    ...
    Times=t1-t0
    cprint(wrap_text('- - - --=[details]=-- - - -',35),c='g')
    cprint('Found {} factors after tried {} times with total {:.2f} s'.format(F_times,T_times,Times),c='')
    cprint('Average factor cost {:.3f} s'.format(F_times/Times),c='')
