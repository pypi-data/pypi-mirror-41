            ...
            if AuthToken.objects.get(username=username, token=password):
                ...
                if 'graph_groups' in request.session:
                    ...
                    if self.use_groups:
                        ...
                        use_special_groupe_membership = False
                        if hasattr(settings, 'ADAL_GROUPS_SPECIAL_MAPPING') \
                                and isinstance(settings.ADAL_GROUPS_SPECIAL_MAPPING, list) \
                                and len(settings.ADAL_GROUPS_SPECIAL_MAPPING) > 0:
                            use_special_groupe_membership = True
                                    ...
                                    for grp in settings.ADAL_GROUPS_MAP.keys():
                                        if group['id'] == settings.ADAL_GROUPS_MAP[grp]:
                                            try:
                                                ...
                                                pass
                                    # Special groups mapping
                                    if use_special_groupe_membership:
                                        if group['id'] in settings.ADAL_GROUPS_SPECIAL_MAPPING.keys():
                                            request.session['ADAL_GROUPS_SPECIAL_MAPPING'] = \
                                                settings.ADAL_GROUPS_SPECIAL_MAPPING[group['id']]
                        usr.save()
