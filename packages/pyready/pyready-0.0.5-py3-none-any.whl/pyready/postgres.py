...
def test_postgres_connection(db_host: str, db_port: int, db_name: str, user: str, pwd: str, n_retries: int, verbose: bool=True):
    def test_func():
        try:
            ...
            str_ex = str(ex)
            if [str_ex.find(x) != -1 for x in ['timeout expired', 'the database system is starting up']] != -1:
                return TestConnectionResult.timeout()
            return TestConnectionResult.error("Connection failed with error: {0}".format(ex))

        return TestConnectionResult.success()

    if not test_tcp_connection(db_host, db_port, n_retries, verbose):
        return False
