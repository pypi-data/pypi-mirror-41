...
class BuildImage(object):
    ...
    def _render_postgres_service(self, psql_version=False):
        ...
        :return: None
        """
        psql_version = psql_version or '9.6'
        env = Environment(loader=FileSystemLoader(self._templates_path))
    ...
    def _get_variables(self):
        ...
        variables_file = shell.run(
            ['find', self.temp_working_folder, '-name', 'variables.sh']
        )
        variables_file = variables_file.output.strip()
        if not path.exists(variables_file):
            return []
        with open(variables_file, 'r') as obj:
            variables = obj.read()
