...
A framework for Multi-Agent Deep Reinforcement Learning in Poker games.
...
![FrameWork](http://www.steinberger-ai.com/wp-content/uploads/2019/01/PokerRL_AgentArchitecture.png)
...
### Distributed Mode on one AWS instance
1. Fire up any AWS instance that suits your needs over the management console. It is important that you add the
following allowance to your security group to be able to view logs:
    ```
...
1. Run the following commands on the instance:
    ...
    pip install PokerRL[distributed]
    ```
1. Grant your instance access to your codebase (including a fork of PokerRL) so that it can `git pull` later on.

1. Create an Image of your instance so you can skip these steps in the future.
...
1. Every time you fire up a new instance with your AMI, execute
    ...
    fixed in PyTorch 1.0, but this version is actually slower for the recurrent networks than 0.4.1.
