...
StateOrdering_AMD = _amici.StateOrdering_AMD
StateOrdering_COLAMD = _amici.StateOrdering_COLAMD
StateOrdering_natural = _amici.StateOrdering_natural
...
class AbstractModel(_object):
    ...
    def fdwdp(self, dwdp, t, x, p, k, h, w, tcl, stcl):
        return _amici.AbstractModel_fdwdp(self, dwdp, t, x, p, k, h, w, tcl, stcl)
...
class IDASolver(Solver):
    ...
    if _newclass:
        ...
        fxdot = _amici.IDASolver_fxdot
    if _newclass:
        fJ = staticmethod(_amici.IDASolver_fJ)
    else:
        fJ = _amici.IDASolver_fJ
    if _newclass:
        fJSparse = staticmethod(_amici.IDASolver_fJSparse)
    else:
        fJSparse = _amici.IDASolver_fJSparse
...
def IDASolver_fJ(N, t, cj, x, dx, xdot, J, user_data, tmp1, tmp2, tmp3):
    return _amici.IDASolver_fJ(N, t, cj, x, dx, xdot, J, user_data, tmp1, tmp2, tmp3)
...
def runAmiciSimulations(solver, edatas, model, num_threads):
    return _amici.runAmiciSimulations(solver, edatas, model, num_threads)
