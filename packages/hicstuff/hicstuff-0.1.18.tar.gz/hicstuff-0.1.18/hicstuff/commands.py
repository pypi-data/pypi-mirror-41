...
class Pipeline(AbstractCommand):
    ...
    usage:
        pipeline [--quality_min=INT] [--duplicates] [--size=INT] [--no-cleanup]
                 ...
                 (<fq1> <fq2> | --frag <sam1> <sam2> | --matrix <bed2D>)
    ...
    options:
        ...
        -r, --frag                    Skip the mapping and start pipeline from fragment attribution using SAM files.
        ...
        -M, --matrix                  Start from the matrix building step using a sorted list of pairs in 2D BED format.
    ...
    def execute(self):
        if self.args["--matrix"] or self.args["--frag"]:
...
class Plot(AbstractCommand):
    ...
    usage:
        plot [--cmap=NAME] [--coord=INT-INT] [--range=INT-INT] [--max=INT]
             [--type={scale|law}] [--output=FILE] [--process] --frags=FILE <contact_map>
