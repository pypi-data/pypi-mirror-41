...
from LbSoftConfDB.SoftConfDB import sortVersions
...
class LbSdbQuery(Script):
    ...
    def main(self):
        ...
        if opts.debugpy2neo:
            logging.getLogger("ariadne.py2neo.cypher").setLevel(logging.DEBUG)
            logging.getLogger("ariadne.py2neo.rest").setLevel(logging.DEBUG)
        else:
            logging.getLogger("ariadne.py2neo.cypher").setLevel(logging.WARNING)
            logging.getLogger("ariadne.py2neo.rest").setLevel(logging.WARNING)
        ...
        cmdShort = { 'l':'listProjects', 'i':'listApplications', 'v':'listVersions',
    ...
    def cmdlistReleaseStacks(self, args):
        ...
        for s in stacks:
            ...
            # Sorting the list according to the distance to the bottom of the stack
            sorter = lambda pv: len( self.mConfDB.listDependencies(*pv))
            sl = sorted(list(s), key=sorter)
            stackdict["projects"] = [ (self._sanitizeProjectName(p2), v2) for (p2, v2) in sl ]
            
            # Now finding the platforms we should compile for
            platforms = set()
            req_platforms = set()
            # Checking if we have explicitly set the platforms for one
            # of the projects
            for (p, v) in s:
                tmp = set(self.mConfDB.listStackPlatformsRequestedForRelease(p, v))
                req_platforms |= tmp
                if(len(tmp)) > 0:
                    self.log.debug("REQUESTED_PLATFORM flag set for %s %s" % (p, v))
            
            # If no requested platform found, look for the dependencies in parent projects
            if len(req_platforms) != 0:
                platforms = req_platforms
            else:
                for (p, v) in s:
                    torelease = self.mConfDB.listStackPlatformsToRelease(p, v)
                    self.log.debug("For %s %s adding platforms: %s " % (p, v, " ".join(torelease)))
                    platforms |= set(torelease)
            
            if len(platforms) == 0:
                # We didn't find any platform for the project
                # as a last solution we check if the project was already released
                # and use the same
                # Just ignore for the moment if we can't find a platform
                for (p, v) in s:
                    platforms &= set(self.mConfDB.listPlatforms(p, v))
        
            stackdict["platforms"] = list(platforms)

            # Now print the source properties
            psources = dict()
            for (p2, v2) in s:
                sp2 = self._sanitizeProjectName(p2)
                psources[sp2] = self.mConfDB.getSourceURI(p2, v2)
            ...
            try:
                ...
                # default is CMake
                #stackdict["build_tool"] = "CMAKE"
                none
            # Now adding the stack to the dictionary
    ...
    def cmdgetProperties(self, args):
        ...
        pname = args[0].upper()
        props = self.mConfDB.getProjectProperties(pname)
        import json
    ...
    def cmdgetPVProperties(self, args):
        ...
        pname = args[0].upper()
        pver = args[1]
        props = self.mConfDB.getPVProperties(pname, pver)
        import json
        print(json.dumps(props, indent=2))
    ...
    def cmdlistRequestedPlatforms(self, args):
        ...
        for p in sorted(self.mConfDB.listPlatforms(pname, pversion, "REQUESTED_PLATFORM")):
            print(p)
    ...
    def cmdlistDependenciesAsDot(self, args):
        ...
        if (len(args) % 2 != 0):
            self.log.error("Please specify project and version for all projects")
            sys.exit(1)

        found = []
        not_found = []
        for (pname, pversion) in (args[pos:pos + 2] for pos in range(0, len(args), 2)):
        ...
        print((self.mConfDB.getDependenciesAsDot(found)))
    ...
    def cmddisplayDependenciesAsDot(self, args):
        ...
        dotfile = self.mConfDB.getDependenciesAsDot(found)
    ...
    def cmdlistReferencesAsDot(self, args):
        ...
        print((self.mConfDB.getReferencesAsDot(found)))
    ...
    def cmddisplayreferencesAsDot(self, args):
        ...
        dotfile = self.mConfDB.getReferencesAsDot(found)
