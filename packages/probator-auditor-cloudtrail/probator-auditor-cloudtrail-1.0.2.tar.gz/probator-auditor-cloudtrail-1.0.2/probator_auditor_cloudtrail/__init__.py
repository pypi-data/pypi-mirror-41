...
from probator import get_aws_session, AWS_REGIONS
...
class CloudTrailAuditor(BaseAuditor):
    ...
    options = (
        ...
        ConfigOption(name='bucket_account', default_value='', type='string', description='Name of account to create the S3 bucket in'),
        ...
        ConfigOption(
            name='sqs_queue_account',
            default_value='',
            type='string',
            description='Name of account of SQS queue for CloudTrail log delivery notifications'
        ),
        ConfigOption(name='sqs_queue_name', default_value='', type='string', description='SQS Queue name'),
        ConfigOption(name='sqs_queue_region', default_value='us-west-2', type='string', description='Region for the SQS queue'),
...
class CloudTrail(object):
    ...
    def __init__(self, account, bucket_name, bucket_region, logger):
        ...
        # Config settings
        self.trail_region = dbconfig.get('cloudtrail_region', self.ns, 'us-west-2')
                    ...
                    if trail['Name'] in ('Default', self.trail_name):
                        if not trail['IsMultiRegionTrail']:
                            ...
                            if trail['HomeRegion'] == aws_region:
                                if self.trail_region != aws_region or trail['Name'] == 'Default':
                                    ct.delete_trail(Name=trail['Name'])
                                    auditlog(
                                        event='cloudtrail.delete_trail',
                                        actor=self.ns,
