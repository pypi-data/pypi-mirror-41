...
from crea.profile import Profile
from crea.wallet import Wallet
from crea.creaconnect import CreaConnect
from crea.asset import Asset
from crea.witness import Witness, WitnessesRankedByVote, WitnessesVotedByAccount
from crea.blockchain import Blockchain
from crea.utils import formatTimeString, construct_authorperm, derive_beneficiaries, derive_tags, seperate_yaml_dict_from_body
from crea.vote import AccountVotes, ActiveVotes
from crea import exceptions
from crea.version import version as __version__
from crea.asciichart import AsciiChart
from crea.transactionbuilder import TransactionBuilder
from timeit import default_timer as timer
from creabase import operations
from creagraphenebase.account import PrivateKey, PublicKey, BrainKey
from creagraphenebase.base58 import Base58
from crea.nodelist import NodeList
from crea.conveyor import Conveyor
from crea.imageuploader import ImageUploader
from crea.rc import RC
...
def asset_callback(ctx, param, value):
    if value not in ["CREA", "CBD"]:
        print("Please CREA or CBD as asset!")
        ctx.abort()
    else:
        return value
...
def unlock_wallet(stm, password=None):
    ...
    if not password and KEYRING_AVAILABLE and password_storage == "keyring":
        password = keyring.get_password("crea", "wallet")
...
def node_answer_time(node):
    try:
        stm_local = Crea(node=node, num_retries=2, num_retries_call=2, timeout=10)
        start = timer()
...
@click.option(
    '--create-link', '-l', is_flag=True, default=False, help="Creates creaconnect links from all broadcast operations")
@click.option(
    '--creaconnect', '-s', is_flag=True, default=False, help="Uses a creaconnect token to broadcast (only broadcast operation with posting permission)")
@click.option(
...
def cli(node, offline, no_broadcast, no_wallet, unsigned, create_link, creaconnect, expires, verbose):
...
def set(key, value):
    ...
    if key == "default_account":
        ...
        if KEYRING_AVAILABLE and value == "keyring":
            ...
            password = keyring.set_password("crea", "wallet", password)
        elif KEYRING_AVAILABLE and value != "keyring":
            try:
                keyring.delete_password("crea", "wallet")
...
def nextnode(results):
    ...
    if not offline:
        ...
        t.add_row(["Version", "creapy is in offline mode..."])
    print(t)
...
def updatenodes(show, test, only_https, only_wss, only_appbase, only_non_appbase):
    ...
    t.align = "l"
    nodelist = NodeList()
    nodelist.update_nodes(crea_instance=stm)
    ...
    if unsafe_import_key:
        for key in unsafe_import_key:
            try:
                ...
                account = Account(account, crea_instance=stm)
...
def upvote(post, vote_weight, account, weight):
    ...
    if not unlock_wallet(stm):
        return
    try:
        post = Comment(post, crea_instance=stm)
        tx = post.upvote(weight, voter=account)
        if stm.unsigned and stm.nobroadcast and stm.creaconnect is not None:
            tx = stm.creaconnect.url_from_tx(tx)
...
def transfer(to, amount, asset, memo, account):
    ...
    if not unlock_wallet(stm):
        return
    acc = Account(account, crea_instance=stm)
...
def powerup(amount, account, to):
    """Power up (vest CREA as CREA ENERGY)"""
    stm = shared_crea_instance()
    if stm.rpc is not None:
        stm.rpc.rpcconnect()
    if not account:
...
def powerdown(amount, account):
    """Power down (start withdrawing VESTS from Crea POWER)
...
def delegate(amount, to_account, account):
    ...
    try:
        ...
        amount = Amount(str(amount), crea_instance=stm)
        if amount.symbol == stm.crea_symbol:
...
@click.option('--auto_vest', help='Set to true if the from account should receive the VESTS as'
              'VESTS, or false if it should receive them as CREA.', is_flag=True)
...
def convert(amount, account):
    """Convert CREADollars to Crea (takes a week to settle)"""
    stm = shared_crea_instance()
    if stm.rpc is not None:
        stm.rpc.rpcconnect()
    if not account:
...
def changewalletpassphrase():
    ...
    password_storage = stm.config["password_storage"]
    if KEYRING_AVAILABLE and password_storage == "keyring":
        keyring.set_password("crea", "wallet", newpassword)
    elif password_storage == "environment":
...
def power(account):
    ...
    if len(account) == 0:
        if "default_account" in stm.config:
            account = [stm.config["default_account"]]
    for name in account:
        a = Account(name, crea_instance=stm)
        print("\n@%s" % a.name)
...
@click.option('--number', '-n', help='Number of subsidized accounts to be claimed (default = 1), when fee = 0 CREA', default=1)
def claimaccount(creator, fee, number):
    ...
    if not unlock_wallet(stm):
        return
    creator = Account(creator, crea_instance=stm)
    fee = Amount("%.3f %s" % (float(fee), stm.crea_symbol), crea_instance=stm)
    tx = None
    if stm.unsigned and stm.nobroadcast and stm.creaconnect is not None:
        tx = stm.claim_account(creator, fee=fee)
        tx = stm.creaconnect.url_from_tx(tx)
    elif float(fee) == 0:
        rc = RC(crea_instance=stm)
...
def beneficiaries(authorperm, beneficiaries):
    ...
    if stm.rpc is not None:
        stm.rpc.rpcconnect()
    c = Comment(authorperm, crea_instance=stm)
    account = c["author"]
    ...
    for b in beneficiaries_list_sorted:
        Account(b["account"], crea_instance=stm)
...
@click.option('--percent-crea-dollars', '-b', help='50% CBD /50% SP is 10000 (default), 100% SP is 0')
@click.option('--max-accepted-payout', '-b', help='Default is 1000000.000 [CBD]')
@click.option('--no-parse-body', help='Disable parsing of links, tags and images', is_flag=True, default=False)
def post(body, account, title, permlink, tags, reply_identifier, community, beneficiaries, percent_crea_dollars, max_accepted_payout, no_parse_body):
    ...
    if percent_crea_dollars is not None:
        parameter["percent_crea_dollars"] = percent_crea_dollars
    elif "percent-crea-dollars" in parameter:
        parameter["percent_crea_dollars"] = parameter["percent-crea-dollars"]
    ...
    percent_crea_dollars = None
    if "percent_crea_dollars" in parameter:
        percent_crea_dollars = parameter["percent_crea_dollars"]
    max_accepted_payout = None
    if "max_accepted_payout" in parameter:
        max_accepted_payout = parameter["max_accepted_payout"]
    comment_options = None
    if max_accepted_payout is not None or percent_crea_dollars is not None:
        comment_options = {}
    if max_accepted_payout is not None:
        if stm.sbd_symbol not in max_accepted_payout:
            max_accepted_payout = str(Amount(float(max_accepted_payout), stm.sbd_symbol, crea_instance=stm))
        comment_options["max_accepted_payout"] = max_accepted_payout
    if percent_crea_dollars is not None:
        comment_options["percent_crea_dollars"] = percent_crea_dollars
    beneficiaries = None
...
@cli.command()
@click.option('--sbd-to-crea', '-i', help='Show ticker in CBD/CREA', is_flag=True, default=False)
def ticker(sbd_to_crea):
    """ Show ticker
    """
    stm = shared_crea_instance()
    if stm.rpc is not None:
    ...
    t = PrettyTable(["Key", "Value"])
    t.align = "l"
    market = Market(crea_instance=stm)
    ticker = market.ticker()
    for key in ticker:
        if key in ["highest_bid", "latest", "lowest_ask"] and sbd_to_crea:
            t.add_row([key, str(ticker[key].as_base("CBD"))])
        elif key in "percent_change" and sbd_to_crea:
...
def pricehistory(width, height, ascii):
    ...
    current_base = Amount(feed_history['current_median_history']["base"], crea_instance=stm)
    current_quote = Amount(feed_history['current_median_history']["quote"], crea_instance=stm)
    price_history = feed_history["price_history"]
    price = []
    for h in price_history:
        base = Amount(h["base"], crea_instance=stm)
        quote = Amount(h["quote"], crea_instance=stm)
    ...
    print("\n            Price history for CREA (median price %4.2f $)\n" % (float(current_base) / float(current_quote)))
...
@click.option('--height', '-h', help='Plot height (default 15)', default=15)
@click.option('--ascii', help='Use only ascii symbols', is_flag=True, default=False)
def tradehistory(days, hours, sbd_to_crea, limit, width, height, ascii):
    """ Show price history
    """
    stm = shared_crea_instance()
    if stm.rpc is not None:
        stm.rpc.rpcconnect()
    m = Market(crea_instance=stm)
    ...
    if sbd_to_crea:
        print("\n     Trade history %s - %s \n\nCBD/CREA" % (formatTimeString(start), formatTimeString(stop)))
    else:
        print("\n     Trade history %s - %s \n\nCREA/CBD" % (formatTimeString(start), formatTimeString(stop)))
    chart.adapt_on_series(price)
    chart.new_chart()
    chart.add_axis()
    chart.add_curve(price)
...
def orderbook(chart, limit, show_date, width, height, ascii):
    ...
    if not show_date:
        header = ["Asks Sum CBD", "Sell Orders", "Bids Sum CBD", "Buy Orders"]
    else:
    ...
    for order in orderbook["asks"]:
        asks.append(order)
        sum_asks += float(order.as_base("CBD")["base"])
        sumsum_asks.append(sum_asks)
    if n < len(asks):
        n = len(asks)
    for order in orderbook["bids"]:
        bids.append(order)
        sum_bids += float(order.as_base("CBD")["base"])
...
def buy(amount, asset, price, account, orderid):
    """Buy CREA or CBD from the internal market

        Limit buy price denoted in (CBD per CREA)
    """
    stm = shared_crea_instance()
    if stm.rpc is not None:
        stm.rpc.rpcconnect()
    ...
    if asset == stm.sbd_symbol:
        market = Market(base=Asset(stm.crea_symbol), quote=Asset(stm.sbd_symbol), crea_instance=stm)
    else:
        market = Market(base=Asset(stm.sbd_symbol), quote=Asset(stm.crea_symbol), crea_instance=stm)
    if price is None:
        orderbook = market.orderbook(limit=1, raw_data=False)
        if asset == stm.crea_symbol and len(orderbook["bids"]) > 0:
            p = Price(orderbook["bids"][0]["base"], orderbook["bids"][0]["quote"], crea_instance=stm).invert()
            p_show = p
        elif len(orderbook["asks"]) > 0:
            p = Price(orderbook["asks"][0]["base"], orderbook["asks"][0]["quote"], crea_instance=stm).invert()
            p_show = p
        price_ok = click.prompt("Is the following Price ok: %s [y/n]" % (str(p_show)))
        if price_ok not in ["y", "ye", "yes"]:
            return
    else:
        p = Price(float(price), u"%s:%s" % (stm.sbd_symbol, stm.crea_symbol), crea_instance=stm)
    if not unlock_wallet(stm):
        return

    a = Amount(float(amount), asset, crea_instance=stm)
    acc = Account(account, crea_instance=stm)
...
def sell(amount, asset, price, account, orderid):
    """Sell CREA or CBD from the internal market

        Limit sell price denoted in (CBD per CREA)
    """
    stm = shared_crea_instance()
    if stm.rpc is not None:
        stm.rpc.rpcconnect()
...
@click.option('--account', '-a', help='Recrea as this user')
def recrea(identifier, account):
    """Recrea an existing post"""
    stm = shared_crea_instance()
    if stm.rpc is not None:
        stm.rpc.rpcconnect()
    if not account:
    ...
    if not unlock_wallet(stm):
        return
    acc = Account(account, crea_instance=stm)
    post = Comment(identifier, crea_instance=stm)
    tx = post.recrea(account=acc)
    if stm.unsigned and stm.nobroadcast and stm.creaconnect is not None:
        tx = stm.creaconnect.url_from_tx(tx)
    tx = json.dumps(tx, indent=4)
    print(tx)
...
@click.option('--sbd_interest_rate', help='CBD interest rate in percent')
...
def witnessupdate(witness, maximum_block_size, account_creation_fee, sbd_interest_rate, url, signing_key):
    ...
    if not unlock_wallet(stm):
        return
    witness = Witness(witness, crea_instance=stm)
    props = witness["props"]
    if account_creation_fee is not None:
        props["account_creation_fee"] = str(
            Amount("%.3f %s" % (float(account_creation_fee), stm.crea_symbol), crea_instance=stm))
...
@click.option('--sbd_interest_rate', help='CBD interest rate in percent', default=0.0)
...
def witnessproperties(witness, wif, account_creation_fee, account_subsidy_budget, account_subsidy_decay, maximum_block_size, sbd_interest_rate, new_signing_key, url):
    ...
    props = {}
    if account_creation_fee is not None:
        props["account_creation_fee"] = Amount("%.3f %s" % (float(account_creation_fee), stm.crea_symbol), crea_instance=stm)
...
@click.option('--quote', '-q', help='Crea quote manually, when not set the base is automatically calculated.')
...
def witnessfeed(witness, wif, base, quote, support_peg):
    ...
    last_published_price = Price(witness["sbd_exchange_rate"], crea_instance=stm)
    crea_usd = None
    print("Old price %.3f (base: %s, quote %s)" % (float(last_published_price), old_base, old_quote))
    if quote is None and not support_peg:
        quote = Amount("1.000 %s" % stm.crea_symbol, crea_instance=stm)
    elif quote is None:
        latest_price = market.ticker()['latest']
        if crea_usd is None:
            crea_usd = market.crea_usd_implied()
        sbd_usd = float(latest_price.as_base(stm.sbd_symbol)) * crea_usd
        quote = Amount(1. / sbd_usd, stm.crea_symbol, crea_instance=stm)
    else:
        if str(quote[-5:]).upper() == stm.crea_symbol:
            quote = Amount(quote, crea_instance=stm)
        else:
            quote = Amount(quote, stm.crea_symbol, crea_instance=stm)
    if base is None:
        if crea_usd is None:
            crea_usd = market.crea_usd_implied()
        base = Amount(crea_usd, stm.sbd_symbol, crea_instance=stm)
    else:
        if str(quote[-3:]).upper() == stm.sbd_symbol:
            base = Amount(base, crea_instance=stm)
        else:
            base = Amount(base, stm.sbd_symbol, crea_instance=stm)
    new_price = Price(base=base, quote=quote, crea_instance=stm)
...
def witness(witness):
    ...
    if "VIRTUAL_SCHEDULE_LAP_LENGTH2" in config:
        ...
        lap_length = int(config["CREA_VIRTUAL_SCHEDULE_LAP_LENGTH2"])
    rank = 0
    active_rank = 0
    found = False
    witnesses = WitnessesRankedByVote(limit=250, crea_instance=stm)
...
def witnesses(account, limit):
    ...
    if account:
        ...
        witnesses = WitnessesVotedByAccount(account_name, crea_instance=stm)
        print("%s: @%s (%d of 30)" % (account_type, account_name, len(witnesses)))
    else:
        witnesses = WitnessesRankedByVote(limit=limit, crea_instance=stm)
...
def votes(account, direction, outgoing, incoming, days, export):
    ...
    if direction == "out" or outgoing:
        votes = AccountVotes(account, start=limit_time, crea_instance=stm)
    ...
    if direction == "in" or incoming:
        ...
        for v in account.history(start=limit_time, only_ops=["vote"]):
            votes_list.append(v)
        votes = ActiveVotes(votes_list, crea_instance=stm)
...
@click.option('--payout', default=None, help="Show the curation for a potential payout in CBD as float")
...
def curation(authorperm, account, limit, min_vote, max_vote, min_performance, max_performance, payout, export, short, length, permlink, title, days):
    ...
    if authorperm == 'all' or authorperm.isdigit():
        ...
        limit_time = utc.localize(datetime.utcnow()) - timedelta(days=days)
        votes = AccountVotes(account, start=limit_time, crea_instance=stm)
        authorperm_list = [Comment(vote.authorperm, crea_instance=stm) for vote in votes]
    ...
    for authorperm in authorperm_list:
        index += 1
        comment = Comment(authorperm, crea_instance=stm)
        if payout is not None and comment.is_pending():
            payout = float(payout)
        elif payout is not None:
            payout = None
        curation_rewards_CBD = comment.get_curation_rewards(pending_payout_CBD=True, pending_payout_value=payout)
        curation_rewards_SP = comment.get_curation_rewards(pending_payout_CBD=False, pending_payout_value=payout)
        rows = []
        sum_curation = [0, 0, 0, 0]
        max_curation = [0, 0, 0, 0, 0, 0]
        highest_vote = [0, 0, 0, 0, 0, 0]
        for vote in comment["active_votes"]:
            vote_CBD = stm.rshares_to_sbd(int(vote["rshares"]))
            curation_CBD = curation_rewards_CBD["active_votes"][vote["voter"]]
            curation_SP = curation_rewards_SP["active_votes"][vote["voter"]]
            if vote_CBD > 0:
                penalty = ((comment.get_curation_penalty(vote_time=vote["time"])) * vote_CBD)
                performance = (float(curation_CBD) / vote_CBD * 100)
            else:
                performance = 0
                penalty = 0
            vote_befor_min = (((vote["time"]) - comment["created"]).total_seconds() / 60)
            sum_curation[0] += vote_CBD
            sum_curation[1] += penalty
            sum_curation[2] += float(curation_SP)
            sum_curation[3] += float(curation_CBD)
            row = [vote["voter"],
                   vote_befor_min,
                   vote_CBD,
                   penalty,
        ...
        if not short and found_voter:
            ...
            t.add_row(sum_line + ["%.1f min" % highest_vote[1],
                                  "%.3f CBD" % float(highest_vote[2]),
                                  "%.3f CBD" % float(highest_vote[3]),
                                  "%.3f SP" % (highest_vote[4]),
                                  "%.1f %%" % (highest_vote[5])])
            sum_line[-1] = "High. Cur."
            t.add_row(sum_line + ["%.1f min" % max_curation[1],
                                  "%.3f CBD" % float(max_curation[2]),
                                  "%.3f CBD" % float(max_curation[3]),
                                  "%.3f SP" % (max_curation[4]),
                                  "%.1f %%" % (max_curation[5])])
            sum_line[-1] = "Sum"
            t.add_row(sum_line + ["-",
                                  "%.3f CBD" % (sum_curation[0]),
                                  "%.3f CBD" % (sum_curation[1]),
...
def rewards(accounts, only_sum, post, comment, curation, length, author, permlink, title, days):
    ...
    for account in accounts:
        ...
        account = Account(account, crea_instance=stm)
        median_price = Price(stm.get_current_median_history(), crea_instance=stm)
        m = Market(crea_instance=stm)
        latest = m.ticker()["latest"]
        if author and permlink:
            t = PrettyTable(["Author", "Permlink", "Payout", "CBD", "SP + CREA", "Liquid USD", "Invested USD"])
        elif author and title:
                t = PrettyTable(["Author", "Title", "Payout", "CBD", "SP + CREA", "Liquid USD", "Invested USD"])
        elif author:
            t = PrettyTable(["Author", "Payout", "CBD", "SP + CREA", "Liquid USD", "Invested USD"])
        elif not author and permlink:
            t = PrettyTable(["Permlink", "Payout", "CBD", "SP + CREA", "Liquid USD", "Invested USD"])
        elif not author and title:
            t = PrettyTable(["Title", "Payout", "CBD", "SP + CREA", "Liquid USD", "Invested USD"])
        else:
            t = PrettyTable(["Received", "CBD", "SP + CREA", "Liquid USD", "Invested USD"])
        t.align = "l"
        rows = []
            ...
            for v in comment_hist:
                ...
                if v["type"] == "author_reward":
                    ...
                    if not comment and c.is_comment():
                        continue
                    payout_CBD = Amount(v["sbd_payout"], crea_instance=stm)
                    payout_CREA = Amount(v["crea_payout"], crea_instance=stm)
                    sum_reward[0] += float(payout_CBD)
                    sum_reward[1] += float(payout_CREA)
                    payout_SP = stm.vests_to_sp(Amount(v["vesting_payout"], crea_instance=stm))
                    sum_reward[2] += float(payout_SP)
                    liquid_USD = float(payout_CBD) / float(latest) * float(median_price) + float(payout_CREA) * float(median_price)
                    ...
                    reward = Amount(v["reward"], crea_instance=stm)
                    ...
                    sum_reward[4] += invested_USD
                    if title:
                        c = Comment(construct_authorperm(v["comment_author"], v["comment_permlink"]), crea_instance=stm)
                        permlink_row = c.title
                    else:
...
def pending(accounts, only_sum, post, comment, curation, length, author, permlink, title, days):
    ...
    for account in accounts:
        ...
        latest = m.ticker()["latest"]
        if author and permlink:
            t = PrettyTable(["Author", "Permlink", "Cashout", "CBD", "SP", "Liquid USD", "Invested USD"])
        elif author and title:
            t = PrettyTable(["Author", "Title", "Cashout", "CBD", "SP", "Liquid USD", "Invested USD"])
        elif author:
            t = PrettyTable(["Author", "Cashout", "CBD", "SP", "Liquid USD", "Invested USD"])
        elif not author and permlink:
            t = PrettyTable(["Permlink", "Cashout", "CBD", "SP", "Liquid USD", "Invested USD"])
        elif not author and title:
            t = PrettyTable(["Title", "Cashout", "CBD", "SP", "Liquid USD", "Invested USD"])
        else:
            t = PrettyTable(["Cashout", "CBD", "SP", "Liquid USD", "Invested USD"])
        t.align = "l"
        rows = []
        c_list = {}
        start_op = account.estimate_virtual_op_num(limit_time)
        ...
        with click.progressbar(map(Comment, account.history(start=start_op, use_block_num=False, only_ops=["comment"])), length=progress_length) as comment_hist:
            for v in comment_hist:
                ...
                if float(author_reward["total_payout_CBD"]) < 0.001:
                    continue
                ...
                if v["author"] != account["name"]:
                    continue
                payout_CBD = author_reward["payout_CBD"]
                sum_reward[0] += float(payout_CBD)
                payout_SP = author_reward["payout_SP"]
                sum_reward[1] += float(payout_SP)
                liquid_USD = float(author_reward["payout_CBD"]) / float(latest) * float(median_price)
        ...
        if curation:
            votes = AccountVotes(account, start=limit_time, crea_instance=stm)
            for vote in votes:
                c = Comment(vote["authorperm"], crea_instance=stm)
...
@cli.command()
@click.argument('account', nargs=1, required=False)
@click.option('--reward_crea', help='Amount of CREA you would like to claim', default=0)
@click.option('--reward_sbd', help='Amount of CBD you would like to claim', default=0)
@click.option('--reward_vests', help='Amount of VESTS you would like to claim', default=0)
@click.option('--claim_all_crea', help='Claim all CREA, overwrites reward_crea', is_flag=True)
@click.option('--claim_all_sbd', help='Claim all CBD, overwrites reward_sbd', is_flag=True)
@click.option('--claim_all_vests', help='Claim all VESTS, overwrites reward_vests', is_flag=True)
def claimreward(account, reward_crea, reward_sbd, reward_vests, claim_all_crea, claim_all_sbd, claim_all_vests):
    ...
    tx = acc.claim_reward_balance(reward_crea, reward_sbd, reward_vests)
    if stm.unsigned and stm.nobroadcast and stm.creaconnect is not None:
        tx = stm.creaconnect.url_from_tx(tx)
    tx = json.dumps(tx, indent=4)
    print(tx)
...
def verify(blocknumber, trx, use_api):
    ...
    try:
        int(blocknumber)
        block = Block(blocknumber, crea_instance=stm)
        if trx is not None:
            i = int(trx)
    ...
    wallet = Wallet(crea_instance=stm)
    ...
    for trx in trxs:
        if not use_api:
            ...
            tx = TransactionBuilder(tx=trx, crea_instance=stm)
...
def info(objects):
    ...
    if not objects:
        ...
        crea_per_mvest = stm.get_crea_per_mvest()
        chain_props = stm.get_chain_properties()
        price = (Amount(median_price["base"], crea_instance=stm).amount / Amount(median_price["quote"], crea_instance=stm).amount)
        for key in info:
    ...
    for obj in objects:
        if re.match("^[0-9-]*$", obj) or re.match("^-[0-9]*$", obj) or re.match("^[0-9-]*:[0-9]", obj) or re.match("^[0-9-]*:-[0-9]", obj):
            ...
            if int(obj) < 1:
                ...
                block_number = obj
            block = Block(block_number, crea_instance=stm)
            if block:
                t = PrettyTable(["Key", "Value"])
                t.align = "l"
            ...
            account = Account(obj, crea_instance=stm)
            t = PrettyTable(["Key", "Value"])
            t.align = "l"
            ...
            # witness available?
            try:
                witness = Witness(obj, crea_instance=stm)
                witness_json = witness.json()
                t = PrettyTable(["Key", "Value"])
                t.align = "l"
                for key in sorted(witness_json):
        ...
        # Post identifier
            ...
            post = Comment(obj, crea_instance=stm)
            post_json = post.json()
            if post_json:
                t = PrettyTable(["Key", "Value"])
                t.align = "l"
...
def userdata(account, signing_account):
    ...
    if signing_account is not None:
        signing_account = Account(signing_account, crea_instance=stm)
    c = Conveyor(crea_instance=stm)
