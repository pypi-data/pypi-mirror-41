...
def map_to_locus(file_path_or_df, code_column, classification_system, crosswalk_path_or_df,
                 column_of_interest,
    ...
    :param crosswalk_path_or_df: (string) full path or dataframe of the crosswalk csv
    ...
    try:
        ...
        df_map = get_mapping(crosswalk_path_or_df=crosswalk_path_or_df,
        ...
        df_data = df_data.merge(df_map, left_on=code_column,
                                right_on=classification_system)

        if code_column!=classification_system:
            df_data.drop(classification_system, 1, inplace=True)
...
def get_mapping(crosswalk_path_or_df, classification_system, column_of_interest,
    ...
    """
    try:
        if isinstance(crosswalk_path_or_df, pd.DataFrame):
            df_cw = crosswalk_path_or_df.copy()
        else:
            df_cw = pd.read_csv(crosswalk_path_or_df, encoding=encoding)
