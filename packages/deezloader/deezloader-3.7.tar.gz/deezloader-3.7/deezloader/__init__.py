...
class Login:
      ...
      def request(self, url, control=True):
          try:
      ...
      def download(self, track, location, quality, check):
          ...
          try:
             ...
             quality = "1"
             qualit = "MP3_128"
          crypt = self.request(genurl(infos['results']['MD5_ORIGIN'], quality, infos['results']['MEDIA_VERSION']), None)
      ...
      def download_trackdee(self, URL, output=localdir + "/Songs/", check=True, quality="MP3_320", recursive=True):
          ...
          image = self.request(image, False).content
          if len(image) == 13:
           image = self.request("https://e-cdns-images.dzcdn.net/images/cover/1200x1200-000000-80-0-0.jpg", False).content
          datas['image'] = image
          ...
          try:
             ...
             url = self.request("https://api.deezer.com/search/track/?q=" + datas['music'].replace("#", "") + " + " + datas['artist'].replace("#", "")).json()
             try:
                ...
                try:
                   url = self.request("https://api.deezer.com/search/track/?q=" + datas['music'].replace("#", "").split(" ")[0] + " + " + datas['artist'].replace("#", "")).json()
                   for a in range(url['total'] + 1):
