...
from compat import httplib, urlencode, PY3
...
class RequestsKeywords(object):
    ...
    def create_session(self, alias, url, headers={}, cookies=None,
        ...
        try:
            ...
            logger.console('创建Session: %s=%s, headers=%s'.decode('utf-8') % ( alias, url, dumps(headers, ensure_ascii=False, encoding='utf-8')))
    ...
    def get_request(
        ...
        logger.console('\n-> 请求地址:[%s] %s'.decode('utf-8') % (alias, uri))
        if headers is not None and len(headers) > 0:
            logger.console('-> 额外Headers:%s'.decode('utf-8') % dumps(headers, ensure_ascii=False, encoding='utf-8'))
        if params is not None and len(params) > 0:
            logger.console('-> 入参:%s'.decode('utf-8') % dumps(params, ensure_ascii=False, encoding='utf-8'))
        ...
        logger.console('<- 返回:%s'.decode('utf-8') % response.content.decode('utf-8'))
        # logger.info(
    ...
    def post_request(
        ...
        if data is not None and len(data) > 0:
            logger.console('-> 入参:%s'.decode('utf-8') % dumps(data, ensure_ascii=False, encoding='utf-8'))
