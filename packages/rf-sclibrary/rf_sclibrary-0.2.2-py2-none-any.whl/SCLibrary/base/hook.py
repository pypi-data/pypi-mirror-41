...
def hook_zh():
    robot_path = imp.find_module("robot")[1]
    print robot_path
    
    if os.path.exists(robot_path + '/HOOK_ZH'):
        return

    try:
        os.rename(robot_path + "/utils/unic.py", robot_path + "/utils/unic_back.py")
        fo = open(robot_path + "/utils/unic.py", "w")
        fo.write(code)
        print 'Hook unic.py成功！'
        fo.close()
    except:
        print 'Hook unic.py失败！'
        return

    open(robot_path + '/HOOK_ZH', 'a').close()
    

code ="""#  Copyright 2008-2015 Nokia Networks
