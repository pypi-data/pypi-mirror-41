...
class RequestsKeywords(object):
    ...
    def create_session(self, alias, url, headers={}, cookies=None,
        ...
        try:
            ...
            logger.console('创建Session: %s=%s, headers=%s' % ( alias, url, headers))
    ...
    def get_request(
        ...
        if headers is not None and len(headers) > 0:
            logger.console('-> 额外Headers:%s' % headers)
        if params is not None and len(params) > 0:
            logger.console('-> 入参:%s' % params)
        ...
        logger.console('<- 返回:%s' % response.content.decode('utf-8'))
        # logger.info(
