...
def LLtoUTM(lat, lon):
	...
	return {'northing': int(math.floor(UTMNorthing+0.5)), 'easting': int(math.floor(UTMEasting+0.5)), 'zoneNumber': int(ZoneNumber), 'zoneLetter': getLetterDesignator(Lat)}
...
def encode(utm, accuracy):	
	...
	return str(utm['zoneNumber']) + utm['zoneLetter'] + str(get100kID(utm['easting'], utm['northing'], utm['zoneNumber'])) + seasting[-5:][0:accuracy] + snorthing[-5:][0:accuracy]
...
def decode(mgrsString):
	...
	return {'easting': easting, 'northing': northing, 'zoneLetter': zoneLetter, 'zoneNumber': zoneNumber, 'accuracy': accuracyBonus}
