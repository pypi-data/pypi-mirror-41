...
class session_reporter():
    ...
    def save_report(self, status_save_directory):
        ...
        with open(filename, 'w') as f:
            ...
            for epoch in ordered_epochs:
                ...
                for batch in ordered_batches:
                    text += "\t".join(["{}:{}".format(key, value) for key, value in self._batch_statuses[epoch][batch].iteritems()])
                    text += "\n"
            f.write('%s' % text)
        ...
        with open(status_save_directory + "tf.epoch.status.txt", 'w') as f:
            ...
            for epoch in ordered_epochs:
                text += "\t".join(["{}:{}".format(key, value) for key, value in self._epoch_statuses[epoch].iteritems()])
                text += "\n"
            f.write('%s' % text)
