...
class RadwareLBaaSV2Driver(base_v2_driver.RadwareLBaaSBaseV2Driver):
    ...
    def __init__(self, plugin, module_fqn=None):
        ...
        provider_name = "radware_" + module_name
        base_v2_driver.RadwareLBaaSBaseV2Driver.__init__(self, plugin, provider_name)

        rad = cfg.CONF.__getattr__(provider_name)
        rad_debug = cfg.CONF.__getattr__(provider_name + "_debug")
        self.plugin = plugin
        self.service = {
            "name": "_REPLACE_",
            "tenantId": "_REPLACE_",
        ...
        self.configure_allowed_address_pairs = rad.configure_allowed_address_pairs
    ...
    def _build_objects_graph(self, ctx, lb, data_model,
        ...
        graph['configure_allowed_address_pairs'] = self.configure_allowed_address_pairs
        ...
        listeners = [
            ...
            listener.id not in deleted_ids and 
        ...
        for listener in listeners:
			...
			if hasattr(listener.default_pool, 'sessionpersistence') and listener.default_pool.sessionpersistence:
				sp_attr = listener.default_pool.sessionpersistence
			elif hasattr(listener.default_pool, 'session_persistence') and listener.default_pool.session_persistence:
				sp_attr = listener.default_pool.session_persistence
			else:
				sp_attr = None
			if (sp_attr):
                sess_pers_dict = {}
                for prop in SESSION_PERSISTENCY_PROPERTIES:
                    sess_pers_dict[prop] = getattr(
                        sp_attr, prop, PROPERTY_DEFAULTS.get(prop))
            ...
            if (listener.default_pool.healthmonitor and
                ...
                constants.PENDING_DELETE and
                listener.default_pool.healthmonitor.id not in deleted_ids):
                hm_dict = {}
