...
from LbSoftConfDb2Server.LbSoftConfDB2.SoftConfDB import SoftConfDB
...
class GitlabProject:
    ...
    def __init__(self, project, version, sourceuri):
        ...
        if len(tmp) == 2:
            ...
            self.log.warning("Path without Group/Name: %s, "
    ...
    def getCMakeLists(self):
        ...
        try:
            ...
            self.log.warning("Could not find CMakeLists at: %s" %
                             self.cmakelistsurl)
            raise
        return data
    ...
    def getProjectCMT(self):
        ...
        try:
            ...
            self.log.warning("Could not find Project.cmt at: %s" %
                             self.projectcmturl)
            raise
        return data
    ...
    def getProjectConfig(self):
        ...
        try:
            ...
            self.log.warning("Could not find projectConfig.json at: %s" %
                             self.projconfigurl)
            raise
        return data
...
class AppImporter:
    ...
    def __init__(self, autorelease=True, platforms=None):
        ...
        if self.mPlatforms is not None:
            ...
            if len(unknown) > 0:
                raise Exception("Unknown platforms: %s" %
                                ", ".join(list(unknown)))
    ...
    def gitlabProcessProjectVersion(self, p, v, alreadyDone=[],
                                    recreate=False,
        ...
        if createNode:
            ...
            if saveURIinPV:
                self.log.warning("Setting sourceuri=%s for %s %s" % (
                    gp.sourceuri, proj, ver))
                self.mConfDB.setPVProperty(proj, corver, "sourceuri",
                                           gp.sourceuri)
        ...
        for (dp, dv) in deps:
            ...
            node_child = self.processProjectVersion(dp, dv, alreadyDone,
                                                    recreate)

            # Now checking if the links exist
            if self.mConfDB.nodesHaveRelationship(node_parent, node_child,
                                                  "REQUIRES"):
                self.log.warning(
                    "Pre-existing dependency (%s, %s)-[:REQUIRES]->(%s, %s)" %
                    (proj, ver, dp, dv))
            else:
                self.log.warning(
                    "Adding dependency (%s, %s)-[:REQUIRES]->(%s, %s)" % (
                        proj, ver, dp, dv))
    ...
    #
    def processProjectVersion(self, p, v, alreadyDone=[],
                              recreate=False, sourceuri=None):
        """ Get the dependencies for a single project """
        ...
        if gitlab:
            ...
            # we'll use this hack
            self.log.warning("Project %s is in Gitlab URI:%s" % (
                proj, sourceuri))
