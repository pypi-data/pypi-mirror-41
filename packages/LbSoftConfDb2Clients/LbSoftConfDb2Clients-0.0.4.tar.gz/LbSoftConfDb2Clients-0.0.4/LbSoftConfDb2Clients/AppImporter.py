...
class AppImporter:
    ...
    def gitlabProcessProjectVersion(self, p, v,
        ...
        if createNode:
            self.log.warning("Will create project %s %s" % (proj, ver))
            if saveURIinPV:
                self.log.warning("Will set sourceuri=%s for %s %s" % (
                    gp.sourceuri, proj, ver))
            # If releasing is needed!
            if self.mAutorelease and proj.upper() not in ["LCG", "LCGCMT"]:
                self.log.warning("Will request release of %s %s" % (proj,
                                                                    corver))
        graph_el = {
            'project': proj,
            'version': corver,
        ...
        if graph == self.graph:
            self.mConfDB.getOrCreateBulkPV(json.dumps(graph))
        return node_parent
    ...
    def processProjectVersion(self, p, v, sourceuri=None, graph=None):
        ...
        if createNode and proj == "LCG":
            ...
            if len(tmplcgcmt) > 0:
                ...
                self.log.warning("Will create project %s %s" % (proj, corver))
        return node_parent
