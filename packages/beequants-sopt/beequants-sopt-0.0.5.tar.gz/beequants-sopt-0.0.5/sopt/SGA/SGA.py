...
class SGA:
    def __init__(self, lower_bound, upper_bound, variables_num, func,
        ...
        self.kw = kw
        self.raw_params = copy.deepcopy(kw['param'])
    ...
    def calculate(self,population,real_population = None,complex_constraints = None,complex_constraints_C = ga_config.complex_constraints_C):
        ...
        rstls = []
        pool = multiprocessing.Pool(self.population_size)
        for i in range(self.population_size):
            for j in range(len(self.param_names)):
                unit = self.raw_params[self.coinType][self.param_names[j]][2]
                self.kw['param'][self.coinType][self.param_names[j]] = round(int(real_population[i][j] / unit) * unit, max(len(str(unit))-2,0))
                real_population[i][j] = self.kw['param'][self.coinType][self.param_names[j]]
            rstl = pool.apply_async(self.func, args=(self.coinType, self.start, self.end, copy.deepcopy(self.kw), self.strategy_name))
            rstls.append([rstl, copy.deepcopy(self.kw)])
        pool.close()
        pool.join()
        for i in range(len(rstls)):
            targets_func[i] = rstls[i][0].get()
            if targets_func[i] > self.best[0]:
                self.best[0] = targets_func[i]
                self.best[1] = rstls[i][1]['param'][self.coinType]
                print(self.best[1], ',  # ', math.log(self.best[0], 1.05))
        # print(time.time() - a)
        ...
        if probs is None:
            ...
            for i in range(self.population_size):
                for j in range(len(self.param_names)):
                    ...
                    self.kw['param'][self.coinType][self.param_names[j]] = round(int(real_population[i][j] / unit) * unit, max(len(str(unit))-2,0))
                    real_population[i][j] = self.kw['param'][self.coinType][self.param_names[j]]
                rslt = pool.apply_async(self.func, args=(self.coinType, self.start, self.end, copy.deepcopy(self.kw), self.strategy_name))
                rstls.append([rslt, copy.deepcopy(self.kw)])
            pool.close()
            pool.join()
            for i in range(len(rstls)):
                targets_func[i] = rstls[i][0].get()
    ...
    def run(self):
        print('----------------初始化初代种群，种群规模 %s--------------------' % self.population_size)
