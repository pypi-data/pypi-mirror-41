...
class GPfit2D(object): # Tested
    ...
    def fit(self, plot = True, plot_save=False):
        ...
        self.x1, self.x2 = np.meshgrid(np.linspace(self.fitlim1[0], self.fitlim1[1], self.res),
                             np.linspace(self.fitlim2[0], self.fitlim2[1], self.res))

        xx = np.vstack([self.x1.reshape(self.x1.size), self.x2.reshape(self.x2.size)]).T
        ...
        lnlik_pred_reshaped = (self.lnlik_pred.T[0]).reshape(self.res,self.res)
        j,k = np.argwhere(lnlik_pred_reshaped == lnlik_pred_reshaped.max())[0]
        self.param1_best, self.param2_best, self.lnlik_best = self.x1[j,k], self.x2[j,k], lnlik_pred_reshaped[j,k]
        ...
        if plot is True:
            ...
            #2D plot
            plt.scatter(self.x1, self.x2, c=lnlik_pred_reshaped, vmin=lnlik_pred_reshaped.min(), vmax=lnlik_pred_reshaped.max(), cmap = cm.magma_r)#gist_earth)
            plt.scatter(self.x1[j,k], self.x2[j,k], c='white')
            plt.show()

            #3D Figure
            fig = plt.figure(figsize=(10,10))
            ax = fig.gca(projection='3d')
            fitplot = ax.plot_surface(self.x1, self.x2, lnlik_pred_reshaped, alpha=0.5, cmap = cm.coolwarm, antialiased=True)
        ...
        return self.x1, self.x2, self.lnlik_pred, self.fit_sigma
