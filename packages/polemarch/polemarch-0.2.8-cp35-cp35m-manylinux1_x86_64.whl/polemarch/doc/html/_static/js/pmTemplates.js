...
gui_project_template={getValue:function(hideReadOnly)
...
{data_field[value]=template_data[value];delete template_data[value];}})
if(!data_field.vars)
{data_field.vars={};}
...
return res;},}
gui_project_template_variables={pkValuePriority:["key"],apiGetDataForQuery:function(query,variable)
{try{if(variable)
{if(query.method=="get")
{let res={status:200,item:"project",type:"mod",data:{},subitem:[this.url_vars['api_pk'],"template",this.url_vars['api_template_id'],"variables",variable]}
let val=this.parent_template.model.data.data['vars'];if(val[variable]!==undefined)
{res.data={key:variable,value:val[variable],}}
else
{res.status=404;res.data.detail="No Variable matches the given query.";}
return res;}
if(query.method=="put")
{let template_data=this.parent_template.model.data
let vars=template_data.data['vars'];if(!vars)
{vars={};}
vars[query.data.key]=query.data.value;var def=new $.Deferred();$.when(this.parent_template.sendToApi("patch",undefined,undefined,template_data)).done(()=>{def.resolve({status:200,item:"project",type:"mod",data:query.data,subitem:[this.url_vars['api_pk'],"template",this.url_vars['api_template_id'],"variables",variable,]})}).fail((e)=>{def.reject(e)})
return def.promise()}}
else
{if(query.method=="get")
{let res={status:200,item:"project",type:"mod",data:{count:0,next:null,previous:null,results:[]},subitem:[this.url_vars['api_pk'],"template",this.url_vars['api_template_id'],"variables",]}
let vars=this.parent_template.model.data.data['vars'];let limit=+query.filters.match(/limit=([0-9]+)/)[1]||guiLocalSettings.get('page_size');let offset=+query.filters.match(/offset=([0-9]+)/)[1]||0;if(vars&&typeof vars=="object"){let vars_keys=Object.keys(vars);for(let i=offset;i<limit+offset;i++){let key=vars_keys[i];if(key&&vars[key]!==undefined){res.data.results.push({id:key,key:key,value:vars[key],})}}
...
let vars=template_data.data['vars'];if(!vars)
{vars={};}
vars[query.data.key]=query.data.value;var def=new $.Deferred();$.when(this.parent_template.sendToApi("patch",undefined,undefined,template_data)).done(()=>{def.resolve({status:201,item:"project",type:"mod",data:query.data,subitem:[this.url_vars['api_pk'],"template",this.url_vars['api_template_id'],"variables",]})}).fail((e)=>{def.reject(e)})
return def.promise()}}}catch(exception){var def=new $.Deferred();def.reject({status:404,data:{detail:"Option not found"}})
return def.promise()}},apiQuery:function(query)
{let variable;if(query.data_type[query.data_type.length-1]!='variables')
...
$.when(this.parent_template.load(query.data_type[3])).done(()=>{$.when(this.apiGetDataForQuery(query,variable)).done((d)=>{if(d.status>=200&&d.status<300){def.resolve(d);}else{def.reject(d);}}).fail((e)=>{def.reject(e);})}).fail((e)=>{def.reject(e);})
return def.promise();},delete:function()
{let url_info=this.url_vars;this.parent_template=new guiObjectFactory("/project/{pk}/template/{template_id}/",this.url_vars);let def=new $.Deferred();$.when(this.parent_template.load(url_info.api_template_id)).done((data)=>{let template_data=data.data;delete template_data.data.vars[url_info.api_variables_id]
def.resolve(this.parent_template.sendToApi("patch",undefined,undefined,template_data))}).fail((e)=>{def.reject(e);})
return def.promise();},deleteArray:function(ids)
{let url_info=this.url_vars;this.parent_template=new guiObjectFactory("/project/{pk}/template/{template_id}/",this.url_vars);let def=new $.Deferred();$.when(this.parent_template.load(url_info.api_template_id)).done((data)=>{let template_data=data.data;for(let i in ids)
{let id=ids[i];delete template_data.data.vars[id];}
guiPopUp.success("Objects of '"+this.api.bulk_name+"' type were successfully deleted");def.resolve(this.parent_template.sendToApi("patch",undefined,undefined,template_data))}).fail((e)=>{def.reject(e);})
return def.promise();},search:function(filters)
{return customTemplateInnerObjectsSearch.apply(this,arguments);},}
gui_project_template_option={load:function(filters)
...
{return gui_list_object.load.apply(this,arguments)}},renderAsNewPage:function(render_options={})
{let def=new $.Deferred();this.parent_template=new guiObjectFactory("/project/{pk}/template/{template_id}/");$.when(this.parent_template.load(this.url_vars['api_template_id'])).done((d1)=>{let template_data=this.parent_template.model.data;prepareOptionFields(template_data,this.api.schema.new);def.resolve(gui_list_object.renderAsNewPage.apply(this,arguments));})
return def.promise();},apiGetDataForQuery:function(query,option)
{try{if(option)
{if(query.method=="get")
{let res={status:200,item:"project",type:"mod",data:{},subitem:[this.url_vars['api_pk'],"template",this.url_vars['api_template_id'],"option",option,]}
let val=this.parent_template.model.data.options[option];res.data={}
...
{template_data.options[query.data.name]=template_data.options[option];delete template_data.options[option];}
...
var def=new $.Deferred();$.when(this.parent_template.sendToApi("patch",undefined,undefined,template_data)).done(()=>{def.resolve({status:200,item:"project",type:"mod",data:query.data,subitem:[this.url_vars['api_pk'],"template",this.url_vars['api_template_id'],"option",option,]})}).fail((e)=>{def.reject(e)})
return def.promise()}}
else
{if(query.method=="get")
{let res={status:200,item:"project",type:"mod",data:{count:0,next:null,previous:null,results:[]},subitem:[this.url_vars['api_pk'],"template",this.url_vars['api_template_id'],"option",]}
let limit=+query.filters.match(/limit=([0-9]+)/)[1]||guiLocalSettings.get('page_size');let offset=+query.filters.match(/offset=([0-9]+)/)[1]||0;if(this.parent_template.model.data.options&&typeof this.parent_template.model.data.options=="object"){let option_keys=Object.keys(this.parent_template.model.data.options);for(let i=offset;i<limit+offset;i++){let key=option_keys[i];if(key&&this.parent_template.model.data.options[key]){let val=this.parent_template.model.data.options[key];res.data.results.push({"id":key,"name":val.name||key,})}}
...
delete template_data.options[query.data.name].name
var def=new $.Deferred();$.when(this.parent_template.sendToApi("patch",undefined,undefined,template_data)).done(()=>{def.resolve({status:201,item:"project",type:"mod",data:query.data,subitem:[this.url_vars['api_pk'],"template",this.url_vars['api_template_id'],"option",],})}).fail((e)=>{def.reject(e)})
return def.promise()}}}catch(exception){var def=new $.Deferred();def.reject({status:404,data:{detail:"Option not found"}})
return def.promise()}},apiQuery:function(query)
{let option;if(query.data_type[query.data_type.length-1]!='option'&&query.data_type.length==6)
{option=query.data_type[query.data_type.length-1];}
let def=new $.Deferred();this.parent_template=new guiObjectFactory("/project/{pk}/template/{template_id}/",this.url_vars)
$.when(this.parent_template.load(query.data_type[3])).done(()=>{$.when(this.apiGetDataForQuery(query,option)).done((d)=>{def.resolve(d)}).fail((e)=>{def.reject(e);})}).fail((e)=>{def.reject(e);})
return def.promise();},delete:function()
{let url_info=this.url_vars;this.parent_template=new guiObjectFactory("/project/{pk}/template/{template_id}/",this.url_vars);let def=new $.Deferred();$.when(this.parent_template.load(url_info.api_template_id)).done((data)=>{let template_data=data.data;delete template_data.options[url_info.api_option_id]
def.resolve(this.parent_template.sendToApi("patch",undefined,undefined,template_data))}).fail((e)=>{def.reject(e);})
return def.promise();},deleteArray:function(ids)
{let url_info=this.url_vars;this.parent_template=new guiObjectFactory("/project/{pk}/template/{template_id}/",this.url_vars);let def=new $.Deferred();$.when(this.parent_template.load(url_info.api_template_id)).done((data)=>{let template_data=data.data;for(let i in ids)
{let id=ids[i];delete template_data.options[id];}
guiPopUp.success("Objects of '"+this.api.bulk_name+"' type were successfully deleted");def.resolve(this.parent_template.sendToApi("patch",undefined,undefined,template_data))}).fail((e)=>{def.reject(e);})
return def.promise();},search:function(filters)
{return customTemplateInnerObjectsSearch.apply(this,arguments);},}
gui_project_template_option_variables={pkValuePriority:["key"],apiGetDataForQuery:function(query,variable)
{try{if(variable)
{if(query.method=="get")
{let res={status:200,item:"project",type:"mod",data:{},subitem:[this.url_vars['api_pk'],"template",this.url_vars['api_template_id'],"option",this.url_vars['api_option_id'],"variables",variable,]}
let val=this.parent_template.model.data.options[query.data_type[5]];if(val.vars&&val.vars[variable]!==undefined)
{res.data={key:variable,value:val.vars[variable],}}
else
{res.status=404;res.data.detail="No Variable matches the given query.";}
return res;}
if(query.method=="put")
{let template_data=this.parent_template.model.data
let option_data=template_data.options[query.data_type[5]];if(!option_data.vars)
{option_data.vars={};}
option_data.vars[query.data.key]=query.data.value;var def=new $.Deferred();$.when(this.parent_template.sendToApi("patch",undefined,undefined,template_data)).done(()=>{def.resolve({status:200,item:"project",type:"mod",data:query.data,subitem:[this.url_vars['api_pk'],"template",this.url_vars['api_template_id'],"option",this.url_vars['api_option_id'],"variables",variable,]})}).fail((e)=>{def.reject(e)})
return def.promise()}}
else
{if(query.method=="get")
{let res={status:200,item:"option",type:"mod",data:{count:0,next:null,previous:null,results:[]},subitem:[this.url_vars['api_pk'],"template",this.url_vars['api_template_id'],"option",this.url_vars['api_option_id'],"variables",]}
let option_data=this.parent_template.model.data.options[query.data_type[5]];let limit=+query.filters.match(/limit=([0-9]+)/)[1]||guiLocalSettings.get('page_size');let offset=+query.filters.match(/offset=([0-9]+)/)[1]||0;if(option_data.vars&&typeof option_data.vars=="object"){let vars_keys=Object.keys(option_data.vars);for(let i=offset;i<limit+offset;i++){let key=vars_keys[i];if(key&&option_data.vars[key]!==undefined){res.data.results.push({id:key,key:key,value:option_data.vars[key],})}}
res.data.count=vars_keys.length;}
return res;}
if(query.method=="post")
{let template_data=this.parent_template.model.data
let option_data=template_data.options[query.data_type[5]];if(!option_data.vars)
{option_data.vars={};}
option_data.vars[query.data.key]=query.data.value;var def=new $.Deferred();$.when(this.parent_template.sendToApi("patch",undefined,undefined,template_data)).done(()=>{def.resolve({status:201,item:"project",type:"mod",data:query.data,subitem:[this.url_vars['api_pk'],"template",this.url_vars['api_template_id'],"option",this.url_vars['api_option_id'],"variables",]})}).fail((e)=>{def.reject(e)})
return def.promise()}}}catch(exception){var def=new $.Deferred();def.reject({status:404,data:{detail:"Option not found"}})
return def.promise()}},apiQuery:function(query)
{let variable;if(query.data_type[query.data_type.length-1]!='variables')
{variable=query.data_type[query.data_type.length-1];}
let def=new $.Deferred();this.parent_template=new guiObjectFactory("/project/{pk}/template/{template_id}/",this.url_vars)
$.when(this.parent_template.load(query.data_type[3])).done(()=>{$.when(this.apiGetDataForQuery(query,variable)).done((d)=>{if(d.status>=200&&d.status<300)
{def.resolve(d);}else{def.reject(d);}}).fail((e)=>{def.reject(e);})}).fail((e)=>{def.reject(e);})
return def.promise();},delete:function()
{let url_info=this.url_vars;this.parent_template=new guiObjectFactory("/project/{pk}/template/{template_id}/",this.url_vars);let def=new $.Deferred();$.when(this.parent_template.load(url_info.api_template_id)).done((data)=>{let template_data=data.data;delete template_data.options[url_info.api_option_id].vars[url_info.api_variables_id]
def.resolve(this.parent_template.sendToApi("patch",undefined,undefined,template_data))}).fail((e)=>{def.reject(e);})
return def.promise();},deleteArray:function(ids)
{let url_info=this.url_vars;this.parent_template=new guiObjectFactory("/project/{pk}/template/{template_id}/",this.url_vars);let def=new $.Deferred();$.when(this.parent_template.load(url_info.api_template_id)).done((data)=>{let template_data=data.data;for(let i in ids)
{let id=ids[i];delete template_data.options[url_info.api_option_id].vars[id];}
guiPopUp.success("Objects of '"+this.api.bulk_name+"' type were successfully deleted");def.resolve(this.parent_template.sendToApi("patch",undefined,undefined,template_data))}).fail((e)=>{def.reject(e);})
return def.promise();},search:function(filters)
{return customTemplateInnerObjectsSearch.apply(this,arguments);},}
gui_project_template_option_Schema={"name":{"title":"Name","type":"string","required":true,"maxLength":512,"minLength":1,"gui_links":[],"definition":{},"name":"name","parent_name_format":"option_name"},"group":{"title":"Group","type":"string","maxLength":512,"minLength":1,"default":"all","gui_links":[],"definition":{},"name":"group","parent_name_format":"option_group","format":"autocomplete","dynamic_properties":{"list_obj":[],"value_field":'name',"view_field":'name',},},"module":{"title":"Module","type":"string","maxLength":512,"minLength":1,"gui_links":[],"definition":{},"name":"module","format":"autocomplete","dynamic_properties":{"list_obj":projPath+"/module/","value_field":'name',"view_field":'path',},"parent_name_format":"option_module"},"args":{"title":"Args","type":"string","maxLength":512,"minLength":1,"gui_links":[],"definition":{},"name":"args","parent_name_format":"option_args"},"playbook":{"title":"Playbook","type":"string","maxLength":512,"minLength":1,"gui_links":[],"definition":{},"name":"playbook","format":"autocomplete","dynamic_properties":{"list_obj":projPath+"/playbook/","value_field":'playbook',"view_field":'playbook',},"parent_name_format":"option_playbook"},}
gui_project_template_option_variables_fields_Schema={"key":{"title":"Key","type":"dynamic","dynamic_properties":{},"required":true,"__func__onInit":"TemplateVariable_key_onInit","gui_links":[],"definition":{},"name":"key","parent_name_format":"variables_key"},"value":{"title":"Value","type":"dynamic","dynamic_properties":{"__func__callback":"TemplateVariable_value_callback",},"required":true,"default":"","gui_links":[],"definition":{},"name":"value","parent_name_format":"variables_value","parent_field":"key"}}
let api_error_responses={"400":{"description":"Validation error or some data error.","schema":{"required":["detail"],"type":"object","properties":{"detail":{"title":"Detail","type":"string","minLength":1,"required":true}},"definition_name":"Error","definition_ref":"#/definitions/Error"}},"401":{"description":"Unauthorized access error.","schema":{"required":["detail"],"type":"object","properties":{"detail":{"title":"Detail","type":"string","minLength":1,"required":true}},"definition_name":"Error","definition_ref":"#/definitions/Error"}},"403":{"description":"Permission denied error.","schema":{"required":["detail"],"type":"object","properties":{"detail":{"title":"Detail","type":"string","minLength":1,"required":true}},"definition_name":"Error","definition_ref":"#/definitions/Error"}},"404":{"description":"Not found error.","schema":{"required":["detail"],"type":"object","properties":{"detail":{"title":"Detail","type":"string","minLength":1,"required":true}},"definition_name":"Error","definition_ref":"#/definitions/Error"}}}
...
{let thisObj=this;let template=new guiObjectFactory("/project/{pk}/template/{template_id}/",parent_object.url_vars);$.when(template.load()).done(function(){let fields={}
...
thisObj.setType("enum",{enum:Object.keys(fields),});thisObj.opt.all_fields=fields
...
{let inventory_path='/inventory/{inventory_id}'
let list_obj=[projPath+inventory_path+'/all_groups/',projPath+inventory_path+'/all_hosts/',]
