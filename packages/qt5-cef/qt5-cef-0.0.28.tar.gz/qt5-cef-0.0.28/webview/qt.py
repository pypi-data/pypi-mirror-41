...
class BrowserView(QMainWindow):
    ...
    def __init__(self, uid, title="", url="", width=default_window_width,
                 ...
                 quit_app_when_main_window_close=True):
    ...
    def get_ime(self):
        if platform.system() == 'Windows':
            import win32api
            import win32con
            print('current ime : ', win32api.GetKeyboardLayout(), win32api.GetKeyboardLayoutName())
...
def find_main_window_hwnd(class_name=None, title=""):
    if platform.system() == 'Windows':
        import win32gui
...
def nest_third_party_application(target_uid='master',
    ...
    third_party_wrapper_window = create_qt_view(default_show=False, window_type='cef', cid=cid)
    third_party_wrapper_window.update_browser_info_one_by_one()
    global pixel_ratio
    if platform.system() == 'Windows':
    ...
    t = Thread(target=launch_third_party_client,
               args=(third_party_application_title, application_path, launch_params, target_window, cid))
    t.start()
    # t.join()
    ...
    if hwnd != 0:
        ...
        logging.debug('temp_window = {temp_window}'.format(temp_window=temp_window))
        logging.debug('hwnd = {hwnd}'.format(hwnd=hwnd))
...
def launch_main_window(uid, title, url, width, height, resizable, full_screen, min_size,
                       ...
                       quit_app_when_main_window_close=True, application_version=app_version_info,
                       enable_cef_cache=True, main_window_class_name='Qt5QWindowIcon'):
