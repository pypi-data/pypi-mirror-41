...
#   - Make the script callable in console
# Version 1.1
# -------------
...
def host_tree_to_html(ip_address, host_tree, base):
    ...
    if host_tree.find('ports').findall('port'):
        for port_tree in host_tree.find('ports').findall('port'):
            ...
            try:
                etree.SubElement(table_summary_port_body, "td", attrib={"class": "extra"}).text = service.get(
                    'extrainfo')
            except:
    ...
    etree.SubElement(body,
                     "p").text = "Report generated by HackRecon version {} (Tool created by Emilien Peretti)".format(
        __version__)
...
def main_with_params(ips, output=".", ports=False, max_threads=5,
    ...
    :param ports: True if all port must be scanned
    :param max_threads: te maximum number of main thread in same time
    :param cache: True if the script must use the cache
    :param css: the css file to use into the HTML report
    :param javascript_head: the javascript file to use in the header of the HTMl report
    ...
    split_list = [ips_list[i * max_threads:(i + 1) * max_threads] for i in
                  range((len(ips_list) + max_threads - 1) // max_threads)]
    logging.basicConfig(filename=os.path.join(output, "hackrecon.log"), level=logging.INFO)
    for ip_list in split_list:
        ...
        for ip in ip_list:
            path = make_folder_for_ip(ip, output)
            if path:
                threads.append(GenericThread(function=scan, arguments=(ip, path, ports), bar=part_bar))
                threads[-1].start()
...
def main_with_args(*args, **kwargs):
    ...
    print("       ------------------------------------------------------")
    print("       |   _   _            _    ____                       |")
    print("       |  | | | | __ _  ___| | _|  _ \ ___  ___ ___  _ __   |")
    print("       |  | |_| |/ _` |/ __| |/ / |_) / _ \/ __/ _ \| '_ \  |")
    print("       |  |  _  | (_| | (__|   <|  _ <  __/ (_| (_) | | | | |")
    print("       |  |_| |_|\__,_|\___|_|\_\_| \_\___|\___\___/|_| |_| |")
    print("       |                                                    |")
    print("       ----Emilien Peretti------------Version {}{}".format(__version__, "-" * (15 - len(__version__))))
